   ...
   Begin VB.Frame frmemain 
      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.Label Label1 
            ...
            Caption         =   "(Discount, Purchase Return etc.)"
            BeginProperty Font 
               Name            =   "Tahoma"
               Size            =   9.75
               Charset         =   0
...
Private Sub CmdSave_Click()
    ...
    If OptCr.Value = True Then
        ...
        rstBILL.Open "Select MAX(CR_NO) From CRDTPYMT WHERE TRX_TYPE = 'CB'", db, adOpenForwardOnly
    ...
    Else
        Set rstBILL = New ADODB.Recordset
        rstBILL.Open "Select MAX(CR_NO) From CRDTPYMT WHERE TRX_TYPE = 'DB'", db, adOpenForwardOnly
        If Not (rstBILL.EOF And rstBILL.BOF) Then
            txtBillNo.text = IIf(IsNull(rstBILL.Fields(0)), 1, rstBILL.Fields(0) + 1)
        End If
        rstBILL.Close
    ...
    If OptBank.Value = True Then
        ...
        Set rstBILL = New ADODB.Recordset
        If OptCr.Value = True Then
            rstBILL.Open "Select MAX(TRX_NO) From BANK_TRX WHERE TRX_TYPE = 'DR' AND BILL_TRX_TYPE = 'DB' AND TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' ", db, adOpenForwardOnly
        Else
            rstBILL.Open "Select MAX(TRX_NO) From BANK_TRX WHERE TRX_TYPE = 'CR' AND BILL_TRX_TYPE = 'CB' AND TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' ", db, adOpenForwardOnly
        End If
        If Not (rstBILL.EOF And rstBILL.BOF) Then
        ...
        If OptDr.Value = True Then
            ...
            RSTTRXFILE!BILL_TRX_TYPE = "CB"
        Else
            RSTTRXFILE!TRX_TYPE = "DR"
            RSTTRXFILE!BILL_TRX_TYPE = "DB"
        End If
...
'        End If
        ...
        RSTITEMMAST.Open "SELECT * FROM CASHATRXFILE WHERE TRX_TYPE = 'CR' AND INV_NO = " & Val(txtBillNo.text) & " AND INV_TYPE = 'DN'  AND INV_TRX_TYPE = 'DN'", db, adOpenStatic, adLockOptimistic, adCmdText
        RSTITEMMAST.AddNew
    ...
    Set rstBILL = New ADODB.Recordset
    If OptCr.Value = True Then
        rstBILL.Open "Select MAX(REC_NO) From CRDTPYMT WHERE TRX_TYPE = 'CB' AND '" & Year(MDIMAIN.DTFROM.Value) & "'", db, adOpenForwardOnly
    Else
        rstBILL.Open "Select MAX(REC_NO) From CRDTPYMT WHERE TRX_TYPE = 'DB' AND '" & Year(MDIMAIN.DTFROM.Value) & "'", db, adOpenForwardOnly
    End If
    If Not (rstBILL.EOF And rstBILL.BOF) Then
