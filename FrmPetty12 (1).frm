...
'
            ...
            db.Execute "delete FROM TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='HI' AND VCH_NO = " & Val(txtBillNo.text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(grdsales.Row, 32)) & ""
    
            db.Execute "START TRANSACTION"
            Set RSTTRXFILE = New ADODB.Recordset
            RSTTRXFILE.Open "Select * FROM TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='HI' AND VCH_NO = " & Val(txtBillNo.text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(grdsales.Row, 32)) & "", db, adOpenStatic, adLockOptimistic, adCmdText
            If (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
                RSTTRXFILE.AddNew
        ...
        '    Set RSTUNBILL = Nothing
            ...
            db.Execute "START TRANSACTION"
            Set RSTTRXFILE = New ADODB.Recordset
            RSTTRXFILE.Open "Select * FROM TRXSUB WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='HI' AND VCH_NO = " & Val(txtBillNo.text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(grdsales.Row, 32)) & "", db, adOpenStatic, adLockOptimistic, adCmdText
            If (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
                RSTTRXFILE.AddNew
                RSTTRXFILE!VCH_NO = Val(txtBillNo.text)
                RSTTRXFILE!TRX_TYPE = "HI"
