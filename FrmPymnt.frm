...
Begin VB.Form FRMPYMNTLENDER 
   ...
   End
End
Attribute VB_Name = "FRMPYMNTLENDER"
...
Private Sub CmdSave_Click()
    ...
    rstBILL.Open "Select MAX(CR_NO) From DBTPYMT WHERE TRX_TYPE = 'PL'", db, adOpenForwardOnly
    ...
    'db.Execute "delete FROM CASHATRXFILE WHERE INV_NO = " & Val(txtBillNo.Text) & " AND INV_TYPE = 'PL'"
    ...
    'db.Execute "delete FROM CASHATRXFILE WHERE INV_NO = " & Val(creditbill.LBLBILLNO.Caption) & " AND INV_TYPE = 'PL'"
    If OptCash.value = True Then
        Set RSTITEMMAST = New ADODB.Recordset
        RSTITEMMAST.Open "SELECT * FROM CASHATRXFILE", db, adOpenStatic, adLockOptimistic, adCmdText
        RSTITEMMAST.AddNew
        RSTITEMMAST!REC_NO = i
        RSTITEMMAST!INV_TYPE = "PL"
        RSTITEMMAST!INV_TRX_TYPE = "PL"
    ...
    If OptBank.value = True Then
        ...
        TRX_NO = 1
        Set rstBILL = New ADODB.Recordset
        rstBILL.Open "Select MAX(TRX_NO) From BANK_TRX WHERE TRX_TYPE = 'DR' AND BILL_TRX_TYPE = 'ML' AND TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' ", db, adOpenForwardOnly
        If Not (rstBILL.EOF And rstBILL.BOF) Then
    ...
    If OptCash.value = True Then
        RSTTRXFILE!C_TRX_TYPE = "CR"
        RSTTRXFILE!C_REC_NO = i
        RSTTRXFILE!C_INV_TRX_TYPE = "PL"
        RSTTRXFILE!C_INV_TYPE = "PL"
        RSTTRXFILE!C_INV_NO = Val(txtBillNo.Text)
    Else
        RSTTRXFILE!B_TRX_NO = TRX_NO
        RSTTRXFILE!B_TRX_TYPE = "DR"
        RSTTRXFILE!B_TRX_YEAR = Year(MDIMAIN.DTFROM.value)
        RSTTRXFILE!B_BILL_TRX_TYPE = "ML"
    End If
    'RSTTRXFILE!INV_DATE = Format(lblinvdate.Caption, "DD/MM/YYYY")
    RSTTRXFILE.Update
    RSTTRXFILE.Close
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    FRMLendReg.Enabled = True
    FRMLendReg.GRDTranx.SetFocus
End Sub
