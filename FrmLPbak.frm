...
Private Sub CMDADD_Click()
    ...
    RSTRTRXFILE.Open "SELECT * From RTRXFILE WHERE TRX_TYPE='PI' AND VCH_NO = " & Val(txtBillNo.Text) & " AND ITEM_CODE='" & Trim(grdsales.TextMatrix(Val(TXTSLNO.Text), 1)) & "'AND LINE_NO=" & Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 16)) & "", db, adOpenStatic, adLockOptimistic, adCmdText
...
Private Sub CmdDelete_Click()
    ...
    grdsales.Rows = 1
    Set RSTRTRXFILE = New ADODB.Recordset
    RSTRTRXFILE.Open "Select * From RTRXFILE WHERE TRX_TYPE='PI' AND VCH_NO = " & Val(txtBillNo.Text) & " ORDER BY [LINE_NO]", db, adOpenStatic, adLockReadOnly
    Do Until RSTRTRXFILE.EOF
...
Private Sub Form_Load()
    ...
    TRXMAST.Open "Select MAX(Val(VCH_NO)) From RTRXFILE WHERE TRX_TYPE = 'PI'", db, adOpenStatic, adLockReadOnly
        ...
        Case vbKeyReturn
            ...
            If Not (RSTRXFILE.EOF And RSTRXFILE.BOF) Then
                ...
                If IsNull(RSTRXFILE!P_RETAIL) Then
                    txtretail.Text = ""
                Else
                    txtretail.Text = Format(Round(Val(RSTRXFILE!P_RETAIL) * Val(Txtpack.Text), 2), ".000")
                End If
...
Private Sub TXTBILLNO_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            rstTRXMAST.Open "Select * From RTRXFILE WHERE TRX_TYPE='PI' AND VCH_NO = " & Val(txtBillNo.Text) & " ORDER BY [LINE_NO]", db, adOpenStatic, adLockReadOnly
            ...
            Set rstTRXMAST = New ADODB.Recordset
            rstTRXMAST.Open "Select * From TRANSMAST WHERE TRX_TYPE='PI' AND VCH_NO = " & Val(txtBillNo.Text) & "", db, adOpenStatic, adLockReadOnly
            ...
            If PHY.RecordCount = 1 Then
                ...
                If Not (RSTRXFILE.EOF And RSTRXFILE.BOF) Then
                    ...
                    If IsNull(RSTRXFILE!PTR) Then
                        TXTPTR.Text = ""
                    Else
                        TXTPTR.Text = Format(Round(Val(RSTRXFILE!PTR) * Val(Txtpack.Text), 2), ".000")
                    End If
                    If IsNull(RSTRXFILE!P_RETAIL) Then
                        txtretail.Text = ""
                    Else
...
Public Sub appendpurchase()
    ...
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "SELECT * from [RTRXFILE] WHERE TRX_TYPE='PI' AND VCH_NO = " & Val(txtBillNo.Text) & "", db, adOpenStatic, adLockOptimistic, adCmdText
...
SKIP:
    Set rstMaxNo = New ADODB.Recordset
    rstMaxNo.Open "Select MAX(Val(VCH_NO)) From RTRXFILE WHERE TRX_TYPE = 'PI'", db, adOpenStatic, adLockReadOnly
    If Not (rstMaxNo.EOF And rstMaxNo.BOF) Then
