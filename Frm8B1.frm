...
Begin VB.Form FRMESTIMATE 
   ...
   Caption         =   "FORM 8B - BILL ENTRY"
   ClientHeight    =   11100
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   17580
   Icon            =   "Frm8B1.frx":0000
   KeyPreview      =   -1  'True
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin VB.Frame FRMEMAIN 
      ...
      Begin VB.Frame FRMEHEAD 
         ...
         Begin VB.ComboBox cmbtype 
            ...
            ForeColor       =   &H00004080&
            Height          =   315
            ItemData        =   "Frm8B1.frx":030A
            Left            =   7455
            List            =   "Frm8B1.frx":0317
...
Private Sub Command1_Click()
    ...
    ElseIf Figre > 0 Then
        LBLFOT.Tag = "(Rupees " & Words_1_all(Num) & " Only)"
    End If
    ReportNameVar = Rptpath & "RPTcommission"
...
Public Function Generateprint()
    ...
    If MDIMAIN.StatusBar.Panels(8).Text = "Y" Then
        If DataList2.BoundText <> "130000" And DataList2.BoundText <> "130001" Then
            ...
            Set RSTTRXFILE = New ADODB.Recordset
            RSTTRXFILE.Open "Select * From DBTPYMT WHERE ACT_CODE = '" & DataList2.BoundText & "' AND NOT(INV_TRX_TYPE = 'RI' AND TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND INV_NO = " & Val(LBLBILLNO.Caption) & ") AND (TRX_TYPE = 'DR' OR TRX_TYPE = 'DB') AND INV_DATE >= '" & Format(TXTINVDATE.Text, "yyyy/mm/dd") & "' ", db, adOpenStatic, adLockReadOnly, adCmdText
            Do Until RSTTRXFILE.EOF
                Select Case RSTTRXFILE!TRX_TYPE
                    Case "DB"
...
'        Set RSTITEMMAST = Nothing
    ...
    If Val(MDIMAIN.StatusBar.Panels(10).Text) = 1 Then
        If Small_Print = True Then
            'ReportNameVar = Rptpath & "rptbillretail"
            ReportNameVar = Rptpath & "RPTBILLA51"
        Else
            ReportNameVar = Rptpath & "rptbillretail1"
        End If
        Set Report = crxApplication.OpenReport(ReportNameVar, 1)
        Set CRXFormulaFields = Report.FormulaFields
    ...
    Else
        If Small_Print = True Then
            'ReportNameVar = Rptpath & "rptbillretail"
            ReportNameVar = Rptpath & "RPTBILLA5"
        Else
            ReportNameVar = Rptpath & "rptbillretail"
        End If
        Set Report = crxApplication.OpenReport(ReportNameVar, 1)
