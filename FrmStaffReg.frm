...
Begin VB.Form FRMStaffReg 
   ...
   Begin VB.Frame FRMEMAIN 
      ...
      Begin VB.CommandButton Command1 
         Caption         =   "Print Ledger for All Employees"
         BeginProperty Font 
            Name            =   "Arial"
            Size            =   8.25
            Charset         =   204
      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.ComboBox CMBYesNo 
            ...
            EndProperty
            Height          =   465
            ItemData        =   "FrmStaffReg.frx":098E
            Left            =   0
            List            =   "FrmStaffReg.frx":0998
      ...
      Begin VB.Label Label1 
         BackStyle       =   0  'Transparent
         Caption         =   "Press F7 to Salary Entry"
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   9.75
            Charset         =   0
...
Private Sub cdmdrcr_Click()
    ...
    Enabled = False
    FRMDRCR3.LBLSUPPLIER.Caption = DataList2.text
    FRMDRCR3.lblactcode.Caption = DataList2.BoundText
    'FRMPYMNTSALARY.lblinvdate.Caption = Format(GRDTranx.TextMatrix(GRDTranx.Row, 2), "DD/MM/YYYY")
    'FRMPYMNTSALARY.lblinvno.Caption = GRDTranx.TextMatrix(GRDTranx.Row, 3)
    'FRMPYMNTSALARY.lblbillamt.Caption = GRDTranx.TextMatrix(GRDTranx.Row, 4)
    'FRMPYMNTSALARY.lblrcvdamt.Caption = GRDTranx.TextMatrix(GRDTranx.Row, 5)
    'FRMPYMNTSALARY.lblbalamt.Caption = GRDTranx.TextMatrix(GRDTranx.Row, 6)
    FRMDRCR3.Show
End Sub
...
Private Sub CMBYesNo_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            'rstTRXMAST.Open "SELECT * From STAFFPYMT WHERE ACT_CODE = '" & DataList2.BoundText & "' AND (TRX_TYPE = 'PY' OR TRX_TYPE = 'CR' OR TRX_TYPE = 'PR') ORDER BY INV_DATE DESC", db, adOpenForwardOnly
            db.BeginTrans
...
Private Sub CmdPrint_Click()
    ...
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * from STAFFPYMT Where ACT_CODE ='" & DataList2.BoundText & "' and (TRX_TYPE ='EX' OR TRX_TYPE ='PR' OR TRX_TYPE ='CB' OR TRX_TYPE ='DB' OR TRX_TYPE ='CR') and INV_DATE < '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' ", db, adOpenStatic, adLockReadOnly, adCmdText
    ...
    Report.RecordSelectionFormula = "({DBTPYMT.ACT_CODE}='" & DataList2.BoundText & "' and ({DBTPYMT.TRX_TYPE} ='DB' OR {DBTPYMT.TRX_TYPE} ='CB' OR {DBTPYMT.TRX_TYPE} ='EX' OR {DBTPYMT.TRX_TYPE} ='PR' OR {DBTPYMT.TRX_TYPE} ='CR') AND {DBTPYMT.INV_DATE} <=# " & Format(DTTo.Value, "MM,DD,YYYY") & " # AND {DBTPYMT.INV_DATE} >=# " & Format(DTFROM.Value, "MM,DD,YYYY") & " #)"
    ...
    If UCase(dbase1) <> "INVSOFT" And UCase(dbase1) <> "INVDB" And UCase(dbase1) <> "INVSOFT3" Then
        ...
        Set oRs = db.Execute("SELECT * FROM STAFFPYMT ")
...
Private Sub CmdPrnRcpt_Click()
    ...
    If GRDTranx.TextMatrix(GRDTranx.Row, 0) = "Payment" Then
        ...
        Set Report = crxApplication.OpenReport(ReportNameVar, 1)
        Report.RecordSelectionFormula = "({CRDTPYMT.TRX_TYPE} ='EX' AND {CRDTPYMT.CR_NO} = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " AND {CRDTPYMT.ACT_CODE}='" & DataList2.BoundText & "')"
    ElseIf GRDTranx.TextMatrix(GRDTranx.Row, 0) = "Debit Note" Then
        ReportNameVar = Rptpath & "RptDNP"
        Set Report = crxApplication.OpenReport(ReportNameVar, 1)
        Report.RecordSelectionFormula = "({CRDTPYMT.TRX_TYPE} ='DN' AND {CRDTPYMT.CR_NO} = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " AND {CRDTPYMT.ACT_CODE}='" & DataList2.BoundText & "')"
    ElseIf GRDTranx.TextMatrix(GRDTranx.Row, 0) = "Credit Note" Then
        ReportNameVar = Rptpath & "RptCNP"
        Set Report = crxApplication.OpenReport(ReportNameVar, 1)
        Report.RecordSelectionFormula = "({CRDTPYMT.TRX_TYPE} ='CN' AND {CRDTPYMT.CR_NO} = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " AND {CRDTPYMT.ACT_CODE}='" & DataList2.BoundText & "')"
    Else
        Screen.MousePointer = vbNormal
        Exit Sub
    End If
...
Private Sub CmdPymnt_Click()
    If DataList2.BoundText = "" Then Exit Sub
    Me.Enabled = False
    FRMPYMNTSALARY.LBLSUPPLIER.Caption = DataList2.text
    FRMPYMNTSALARY.lblactcode.Caption = DataList2.BoundText
    'FRMPYMNTSALARY.lblinvdate.Caption = Format(GRDTranx.TextMatrix(GRDTranx.Row, 2), "DD/MM/YYYY")
    'FRMPYMNTSALARY.lblinvno.Caption = GRDTranx.TextMatrix(GRDTranx.Row, 3)
    'FRMPYMNTSALARY.lblbillamt.Caption = GRDTranx.TextMatrix(GRDTranx.Row, 4)
    'FRMPYMNTSALARY.lblrcvdamt.Caption = GRDTranx.TextMatrix(GRDTranx.Row, 5)
...
Private Sub Command1_Click()
    ...
    rstCustomer.Open "SELECT * FROM ACTMAST WHERE (Mid(ACT_CODE, 1, 3)='321')And (LENGTH(ACT_CODE)>3) ORDER BY ACT_NAME", db, adOpenStatic, adLockOptimistic, adCmdText
    Do Until rstCustomer.EOF
        ...
        Set RSTTRXFILE = New ADODB.Recordset
        RSTTRXFILE.Open "Select * from STAFFPYMT Where ACT_CODE ='" & rstCustomer!ACT_CODE & "' and (TRX_TYPE ='EX' OR TRX_TYPE ='PR' OR TRX_TYPE ='CR') and INV_DATE < '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' ", db, adOpenStatic, adLockReadOnly, adCmdText
        Do Until RSTTRXFILE.EOF
            Select Case RSTTRXFILE!TRX_TYPE
                Case "CR"
                    OP_Sale = OP_Sale + IIf(IsNull(RSTTRXFILE!INV_AMT), 0, RSTTRXFILE!INV_AMT)
        ...
        BAL_AMOUNT = 0
        Set RSTTRXFILE = New ADODB.Recordset
        RSTTRXFILE.Open "Select * from STAFFPYMT Where ACT_CODE ='" & rstCustomer!ACT_CODE & "' and (TRX_TYPE ='EX' OR TRX_TYPE ='PR' OR TRX_TYPE ='CR') AND INV_DATE <='" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND INV_DATE >='" & Format(DTFROM.Value, "yyyy/mm/dd") & "' ORDER BY INV_DATE, TRX_TYPE, INV_NO", db, adOpenStatic, adLockOptimistic, adCmdText
        Do Until RSTTRXFILE.EOF
            'RSTTRXFILE!BAL_AMT = Op_Bal
            Select Case RSTTRXFILE!TRX_TYPE
                Case "CR"
    ...
    End If
    Set Report = crxApplication.OpenReport(ReportNameVar, 1)
    Report.RecordSelectionFormula = "(({DBTPYMT.TRX_TYPE} ='EX' OR {DBTPYMT.TRX_TYPE} ='PR' OR {DBTPYMT.TRX_TYPE} ='CR') AND {DBTPYMT.INV_DATE} <=# " & Format(DTTo.Value, "MM,DD,YYYY") & " # AND {DBTPYMT.INV_DATE} >=# " & Format(DTFROM.Value, "MM,DD,YYYY") & " #)"
    Set CRXFormulaFields = Report.FormulaFields
...
Private Sub Form_Load()
    ...
    GRDTranx.TextMatrix(0, 3) = "VCH NO."
    GRDTranx.TextMatrix(0, 4) = "SALARY AMT"
    ...
    GRDTranx.ColWidth(20) = 0 '1100
    GRDTranx.ColWidth(21) = 1100
    GRDTranx.ColWidth(22) = 1100
    GRDTranx.ColWidth(23) = 800
    GRDTranx.ColWidth(24) = 800
...
Private Sub GRDTranx_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            On Error GoTo ERRHAND
            If GRDTranx.rows = 1 Then Exit Sub
            If GRDTranx.TextMatrix(GRDTranx.Row, 0) <> "Salary" Then Exit Sub
            If GRDTranx.TextMatrix(GRDTranx.Row, 0) = "Salary" Then
                grdreceipts.rows = 1
                i = 0
...
'                        Set rstTRANX2 = Nothing
        ...
        Case 113
            ...
            Select Case GRDTranx.Col
                 Case 2
                    If Not (GRDTranx.TextMatrix(GRDTranx.Row, 0) = "Payment" Or GRDTranx.TextMatrix(GRDTranx.Row, 0) = "Debit Note" Or GRDTranx.TextMatrix(GRDTranx.Row, 0) = "Credit Note") Then Exit Sub
                ...
                Case 5
                    ...
                    TXTsample.SetFocus
                Case 4
                    If Not (GRDTranx.TextMatrix(GRDTranx.Row, 0) = "Credit Note") Then Exit Sub
                    TXTsample.Visible = True
                    TXTsample.Top = GRDTranx.CellTop '+ 120
                    TXTsample.Left = GRDTranx.CellLeft '+ 20
                    TXTsample.Width = GRDTranx.CellWidth
            ...
            If GRDTranx.TextMatrix(GRDTranx.Row, 0) = "Salary" Then
                If Val(CMDDISPLAY.Tag) = 68 Or Val(CMDDISPLAY.Tag) = 100 Or Val(CMDEXIT.Tag) = 69 Or Val(CMDEXIT.Tag) = 101 Then
                    If MsgBox("Are you sure you want to delete this entry", vbYesNo, "DELETE !!!") = vbYes Then
                        db.BeginTrans
                        db.Execute "delete From STAFFPYMT WHERE TRX_TYPE='" & GRDTranx.TextMatrix(GRDTranx.Row, 19) & "' AND CR_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " AND INV_TRX_TYPE = '" & GRDTranx.TextMatrix(GRDTranx.Row, 8) & "'"
'                        db.Execute "delete FROM CASHATRXFILE WHERE INV_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " AND INV_TYPE = 'EX' AND INV_TRX_TYPE = 'PY' AND TRX_TYPE = 'DR' AND TRX_YEAR = '" & GRDTranx.TextMatrix(GRDTranx.Row, 18) & "'"
'                        db.Execute "delete FROM BANK_TRX WHERE TRX_TYPE = '" & GRDTranx.TextMatrix(GRDTranx.Row, 9) & "' AND TRX_NO = " & Val(GRDTranx.TextMatrix(GRDTranx.Row, 10)) & " AND BILL_TRX_TYPE = '" & GRDTranx.TextMatrix(GRDTranx.Row, 11) & "' AND TRX_YEAR = '" & GRDTranx.TextMatrix(GRDTranx.Row, 12) & "' AND BANK_CODE = '" & GRDTranx.TextMatrix(GRDTranx.Row, 13) & "' "
                        db.Execute "delete From trnxrcpt WHERE TRX_TYPE='EX' AND CR_TRX_TYPE= 'CR' AND CR_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " "
                        db.CommitTrans
                        Call Fillgrid
                    Else
                        GRDTranx.SetFocus
                    End If
                End If
            ElseIf GRDTranx.TextMatrix(GRDTranx.Row, 0) = "Payment" Then
                If Val(CMDDISPLAY.Tag) = 68 Or Val(CMDDISPLAY.Tag) = 100 Or Val(CMDEXIT.Tag) = 69 Or Val(CMDEXIT.Tag) = 101 Then
                    If MsgBox("Are you sure you want to delete this entry", vbYesNo, "DELETE !!!") = vbYes Then
                        db.BeginTrans
                        db.Execute "delete From STAFFPYMT WHERE TRX_TYPE='" & GRDTranx.TextMatrix(GRDTranx.Row, 19) & "' AND CR_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " AND INV_TRX_TYPE = '" & GRDTranx.TextMatrix(GRDTranx.Row, 8) & "' "
                        db.Execute "delete FROM CASHATRXFILE WHERE INV_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " AND INV_TYPE = '" & GRDTranx.TextMatrix(GRDTranx.Row, 19) & "' AND INV_TRX_TYPE = '" & GRDTranx.TextMatrix(GRDTranx.Row, 8) & "' AND TRX_TYPE = 'DR' AND TRX_YEAR = '" & GRDTranx.TextMatrix(GRDTranx.Row, 18) & "'"
...
'                    If MsgBox("Are you sure you want to delete this entry", vbYesNo, "DELETE !!!") = vbYes Then
'                        db.BeginTrans
'                        db.Execute "delete From STAFFPYMT WHERE TRX_TYPE='CB' AND CR_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " AND INV_TRX_TYPE = 'CN'"
'                        db.Execute "delete FROM CASHATRXFILE WHERE INV_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " AND INV_TYPE = 'EX' AND INV_TRX_TYPE = 'CN' AND TRX_TYPE = 'CT' AND TRX_YEAR = '" & GRDTranx.TextMatrix(GRDTranx.Row, 18) & "'"
'                        If Not GRDTranx.TextMatrix(GRDTranx.Row, 10) = "" Then
...
Private Sub TXTDEALER_Change()
    ...
    If flagchange.Caption <> "1" Then
        If ACT_FLAG = True Then
            ACT_REC.Open "select ACT_CODE, ACT_NAME from ACTMAST  WHERE (Mid(ACT_CODE, 1, 3)='321')And (LENGTH(ACT_CODE)>3) And ACT_NAME Like '" & TXTDEALER.text & "%'ORDER BY ACT_NAME", db, adOpenForwardOnly
            ACT_FLAG = False
        Else
            ACT_REC.Close
            ACT_REC.Open "select ACT_CODE, ACT_NAME from ACTMAST  WHERE (Mid(ACT_CODE, 1, 3)='321')And (LENGTH(ACT_CODE)>3) And ACT_NAME Like '" & TXTDEALER.text & "%'ORDER BY ACT_NAME", db, adOpenForwardOnly
...
Private Function Fillgrid()
    ...
    rstTRANX.Open "SELECT * From STAFFPYMT WHERE ACT_CODE = '" & DataList2.BoundText & "' AND (TRX_TYPE = 'CB' OR TRX_TYPE = 'DB' OR TRX_TYPE = 'EX' OR TRX_TYPE = 'CR' OR TRX_TYPE = 'PR') ORDER BY INV_DATE, CR_NO", db, adOpenForwardOnly
    Do Until rstTRANX.EOF
        ...
        Select Case rstTRANX!TRX_TYPE
            Case "CR"
                ...
                GRDTranx.TextMatrix(i, 0) = "Salary"
                GRDTranx.CellForeColor = vbRed
                Set rstTRXMAST = New ADODB.Recordset
                rstTRXMAST.Open "select SUM(RCPT_AMOUNT) from trnxrcpt WHERE TRX_TYPE = 'EX' AND ACT_CODE = '" & rstTRANX!ACT_CODE & "' AND INV_NO  = " & rstTRANX!INV_NO & " AND INV_TRX_TYPE = '" & rstTRANX!INV_TRX_TYPE & "' AND INV_TRX_YEAR = '" & rstTRANX!TRX_YEAR & "' ", db, adOpenStatic, adLockReadOnly, adCmdText
                If Not (rstTRXMAST.EOF And rstTRXMAST.BOF) Then
        ...
        If GRDTranx.TextMatrix(i, 0) = "Salary" And Val(GRDTranx.TextMatrix(i, 22)) <= 0 Then
            ...
            Select Case GRDTranx.Col
                Case 2
                    ...
                    If GRDTranx.TextMatrix(GRDTranx.Row, 0) = "Payment" Then
                        ...
                        rststock.Open "SELECT * From STAFFPYMT WHERE TRX_TYPE='EX' AND CR_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " ", db, adOpenStatic, adLockOptimistic, adCmdText
                        ...
                        Set rststock = New ADODB.Recordset
                        rststock.Open "SELECT * From TRNXRCPT WHERE TRX_TYPE='EX' AND CR_TRX_TYPE= 'CR' AND CR_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " ", db, adOpenStatic, adLockOptimistic, adCmdText
                        If Not (rststock.EOF And rststock.BOF) Then
                            rststock!RCPT_DATE = Format(TXTEXPIRY.text, "dd/mm/yyyy")
                            rststock.Update
                        End If
                        ...
                        Set rststock = New ADODB.Recordset
                        rststock.Open "SELECT * FROM CASHATRXFILE WHERE INV_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " AND INV_TYPE = 'EX' AND INV_TRX_TYPE = 'PY' AND TRX_TYPE = 'DR' AND TRX_YEAR = '" & GRDTranx.TextMatrix(GRDTranx.Row, 18) & "'", db, adOpenStatic, adLockOptimistic, adCmdText
                        If Not (rststock.EOF And rststock.BOF) Then
                    ...
                    ElseIf GRDTranx.TextMatrix(GRDTranx.Row, 0) = "Debit Note" Then
                        Set rststock = New ADODB.Recordset
                        rststock.Open "SELECT * FROM STAFFPYMT WHERE TRX_TYPE='DB' AND CR_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " AND INV_TRX_TYPE = 'DN' ", db, adOpenStatic, adLockOptimistic, adCmdText
                        If Not (rststock.EOF And rststock.BOF) Then
                            rststock!RCPT_DATE = Format(TXTEXPIRY.text, "dd/mm/yyyy")
                            rststock.Update
                        End If
                        ...
                        Set rststock = New ADODB.Recordset
                        rststock.Open "SELECT * FROM CASHATRXFILE WHERE INV_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " AND INV_TYPE = 'EX' AND INV_TRX_TYPE = 'DN' AND TRX_TYPE = 'CT' AND TRX_YEAR = '" & GRDTranx.TextMatrix(GRDTranx.Row, 18) & "'", db, adOpenStatic, adLockOptimistic, adCmdText
                        If Not (rststock.EOF And rststock.BOF) Then
                            rststock!VCH_DATE = Format(TXTEXPIRY.text, "dd/mm/yyyy")
                            rststock.Update
                        End If
                    ...
                    ElseIf GRDTranx.TextMatrix(GRDTranx.Row, 0) = "Credit Note" Then
                        Set rststock = New ADODB.Recordset
                        rststock.Open "SELECT * FROM STAFFPYMT WHERE TRX_TYPE='CB' AND CR_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " AND INV_TRX_TYPE = 'CN'", db, adOpenStatic, adLockOptimistic, adCmdText
                        If Not (rststock.EOF And rststock.BOF) Then
                            rststock!RCPT_DATE = Format(TXTEXPIRY.text, "dd/mm/yyyy")
                            rststock.Update
                        End If
                        ...
                        Set rststock = New ADODB.Recordset
                        rststock.Open "SELECT * FROM CASHATRXFILE WHERE INV_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " AND INV_TYPE = 'EX' AND INV_TRX_TYPE = 'CN' AND TRX_TYPE = 'CT' AND TRX_YEAR = '" & GRDTranx.TextMatrix(GRDTranx.Row, 18) & "'", db, adOpenStatic, adLockOptimistic, adCmdText
                        If Not (rststock.EOF And rststock.BOF) Then
                            rststock!VCH_DATE = Format(TXTEXPIRY.text, "dd/mm/yyyy")
                            rststock.Update
                        End If
        ...
        Case vbKeyReturn
            Select Case GRDTranx.Col
                Case 5
                    ...
                    If GRDTranx.TextMatrix(GRDTranx.Row, 0) = "Payment" Then
                        db.Execute "Update STAFFPYMT SET RCPT_AMOUNT = " & Val(TXTsample.text) & " WHERE TRX_TYPE='EX' AND CR_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " "
                        db.Execute "Update TRNXRCPT SET RCPT_AMOUNT = " & Val(TXTsample.text) & " WHERE TRX_TYPE='EX' AND CR_TRX_TYPE= 'CR' AND CR_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " "
                        db.Execute "Update CASHATRXFILE SET AMOUNT = " & Val(TXTsample.text) & " WHERE INV_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " AND INV_TYPE = 'EX' AND INV_TRX_TYPE = 'PY' AND TRX_TYPE = 'DR' AND TRX_YEAR = '" & GRDTranx.TextMatrix(GRDTranx.Row, 18) & "'"
                    ...
                    ElseIf GRDTranx.TextMatrix(GRDTranx.Row, 0) = "Debit Note" Then
                        ...
                        db.Execute "Update STAFFPYMT SET RCPT_AMOUNT = " & Val(TXTsample.text) & " WHERE TRX_TYPE='DB' AND CR_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " AND INV_TRX_TYPE = 'DN' "
                        db.Execute "Update CASHATRXFILE SET AMOUNT = " & Val(TXTsample.text) & " WHERE INV_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " AND INV_TYPE = 'EX' AND INV_TRX_TYPE = 'DN' AND TRX_TYPE = 'CT' AND TRX_YEAR = '" & GRDTranx.TextMatrix(GRDTranx.Row, 18) & "'"
                        If Not GRDTranx.TextMatrix(GRDTranx.Row, 10) = "" Then
                    ...
                    Exit Sub
                Case 4
                    If GRDTranx.TextMatrix(GRDTranx.Row, 0) = "Credit Note" Then
                        db.Execute "Update STAFFPYMT SET INV_AMT = " & Val(TXTsample.text) & " WHERE TRX_TYPE='CB' AND CR_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " AND INV_TRX_TYPE = 'CN'"
                        db.Execute "Update CASHATRXFILE SET AMOUNT = " & Val(TXTsample.text) & " WHERE INV_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " AND INV_TYPE = 'EX' AND INV_TRX_TYPE = 'CN' AND TRX_TYPE = 'CT' AND TRX_YEAR = '" & GRDTranx.TextMatrix(GRDTranx.Row, 18) & "'"
                        If Not GRDTranx.TextMatrix(GRDTranx.Row, 10) = "" Then
                ...
                Case 6  ' Ref No
                    ...
                    db.Execute "Update STAFFPYMT set REF_NO = '" & Trim(TXTsample.text) & "' where CR_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & "  and TRX_TYPE = '" & GRDTranx.TextMatrix(GRDTranx.Row, 19) & "' "
...
Private Sub GRDTranx_Click()
    ...
    With GRDTranx
        If .TextMatrix(.Row, 0) = "Salary" Then
            oldx = .Col
            oldy = .Row
...
Private Sub CmdPay_Click()
    If MsgBox("Are you sure you want to make the selected Salary as Paid", vbYesNo + vbDefaultButton2, "Payment Entry") = vbNo Then Exit Sub
    
    Dim i As Long
    Dim RSTTRXFILE As ADODB.Recordset
    On Error GoTo ERRHAND
    For i = 0 To grdcount.rows - 1
        db.Execute "UPDATE STAFFPYMT SET PAID_FLAG = 'Y' WHERE TRX_TYPE='CB' AND CR_NO = " & grdcount.TextMatrix(i, 7) & " AND INV_TRX_TYPE = '" & grdcount.TextMatrix(i, 8) & "' AND TRX_YEAR= '" & grdcount.TextMatrix(i, 18) & "'"
...
Private Sub Cmdremove_Click()
    If MsgBox("Are you sure you want to make the selected Salary as Not Paid", vbYesNo + vbDefaultButton2, "Payment Entry") = vbNo Then Exit Sub
    Dim i As Long
    Dim RSTTRXFILE As ADODB.Recordset
    On Error GoTo ERRHAND
    For i = 0 To grdcount.rows - 1
        'db.Execute "UPDATE STAFFPYMT SET PAID_FLAG = 'N' WHERE TRX_TYPE='CR' AND CR_NO = " & grdcount.TextMatrix(i, 7) & " AND INV_TRX_TYPE = '" & grdcount.TextMatrix(i, 8) & "' AND TRX_YEAR= '" & grdcount.TextMatrix(i, 18) & "'"
        db.Execute "UPDATE STAFFPYMT SET PAID_FLAG = 'N' WHERE TRX_TYPE='CB' AND CR_NO = " & grdcount.TextMatrix(i, 7) & " AND INV_TRX_TYPE = '" & grdcount.TextMatrix(i, 8) & "' AND TRX_YEAR= '" & grdcount.TextMatrix(i, 18) & "'"
