...
Begin VB.Form frmSTKTRANSFER 
   ...
   Caption         =   "STOCK TRANSEFR FROM HO/ BRANCH "
   ClientHeight    =   10995
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   18645
   ControlBox      =   0   'False
   Icon            =   "FrmSTKTRANSF.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin VB.Frame Fram 
      ...
      Begin VB.Frame FRMECONTROLS 
         ...
         Begin VB.ComboBox cmbfull 
            ...
            ForeColor       =   &H000000FF&
            Height          =   360
            ItemData        =   "FrmSTKTRANSF.frx":030A
            Left            =   7500
            List            =   "FrmSTKTRANSF.frx":034A
         ...
         Begin VB.ComboBox CmbWrnty 
            ...
            ForeColor       =   &H000000FF&
            Height          =   360
            ItemData        =   "FrmSTKTRANSF.frx":03BF
            Left            =   12360
            List            =   "FrmSTKTRANSF.frx":03C9
            Style           =   2  'Dropdown List
            TabIndex        =   37
            Top             =   1155
            Width           =   840
         ...
         Begin VB.ComboBox CmbPack 
            ...
            ForeColor       =   &H000000FF&
            Height          =   360
            ItemData        =   "FrmSTKTRANSF.frx":03DA
            Left            =   9585
            List            =   "FrmSTKTRANSF.frx":041D
            Style           =   2  'Dropdown List
            TabIndex        =   8
            Top             =   495
            Width           =   780
...
'            End If
    ...
    If OLD_BILL = False And Val(txtBillNo.text) <> 1 Then
        RSTTRXFILE.Open "Select * From TRANSMAST WHERE VCH_NO= (SELECT MAX(VCH_NO) FROM TRANSMAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE = 'TF')", db, adOpenStatic, adLockOptimistic, adCmdText
...
'                Else
    ...
    'RSTRTRXFILE!VCH_DESC = "Received From TF Stock"
...
Private Sub Command5_Click()
    ...
    rstBILL.Open "Select MAX(VCH_NO) From TRANSMAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE = 'TF'", db, adOpenStatic, adLockReadOnly
...
Private Sub grdsales_KeyDown(KeyCode As Integer, Shift As Integer)
    Select Case KeyCode
        ...
        Case vbKeyDelete
            ...
            Screen.MousePointer = vbHourglass
            On Error GoTo ERRHAND
            db.Execute "delete  From RTRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='TF' AND VCH_NO = " & Val(txtBillNo.text) & " AND ITEM_CODE='" & Trim(grdsales.TextMatrix(grdsales.Row, 1)) & "' AND LINE_NO=" & Val(grdsales.TextMatrix(grdsales.Row, 16)) & ""
            Set rststock = New ADODB.Recordset
...
'            RSTTRNSMAST.Open "Select CHECK_FLAG From TRANSMAST WHERE TRX_TYPE='TF' AND VCH_NO = " & Val(txtBillNo.Text) & "", db, adOpenStatic, adLockReadOnly
...
Public Sub appendpurchase()
    ...
    Do Until RSTTRXFILE.EOF
        ...
        RSTTRXFILE!VCH_DESC = "Received From TF Stock"
        RSTTRXFILE!PINV = ""
        RSTTRXFILE!TRX_GODOWN = Trim(CMBDISTRICT.text)
        RSTTRXFILE!M_USER_ID = ""
        RSTTRXFILE.Update
...
Private Sub txtcramt_LostFocus()
    ...
    Exit Sub
ERRHAND:
    MsgBox "Please enter a Numeric Value", , "EzBiz"
    txtcramt.SetFocus
End Sub
...
Private Function find_small_number()
    ...
    For i = 1 To grdsales.rows - 1
        ...
        lblqty.Caption = Format(Val(lblqty.Caption) + Val(grdsales.TextMatrix(i, 3)), ".00")
        db.Execute "Update RTRXFILE set ITEM_NET_COST_PRICE = " & NETCOST & ", EXPENSE = " & Val(grdsales.TextMatrix(i, 32)) & " WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='TF' AND VCH_NO = " & Val(txtBillNo.text) & " AND ITEM_CODE='" & Trim(grdsales.TextMatrix(i, 1)) & "'AND LINE_NO=" & Val(grdsales.TextMatrix(i, 16)) & "  "
        'db.Execute "Update RTRXFILE set EXPENSE = " & Val(grdsales.TextMatrix(i, 32)) & " WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='TF' AND VCH_NO = " & Val(txtBillNo.text) & " AND ITEM_CODE='" & Trim(grdsales.TextMatrix(i, 1)) & "'AND LINE_NO=" & Val(grdsales.TextMatrix(i, 16)) & "  "
