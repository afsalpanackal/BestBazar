   ...
   Begin VB.Frame FRMEMAIN 
      ...
      Begin VB.Frame Frmeperiod 
         ...
         Begin VB.Frame FrmePetty 
            ...
            Visible         =   0   'False
            Width           =   2520
            Begin VB.CheckBox chkPetty 
               BackColor       =   &H00C0C0FF&
               Caption         =   "Include Petty"
               Height          =   300
               Left            =   75
               TabIndex        =   37
               Top             =   195
...
Private Sub CmDDisplay_Click()
    ...
    Else
        RSTTRXFILE.Open "select SUM(AMOUNT) FROM CASHATRXFILE WHERE INV_TRX_TYPE <> 'WO' AND VCH_DATE < '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND CHECK_FLAG='P'", db, adOpenStatic, adLockReadOnly, adCmdText
    End If
    If Not (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
    ...
    Else
        RSTTRXFILE.Open "SELECT *  FROM CASHATRXFILE WHERE INV_TRX_TYPE <> 'WO' AND AMOUNT <> 0 AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' ORDER BY VCH_DATE DESC", db, adOpenStatic, adLockReadOnly, adCmdText
    End If
...
Private Sub CMDREGISTER_Click()
    ...
    Else
        Report.RecordSelectionFormula = "({CASHATRXFILE.INV_TRX_TYPE} <> 'WO' AND {CASHATRXFILE.VCH_DATE}<=# " & Format(DTTo.Value, "MM,DD,YYYY") & " #) AND ({CASHATRXFILE.VCH_DATE} >=# " & Format(DTFROM.Value, "MM,DD,YYYY") & " # )"
    End If
...
Private Sub lblcloscash_DblClick()
    If FrmePetty.Visible = True Then
        FrmePetty.Visible = False
        chkPetty.Value = 0
    Else
        FrmePetty.Visible = True
        chkPetty.Value = 1
    End If
End Sub
