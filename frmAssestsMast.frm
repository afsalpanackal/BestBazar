...
Begin VB.Form frmAssetsMast 
   BackColor       =   &H00E0E0E0&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Assets Head Creation"
   ClientHeight    =   3180
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   7155
   ControlBox      =   0   'False
   Icon            =   "frmAssestsMast.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   End
End
Attribute VB_Name = "frmAssetsMast"
...
Private Sub CmdDelete_Click()
    ...
    RSTSUPMAST.Open "SELECT M_USER_ID From TRXEXPENSE WHERE [ACT_CODE] = '" & Txtsuplcode.Tag & "'", db, adOpenStatic, adLockReadOnly
...
Private Sub CmdSave_Click()
    ...
    Txtsuplcode.Tag = "841" & Txtsuplcode.Text
    ...
    MsgBox "SAVED SUCCESSFULLY..", vbOKOnly, "ASSETS CREATION"
    Set TRXMAST = New ADODB.Recordset
    TRXMAST.Open "Select MAX(Val(ACT_CODE)) From ACTMAST WHERE (Mid(ACT_CODE, 1, 3)='841')And (len(ACT_CODE)>3)", db, adOpenStatic, adLockReadOnly
...
Private Sub txtsupplist_Change()
    ...
    If REPFLAG = True Then
        RSTREP.Open "Select [ACT_CODE],[ACT_NAME] From ACTMAST  WHERE (Mid(ACT_CODE, 1, 3)='841')And (len(ACT_CODE)>3) And ACT_NAME Like '" & Me.txtsupplist.Text & "%'ORDER BY [ACT_NAME]", db, adOpenStatic, adLockReadOnly
        REPFLAG = False
    Else
        RSTREP.Close
        RSTREP.Open "Select [ACT_CODE],[ACT_NAME] From ACTMAST  WHERE (Mid(ACT_CODE, 1, 3)='841')And (len(ACT_CODE)>3) And ACT_NAME Like '" & Me.txtsupplist.Text & "%'ORDER BY [ACT_NAME]", db, adOpenStatic, adLockReadOnly
