...
Begin VB.Form frmExpenseStaff 
   ...
   End
End
Attribute VB_Name = "frmExpenseStaff"
...
Private Sub cmdcancel_Click()
    ...
    TRXMAST.Open "Select MAX(VCH_NO) From TRXEXP_MAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "'", db, adOpenStatic, adLockReadOnly
...
Private Sub CmdPrint_Click()
    ...
    ReportNameVar = Rptpath & "RPTExpenseStaff"
    Set Report = crxApplication.OpenReport(ReportNameVar, 1)
    
    Report.RecordSelectionFormula = "({TRXEXPMAST.TRX_TYPE} ='EX' AND {TRXEXPMAST.VCH_NO}= " & Val(txtBillNo.Text) & " AND {TRXEXPMAST.TRX_YEAR}= '" & Year(MDIMAIN.DTFROM.Value) & "' )"
...
Private Sub Command4_Click()
    ...
    If M_ADD = True Then
        If MsgBox("Changes have been made. Do you want to Cancel?", vbYesNo, "STAFF EXPENSE") = vbNo Then Exit Sub
    End If
    txtBillNo.Enabled = True
    'txtBillNo.Text = TXTLASTBILL.Text
    FRMEMASTER.Enabled = False
...
'
'    Set TRXMAST = New ADODB.Recordset
'    TRXMAST.Open "Select MAX(VCH_NO) From TRXEXPMAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "'", db, adOpenStatic, adLockReadOnly
'    If Not (TRXMAST.EOF And TRXMAST.BOF) Then
'        txtBillNo.Text = IIf(IsNull(TRXMAST.Fields(0)), 1, TRXMAST.Fields(0) + 1)
'        TXTLASTBILL.Text = txtBillNo.Text
'    End If
...
Private Sub txtBillNo_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            rstTRXMAST.Open "Select * From TRXEXP_MAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND VCH_NO = " & Val(txtBillNo.Text) & " AND TRX_TYPE ='EX' ORDER BY VCH_NO, VCH_DATE", db, adOpenStatic, adLockReadOnly
            ...
            Set rstTRXMAST = New ADODB.Recordset
            rstTRXMAST.Open "Select * From TRXFILE_EXP WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND VCH_NO = " & Val(txtBillNo.Text) & " AND TRX_TYPE ='EX' ORDER BY VCH_NO, VCH_DATE", db, adOpenStatic, adLockReadOnly
...
Public Sub appendpurchase()
    ...
    db.BeginTrans
    db.Execute "delete From TRXEXP_MAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND VCH_NO = " & Val(txtBillNo.Text) & ""
    db.Execute "delete From TRXFILE_EXP WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND VCH_NO = " & Val(txtBillNo.Text) & ""
    db.Execute "delete FROM CASHATRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND INV_NO = " & Val(txtBillNo.Text) & " AND INV_TYPE = 'ES' AND INV_TRX_TYPE = 'ES'"
    db.Execute "delete From BANK_TRX WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE ='DR' AND BILL_TRX_TYPE = 'ES' AND B_VCH_NO = " & Val(txtBillNo.Text) & " "
    ...
    If lblcash.Caption = "Y" Then
        ...
        End If
        RSTITEMMAST.AddNew
        RSTITEMMAST!REC_NO = i
        RSTITEMMAST!INV_TYPE = "ES"
        RSTITEMMAST!INV_TRX_TYPE = "ES"
        RSTITEMMAST!INV_NO = Val(txtBillNo.Text)
        RSTITEMMAST!TRX_YEAR = Year(MDIMAIN.DTFROM.Value)
        RSTITEMMAST!TRX_TYPE = "DR"
        RSTITEMMAST!ACT_CODE = Dlstemployee.BoundText
        RSTITEMMAST!ACT_NAME = Dlstemployee.Text
    ...
    Else
        Set RSTITEMMAST = New ADODB.Recordset
        RSTITEMMAST.Open "Select * From BANK_TRX WHERE TRX_NO= (SELECT MAX(TRX_NO) FROM BANK_TRX WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE = 'DR' AND BILL_TRX_TYPE = 'ES')", db, adOpenStatic, adLockOptimistic, adCmdText
        If RSTITEMMAST.RecordCount = 0 Then
            i = 1
        Else
...
'    Dim RSTITEMMAST As ADODB.Recordset
'    Set RSTITEMMAST = New ADODB.Recordset
'    RSTITEMMAST.Open "SELECT * FROM CASHATRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND INV_NO = " & Val(txtBillNo.Text) & " AND INV_TYPE = 'ES' AND INV_TRX_TYPE = 'ES'", db, adOpenStatic, adLockOptimistic, adCmdText
...
'    Set RSTITEMMAST = Nothing
    ...
    RSTTRXFILE.Open "SELECT * from TRXEXP_MAST", db, adOpenStatic, adLockOptimistic, adCmdText
    RSTTRXFILE.AddNew
    ...
    RSTTRXFILE!C_USER_ID = Left(frmLogin.rs!USER_NAME, 23)
    ...
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "SELECT * from TRXFILE_EXP", db, adOpenStatic, adLockOptimistic, adCmdText
    For i = 1 To grdsales.rows - 1
        RSTTRXFILE.AddNew
        RSTTRXFILE!TRX_TYPE = "EX"
        RSTTRXFILE!VCH_NO = Val(txtBillNo.Text)
...
SKIP:
    ...
    M_ADD = False
    CMDEXIT.Enabled = True
    MsgBox "SAVED SUCCESSFULLY", vbOKOnly, "STAFF EXPENSE"
    Exit Sub
    
    Dim rstMaxNo As ADODB.Recordset
    Set rstMaxNo = New ADODB.Recordset
    rstMaxNo.Open "Select MAX(VCH_NO) From TRXEXP_MAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "'", db, adOpenStatic, adLockReadOnly
    ...
    Select Case KeyCode
        Case vbKeyReturn, 40
            ...
            If DataList2.VisibleCount = 0 Then
                If MsgBox("Expense head not found. Do you want to create it?", vbYesNo + vbDefaultButton2, "EXPENSE MASTER") = vbNo Then Exit Sub
                ...
                Set RSTITEMMAST = New ADODB.Recordset
                RSTITEMMAST.Open "Select MAX(ACT_CODE) From ACTMAST WHERE (Mid(ACT_CODE, 1, 3)='641')And (LENGTH(ACT_CODE)>3)", db, adOpenStatic, adLockReadOnly
                If Not (RSTITEMMAST.EOF And RSTITEMMAST.BOF) Then
                    expcode = IIf(IsNull(RSTITEMMAST.Fields(0)), "001", Mid(RSTITEMMAST.Fields(0), 4) + 1)
                End If
                RSTITEMMAST.Close
                Set RSTITEMMAST = Nothing
                
                expcode = Format(expcode, "000")
                expcode = "641" & expcode
                Set RSTITEMMAST = New ADODB.Recordset
                RSTITEMMAST.Open "SELECT * FROM ACTMAST WHERE ACT_CODE = '" & expcode & "'", db, adOpenStatic, adLockOptimistic, adCmdText
                If (RSTITEMMAST.EOF And RSTITEMMAST.BOF) Then
                    RSTITEMMAST.AddNew
                    RSTITEMMAST!ACT_CODE = expcode
                End If
                RSTITEMMAST!ACT_NAME = Trim(TXTDEALER.Text)
                RSTITEMMAST!OPEN_DB = 0
                RSTITEMMAST!OPEN_CR = 0
                RSTITEMMAST!YTD_DB = 0
                RSTITEMMAST!YTD_CR = 0
                RSTITEMMAST!CREATE_DATE = Date
                RSTITEMMAST!C_USER_ID = frmLogin.rs!USER_ID
...
Private Sub TXTEMPLOYEE_KeyDown(KeyCode As Integer, Shift As Integer)
    Select Case KeyCode
        Case vbKeyReturn, 40
            If Trim(TXTEMPLOYEE.Text) = "" Then Exit Sub
            If Dlstemployee.VisibleCount = 0 Then
                If MsgBox("Employee not found. Do you want to create a new employee?", vbYesNo + vbDefaultButton2, "EMPLOYEE MASTER") = vbNo Then Exit Sub
                ...
                Set RSTITEMMAST = New ADODB.Recordset
                RSTITEMMAST.Open "Select MAX(ACT_CODE) From ACTMAST WHERE ((Mid(ACT_CODE, 1, 3)='321') or (Mid(ACT_CODE, 1, 3)='911')) And (LENGTH(ACT_CODE)>3)", db, adOpenStatic, adLockReadOnly
                If Not (RSTITEMMAST.EOF And RSTITEMMAST.BOF) Then
                    expcode = IIf(IsNull(RSTITEMMAST.Fields(0)), "001", Mid(RSTITEMMAST.Fields(0), 4) + 1)
                End If
                RSTITEMMAST.Close
                ...
                If (RSTITEMMAST.EOF And RSTITEMMAST.BOF) Then
                    ...
                    RSTITEMMAST!ACT_CODE = expcode
                End If
                RSTITEMMAST!ACT_NAME = Trim(TXTEMPLOYEE.Text)
                RSTITEMMAST!OPEN_DB = 0
                RSTITEMMAST!OPEN_CR = 0
                RSTITEMMAST!YTD_DB = 0
                RSTITEMMAST!YTD_CR = 0
...
Private Sub grdsales_KeyDown(KeyCode As Integer, Shift As Integer)
    Select Case KeyCode
        Case vbKeyEscape
            ...
            If TXTAMOUNT.Enabled = True Then Exit Sub
            FRMECONTROLS.Enabled = True
            TXTSLNO.Enabled = True
            TXTSLNO.SetFocus
        Case vbKeyDelete
            ...
            For i = grdsales.Row - 1 To grdsales.rows - 2
                'grdsales.TextMatrix(grdsales.Row, 0) = i + 1 'grdsales.TextMatrix(i + 1, 0)
