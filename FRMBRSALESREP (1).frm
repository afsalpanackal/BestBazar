...
Begin VB.Form FRMBILLPRINTBR 
   ...
   Begin VB.Frame FRMEMAIN 
      ...
      Begin VB.CommandButton Command4 
         Caption         =   "Print HSN Code Wise Report"
         BeginProperty Font 
            Name            =   "Arial"
            Size            =   8.25
            Charset         =   204
      ...
      Begin VB.Frame Frmeperiod 
         ...
         Begin vbalProgBarLib6.vbalProgressBar vbalProgressBar1 
            ...
            _ExtentX        =   8943
            _ExtentY        =   423
            Picture         =   "FRMBRSALESREP.frx":030A
            ForeColor       =   0
            BarPicture      =   "FRMBRSALESREP.frx":0326
            Max             =   150
            Text            =   "PLEASE WAIT..."
            BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
               Name            =   "Tahoma"
   ...
   End
End
Attribute VB_Name = "FRMBILLPRINTBR"
...
'        Exit Sub
'    End If
    db.Execute "Update trxfileVAN set mfgr ='' where isnull(mfgr)"
    ...
    ReportNameVar = Rptpath & "RPTCompRepBR"
    ...
    Else
        If OPTCUSTOMER.Value = False Then
            Report.RecordSelectionFormula = "({TRXFILE.MFGR}='" & DataList1.BoundText & "' AND {TRXMAST.VCH_DATE} <=# " & Format(DTTo.Value, "MM,DD,YYYY") & " # AND {TRXMAST.VCH_DATE} >=# " & Format(DTFROM.Value, "MM,DD,YYYY") & " # )"
        Else
            Report.RecordSelectionFormula = "({TRXMAST.ACT_CODE} = '" & DataList2.BoundText & "' AND {TRXFILE.MFGR}='" & DataList1.BoundText & "' AND {TRXMAST.VCH_DATE} <=# " & Format(DTTo.Value, "MM,DD,YYYY") & " # AND {TRXMAST.VCH_DATE} >=# " & Format(DTFROM.Value, "MM,DD,YYYY") & " # )"
        End If
    End If
    ...
    For i = 1 To Report.Database.Tables.COUNT
        ...
        If UCase(dbase1) <> "INVSOFT" And UCase(dbase1) <> "INVDB" And UCase(dbase1) <> "INVSOFT3" Then
            ...
            ElseIf Report.Database.Tables(i).Name = "itemmast" Then
                Set oRs = db.Execute("SELECT * From TRXFILEVAN INNER JOIN " & Report.Database.Tables(i).Name & " USING(ITEM_CODE) WHERE VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' ")
            Else
                Set oRs = db.Execute("SELECT * FROM " & Report.Database.Tables(i).Name & " ")
            End If
...
Private Sub CmdDaily_Click()
    ...
    ReportNameVar = Rptpath & "RptDaywiseBR"
    Set Report = crxApplication.OpenReport(ReportNameVar, 1)
    Report.RecordSelectionFormula = "({TRXMAST.VCH_DATE} <=# " & Format(DTTo.Value, "MM,DD,YYYY") & " # AND {TRXMAST.VCH_DATE} >=# " & Format(DTFROM.Value, "MM,DD,YYYY") & " # AND ({TRXMAST.TRX_TYPE}='TF' OR {TRXMAST.TRX_TYPE}='SV' OR {TRXMAST.TRX_TYPE}='HI' OR {TRXMAST.TRX_TYPE}='GI' OR {TRXMAST.TRX_TYPE}='SI' OR {TRXMAST.TRX_TYPE}='VI' OR {TRXMAST.TRX_TYPE}='RI' OR {TRXMAST.TRX_TYPE}='WO') )"
    Set CRXFormulaFields = Report.FormulaFields
    For i = 1 To Report.Database.Tables.COUNT
        Report.Database.Tables.Item(i).SetLogOnInfo strConnection
        If UCase(dbase1) <> "INVSOFT" And UCase(dbase1) <> "INVDB" And UCase(dbase1) <> "INVSOFT3" Then
...
Private Sub Cmdday_Click()
    ...
    rstTRANX.Open "SELECT * From TRXMASTVAN WHERE VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND TRX_TYPE='GI' ", db, adOpenStatic, adLockReadOnly
    ...
    Do Until FROMDATE > TODATE
        ...
        RSTTRXFILE.Open "SELECT * From TRXMASTVAN WHERE VCH_DATE = '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND TRX_TYPE='GI' ORDER BY VCH_NO", db, adOpenStatic, adLockReadOnly
...
Private Sub CmDDisplay_Click()
    ...
    Set rstTRANX = Nothing
SKIP_VERIFY:
    db.Execute "Update TRXMASTVAN Set PHONE='' where isnull(PHONE)"
    db.Execute "Update TRXMASTVAN Set BILL_NAME='Cash' where isnull(BILL_NAME)"
    db.Execute "Update TRXMASTVAN Set ACT_CODE='130001' where isnull(ACT_CODE)"
    db.Execute "Update TRXMASTVAN Set ACT_NAME='Cash' where isnull(ACT_NAME)"
    Set rstTRANX = New ADODB.Recordset
    If OPTPERIOD.Value = True Then
        rstTRANX.Open "SELECT * From TRXMASTVAN WHERE PHONE Like '%" & Trim(TxtPhone.text) & "%' AND (BILL_NAME Like '%" & Trim(txtCustomerName.text) & "%' OR ACT_NAME Like '%" & Trim(txtCustomerName.text) & "%') AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND (TRX_TYPE='GI')  ORDER BY TRX_TYPE, VCH_DATE,VCH_NO", db, adOpenStatic, adLockReadOnly
    Else
...
'        End If
        ...
        Set RSTTRXFILE = New ADODB.Recordset
        RSTTRXFILE.Open "Select DISTINCT SALES_TAX From TRXFILEVAN WHERE TRX_YEAR = '" & rstTRANX!TRX_YEAR & "' AND VCH_NO = " & rstTRANX!VCH_NO & " AND TRX_TYPE = '" & rstTRANX!TRX_TYPE & "' AND CATEGORY LIKE '%" & DataList3.BoundText & "%'", db, adOpenStatic, adLockReadOnly, adCmdText
        Do Until RSTTRXFILE.EOF
            EXSALEAMT = 0
            TAXSALEAMT = 0
            TaxAmt = 0
            ...
            RSTtax.Open "Select * From TRXFILEVAN WHERE TRX_YEAR = '" & rstTRANX!TRX_YEAR & "' AND VCH_NO = " & rstTRANX!VCH_NO & " AND TRX_TYPE = '" & rstTRANX!TRX_TYPE & "' AND SALES_TAX = " & RSTTRXFILE!SALES_TAX & " AND CATEGORY LIKE '%" & DataList3.BoundText & "%'", db, adOpenStatic, adLockReadOnly, adCmdText
            Do Until RSTtax.EOF
...
'            Set RSTACTCODE = Nothing
        ...
        Else
            ...
            If MDIMAIN.lblgst.Caption = "C" Or MDIMAIN.lblgst.Caption = "N" Or rstTRANX!TRX_TYPE = "WO" Then
                GRDTranx.TextMatrix(M, 16) = Format(Round(Val(GRDTranx.TextMatrix(M, 20)) - Val(GRDTranx.TextMatrix(M, 9)), 2), "0.00")
            Else
                GRDTranx.TextMatrix(M, 16) = Format(Round((Val(GRDTranx.TextMatrix(M, 20)) - (Val(GRDTranx.TextMatrix(M, 6)) + Val(GRDTranx.TextMatrix(M, 8)))) - Val(GRDTranx.TextMatrix(M, 9)), 2), "0.00")
            End If
            ...
            Else
                GRDTranx.TextMatrix(M, 17) = Format(Round((((Val(GRDTranx.TextMatrix(M, 20)) - (Val(GRDTranx.TextMatrix(M, 6)) + Val(GRDTranx.TextMatrix(M, 8)))) * 100) / Val(GRDTranx.TextMatrix(M, 9))) - 100, 2), "0.00")
            End If
        ...
        If frmLogin.rs!Level = "0" Then
            If MDIMAIN.lblgst.Caption = "C" Or MDIMAIN.lblgst.Caption = "N" Then
                ...
                lbltaxamt.Caption = ""
            Else
                LBLPROFIT.Caption = Val(LBLPROFIT.Caption) + Val(GRDTranx.TextMatrix(M, 16)) 'Format(Val(LBLNET.Caption) - (Val(lblcost.Caption) + Val(lblCommi.Caption)), "0.00")
                lbltaxamt.Caption = Val(lbltaxamt.Caption) + (Val(GRDTranx.TextMatrix(M, 7)) - Val(GRDTranx.TextMatrix(M, 20)))
...
Private Sub CmdPrintBills_Click()
    ...
    If UCase(dbase1) <> "INVSOFT" And UCase(dbase1) <> "INVDB" And UCase(dbase1) <> "INVSOFT3" Then
        ...
        Set oRs = New ADODB.Recordset
        Set oRs = db.Execute("SELECT * FROM ITEMMASTVAN ")
...
Private Sub CmdUserrep_Click()
    ...
    On Error GoTo ERRHAND
    ReportNameVar = Rptpath & "RPTUSERREPORTBR"
    Set Report = crxApplication.OpenReport(ReportNameVar, 1)
    'ACT_CODE = '" & DataList2.BoundText & "' AND
    If OPTPERIOD.Value = True Then
...
Private Sub CmdZeroBills_Click()
    ...
    Set rstTRANX = New ADODB.Recordset
    rstTRANX.Open "SELECT * From TRXMASTVAN WHERE NET_AMOUNT <= 0 AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND (TRX_TYPE='SV' OR TRX_TYPE='GI' OR TRX_TYPE='SI' or TRX_TYPE='HI' OR TRX_TYPE='RI' OR TRX_TYPE='VI') ORDER BY TRX_TYPE, VCH_DATE,VCH_NO", db, adOpenStatic, adLockReadOnly
...
Private Sub Command1_Click()
    ...
    On Error GoTo ERRHAND
    ReportNameVar = Rptpath & "RPTSALESDAYBR"
    Set Report = crxApplication.OpenReport(ReportNameVar, 1)
    'ACT_CODE = '" & DataList2.BoundText & "' AND
    If OPTPERIOD.Value = True Then
        Report.RecordSelectionFormula = "({TRXMAST.TRX_TYPE}='GI' AND {TRXMAST.VCH_DATE} <=# " & Format(DTTo.Value, "MM,DD,YYYY") & " # AND {TRXMAST.VCH_DATE} >=# " & Format(DTFROM.Value, "MM,DD,YYYY") & " # )"
...
Private Sub CMDEfile_Click()
   ...
   Dim RSTTRXFILE As ADODB.Recordset
    ...
    Else
        rstTRANX.Open "SELECT * From TRXMASTVAN WHERE ACT_CODE = '" & DataList2.BoundText & "' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND TRX_TYPE='GI'  ORDER BY TRX_TYPE, VCH_DATE,VCH_NO", db, adOpenStatic, adLockReadOnly
    End If
...
Private Sub CmdMonthly_Click()
    ...
    On Error GoTo ERRHAND
    ReportNameVar = Rptpath & "RptMonthwiseBR"
    Set Report = crxApplication.OpenReport(ReportNameVar, 1)
...
Private Sub CMDREGISTER_Click()
    ...
    On Error GoTo ERRHAND
    ReportNameVar = Rptpath & "RPTLEDREPBR"
    Set Report = crxApplication.OpenReport(ReportNameVar, 1)
...
Private Sub CmdReport_Click()
    ...
    On Error GoTo ERRHAND
    ReportNameVar = Rptpath & "RPTSALESREP1BR"
    Set Report = crxApplication.OpenReport(ReportNameVar, 1)
    If OPTPERIOD.Value = True Then
        Report.RecordSelectionFormula = "({TRXMAST.TRX_TYPE} = 'GI' AND {TRXMAST.VCH_DATE} <=# " & Format(DTTo.Value, "MM,DD,YYYY") & " # AND {TRXMAST.VCH_DATE} >=# " & Format(DTFROM.Value, "MM,DD,YYYY") & " # )"
    Else
...
Private Sub cmdview_Click()
    ...
    On Error GoTo ERRHAND
    ReportNameVar = Rptpath & "RPTSALESREPORTAGBR"
...
'        rstTRANX.Open "SELECT * From TRXMASTVAN WHERE VCH_DATE <= '" & Format(DTTO.value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.value, "yyyy/mm/dd") & "' AND TRX_TYPE='RI' ", db, adOpenStatic, adLockReadOnly
'    ElseIf OptService.value = True Then
'        rstTRANX.Open "SELECT * From TRXMASTVAN WHERE VCH_DATE <= '" & Format(DTTO.value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.value, "yyyy/mm/dd") & "' AND TRX_TYPE='SV' ", db, adOpenStatic, adLockReadOnly
'    ElseIf Optpetty.value = True Then
'        rstTRANX.Open "SELECT * From TRXMASTVAN WHERE VCH_DATE <= '" & Format(DTTO.value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.value, "yyyy/mm/dd") & "' AND TRX_TYPE='WO' ", db, adOpenStatic, adLockReadOnly
'    ElseIf OptWS.value = True Then
'        rstTRANX.Open "SELECT * From TRXMASTVAN WHERE VCH_DATE <= '" & Format(DTTO.value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.value, "yyyy/mm/dd") & "' AND TRX_TYPE='SI' ", db, adOpenStatic, adLockReadOnly
'    ElseIf OptVan.value = True Then
'        rstTRANX.Open "SELECT * From TRXMASTVAN WHERE VCH_DATE <= '" & Format(DTTO.value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.value, "yyyy/mm/dd") & "' AND TRX_TYPE='HI' ", db, adOpenStatic, adLockReadOnly
'    Else
'        rstTRANX.Open "SELECT * From TRXMASTVAN WHERE VCH_DATE <= '" & Format(DTTO.value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.value, "yyyy/mm/dd") & "' AND TRX_TYPE='GI' ", db, adOpenStatic, adLockReadOnly
'    End If
'
...
'
'        Set RSTTRXFILE = New ADODB.Recordset
'        RSTTRXFILE.Open "SELECT * From TRXMASTVAN WHERE VCH_DATE = '" & Format(DTFROM.value, "yyyy/mm/dd") & "' AND TRX_TYPE='SI' ORDER BY VCH_NO", db, adOpenStatic, adLockReadOnly
'        Do Until RSTTRXFILE.EOF
'            CmdDisplay.Tag = ""
'            If RSTTRXFILE!SLSM_CODE = "A" Then
'                CmdDisplay.Tag = IIf(IsNull(RSTTRXFILE!DISCOUNT), "", Format(RSTTRXFILE!DISCOUNT, "0.00"))
...
Private Sub Command2_Click()
    ...
    On Error GoTo ERRHAND
    ReportNameVar = Rptpath & "RPTCRSALEBR"
    Set Report = crxApplication.OpenReport(ReportNameVar, 1)
...
'    End If
    ...
    Screen.MousePointer = vbHourglass
    ReportNameVar = Rptpath & "RPTAreaRepBR"
    Set Report = crxApplication.OpenReport(ReportNameVar, 1)
    Report.DiscardSavedData
    Report.VerifyOnEveryPrint = True
    If DataList4.BoundText = "" Then
...
Private Sub Command4_Click()
    ...
    On Error GoTo ERRHAND
    ReportNameVar = Rptpath & "RPTHSNREPORTBR"
    Set Report = crxApplication.OpenReport(ReportNameVar, 1)
    Report.RecordSelectionFormula = "(((ISNULL({ITEMMAST.UN_BILL}) OR {ITEMMAST.UN_BILL} <> 'Y') AND {TRXFILE.TRX_TYPE}='GI')AND {TRXFILE.VCH_DATE} <=# " & Format(DTTo.Value, "MM,DD,YYYY") & " # AND {TRXFILE.VCH_DATE} >=# " & Format(DTFROM.Value, "MM,DD,YYYY") & " # )"
...
Private Sub Command5_Click()
    ...
    On Error GoTo ERRHAND
    ReportNameVar = Rptpath & "RPTSALEITEMBR"
    Set Report = crxApplication.OpenReport(ReportNameVar, 1)
...
Private Sub Command6_Click()
    ...
    On Error GoTo ERRHAND
    If DataList3.BoundText = "" Then
        ReportNameVar = Rptpath & "RPTSALESREPORT1BR"
    Else
        ReportNameVar = Rptpath & "RPTSALESREPORT2BR"
    End If
    Set Report = crxApplication.OpenReport(ReportNameVar, 1)
    'ACT_CODE = '" & DataList2.BoundText & "' AND
    If DataList3.BoundText = "" Then
    ...
    Else
        If OPTPERIOD.Value = True Then
            Report.RecordSelectionFormula = "({ITEMMAST.CATEGORY} = '" & DataList3.BoundText & "' AND ({TRXFILE.TRX_TYPE}='SV' OR {TRXFILE.TRX_TYPE}='HI' OR {TRXFILE.TRX_TYPE}='GI' OR {TRXFILE.TRX_TYPE}='VI' OR {TRXFILE.TRX_TYPE}='WO' OR {TRXFILE.TRX_TYPE}='RI' OR {TRXFILE.TRX_TYPE}='SI') AND {TRXFILE.VCH_DATE} <=# " & Format(DTTo.Value, "MM,DD,YYYY") & " # AND {TRXFILE.VCH_DATE} >=# " & Format(DTFROM.Value, "MM,DD,YYYY") & " # )"
        Else
            Report.RecordSelectionFormula = "({ITEMMAST.CATEGORY} = '" & DataList3.BoundText & "' AND {TRXFILE.ACT_CODE} = '" & DataList2.BoundText & "' AND ({TRXFILE.TRX_TYPE}='SV' OR {TRXFILE.TRX_TYPE}='HI' OR {TRXFILE.TRX_TYPE}='GI' OR {TRXFILE.TRX_TYPE}='VI' OR {TRXFILE.TRX_TYPE}='WO' OR {TRXFILE.TRX_TYPE}='RI' OR {TRXFILE.TRX_TYPE}='SI') AND {TRXFILE.VCH_DATE} <=# " & Format(DTTo.Value, "MM,DD,YYYY") & " # AND {TRXFILE.VCH_DATE} >=# " & Format(DTFROM.Value, "MM,DD,YYYY") & " # )"
        End If
    End If
    Set CRXFormulaFields = Report.FormulaFields
    For i = 1 To Report.Database.Tables.COUNT
...
Private Sub CMDPRINTREGISTER_Click()
    ...
    Screen.MousePointer = vbHourglass
    On Error GoTo ERRHAND
    ReportNameVar = Rptpath & "RPTSALESREPORTBR"
    Set Report = crxApplication.OpenReport(ReportNameVar, 1)
    Report.DiscardSavedData
    Report.VerifyOnEveryPrint = True
...
'    rstTRXMAST.Open "SELECT * From TRXMASTVAN WHERE VCH_DATE >= '" & Format(dt_from, "yyyy/mm/dd") & "' ", db, adOpenStatic, adLockReadOnly
    ...
    Select Case Trim(GRDTranx.TextMatrix(GRDTranx.Row, 1))
        Case "HI"
            ...
            Else
                ...
                ElseIf IsFormLoaded(FRMGSTR) <> True Then
                    FRMGSTR.txtBillNo.text = Val(GRDTranx.TextMatrix(GRDTranx.Row, 3))
                    FRMGSTR.LBLBILLNO.Caption = Val(GRDTranx.TextMatrix(GRDTranx.Row, 3))
                    FRMGSTR.Show
                    FRMGSTR.SetFocus
...
Private Sub GRDTranx_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            i = 0
            Set RSTTRXFILE = New ADODB.Recordset
            RSTTRXFILE.Open "Select * From TRXFILEVAN WHERE VCH_NO = " & Val(LBLBILLNO.Caption) & "  AND TRX_TYPE = '" & Trim(GRDTranx.TextMatrix(GRDTranx.Row, 1)) & "' AND TRX_YEAR = '" & Trim(GRDTranx.TextMatrix(GRDTranx.Row, 15)) & "'", db, adOpenStatic, adLockReadOnly
            Do Until RSTTRXFILE.EOF
                i = i + 1
...
CLOSEFILE:
    ...
    'RSTTRXFILE.Open "SELECT * From SALESREG ORDER BY VCH_NO", DB, adOpenStatic,adLockReadOnly
    RSTTRXFILE.Open "SELECT * From TRXMASTVAN WHERE VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND (TRX_TYPE='GI' OR TRX_TYPE='HI' OR TRX_TYPE='SV' OR TRX_TYPE='SI' OR TRX_TYPE='RI') ORDER BY VCH_NO", db, adOpenStatic, adLockReadOnly
    Do Until RSTTRXFILE.EOF
        SN = SN + 1
        CMDDISPLAY.Tag = ""
        If RSTTRXFILE!SLSM_CODE = "A" Then
...
Private Function fillcombo()
    ...
    rstfillcombo.Open "Select DISTINCT BILL_NAME From TRXMASTVAN ORDER BY BILL_NAME", db, adOpenStatic, adLockReadOnly
...
Private Sub Report_Generate(search_type As String)
    ...
    If search_type = "M" Then
        ReportNameVar = Rptpath & "RPTMONWISEBR"
    ElseIf search_type = "D" Then
        ReportNameVar = Rptpath & "RPTDAILYWISEBR"
    ElseIf search_type = "Y" Then
        ReportNameVar = Rptpath & "RPTYEARWISEBR"
    Else
        Exit Sub
    End If
    Screen.MousePointer = vbHourglass
