...
'    Set RSTTRXFILE = Nothing
    ...
    For n = 1 To grdsales.rows - 1
        Set RSTTRXFILE = New ADODB.Recordset
        RSTTRXFILE.Open "SELECT *  From ITEMMASTVAN WHERE ITEM_CODE = '" & grdsales.TextMatrix(n, 13) & "' AND CATEGORY <> 'SERVICES' AND UCASE(CATEGORY) <> 'SERVICE CHARGE'  AND CATEGORY <> 'SELF'", db, adOpenStatic, adLockOptimistic, adCmdText
        With RSTTRXFILE
            If Not (.EOF And .BOF) Then
                .Properties("Update Criteria").Value = adCriteriaKey
                '!ISSUE_QTY = !ISSUE_QTY - (Val(grdsales.TextMatrix(N, 3)) + Val(grdsales.TextMatrix(N, 20)))
...
Private Sub CMDADD_Click()
    ...
    If ZERO_WARN_FLAG = True And MINUS_BILL = "N" And M_EDIT = False Then
        Set rststock = New ADODB.Recordset
        rststock.Open "SELECT *  FROM ITEMMAST WHERE UCASE(CATEGORY) <> 'SERVICE CHARGE' AND ITEM_CODE = '" & TXTITEMCODE.Text & "' AND CLOSE_QTY < " & Val(TXTQTY.Text) + Val(TXTFREE.Text) & " ", db, adOpenStatic, adLockOptimistic, adCmdText
        With rststock
            If Not (.EOF And .BOF) Then
