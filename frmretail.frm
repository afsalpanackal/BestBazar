   ...
   Begin VB.Frame FRMEMAIN 
      ...
      Begin VB.Frame FRMEHEAD 
         ...
         Begin VB.ComboBox cmbtype 
            ...
            ForeColor       =   &H00004080&
            Height          =   315
            ItemData        =   "frmretail.frx":030A
            Left            =   6585
            List            =   "frmretail.frx":0317
...
Private Sub CMDSALERETURN_Click()
    ...
    Do Until RSTTRXFILE.EOF
        ...
        grdsales.TextMatrix(grdsales.Rows - 1, 24) = "0"
...
Private Sub DataList2_Click()
    ...
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "SELECT *  FROM TEMPCN WHERE ACT_CODE = '" & Trim(DataList2.BoundText) & "' AND CHECK_FLAG <> 'Y' AND TRX_TYPE = 'RI'", db, adOpenStatic, adLockReadOnly, adCmdText
...
Private Sub CMDDELIVERY_Click()
    ...
    'If grdsales.Rows <= Val(TXTSLNO.Text) Then grdsales.Rows = grdsales.Rows + 1
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "SELECT *  FROM TEMPCN WHERE ACT_CODE = '" & Trim(DataList2.BoundText) & "' AND CHECK_FLAG <> 'Y' AND TRX_TYPE = 'RI'", db, adOpenStatic, adLockOptimistic, adCmdText
    Do Until RSTTRXFILE.EOF
        grdsales.Rows = grdsales.Rows + 1
        grdsales.FixedRows = 1
        grdsales.TextMatrix(grdsales.Rows - 1, 0) = grdsales.Rows
        ...
        Set RSTMFGR = New ADODB.Recordset
        RSTMFGR.Open "SELECT MANUFACTURER  FROM ITEMMASTWO WHERE ITEM_CODE = '" & Trim(grdsales.TextMatrix(grdsales.Rows - 1, 1)) & "'", db, adOpenStatic, adLockReadOnly
        If Not (RSTMFGR.EOF And RSTMFGR.BOF) Then
...
Private Sub cmdPrint_Click()
    ...
    If DataList2.BoundText = "130000" Then
        FRMRETAIL.lblcredit.Caption = "0"
        FRMRETAIL.Generateprint
    Else
        Me.Enabled = False
        FRMDEBITRT.Show
    End If
...
'    End If
    ...
    For i = 1 To Report.Database.Tables.Count
        Report.Database.Tables(i).SetLogOnInfo "ConnectionName", "G:\dbase\YEAR13-14\MDINV.MDB", "admin", "!@#$%^&*())(*&^%$#@!"
    Next i
...
''''''                RSTNONSTOCK.Open "Select * From NONRCVD WHERE Item_Code = '" & TXTITEMCODE.Text & "'", DB, adOpenStatic, adLockOptimistic, adCmdText
...
''''''                End If
            ...
            If ITEM_FLAG = True Then
                If NONSTOCKFLAG = True Then
                    PHY_ITEM.Open "Select ITEM_CODE, ITEM_NAME, BAL_QTY, SALES_PRICE, SALES_TAX, LINE_DISC, REF_NO, EXP_DATE, VCH_NO, LINE_NO, TRX_TYPE, MRP, CHECK_FLAG, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN  From RTRXFILE  WHERE ITEM_CODE = '" & TxtItemcode.Text & "' ORDER BY [VCH_DATE]", db, adOpenStatic, adLockReadOnly
                Else
                    PHY_ITEM.Open "Select ITEM_CODE, ITEM_NAME, BAL_QTY, SALES_PRICE, SALES_TAX, LINE_DISC, REF_NO, EXP_DATE, VCH_NO, LINE_NO, TRX_TYPE, MRP, CHECK_FLAG, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN  From RTRXFILE  WHERE ITEM_CODE = '" & TxtItemcode.Text & "' AND BAL_QTY > 0 ORDER BY [VCH_DATE]", db, adOpenStatic, adLockReadOnly
                End If
                ITEM_FLAG = False
            Else
                PHY_ITEM.Close
        ...
        Case vbKeyReturn
            ...
            Do Until TRXSUB.EOF
                ...
                If Not (TRXFILE.EOF And TRXFILE.BOF) Then
                    ...
                    TRXMAST.Open "SELECT LINE_DISC FROM RTRXFILE WHERE RTRXFILE.TRX_TYPE = '" & Trim(TRXSUB!R_TRX_TYPE) & "' AND RTRXFILE.VCH_NO = " & Val(TRXSUB!R_VCH_NO) & " AND RTRXFILE.LINE_NO = " & Val(TRXSUB!R_LINE_NO) & "", db, adOpenStatic, adLockReadOnly
                    If Not (TRXMAST.EOF Or TRXMAST.BOF) Then
                        grdsales.TextMatrix(i, 4) = Val(TRXMAST!LINE_DISC)
                    End If
                    TRXMAST.Close
                    Set TRXMAST = Nothing
            ...
            If Not (TRXMAST.EOF Or TRXMAST.BOF) Then
                ...
                If TRXMAST!BILL_TYPE = "V" Then
                    cmbtype.ListIndex = 0
                ElseIf TRXMAST!BILL_TYPE = "C" Then
                    cmbtype.ListIndex = 1
                End If
            End If
            TRXMAST.Close
    ...
    Select Case KeyCode
        ...
        Case vbKeyReturn
            ...
            If PHYFLAG = True Then
                PHY.Open "Select * From ITEMMAST  WHERE ITEM_NAME Like '" & Me.TxtProduct.Text & "%'ORDER BY [ITEM_NAME]", db, adOpenStatic, adLockReadOnly
                PHYFLAG = False
            Else
                PHY.Close
                PHY.Open "Select * From ITEMMAST  WHERE ITEM_NAME Like '" & Me.TxtProduct.Text & "%'ORDER BY [ITEM_NAME]", db, adOpenStatic, adLockReadOnly
...
Function FILL_ITEMGRID()
    ...
    If ITEM_FLAG = True Then
        PHY_ITEM.Open "Select * From ITEMMAST  WHERE ITEM_NAME Like '" & TxtProduct.Text & "%'ORDER BY [ITEM_NAME]", db, adOpenStatic, adLockReadOnly
        ITEM_FLAG = False
    Else
        PHY_ITEM.Close
        PHY_ITEM.Open "Select * From ITEMMAST  WHERE ITEM_NAME Like '" & TxtProduct.Text & "%'ORDER BY [ITEM_NAME]", db, adOpenStatic, adLockReadOnly
...
Private Function AppendSale()
    ...
    If grdsales.Rows = 1 Then
        db.Execute "delete * FROM CASHATRXFILE WHERE INV_NO = " & Val(LBLBILLNO.Caption) & " AND INV_TYPE = 'RT' AND BILL_TRX_TYPE = 'RI'"
    Else
        Set RSTITEMMAST = New ADODB.Recordset
        RSTITEMMAST.Open "SELECT * FROM CASHATRXFILE WHERE INV_NO = " & Val(LBLBILLNO.Caption) & " AND INV_TYPE = 'RT' AND BILL_TRX_TYPE = 'RI'", db, adOpenStatic, adLockOptimistic, adCmdText
...
SKIP:
    ...
    If lblcredit.Caption <> "0" Then
        ...
        Set RSTITEMMAST = New ADODB.Recordset
        RSTITEMMAST.Open "SELECT * FROM DBTPYMT WHERE INV_NO = " & Val(LBLBILLNO.Caption) & " AND TRX_TYPE = 'DR' AND BILL_TRX_TYPE = 'RI'", db, adOpenStatic, adLockOptimistic, adCmdText
        If (RSTITEMMAST.EOF And RSTITEMMAST.BOF) Then
            RSTITEMMAST.AddNew
            RSTITEMMAST!TRX_TYPE = "DR"
            RSTITEMMAST!CR_NO = i
        ...
        Set RSTITEMMAST = Nothing
    Else
        db.Execute "delete * From DBTPYMT WHERE TRX_TYPE='DR' AND INV_NO = " & Val(LBLBILLNO.Caption) & " AND BILL_TRX_TYPE = 'RI'"
    End If
...
Function FILL_PREVIIOUSRATE()
    ...
    If PRERATE_FLAG = True Then
        PHY_PRERATE.Open "Select ITEM_CODE, ITEM_NAME, VCH_DATE, QTY, P_RETAIL, M_USER_ID, VCH_NO  From TRXFILE  WHERE ITEM_CODE = '" & TxtItemcode.Text & "' AND M_USER_ID = '" & DataList2.BoundText & "' AND VCH_NO <> " & Val(txtBillNo.Text) & " ORDER BY [VCH_DATE] ", db, adOpenStatic, adLockReadOnly
        PRERATE_FLAG = False
    Else
        PHY_PRERATE.Close
        PHY_PRERATE.Open "Select ITEM_CODE, ITEM_NAME, VCH_DATE, QTY, P_RETAIL, M_USER_ID, VCH_NO  From TRXFILE  WHERE ITEM_CODE = '" & TxtItemcode.Text & "' AND M_USER_ID = '" & DataList2.BoundText & "' AND VCH_NO <> " & Val(txtBillNo.Text) & " ORDER BY [VCH_DATE] ", db, adOpenStatic, adLockReadOnly
