...
Begin VB.Form FRMEXPINREGISTER 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "EXPENSE (NPUT TAX) REPORT"
   ClientHeight    =   10650
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   18105
   ...
   Begin VB.Frame FRMEMAIN 
      ...
      Begin vbalProgBarLib6.vbalProgressBar vbalProgressBar1 
         ...
         _ExtentX        =   8864
         _ExtentY        =   609
         Picture         =   "FrmExpInreg.frx":030A
         ForeColor       =   0
         BarPicture      =   "FrmExpInreg.frx":0326
         Max             =   150
         Text            =   "PLEASE WAIT..."
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "Tahoma"
   ...
   End
End
Attribute VB_Name = "FRMEXPINREGISTER"
...
Private Sub CmDDisplay_Click()
    ...
    If OPTPERIOD.Value = True Then
        If OptSortSupp.Value = True Then
            rstTRANX.Open "SELECT * From ASTRXMAST WHERE PINV Like '%" & Trim(TxtBillNos.text) & "%' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND TRX_TYPE='EP' ORDER BY ACT_NAME, VCH_DATE, VCH_NO", db, adOpenStatic, adLockReadOnly
        ElseIf OptSortDate.Value = True Then
            rstTRANX.Open "SELECT * From ASTRXMAST WHERE PINV Like '%" & Trim(TxtBillNos.text) & "%' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND TRX_TYPE='EP' ORDER BY VCH_DATE, VCH_NO", db, adOpenStatic, adLockReadOnly
        Else
            rstTRANX.Open "SELECT * From ASTRXMAST WHERE PINV Like '%" & Trim(TxtBillNos.text) & "%' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND TRX_TYPE='EP' ORDER BY TRX_TYPE, VCH_NO, VCH_DATE", db, adOpenStatic, adLockReadOnly
        End If
    Else
        If OptSortSupp.Value = True Then
            rstTRANX.Open "SELECT * From ASTRXMAST WHERE PINV Like '%" & Trim(TxtBillNos.text) & "%' AND ACT_CODE = '" & DataList2.BoundText & "' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND TRX_TYPE='EP' ORDER BY ACT_NAME, VCH_DATE, VCH_NO", db, adOpenStatic, adLockReadOnly
        ElseIf OptSortDate.Value = True Then
            rstTRANX.Open "SELECT * From ASTRXMAST WHERE PINV Like '%" & Trim(TxtBillNos.text) & "%' AND ACT_CODE = '" & DataList2.BoundText & "' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND TRX_TYPE='EP' ORDER BY VCH_DATE, VCH_NO", db, adOpenStatic, adLockReadOnly
        Else
            rstTRANX.Open "SELECT * From ASTRXMAST WHERE PINV Like '%" & Trim(TxtBillNos.text) & "%' AND ACT_CODE = '" & DataList2.BoundText & "' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND TRX_TYPE='EP' ORDER BY TRX_TYPE, VCH_NO, VCH_DATE", db, adOpenStatic, adLockReadOnly
        End If
    End If
    ...
    Do Until rstTRANX.EOF
        ...
        'If rstTRANX!TRX_TYPE <> "EP" Then GoTo SKIP
...
Private Sub CMDPRINTREGISTER_Click()
    ...
    Screen.MousePointer = vbHourglass
    ReportNameVar = Rptpath & "RPTEXPINREG"
...
Private Sub CMDPRINTREGISTER2_Click()
    ...
    Screen.MousePointer = vbHourglass
    On Error GoTo ERRHAND
    ReportNameVar = Rptpath & "RPTEXPINREPORT"
    Set Report = crxApplication.OpenReport(ReportNameVar, 1)
    Report.RecordSelectionFormula = "({TRXFILE.TRX_TYPE}='EP' AND {TRXFILE.VCH_DATE} <=# " & Format(DTTo.Value, "MM,DD,YYYY") & " # AND {TRXFILE.VCH_DATE} >=# " & Format(DTFROM.Value, "MM,DD,YYYY") & " # )"
    Set CRXFormulaFields = Report.FormulaFields
    For i = 1 To Report.Database.Tables.COUNT
        Report.Database.Tables.Item(i).SetLogOnInfo strConnection
    Next i
...
Private Sub CmdReport_Click()
    ...
    On Error GoTo ERRHAND
    ReportNameVar = Rptpath & "RPTEXPINSUM"
    Set Report = crxApplication.OpenReport(ReportNameVar, 1)
    Report.RecordSelectionFormula = "({ASTRXMAST.TRX_TYPE}='EP'AND {ASTRXMAST.VCH_DATE} <=# " & Format(DTTo.Value, "MM,DD,YYYY") & " # AND {ASTRXMAST.VCH_DATE} >=# " & Format(DTFROM.Value, "MM,DD,YYYY") & " # )"
    Set CRXFormulaFields = Report.FormulaFields
    For i = 1 To Report.Database.Tables.COUNT
        Report.Database.Tables.Item(i).SetLogOnInfo strConnection
        If UCase(dbase1) <> "INVSOFT" And UCase(dbase1) <> "INVDB" And UCase(dbase1) <> "INVSOFT3" Then
...
Private Sub TMPDELETE_Click()
    ...
    db.Execute ("DELETE from SALESREG WHERE SALESREG.VCH_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 1) & " AND SALESREG.TRX_TYPE = 'EP'")
    Call fillSTOCKREG
End Sub
...
Private Sub ReportGeneratION()
   ...
   ' On Error GoTo errHand
    ...
    'MsgBox "Report file generated at " & Rptpath & "Report.txt" & vbCrLf & "Click Print Report Button to print on paper."
    Exit Sub
