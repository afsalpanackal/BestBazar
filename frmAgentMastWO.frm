...
Begin VB.Form frmAgentMastwo 
   ...
   End
End
Attribute VB_Name = "frmAgentMastwo"
...
Private Sub Form_Load()
    ...
    TRXMAST.Open "Select MAX(Val(ACT_CODE)) From ACTMAST WHERE (Mid(ACT_CODE, 1, 3)='911')And (len(ACT_CODE)>3)", db2, adOpenStatic, adLockReadOnly
...
Private Sub txtsupplist_Change()
    ...
    If REPFLAG = True Then
        RSTREP.Open "Select [ACT_CODE],[ACT_NAME] From ACTMAST  WHERE (Mid(ACT_CODE, 1, 3)='911')And (len(ACT_CODE)>3) And ACT_NAME Like '" & Me.txtsupplist.Text & "%'ORDER BY [ACT_NAME]", db2, adOpenStatic, adLockReadOnly
        REPFLAG = False
    Else
        RSTREP.Close
        RSTREP.Open "Select [ACT_CODE],[ACT_NAME] From ACTMAST  WHERE (Mid(ACT_CODE, 1, 3)='911')And (len(ACT_CODE)>3) And ACT_NAME Like '" & Me.txtsupplist.Text & "%'ORDER BY [ACT_NAME]", db2, adOpenStatic, adLockReadOnly
