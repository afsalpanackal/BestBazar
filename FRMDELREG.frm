...
Private Sub CmDDisplay_Click()
    ...
    Select Case OptAll.Value
        Case True
            If OPTPERIOD.Value = True Then
                ...
                rstTRANX.Open "SELECT DISTINCT VCH_NO, TRX_TYPE, TRX_YEAR From TEMPCN WHERE VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' ORDER BY TRX_TYPE, TRX_YEAR, VCH_NO", db, adOpenStatic, adLockReadOnly
            Else
                Set rstTRANX = New ADODB.Recordset
                rstTRANX.Open "SELECT DISTINCT VCH_NO, TRX_TYPE, TRX_YEAR From TEMPCN WHERE ACT_CODE = '" & DataList2.BoundText & "' ORDER BY TRX_TYPE, TRX_YEAR, VCH_NO", db, adOpenStatic, adLockReadOnly
            End If
        Case Else
            If OPTPERIOD.Value = True Then
                Set rstTRANX = New ADODB.Recordset
                rstTRANX.Open "SELECT DISTINCT VCH_NO, TRX_TYPE, TRX_YEAR From TEMPCN WHERE CHECK_FLAG <>'Y' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' ORDER BY TRX_TYPE, TRX_YEAR, VCH_NO", db, adOpenStatic, adLockReadOnly
            Else
                Set rstTRANX = New ADODB.Recordset
                rstTRANX.Open "SELECT DISTINCT VCH_NO, TRX_TYPE, TRX_YEAR From TEMPCN WHERE ACT_CODE = '" & DataList2.BoundText & "' AND CHECK_FLAG <>'Y' ORDER BY TRX_TYPE, TRX_YEAR, VCH_NO", db, adOpenStatic, adLockReadOnly
            End If
    End Select
    Do Until rstTRANX.EOF
        ...
        GRDTranx.TextMatrix(i, 10) = rstTRANX!TRX_YEAR
        Select Case rstTRANX!TRX_TYPE
            Case "GI"
                GRDTranx.TextMatrix(i, 3) = "GST"
            Case "SI"
                GRDTranx.TextMatrix(i, 3) = "Delivery"
            Case Else
                GRDTranx.TextMatrix(i, 3) = "Retail"
        End Select
        Set RSTTRXFILE = New ADODB.Recordset
        RSTTRXFILE.Open "SELECT * From TEMPCN WHERE VCH_NO = " & rstTRANX!VCH_NO & " AND TRX_TYPE = '" & rstTRANX!TRX_TYPE & "' AND TRX_YEAR = '" & rstTRANX!TRX_YEAR & "' ", db, adOpenStatic, adLockReadOnly
        Do Until RSTTRXFILE.EOF
            GRDTranx.TextMatrix(i, 4) = IIf(IsDate(RSTTRXFILE!VCH_DATE), RSTTRXFILE!VCH_DATE, "")
            GRDTranx.TextMatrix(i, 5) = Format(Val(GRDTranx.TextMatrix(i, 5)) + RSTTRXFILE!TRX_TOTAL, "0.00")
            GRDTranx.TextMatrix(i, 6) = IIf(IsNull(RSTTRXFILE!VCH_DESC), "", Mid(RSTTRXFILE!VCH_DESC, 15))
            GRDTranx.TextMatrix(i, 7) = IIf((RSTTRXFILE!check_flag = "Y"), "Billed", "Pending")
...
'    rptPRINT.ReportFileName = Rptpath & "RPTSALESREG"
...
Private Sub Form_Load()
    ...
    GRDTranx.TextMatrix(0, 9) = "INV. DATE"
...
Private Sub GRDTranx_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            i = 0
            Set RSTTRXFILE = New ADODB.Recordset
            RSTTRXFILE.Open "Select * From TEMPCN WHERE VCH_NO = " & Val(LBLBILLNO.Caption) & " AND TRX_TYPE = '" & Trim(GRDTranx.TextMatrix(GRDTranx.Row, 2)) & "' AND TRX_YEAR = '" & Trim(GRDTranx.TextMatrix(GRDTranx.Row, 10)) & "'", db, adOpenStatic, adLockReadOnly, adCmdText
            Do Until RSTTRXFILE.EOF
                i = i + 1
