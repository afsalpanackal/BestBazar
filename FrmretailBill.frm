...
Private Sub CmdPrint_Click()
    ...
    If Month(Date) >= 5 And Year(Date) = 2016 Then
        Exit Sub
    End If
'    Set TRXMAST = New ADODB.Recordset
'    TRXMAST.Open "Select MAX(Val(VCH_NO)) From TRXMAST", db, adOpenForwardOnly
...
'    End If
    ...
    Do Until rstTRXMAST.EOF
        ...
        RSTTRXFILE!LINE_DISC = rstTRXMAST!LINE_DISC
        RSTTRXFILE!SCHEME = 0
        RSTTRXFILE!EXP_DATE = Null
    ...
    If DataList2.BoundText <> "130000" Then
        ...
        m_Rcpt_Amt = 0
        Set RSTTRXFILE = New ADODB.Recordset
        RSTTRXFILE.Open "Select * From DBTPYMT WHERE ACT_CODE = '" & DataList2.BoundText & "' AND (TRX_TYPE = 'RT' OR TRX_TYPE = 'SR')", db, adOpenStatic, adLockReadOnly, adCmdText
        Do Until RSTTRXFILE.EOF
    ...
    For i = 1 To Report.Database.Tables.Count
        Report.Database.Tables(i).SetLogOnInfo "ConnectionName", "D:\dbase\YEAR14-15\INVSOFT.SQL", "admin", "###DATABASE%%%RET"
    Next i
...
Private Sub Form_Load()
    ...
    rstBILL.Open "Select MAX(Val(VCH_NO)) From TRXMAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE = 'RI'", db, adOpenStatic, adLockReadOnly
...
Private Sub LblPack_LostFocus()
    txtretail.Text = Val(lblretail.Caption) * Val(LblPack.Text)
...
Private Sub txtBillNo_LostFocus()
    ...
    On Error GoTo eRRHAND
    Set TRXMAST = New ADODB.Recordset
    TRXMAST.Open "Select MAX(Val(VCH_NO)) FROM TRXMAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE = 'RI'", db, adOpenStatic, adLockReadOnly
    If Not (TRXMAST.EOF And TRXMAST.BOF) Then
...
Private Function AppendSale()
    ...
    i = 0
    If grdsales.Rows = 1 Then
        db.Execute "delete * FROM CASHATRXFILE WHERE INV_NO = " & Val(LBLBILLNO.Caption) & " AND INV_TYPE = 'RT' AND INV_TRX_TYPE = 'RI'"
    Else
        Set RSTITEMMAST = New ADODB.Recordset
        RSTITEMMAST.Open "SELECT * FROM CASHATRXFILE WHERE INV_NO = " & Val(LBLBILLNO.Caption) & " AND INV_TYPE = 'RT' AND INV_TRX_TYPE = 'RI'", db, adOpenStatic, adLockOptimistic, adCmdText
