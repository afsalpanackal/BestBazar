...
''''    Exit Sub
    ...
    If Not (RSTITEMMAST.EOF And RSTITEMMAST.BOF) Then
        ...
        db.Execute "Update RTRXFILE set BAL_QTY = 0 WHERE ITEM_CODE = '" & DataList2.BoundText & "' AND ISNULL(BAL_QTY) OR BAL_QTY <0 "
        BALQTY = 0
...
'    Set rstTRXMAST = Nothing
    ...
    Select Case Trim(GRDOUTWARD.TextMatrix(GRDOUTWARD.Row, 11))
        ...
        Case "SV"
            If Year(MDIMAIN.DTFROM.Value) <> Val(GRDOUTWARD.TextMatrix(GRDOUTWARD.Row, 13)) Then Exit Sub
            If IsFormLoaded(FRMSERVICEC) <> True Then
                FRMSERVICEC.txtBillNo.Text = Val(GRDOUTWARD.TextMatrix(GRDOUTWARD.Row, 12))
                FRMSERVICEC.LBLBILLNO.Caption = Val(GRDOUTWARD.TextMatrix(GRDOUTWARD.Row, 12))
                FRMSERVICEC.Show
                FRMSERVICEC.SetFocus
                Call FRMSERVICEC.txtBillNo_KeyDown(13, 0)
            ElseIf IsFormLoaded(FRMGST1) <> True Then
                FRMGST1.txtBillNo.Text = Val(GRDOUTWARD.TextMatrix(GRDOUTWARD.Row, 12))
                FRMGST1.LBLBILLNO.Caption = Val(GRDOUTWARD.TextMatrix(GRDOUTWARD.Row, 12))
                FRMGST1.Show
...
Private Sub TXTCODE_Change()
    ...
    If REPFLAG = True Then
        If FrmeAll.Visible = False Then
            RSTREP.Open "Select * From ITEMMAST  WHERE (ISNULL(UN_BILL) OR UN_BILL <> 'Y') AND ITEM_CODE Like '" & TxtCode.Text & "%'   ORDER BY ITEM_NAME", db, adOpenStatic, adLockReadOnly
        Else
            RSTREP.Open "Select * From ITEMMAST  WHERE ITEM_CODE Like '" & TxtCode.Text & "%'   ORDER BY ITEM_NAME", db, adOpenStatic, adLockReadOnly
        End If
        REPFLAG = False
    Else
        RSTREP.Close
...
Private Sub tXTMEDICINE_Change()
    ...
    If REPFLAG = True Then
        If FrmeAll.Visible = False Then
            RSTREP.Open "Select * From ITEMMAST  WHERE (ISNULL(UN_BILL) OR UN_BILL <> 'Y') AND ITEM_NAME Like '" & Me.TxtItemName.Text & "%' AND ITEM_NAME Like '%" & Me.tXTMEDICINE.Text & "%' AND ITEM_CODE Like '%" & Me.TxtCode.Text & "%'   ORDER BY ITEM_NAME", db, adOpenStatic, adLockReadOnly
        Else
            RSTREP.Open "Select * From ITEMMAST  WHERE ITEM_NAME Like '" & Me.TxtItemName.Text & "%' AND ITEM_NAME Like '%" & Me.tXTMEDICINE.Text & "%' AND ITEM_CODE Like '%" & Me.TxtCode.Text & "%'   ORDER BY ITEM_NAME", db, adOpenStatic, adLockReadOnly
        End If
        REPFLAG = False
    Else
        RSTREP.Close
...
Private Function Fillgrid()
    ...
    If Not (RSTITEM.EOF And RSTITEM.BOF) Then
        If UCase(RSTITEM!Category) = "SELF" Or UCase(RSTITEM!Category) = "SERVICE CHARGE" Or UCase(RSTITEM!Category) = "SERVICES" Then
            Label1(6).Visible = False
            Label1(8).Visible = False
            Label1(4).Visible = False
            LBLINWARD.Visible = False
            lblinval.Visible = False
            LBLOPQTY.Visible = False
        ...
        Else
            ...
            LBLINWARD.Visible = True
            lblinval.Visible = True
            LBLOPQTY.Visible = True
        ...
        Set RSTTRXFILE = New ADODB.Recordset
        RSTTRXFILE.Open "SELECT SUM(QTY * ITEM_COST) FROM RTRXFILE WHERE ITEM_CODE = '" & RSTITEM!ITEM_CODE & "' AND VCH_DATE <'" & Format(DTFROM.Value, "yyyy/mm/dd") & "'", db, adOpenForwardOnly
        If Not (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
