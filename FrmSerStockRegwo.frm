...
Begin VB.Form FRMServRegwo 
   ...
   EndProperty
   Icon            =   "FrmSerStockRegwo.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   End
End
Attribute VB_Name = "FRMServRegwo"
...
Private Sub CMDDISPLAY_Click()
    ...
    rstTRANX.Open "SELECT DISTINCT VCH_NO From SERVICE_STK WHERE [VCH_DATE] <=# " & TODATE & " # AND [VCH_DATE] >=# " & FROMDATE & " # ORDER BY VCH_NO", db2, adOpenStatic, adLockReadOnly
    Do Until rstTRANX.EOF
        ...
        RSTTRXFILE.Open "SELECT * From SERVICE_STK WHERE VCH_NO = " & rstTRANX!VCH_NO & "", db2, adOpenStatic, adLockReadOnly
...
Private Sub GRDTranx_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            i = 0
            Set RSTTRXFILE = New ADODB.Recordset
            RSTTRXFILE.Open "Select * From SERVICE_STK WHERE VCH_NO = " & Val(LBLBILLNO.Caption) & " AND TRX_TYPE = '" & Trim(GRDTranx.TextMatrix(GRDTranx.Row, 5)) & "'", db2, adOpenStatic, adLockReadOnly, adCmdText
            Do Until RSTTRXFILE.EOF
                i = i + 1
