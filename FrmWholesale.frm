   ...
   Begin VB.Frame FRMEMAIN 
      ...
      Begin VB.Frame FRMEHEAD 
         ...
         Begin VB.ComboBox cmbtype 
            ...
            ForeColor       =   &H00004080&
            Height          =   315
            ItemData        =   "FrmWholesale.frx":030A
            Left            =   7455
            List            =   "FrmWholesale.frx":0314
...
Private Sub CmdPrintA5_Click()
    ...
    If Month(Date) >= 5 And Year(Date) = 2017 Then Exit Sub
    If Month(MDIMAIN.DTFROM.value) >= 4 And Year(MDIMAIN.DTFROM.value) = 2017 Then
...
'    End If
    ...
    If DataList2.BoundText <> "130000" Then
        ...
        m_Rcpt_Amt = 0
        Set RSTTRXFILE = New ADODB.Recordset
        RSTTRXFILE.Open "Select * From DBTPYMT WHERE ACT_CODE = '" & DataList2.BoundText & "' AND (TRX_TYPE = 'RT' OR TRX_TYPE = 'SR') AND NOT(TRX_TYPE= 'RT' AND INV_TRX_TYPE ='SI' AND TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND INV_NO = " & Val(txtBillNo.Text) & ") ", db, adOpenStatic, adLockReadOnly, adCmdText
        Do Until RSTTRXFILE.EOF
    ...
    If Trim(TXTTIN.Text) <> "" Then
        If Small_Print = True Then
            ReportNameVar = "D:\EzBiz\RPTBILL8A5.RPT"
        Else
            ReportNameVar = "D:\EzBiz\rptbillretail8.RPT"
        End If
    Else
        If Small_Print = True Then
            ReportNameVar = "D:\EzBiz\RPTBILLA5.RPT"
        Else
            ReportNameVar = "D:\EzBiz\rptbillretail.RPT"
        End If
    End If
...
'    Next i
    For i = 1 To Report.OpenSubreport("RPTBILL1.rpt").Database.Tables.Count
        Report.OpenSubreport("RPTBILL1.rpt").Database.Tables(i).SetLogOnInfo "ConnectionName", "D:\EzBiz\INVSOFT.SQL", "admin", "###DATABASE%%%RET"
    Next i
    For i = 1 To Report.OpenSubreport("RPTBILL2.rpt").Database.Tables.Count
        Report.OpenSubreport("RPTBILL2.rpt").Database.Tables(i).SetLogOnInfo "ConnectionName", "D:\EzBiz\INVSOFT.SQL", "admin", "###DATABASE%%%RET"
    Next i
    For i = 1 To Report.OpenSubreport("RPTBILL3.rpt").Database.Tables.Count
        Report.OpenSubreport("RPTBILL3.rpt").Database.Tables(i).SetLogOnInfo "ConnectionName", "D:\EzBiz\INVSOFT.SQL", "admin", "###DATABASE%%%RET"
    Next i
...
Private Sub TXTDEALER_Change()
    ...
    If flagchange.Caption <> "1" Then
        If ACT_FLAG = True Then
            ACT_REC.Open "select ACT_CODE, ACT_NAME from [CUSTMAST] WHERE ACT_NAME Like '" & Me.TXTDEALER.Text & "%' AND ACT_CODE <> '130000' ORDER BY ACT_NAME", db, adOpenStatic, adLockReadOnly, adCmdText
            ACT_FLAG = False
        Else
            ACT_REC.Close
            ACT_REC.Open "select ACT_CODE, ACT_NAME from [CUSTMAST] WHERE ACT_NAME Like '" & Me.TXTDEALER.Text & "%' AND ACT_CODE <> '130000' ORDER BY ACT_NAME", db, adOpenStatic, adLockReadOnly, adCmdText
