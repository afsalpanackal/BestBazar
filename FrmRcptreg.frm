...
Begin VB.Form FRMRcptReg 
   ...
   Begin VB.Frame FRMEMAIN 
      ...
      Begin VB.Frame Frame2 
         ...
         Top             =   8280
         Width           =   18585
         Begin VB.CommandButton CmdAreaLegder 
            Caption         =   "Area Wise Ledger"
            BeginProperty Font 
               Name            =   "Arial"
               Size            =   9
               Charset         =   204
...
Private Sub CmdAreaLegder_Click()
    ...
    Else
        RSTTRXFILE.Open "Select * from DBTPYMT Where BR_ADDRESS = '" & Datacompany.Text & "' and ACT_CODE ='" & DataList2.BoundText & "' and (TRX_TYPE ='CB' OR TRX_TYPE ='DB' OR TRX_TYPE ='RT' OR TRX_TYPE ='RW' OR TRX_TYPE ='SR' OR TRX_TYPE ='DR' OR TRX_TYPE = 'EP' OR TRX_TYPE = 'VR' OR TRX_TYPE = 'ER' OR TRX_TYPE = 'PY' OR TRX_TYPE = 'RD') and INV_DATE < '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' ", db, adOpenStatic, adLockReadOnly, adCmdText
    End If
...
'        RSTTRXFILE!BANK_CODE = Null
    ...
    Else
        Report.RecordSelectionFormula = "({DBTPYMT.ACT_CODE}='" & DataList2.BoundText & "') and ({DBTPYMT.BR_ADDRESS}='" & Datacompany.Text & "') and ({DBTPYMT.TRX_TYPE} ='AA' OR {DBTPYMT.TRX_TYPE} ='CB' OR {DBTPYMT.TRX_TYPE} ='DB' OR {DBTPYMT.TRX_TYPE} ='RT' OR {DBTPYMT.TRX_TYPE} ='DR' OR {DBTPYMT.TRX_TYPE} ='RW' OR {DBTPYMT.TRX_TYPE} ='SR' OR {DBTPYMT.TRX_TYPE} = 'EP' OR {DBTPYMT.TRX_TYPE} = 'VR' OR {DBTPYMT.TRX_TYPE} = 'ER' OR {DBTPYMT.TRX_TYPE} = 'PY' OR {DBTPYMT.TRX_TYPE} = 'RD') AND ({DBTPYMT.INV_DATE} <=# " & Format(DTTO.Value, "MM,DD,YYYY") & " # AND {DBTPYMT.INV_DATE} >=# " & Format(DTFROM.Value, "MM,DD,YYYY") & " #)"
    End If
...
'    db.Execute "Update DBTPYMT set INV_AMT =0 where isnull(INV_AMT)"
    ...
    grdreceipts.TextMatrix(0, 7) = "Ref No."
    ...
    grdreceipts.ColWidth(6) = 1600
    grdreceipts.ColWidth(7) = 3500
    ...
    DTFROM.Value = DateAdd("m", -3, Date)
    DTFROM.Value = "01/" & Month(DTFROM.Value) & "/" & Year(DTFROM.Value)
...
'                        Set rstTRANX2 = Nothing
                    ...
                    Do Until RSTTRXFILE.EOF
                        ...
                        grdreceipts.TextMatrix(i, 7) = IIf(IsNull(RSTTRXFILE!REF_NO), "", RSTTRXFILE!REF_NO)
                        RSTTRXFILE.MoveNext
                    Loop
                    RSTTRXFILE.Close
                    Set RSTTRXFILE = Nothing
...
'            End If
        ...
        Case vbKeyF6
            ...
            FRMRECEIPT.lblarea.Caption = GRDTranx.TextMatrix(GRDTranx.Row, 29)
...
Private Sub txtcompany_Change()
    ...
    If RSTCOMPANY.State = 1 Then RSTCOMPANY.Close
    RSTCOMPANY.Open "Select DISTINCT BR_ADDRESS From DBTPYMT WHERE ACT_CODE = '" & DataList2.BoundText & "' AND BR_ADDRESS <> '' AND BR_ADDRESS Like '" & txtcompany.Text & "%' ORDER BY BR_ADDRESS", db, adOpenStatic, adLockReadOnly
    Set Me.Datacompany.RowSource = RSTCOMPANY
    Datacompany.ListField = "BR_ADDRESS"
    Datacompany.BoundColumn = "BR_ADDRESS"
