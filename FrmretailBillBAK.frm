...
Begin VB.Form FRMESTIMATE 
   ...
   Caption         =   "RETAIL BILL ENTRY"
   ClientHeight    =   9060
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   16740
   Icon            =   "FrmretailBill.frx":0000
   KeyPreview      =   -1  'True
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin VB.Frame FRMEMAIN 
      ...
      Begin VB.Frame FRMEHEAD 
         ...
         Begin VB.ComboBox cmbtype 
            ...
            ForeColor       =   &H00004080&
            Height          =   315
            ItemData        =   "FrmretailBill.frx":030A
            Left            =   6855
            List            =   "FrmretailBill.frx":0317
...
'    End If
    ...
    db.Execute "delete * From TRXFILE WHERE TRX_TYPE='RI' AND VCH_NO = " & Val(txtBillNo.Text) & ""
    db.Execute "delete * From TRXMAST WHERE TRX_TYPE='RI' AND VCH_NO = " & Val(txtBillNo.Text) & ""
    ...
    If Dos_Print = False Then
        If DN_CN_FLag = False Then
            If Trim(TXTTIN.Text) <> "" Then
                If Small_Print = True Then
                    ReportNameVar = App.Path & "\RPTBILL8A5.RPT"
                Else
                    ReportNameVar = App.Path & "\rptbillretail8.RPT"
                End If
            Else
                If Small_Print = True Then
                    ReportNameVar = App.Path & "\RPTBILLA5.RPT"
                Else
        ...
        Else
            If Trim(TXTTIN.Text) <> "" Then
                If Small_Print = True Then
                    ReportNameVar = App.Path & "\RPTBILL8A5CN.RPT"
                Else
                    ReportNameVar = App.Path & "\rptbillretail8CN.RPT"
                End If
            Else
                If Small_Print = True Then
                    ReportNameVar = App.Path & "\RPTBILLA5CN.RPT"
                Else
        ...
        For i = 1 To Report.Database.Tables.Count
            Report.Database.Tables(i).SetLogOnInfo "ConnectionName", "E:\dbase\YEAR14-15\MDINV.MDB", "admin", "!@#$%^&*())(*&^%$#@!"
        Next i
...
Private Sub Form_Load()
    ...
    rstBILL.Open "Select MAX(Val(VCH_NO)) From TRXMAST WHERE TRX_TYPE = 'RI'", db, adOpenStatic, adLockReadOnly
...
'                        End If
            ...
            If ITEM_FLAG = True Then
                PHY_ITEM.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN, CHECK_FLAG, LOOSE_PACK, PACK_TYPE, CATEGORY, WARRANTY, WARRANTY_TYPE  From ITEMMAST  WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' ORDER BY ITEM_NAME", db, adOpenStatic, adLockReadOnly
                ITEM_FLAG = False
            Else
                PHY_ITEM.Close
                PHY_ITEM.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN, CHECK_FLAG, LOOSE_PACK, PACK_TYPE, CATEGORY, WARRANTY, WARRANTY_TYPE  From ITEMMAST  WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' ORDER BY ITEM_NAME", db, adOpenStatic, adLockReadOnly
...
Private Sub TXTBILLNO_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            TRXSUB.Open "Select * FROM TRXSUB WHERE TRX_TYPE='RI' AND VCH_NO = " & Val(txtBillNo.Text) & " ORDER BY LINE_NO", db, adOpenStatic, adLockReadOnly
            Do Until TRXSUB.EOF
                Set TRXFILE = New ADODB.Recordset
                TRXFILE.Open "Select * From TRXFILE WHERE TRX_TYPE='RI' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(TRXSUB!LINE_NO) & "", db, adOpenStatic, adLockReadOnly
            ...
            Set TRXMAST = New ADODB.Recordset
            TRXMAST.Open "Select * FROM TRXMAST WHERE TRX_TYPE='RI' AND VCH_NO = " & Val(txtBillNo.Text) & "", db, adOpenStatic, adLockReadOnly
            If Not (TRXMAST.EOF Or TRXMAST.BOF) Then
...
Private Sub txtBillNo_LostFocus()
    ...
    On Error GoTo ErrHand
    Set TRXMAST = New ADODB.Recordset
    TRXMAST.Open "Select MAX(Val(VCH_NO)) FROM TRXMAST WHERE TRX_TYPE = 'RI'", db, adOpenStatic, adLockReadOnly
...
'    Set TRXMAST = New ADODB.Recordset
'    TRXMAST.Open "Select MIN(Val(VCH_NO)) FROM TRXFILE WHERE TRX_TYPE = 'RI'", db, adOpenStatic, adLockReadOnly
'    If Not (TRXMAST.EOF And TRXMAST.BOF) Then
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            If PHYFLAG = True Then
                PHY.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE  From ITEMMAST  WHERE ITEM_NAME Like '" & Me.TXTPRODUCT.Text & "%'ORDER BY [ITEM_NAME] ", db, adOpenStatic, adLockReadOnly
                PHYFLAG = False
            Else
                PHY.Close
                PHY.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE   From ITEMMAST  WHERE ITEM_NAME Like '" & Me.TXTPRODUCT.Text & "%'ORDER BY [ITEM_NAME] ", db, adOpenStatic, adLockReadOnly
...
Private Sub TXTQTY_LostFocus()
    ...
    If Not (RSTITEMCOST.EOF Or RSTITEMCOST.BOF) Then
        If OptLoose.value = True Then
            ...
            LBLSELPRICE.Caption = IIf(IsNull(RSTITEMCOST!SALES_PRICE), "", RSTITEMCOST!SALES_PRICE / Val(lblpack.Caption))
        Else
...
Private Function AppendSale()
    ...
    If OLD_BILL = False Then Call checklastbill
    db.Execute "delete * FROM TRXMAST WHERE TRX_TYPE='RI' AND VCH_NO = " & Val(txtBillNo.Text) & ""
    db.Execute "delete * FROM TRXSUB WHERE TRX_TYPE='RI' AND VCH_NO = " & Val(txtBillNo.Text) & ""
    db.Execute "delete * FROM TRXFILE WHERE TRX_TYPE='RI' AND VCH_NO = " & Val(txtBillNo.Text) & ""
    ...
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * FROM TRXMAST WHERE TRX_TYPE='RI' AND VCH_NO = " & Val(txtBillNo.Text) & "", db, adOpenStatic, adLockOptimistic, adCmdText
...
CLOSEFILE:
    ...
    If Not (RSTCOMPANY.EOF And RSTCOMPANY.BOF) Then
       ...
       ' Print #1, Chr(27) & Chr(72) &  "Salesman: CS"
        ...
        Print #1, "For GEO TRADERS & AGENCIES"
...
Private Sub TXTITEMCODE_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            Set grdtmp.DataSource = Nothing
            If PHYFLAG = True Then
                PHY.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE  From ITEMMAST  WHERE ITEM_CODE = '" & Me.TXTITEMCODE.Text & "' ", db, adOpenStatic, adLockReadOnly
                PHYFLAG = False
            Else
                PHY.Close
                PHY.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE  From ITEMMAST  WHERE ITEM_CODE = '" & Me.TXTITEMCODE.Text & "' ", db, adOpenStatic, adLockReadOnly
...
Function FILL_BATCHGRID()
    ...
    If BATCH_FLAG = True Then
        PHY_BATCH.Open "Select REF_NO, BAL_QTY, VCH_NO, LINE_NO, TRX_TYPE, VCH_DATE, ITEM_NAME, WARRANTY, WARRANTY_TYPE From RTRXFILE  WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY > 0 ORDER BY VCH_DATE DESC", db, adOpenForwardOnly
        BATCH_FLAG = False
    Else
        PHY_BATCH.Close
        PHY_BATCH.Open "Select REF_NO, BAL_QTY, VCH_NO, LINE_NO, TRX_TYPE, VCH_DATE, ITEM_NAME, WARRANTY, WARRANTY_TYPE From RTRXFILE  WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY > 0 ORDER BY VCH_DATE DESC", db, adOpenForwardOnly
...
Function FILL_PREVIIOUSRATE2()
    ...
    If PRERATE_FLAG = True Then
        PHY_PRERATE.Open "Select ITEM_CODE, VCH_DESC, VCH_DATE, QTY, P_RETAIL, M_USER_ID, VCH_NO, ITEM_NAME  From TRXFILE  WHERE TRX_TYPE ='RI' AND ITEM_CODE = '" & TXTITEMCODE.Text & "' AND VCH_NO <> " & Val(txtBillNo.Text) & " ORDER BY [VCH_DATE] DESC", db, adOpenStatic, adLockReadOnly
        PRERATE_FLAG = False
    Else
        PHY_PRERATE.Close
        PHY_PRERATE.Open "Select ITEM_CODE, VCH_DESC, VCH_DATE, QTY, P_RETAIL, M_USER_ID, VCH_NO, ITEM_NAME  From TRXFILE  WHERE TRX_TYPE ='RI' AND ITEM_CODE = '" & TXTITEMCODE.Text & "' AND VCH_NO <> " & Val(txtBillNo.Text) & " ORDER BY [VCH_DATE] DESC", db, adOpenStatic, adLockReadOnly
...
Private Function checklastbill()
    ...
    Set rstBILL = New ADODB.Recordset
    rstBILL.Open "Select MAX(Val(VCH_NO)) From TRXMAST WHERE TRX_TYPE = 'RI'", db, adOpenForwardOnly
    If Not (rstBILL.EOF And rstBILL.BOF) Then
        txtBillNo.Text = IIf(IsNull(rstBILL.Fields(0)), 1, rstBILL.Fields(0) + 1)
    End If
    rstBILL.Close
