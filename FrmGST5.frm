...
Begin VB.Form FRMGSTR 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "GST RETAIL (B2C) - BILL NO. I"
   ClientHeight    =   11100
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   18495
   ForeColor       =   &H00FF0000&
   Icon            =   "FrmGST5.frx":0000
   KeyPreview      =   -1  'True
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin VB.Frame FRMEMAIN 
      ...
      Begin VB.Frame FRMEHEAD 
         ...
         Begin VB.ComboBox cmbtype 
            ...
            ForeColor       =   &H00FF0000&
            Height          =   315
            ItemData        =   "FrmGST5.frx":030A
            Left            =   7455
            List            =   "FrmGST5.frx":0317
...
'                        End If
            ...
            Select Case cmbtype.ListIndex
                ...
                Case 1
                    ...
                    'RSTBATCH.Open "Select * From RTRXFILE WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY >0 ", db, adOpenStatic, adLockReadOnly
                    RSTBATCH.Open "Select DISTINCT ITEM_CODE, P_WS, REF_NO From RTRXFILE WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY >0 ANDP_WS >0 ", db, adOpenStatic, adLockReadOnly
                Case Else
...
Private Sub TXTBATCH_KeyDown(KeyCode As Integer, Shift As Integer)
    Select Case KeyCode
        Case vbKeyReturn
            ...
            For i = 1 To grdsales.Rows - 1
                'If Trim(grdsales.TextMatrix(i, 13)) = Trim(TXTITEMCODE.Text) Then
                If i <> Val(TXTSLNO.Text) And Trim(txtBatch.Text) <> "" And Trim(grdsales.TextMatrix(i, 10)) = Trim(txtBatch.Text) Then
                    If MsgBox("This Serial No Already exists.... Do yo want to add this again", vbYesNo + vbDefaultButton2, "BILL..") = vbNo Then
...
Private Sub TXTQTY_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            If FrmeType.Visible = True And OptNormal.value = False And OptLoose.value = False Then
                MsgBox "Please select Loose / Full Option" & Chr(13) & "Press 'L' of 'F'", vbOKOnly, "EzBiz"
                TXTQTY.SetFocus
                Exit Sub
            End If
...
'    End If
    'Print #1, RepeatString("-", 80)
    Print #1, "D.N. NO & Date" & Space(3) & "P.O. NO. & Date" & Space(3) & "D.Doc.NO & Date" & Space(3) & "Del Terms" & Space(5) & "Veh. No"
    Print #1,
    Print #1, RepeatString("-", 80)
    'Print #1, Chr(27) & Chr(71) & Chr(10) & Space(41) & AlignLeft("INVOICE FORM 8H", 16)
   ...
   ' Print #1, Chr(27) & Chr(72) &  "Salesman: CS"
    ...
    Print #1, RepeatString("-", 80)
    Print #1, AlignLeft("Description", 20) & _
            AlignLeft("HSN", 7) & Space(1) & _
            AlignLeft("Qty", 6) & Space(0) & _
    ...
    For i = 1 To grdsales.Rows - 1
        ...
        Print #1, AlignLeft(grdsales.TextMatrix(i, 2), 20) & Space(0) & _
            AlignLeft(HSNCODE, 5) & _
...
SKIP_UNBILL:
    ...
    Do Until RSTTRXFILE.EOF
        ...
        RSTtax.Close
        Set RSTtax = Nothing
        Taxsplit = Taxsplit & "Taxable: " & Format(Round(TaxableAmt, 2), "0.00") & "SGST " & RSTTRXFILE!SALES_TAX / 2 & "%: Tax: " & Format(Round(TaxAmt / 2, 2), "0.00") & " CGST " & RSTTRXFILE!SALES_TAX / 2 & "%: Tax: " & Format(Round(TaxAmt / 2, 2), "0.00") & Chr(13)  '"|"
        RSTTRXFILE.MoveNext
    Loop
    RSTTRXFILE.Close
    Set RSTTRXFILE = Nothing
    ...
    Else
        If TotalTax > 0 Then
            ...
            Print #1, AlignLeft("CGST Tax Amt: " & Format(Round(TotalTax / 2, 2), "0.00"), 48)
            Print #1, AlignLeft("SGST Tax Amt: " & Format(Round(TotalTax / 2, 2), "0.00"), 48)
            Print #1, AlignLeft("IGST Tax Amt: " & "0.00", 48)
        End If
...
'    End If
    ...
    If DataList2.BoundText <> "130000" And DataList2.BoundText <> "130001" Then
        ...
        txtOutstanding.Text = Round((m_OP_Bal + m_Bal_Amt) - (m_Rcpt_Amt), 2)
    End If
SKIP_OUT:
    'If OLD_BILL = False Then Call checklastbill
    Set RSTTRXFILE = New ADODB.Recordset
