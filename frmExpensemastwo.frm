...
Begin VB.Form frmExpmastwo 
   BackColor       =   &H0080C0FF&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Expense Head Creation"
   ClientHeight    =   3180
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   7155
   ControlBox      =   0   'False
   Icon            =   "frmExpensemastwo.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   End
End
Attribute VB_Name = "frmExpmastwo"
...
Private Sub CmdDelete_Click()
    ...
    RSTSUPMAST.Open "SELECT M_USER_ID From TRXEXPENSE WHERE [ACT_CODE] = '" & Txtsuplcode.Tag & "'", db2, adOpenStatic, adLockReadOnly
...
Private Sub CmdSave_Click()
    ...
    Txtsuplcode.Tag = "641" & Txtsuplcode.Text
...
Private Sub Form_Load()
    ...
    TRXMAST.Open "Select MAX(Val(ACT_CODE)) From ACTMAST WHERE (Mid(ACT_CODE, 1, 3)='641')And (len(ACT_CODE)>3)", db2, adOpenStatic, adLockReadOnly
...
Private Sub txtsupplist_Change()
    ...
    If REPFLAG = True Then
        RSTREP.Open "Select [ACT_CODE],[ACT_NAME] From ACTMAST  WHERE (Mid(ACT_CODE, 1, 3)='641')And (len(ACT_CODE)>3) And ACT_NAME Like '" & Me.txtsupplist.Text & "%'ORDER BY [ACT_NAME]", db2, adOpenStatic, adLockReadOnly
        REPFLAG = False
    Else
        RSTREP.Close
        RSTREP.Open "Select [ACT_CODE],[ACT_NAME] From ACTMAST  WHERE (Mid(ACT_CODE, 1, 3)='641')And (len(ACT_CODE)>3) And ACT_NAME Like '" & Me.txtsupplist.Text & "%'ORDER BY [ACT_NAME]", db2, adOpenStatic, adLockReadOnly
