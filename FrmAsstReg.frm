...
Begin VB.Form FRMAsstReg 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "ASSET PURCHASE REPORT"
   ClientHeight    =   10650
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   18120
   ...
   Begin VB.Frame FRMEMAIN 
      ...
      Begin vbalProgBarLib6.vbalProgressBar vbalProgressBar1 
         ...
         _ExtentX        =   8864
         _ExtentY        =   609
         Picture         =   "FrmAsstReg.frx":030A
         ForeColor       =   0
         BarPicture      =   "FrmAsstReg.frx":0326
         Max             =   150
         Text            =   "PLEASE WAIT..."
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "Tahoma"
...
Private Sub CmDDisplay_Click()
    ...
    If OPTPERIOD.Value = True Then
        If OptSortSupp.Value = True Then
            rstTRANX.Open "SELECT * From ASTRXMAST WHERE PINV Like '%" & Trim(TxtBillNos.text) & "%' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND (TRX_TYPE='AP' OR TRX_TYPE='PW' OR TRX_TYPE='LP') ORDER BY ACT_NAME, VCH_DATE, VCH_NO", db, adOpenStatic, adLockReadOnly
        ElseIf OptSortDate.Value = True Then
            rstTRANX.Open "SELECT * From ASTRXMAST WHERE PINV Like '%" & Trim(TxtBillNos.text) & "%' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND (TRX_TYPE='AP' OR TRX_TYPE='PW' OR TRX_TYPE='LP') ORDER BY VCH_DATE, VCH_NO", db, adOpenStatic, adLockReadOnly
        Else
            rstTRANX.Open "SELECT * From ASTRXMAST WHERE PINV Like '%" & Trim(TxtBillNos.text) & "%' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND (TRX_TYPE='AP' OR TRX_TYPE='PW' OR TRX_TYPE='LP') ORDER BY TRX_TYPE, VCH_NO, VCH_DATE", db, adOpenStatic, adLockReadOnly
        End If
    Else
        If OptSortSupp.Value = True Then
            rstTRANX.Open "SELECT * From ASTRXMAST WHERE PINV Like '%" & Trim(TxtBillNos.text) & "%' AND ACT_CODE = '" & DataList2.BoundText & "' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND (TRX_TYPE='AP' OR TRX_TYPE='PW' OR TRX_TYPE='LP') ORDER BY ACT_NAME, VCH_DATE, VCH_NO", db, adOpenStatic, adLockReadOnly
        ElseIf OptSortDate.Value = True Then
            rstTRANX.Open "SELECT * From ASTRXMAST WHERE PINV Like '%" & Trim(TxtBillNos.text) & "%' AND ACT_CODE = '" & DataList2.BoundText & "' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND (TRX_TYPE='AP' OR TRX_TYPE='PW' OR TRX_TYPE='LP') ORDER BY VCH_DATE, VCH_NO", db, adOpenStatic, adLockReadOnly
        Else
    ...
    Do Until rstTRANX.EOF
        ...
        'If rstTRANX!TRX_TYPE <> "AP" Then GoTo SKIP
        Set RSTTRXFILE = New ADODB.Recordset
        RSTTRXFILE.Open "Select DISTINCT SALES_TAX From ASTRXFILE WHERE TRX_YEAR = '" & rstTRANX!TRX_YEAR & "' AND VCH_NO = " & rstTRANX!VCH_NO & " AND TRX_TYPE = '" & rstTRANX!TRX_TYPE & "'", db, adOpenStatic, adLockReadOnly, adCmdText
        Do Until RSTTRXFILE.EOF
            ...
            RSTtax.Open "Select * From ASTRXFILE WHERE TRX_YEAR = '" & rstTRANX!TRX_YEAR & "' AND VCH_NO = " & rstTRANX!VCH_NO & " AND TRX_TYPE = '" & rstTRANX!TRX_TYPE & "' AND SALES_TAX = " & RSTTRXFILE!SALES_TAX & "", db, adOpenStatic, adLockReadOnly, adCmdText
            Do Until RSTtax.EOF
...
Private Sub CMDPRINTREGISTER_Click()
    ...
    On Error GoTo ERRHAND
    Screen.MousePointer = vbHourglass
    ReportNameVar = Rptpath & "RPTASTREG"
...
Private Sub CMDPRINTREGISTER2_Click()
    ...
    Screen.MousePointer = vbHourglass
    On Error GoTo ERRHAND
    ReportNameVar = Rptpath & "RPTASTREPORT"
    Set Report = crxApplication.OpenReport(ReportNameVar, 1)
    If OPTCUSTOMER.Value = True Then
        Report.RecordSelectionFormula = "({TRXMAST.ACT_CODE}='" & DataList2.BoundText & "' AND {TRXFILE.TRX_TYPE}='AP' AND {TRXFILE.VCH_DATE} <=# " & Format(DTTo.Value, "MM,DD,YYYY") & " # AND {TRXFILE.VCH_DATE} >=# " & Format(DTFROM.Value, "MM,DD,YYYY") & " # )"
    Else
        Report.RecordSelectionFormula = "({TRXFILE.TRX_TYPE}='AP' AND {TRXFILE.VCH_DATE} <=# " & Format(DTTo.Value, "MM,DD,YYYY") & " # AND {TRXFILE.VCH_DATE} >=# " & Format(DTFROM.Value, "MM,DD,YYYY") & " # )"
    End If
    Set CRXFormulaFields = Report.FormulaFields
    For i = 1 To Report.Database.Tables.COUNT
        Report.Database.Tables.Item(i).SetLogOnInfo strConnection
    ...
    If UCase(dbase1) <> "INVSOFT" And UCase(dbase1) <> "INVDB" And UCase(dbase1) <> "INVSOFT3" Then
        Set oRs = New ADODB.Recordset
        Set oRs = db.Execute("SELECT * FROM ASTRXFILE ")
        Report.Database.SetDataSource oRs, 3, 1
        Set oRs = Nothing
        
        Set oRs = New ADODB.Recordset
        Set oRs = db.Execute("SELECT * From ASTRXMAST ")
...
Private Sub CmdReport_Click()
    ...
    On Error GoTo ERRHAND
    ReportNameVar = Rptpath & "RPTASTSUM"
    Set Report = crxApplication.OpenReport(ReportNameVar, 1)
    Report.RecordSelectionFormula = "(({TRANSMAST.TRX_TYPE}='AP' OR {TRANSMAST.TRX_TYPE}='LP' OR {TRANSMAST.TRX_TYPE}='PW')AND {TRANSMAST.VCH_DATE} <=# " & Format(DTTo.Value, "MM,DD,YYYY") & " # AND {TRANSMAST.VCH_DATE} >=# " & Format(DTFROM.Value, "MM,DD,YYYY") & " # )"
    Set CRXFormulaFields = Report.FormulaFields
    For i = 1 To Report.Database.Tables.COUNT
        Report.Database.Tables.Item(i).SetLogOnInfo strConnection
        If UCase(dbase1) <> "INVSOFT" And UCase(dbase1) <> "INVDB" And UCase(dbase1) <> "INVSOFT3" Then
...
Private Sub GRDTranx_DblClick()
    Select Case Trim(GRDTranx.TextMatrix(GRDTranx.Row, 10))
        Case "AP"
            ...
            If IsFormLoaded(frmAstPurchase) <> True Then
                frmAstPurchase.txtBillNo.text = Val(GRDTranx.TextMatrix(GRDTranx.Row, 1))
                frmAstPurchase.Show
                frmAstPurchase.SetFocus
                Call frmAstPurchase.txtBillNo_KeyDown(13, 0)
            End If
    End Select
End Sub
...
Private Sub GRDTranx_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            i = 0
            Set RSTTRXFILE = New ADODB.Recordset
            RSTTRXFILE.Open "Select * From ASTRXFILE WHERE TRX_YEAR = '" & GRDTranx.TextMatrix(GRDTranx.Row, 12) & "' AND VCH_NO = " & Val(LBLBILLNO.Caption) & " AND TRX_TYPE = '" & Trim(GRDTranx.TextMatrix(GRDTranx.Row, 10)) & "'", db, adOpenStatic, adLockReadOnly
            Do Until RSTTRXFILE.EOF
                i = i + 1
...
Private Sub TMPDELETE_Click()
    ...
    db.Execute ("DELETE from SALESREG WHERE SALESREG.VCH_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 1) & " AND SALESREG.TRX_TYPE = 'AP'")
    Call fillSTOCKREG
End Sub
