...
'    Set RSTTRXFILE = Nothing
    ...
    db.Execute "delete  From RTRXFILE WHERE TRX_TYPE='RI' AND VCH_NO = " & Val(TxtCN.Text) & ""
    db.Execute "delete  FROM RTRXFILE WHERE TRX_TYPE='MI' AND VCH_NO = " & Val(txtBillNo.Text) & ""
    ...
    For N = 1 To grdsales.Rows - 1
        ...
        db.Execute "delete  FROM TRXFILE_FORMULA WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE='HI' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_REF_NO = " & Val(grdsales.TextMatrix(N, 32)) & ""
        db.Execute "delete  FROM TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE='MI' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_REF_NO = " & Val(grdsales.TextMatrix(N, 32)) & ""
        
        Set RSTITEMMAST = New ADODB.Recordset
        RSTITEMMAST.Open "SELECT * FROM  TRXFORMULASUB WHERE FOR_NAME = '" & grdsales.TextMatrix(N, 13) & "' ", db, adOpenStatic, adLockReadOnly, adCmdText
        With RSTITEMMAST
            Do Until .EOF
                ...
                With rstTRXMAST
                    If Not (.EOF And .BOF) Then
                        !ISSUE_QTY = !ISSUE_QTY - (RSTITEMMAST!QTY / FOR_QTY) * Val(grdsales.TextMatrix(N, 3))
                        !FREE_QTY = 0
                        !ISSUE_VAL = 0
                        !CLOSE_QTY = !CLOSE_QTY + (RSTITEMMAST!QTY / FOR_QTY) * Val(grdsales.TextMatrix(N, 3))
                        !CLOSE_VAL = 0
                        'ITEMCOST = ITEMCOST + IIf(IsNull(!ITEM_COST), 0, !ITEM_COST * Val(grdsales.TextMatrix(N, 3)))
                        rstTRXMAST.Update
                    End If
                End With
                rstTRXMAST.Close
...
Private Sub CMDSALERETURN_Click()
    ...
    Do Until RSTTRXFILE.EOF
        ...
        grdsales.TextMatrix(grdsales.Rows - 1, 5) = Format(RSTTRXFILE!MRP, ".00")
        grdsales.TextMatrix(grdsales.Rows - 1, 6) = Format(RSTTRXFILE!PTR, ".00")
        grdsales.TextMatrix(grdsales.Rows - 1, 7) = Format(RSTTRXFILE!SALES_PRICE, ".00")
        ...
        grdsales.TextMatrix(grdsales.Rows - 1, 12) = Format(RSTTRXFILE!TRX_TOTAL, ".00")
...
'    End If
    ...
    grdsales.TextMatrix(Val(TXTSLNO.Text), 5) = Format(Val(TxtMRP.Text), ".00")
    grdsales.TextMatrix(Val(TXTSLNO.Text), 6) = Format(Val(TXTRETAILNOTAX.Text), ".00")
    grdsales.TextMatrix(Val(TXTSLNO.Text), 7) = Format(Val(txtretail.Text), ".00")
    ...
    Else
        ...
        TXTAPPENDTOTAL.Text = Format((Val(TXTAPPENDQTY.Text) * Round(Val(txtretail.Text), 3)) - Val(TXTDISC.Tag), ".00")
    End If
    
    TXTAPPENDTOTAL.Text = ""
    
    grdsales.TextMatrix(Val(TXTSLNO.Text), 12) = Format(Val(LBLSUBTOTAL.Caption) + Val(TXTAPPENDTOTAL.Text), ".00")
    ...
    grdsales.TextMatrix(Val(TXTSLNO.Text), 21) = Format(Val(txtretail.Text), ".00")
    grdsales.TextMatrix(Val(TXTSLNO.Text), 22) = Format(Val(TXTRETAILNOTAX.Text), ".00")
...
'                End If
        ...
        db.Execute "delete  FROM TRXFILE_FORMULA WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE='HI' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_REF_NO = " & Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 32)) & ""
        db.Execute "delete  FROM TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE='MI' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_REF_NO = " & Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 32)) & ""
            
        Set rstMaxNo = New ADODB.Recordset
        rstMaxNo.Open "Select MAX(LINE_NO) From TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE='MI' AND VCH_NO = " & Val(txtBillNo.Text) & "", db, adOpenStatic, adLockReadOnly
        If Not (rstMaxNo.EOF And rstMaxNo.BOF) Then
        ...
        ITEMCOST = 0
        Set RSTITEMMAST = New ADODB.Recordset
        RSTITEMMAST.Open "SELECT * FROM  TRXFORMULASUB WHERE FOR_NAME = '" & grdsales.TextMatrix(Val(TXTSLNO.Text), 13) & "' ", db, adOpenStatic, adLockReadOnly, adCmdText
        With RSTITEMMAST
            Do Until .EOF
                Set RSTTRXFILE = New ADODB.Recordset
                RSTTRXFILE.Open "Select * FROM TRXFILE_FORMULA", db, adOpenStatic, adLockOptimistic, adCmdText
                'RSTTRXFILE.Open "Select * FROM TRXFILE_FORMULA WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE='HI' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 32)) & "", db, adOpenStatic, adLockOptimistic, adCmdText
                RSTTRXFILE.AddNew
                RSTTRXFILE!TRX_TYPE = "HI"
                RSTTRXFILE!TRX_YEAR = Year(MDIMAIN.DTFROM.value)
                RSTTRXFILE!VCH_NO = Val(txtBillNo.Text)
                RSTTRXFILE!LINE_REF_NO = Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 32))
                RSTTRXFILE!line_no = i
                RSTTRXFILE!CREATE_DATE = Format(Date, "DD/MM/YYYY")
                'RSTTRXFILE!C_USER_ID = MDIMAIN.StatusBar.Panels(1).Text & "'"
                RSTTRXFILE!FOR_CODE = grdsales.TextMatrix(Val(TXTSLNO.Text), 13)
                RSTTRXFILE!FOR_NAME = grdsales.TextMatrix(Val(TXTSLNO.Text), 2)
                ...
                Else
                    RSTTRXFILE!QTY = (RSTITEMMAST!QTY / FOR_QTY) * Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 3))
                ...
                With rstTRXMAST
                    If Not (.EOF And .BOF) Then
                        !ISSUE_QTY = !ISSUE_QTY + (RSTITEMMAST!QTY / FOR_QTY) * Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 3))
                        !FREE_QTY = 0
                        !ISSUE_VAL = 0
                        !CLOSE_QTY = !CLOSE_QTY - (RSTITEMMAST!QTY / FOR_QTY) * Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 3))
                        !CLOSE_VAL = 0
                ...
                RSTTRXFILEFOR!VCH_NO = Val(txtBillNo.Text)
                RSTTRXFILEFOR!VCH_DATE = Format(TXTINVDATE.Text, "DD/MM/YYYY")
                RSTTRXFILEFOR!line_no = SN
                RSTTRXFILEFOR!LINE_REF_NO = Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 32))
                ...
                RSTTRXFILEFOR!VCH_DESC = "Issued to      Kitchen" '& Trim(DataList2.Text)
            ...
            RSTITEMCOST.Open "Select * FROM ITEMMAST WHERE ITEM_CODE = '" & grdsales.TextMatrix(Val(TXTSLNO.Text), 1) & "' ", db, adOpenStatic, adLockOptimistic, adCmdText
    ...
    '        Set rstMaxNo = New ADODB.Recordset
    '        rstMaxNo.Open "Select MAX(LINE_NO) From RTRXFILE WHERE TRX_TYPE='MI' AND VCH_NO = " & Val(txtBillNo.Text) & " ", db, adOpenStatic, adLockReadOnly, adCmdText
    '        If Not (rstMaxNo.EOF And rstMaxNo.BOF) Then
    '            SN = IIf(IsNull(rstMaxNo.Fields(0)), 1, rstMaxNo.Fields(0) + 1)
    '        Else
    '            SN = 1
    ...
    '        Set rstMaxNo = Nothing
            ...
            If RSTITEMMAST.RecordCount > 0 Then
                ...
                RSTRTRXFILE.Open "SELECT * From RTRXFILE WHERE TRX_TYPE='MI' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO= " & grdsales.TextMatrix(Val(TXTSLNO.Text), 32) & " ", db, adOpenStatic, adLockOptimistic, adCmdText
                If (RSTRTRXFILE.EOF And RSTRTRXFILE.BOF) Then
                    RSTRTRXFILE.AddNew
                    RSTRTRXFILE!TRX_TYPE = "MI"
                    RSTRTRXFILE!VCH_NO = Val(txtBillNo.Text)
                    RSTRTRXFILE!line_no = Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 32))
                    RSTRTRXFILE!ITEM_CODE = grdsales.TextMatrix(Val(TXTSLNO.Text), 13)
                    ...
                    Set rststock = New ADODB.Recordset
                    rststock.Open "SELECT *  From ITEMMAST WHERE ITEM_CODE = '" & grdsales.TextMatrix(Val(TXTSLNO.Text), 13) & "'", db, adOpenStatic, adLockOptimistic, adCmdText
                    With rststock
                        If Not (.EOF And .BOF) Then
                            ...
                            !RCPT_QTY = !RCPT_QTY + Round((Val(TXTQTY.Text) + Val(TXTFREE.Text)) * Val(LblPack.Text), 3) '* Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 5))
                    ...
                    Set rststock = Nothing
                Else
                    M_DATA = Round((Val(TXTQTY.Text) + Val(TXTFREE.Text)) * Val(LblPack.Text), 3) '* Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 5))
                    M_DATA = M_DATA - (RSTRTRXFILE!QTY - RSTRTRXFILE!BAL_QTY)
                    RSTRTRXFILE!BAL_QTY = M_DATA
                    RSTRTRXFILE!QTY = Round((Val(TXTQTY.Text) + Val(TXTFREE.Text)) * Val(LblPack.Text), 3) '* Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 5))
                    Set rststock = New ADODB.Recordset
                    ...
                    With rststock
                        If Not (.EOF And .BOF) Then
                            ...
                            !CLOSE_QTY = !CLOSE_QTY + Round((Val(TXTQTY.Text) + Val(TXTFREE.Text)) * Val(LblPack.Text), 3) '* Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 5))
                            If (IsNull(!CLOSE_VAL)) Then !CLOSE_VAL = 0
                ...
                RSTRTRXFILE!ITEM_NAME = grdsales.TextMatrix(Val(TXTSLNO.Text), 2)
                ...
                RSTRTRXFILE!MRP = 0 'Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 6))
...
Private Sub CmdDelete_Click()
    ...
    If Not (UCase(grdsales.TextMatrix(Val(TXTSLNO.Text), 25)) = "SERVICES" Or UCase(grdsales.TextMatrix(Val(TXTSLNO.Text), 25)) = "SELF") Then
        ...
        db.Execute "delete  FROM TRXFILE_FORMULA WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE='HI' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_REF_NO = " & Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 32)) & ""
        db.Execute "delete  FROM TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE='MI' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_REF_NO = " & Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 32)) & ""
        db.Execute "delete  FROM RTRXFILE WHERE TRX_TYPE='MI' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO= " & grdsales.TextMatrix(Val(TXTSLNO.Text), 32) & " "
...
Private Sub CmdDelivery_Click()
    ...
    For N = 0 To grdcount.Rows - 1
        ...
        RSTTRXFILE!GROSS_AMOUNT = Format(Round((Val(grdsales.TextMatrix(grdsales.Rows - 1, 3)) * Val(grdsales.TextMatrix(grdsales.Rows - 1, 6))) - Val(TXTDISC.Tag), 2), ".00")
...
'    End If
    ...
    grdsales.ColWidth(5) = 0 '1200
    grdsales.ColWidth(6) = 0 '1300
    grdsales.ColWidth(7) = 1300
    grdsales.ColWidth(8) = 900
    grdsales.ColWidth(9) = 0 '900
...
'                        End If
            ...
            Select Case cmbtype.ListIndex
                Case 0
                    ...
                    RSTBATCH.Open "Select DISTINCT ITEM_CODE, P_RETAIL, REF_NO, TRX_GODOWN From RTRXFILE WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY >0 ", db, adOpenStatic, adLockReadOnly
                Case 1
                    ...
                    'RSTBATCH.Open "Select * From RTRXFILE WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY >0 ", db, adOpenStatic, adLockReadOnly
                    RSTBATCH.Open "Select DISTINCT ITEM_CODE, P_WS, REF_NO, TRX_GODOWN From RTRXFILE WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY >0 ", db, adOpenStatic, adLockReadOnly
                Case Else
                    ...
                    'RSTBATCH.Open "Select * From RTRXFILE WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY >0 ", db, adOpenStatic, adLockReadOnly
                    RSTBATCH.Open "Select DISTINCT ITEM_CODE, P_VAN, REF_NO, TRX_GODOWN From RTRXFILE WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY >0 ", db, adOpenStatic, adLockReadOnly
            End Select
...
'            Set TRXMAST = Nothing
            ...
            Do Until TRXSUB.EOF
                ...
                If Not (TRXFILE.EOF And TRXFILE.BOF) Then
                    ...
                    grdsales.TextMatrix(i, 5) = Format(TRXFILE!MRP, ".00")
                    grdsales.TextMatrix(i, 6) = Format(TRXFILE!PTR, ".00")
                    grdsales.TextMatrix(i, 7) = Format(TRXFILE!SALES_PRICE, ".00")
                    ...
                    grdsales.TextMatrix(i, 12) = Format(Val(TRXFILE!TRX_TOTAL), ".00")
                    ...
                    grdsales.TextMatrix(i, 21) = IIf(IsNull(TRXFILE!P_RETAIL), "0.00", Format(TRXFILE!P_RETAIL, ".00"))
                    grdsales.TextMatrix(i, 22) = IIf(IsNull(TRXFILE!P_RETAILWOTAX), "0.00", Format(TRXFILE!P_RETAILWOTAX, ".00"))
...
Private Sub TXTDISC_LostFocus()
    ...
    If UCase(txtcategory.Text) = "SERVICE CHARGE" Then
        ...
        LBLSUBTOTAL.Caption = Format(Round(Val(txtretail.Text) - Val(TXTDISC.Tag), 2), ".00")
        LblGross.Caption = Format(Round(Val(TXTRETAILNOTAX.Text) - Val(TXTDISC.Tag), 2), ".00")
    Else
        TXTDISC.Tag = Val(TXTQTY.Text) * Val(txtretail.Text) * Val(TXTDISC.Text) / 100
        LBLSUBTOTAL.Caption = Format(Round((Val(TXTQTY.Text) * Val(txtretail.Text)) - Val(TXTDISC.Tag), 2), ".00")
        LblGross.Caption = Format(Round((Val(TXTQTY.Text) * Val(TXTRETAILNOTAX.Text)) - Val(TXTDISC.Tag), 2), ".00")
    ...
    ''TXTDISC.Text = Format(TXTDISC.Text, ".00")
...
Private Sub TxtMRP_LostFocus()
    TxtMRP.Text = Format(TxtMRP.Text, ".00")
End Sub
...
Private Sub TxtName1_Change()
    ...
    If PHYFLAG = True Then
        ...
        PHY.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE, MRP, CUST_DISC, MANUFACTURER, BIN_LOCATION, CESS_PER, CESS_AMT, P_LWS From ITEMMAST  WHERE ITEM_NAME Like '%" & Trim(Me.TxtName1.Text) & "%' OR MRP Like '" & Trim(Me.TxtName1.Text) & "' AND (ISNULL(DEAD_STOCK) OR DEAD_STOCK <> 'Y') ORDER BY ITEM_NAME ", db, adOpenStatic, adLockReadOnly
        PHYFLAG = False
    Else
        PHY.Close
        'PHY.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE, MRP, CUST_DISC, MANUFACTURER, BIN_LOCATION  From ITEMMAST  WHERE ucase(CATEGORY) = 'OWN' AND ITEM_NAME Like '%" & Trim(Me.TxtName1.Text) & "%' OR MRP Like '" & Trim(Me.TxtName1.Text) & "' ORDER BY ITEM_NAME ", db, adOpenStatic, adLockReadOnly
...
Private Sub TXTQTY_LostFocus()
    ...
    If Val(TXTRETAILNOTAX.Text) = 0 Then
        ...
        LBLSUBTOTAL.Caption = Format((Val(TXTQTY.Text) * Round(Val(txtretail.Text), 3)) - Val(TXTDISC.Tag) + Val(TXTTAX.Tag), ".00")
        LblGross.Caption = Format((Val(TXTQTY.Text) * Round(Val(TXTRETAILNOTAX.Text), 3)) - Val(TXTDISC.Tag), ".00")
    Else
        ...
        LBLSUBTOTAL.Caption = Format((Val(TXTQTY.Text) * Round(Val(TXTRETAILNOTAX.Text), 3)) - Val(TXTDISC.Tag) + Val(TXTTAX.Tag), ".00")
        LblGross.Caption = Format((Val(TXTQTY.Text) * Round(Val(TXTRETAILNOTAX.Text), 3)) - Val(TXTDISC.Tag), ".00")
    End If
...
'            End If
            ...
            If Val(TXTSLNO.Text) < grdsales.Rows Then
                ...
                LBLSUBTOTAL.Caption = Format(grdsales.TextMatrix(Val(TXTSLNO.Text), 12), ".00")
...
'    LBLSUBTOTAL.Caption = Format((Val(TXTQTY.Text) * Round(Val(TXTRATE.Text), 3)) - Val(TXTDISC.Tag) + Val(TXTTAX.Tag), ".00")
...
'    TXTFREE.Text = Format(TXTFREE.Text, ".00")
'    TXTDISC.Tag = 0
'    TXTTAX.Tag = 0
'    If Val(TXTRATE.Text) = 0 Then
'        TXTDISC.Tag = Val(TXTDISC.Text) / 100
'        TXTTAX.Tag = Val(TXTTAX.Text) / 100
'        LBLSUBTOTAL.Caption = Format((Val(TXTFREE.Text) * Round(Val(TXTRATE.Text), 3)) - Val(TXTDISC.Tag) + Val(TXTTAX.Tag), ".00")
'    Else
...
Private Sub TXTRETAILNOTAX_LostFocus()
    TXTRETAILNOTAX.Text = Format(Val(TXTRETAILNOTAX.Text), "0.00")
    If UCase(txtcategory.Text) = "PARDHA" Or UCase(txtcategory.Text) = "CLOTHES" Then
        If Val(TXTRETAILNOTAX.Text) < 1000 Then
            TXTTAX.Text = "5"
        Else
...
'    End If
    ...
    'TXTDISC.Tag = 0
    'TXTDISC.Tag = Val(TXTQTY.Text) * Val(TXTRETAILNOTAX.Text) * Val(TXTDISC.Text) / 100
    'LBLSUBTOTAL.Caption = Format((Val(TXTQTY.Text) * Round(Val(TXTRETAILNOTAX.Text), 3)) - Val(TXTDISC.Tag), ".00")
End Sub
...
Private Sub txtcommi_LostFocus()
    txtcommi.Text = Format(txtcommi.Text, ".00")
End Sub
...
Private Sub TXTTYPE_LostFocus()
    If cmbtype.ListIndex = -1 Then
        ...
        Exit Sub
    End If
     'If Val(TXTTYPE.Text) = 0 Then TXTTYPE.Text = 3
    ...
    Select Case KeyCode
        Case vbKeyReturn
            Select Case grdsales.Col
                        ...
                        With RSTTRXFILE
                            If Not (.EOF And .BOF) Then
                                ...
                                If UCase(grdsales.TextMatrix(grdsales.Row, 25)) = "SERVICE CHARGE" Then
                                    ...
                                    grdsales.TextMatrix(grdsales.Row, 12) = Format(Round(Val(grdsales.TextMatrix(grdsales.Row, 7)) - Val(TXTDISC.Tag), 4), ".00")
                                    grdsales.TextMatrix(grdsales.Row, 34) = Format(Round(Val(grdsales.TextMatrix(grdsales.Row, 6)) - Val(TXTDISC.Tag), 4), ".00")
                                Else
                                    ...
                                    grdsales.TextMatrix(grdsales.Row, 12) = Format(Round((Val(grdsales.TextMatrix(grdsales.Row, 3)) * Val(grdsales.TextMatrix(grdsales.Row, 7))) - Val(TXTDISC.Tag), 4), ".00")
                                    grdsales.TextMatrix(grdsales.Row, 34) = Format(Round((Val(grdsales.TextMatrix(grdsales.Row, 3)) * Val(grdsales.TextMatrix(grdsales.Row, 6))) - Val(TXTDISC.Tag), 4), ".00")
                                End If
                ...
                Case 6  'RATE
                    TXTDISC.Tag = 0
                    grdsales.TextMatrix(grdsales.Row, 7) = Format(Round(Val(TXTsample.Text) + Val(TXTsample.Text) * Val(grdsales.TextMatrix(grdsales.Row, 9)) / 100, 4), "0.00")
                    grdsales.TextMatrix(grdsales.Row, 21) = Format(Round(Val(TXTsample.Text) + Val(TXTsample.Text) * Val(grdsales.TextMatrix(grdsales.Row, 9)) / 100, 4), "0.00")
                    If UCase(grdsales.TextMatrix(grdsales.Row, 25)) = "SERVICE CHARGE" Then
                        TXTDISC.Tag = Val(grdsales.TextMatrix(grdsales.Row, 7)) * Val(grdsales.TextMatrix(grdsales.Row, 8)) / 100
                        grdsales.TextMatrix(grdsales.Row, 12) = Format(Round(Val(grdsales.TextMatrix(grdsales.Row, 7)) - Val(TXTDISC.Tag), 4), ".00")
                        grdsales.TextMatrix(grdsales.Row, 34) = Format(Round(Val(TXTsample.Text) - Val(TXTDISC.Tag), 4), ".00")
                    Else
                        TXTDISC.Tag = Val(grdsales.TextMatrix(grdsales.Row, 3)) * Val(grdsales.TextMatrix(grdsales.Row, 7)) * Val(grdsales.TextMatrix(grdsales.Row, 8)) / 100
                        grdsales.TextMatrix(grdsales.Row, 12) = Format(Round((Val(grdsales.TextMatrix(grdsales.Row, 3)) * Val(grdsales.TextMatrix(grdsales.Row, 7))) - Val(TXTDISC.Tag), 4), ".00")
                        grdsales.TextMatrix(grdsales.Row, 34) = Format(Round((Val(grdsales.TextMatrix(grdsales.Row, 3)) * Val(TXTsample.Text)) - Val(TXTDISC.Tag), 4), ".00")
                    End If
                    grdsales.TextMatrix(grdsales.Row, grdsales.Col) = Format(Val(TXTsample.Text), "0.00")
                    LBLTOTAL.Caption = ""
                    lblnetamount.Caption = ""
                ...
                Case 7  'NET RATE
                    ...
                    End If
                    'TXTRETAILNOTAX.Text = Round(Val(TXTRETAIL.Text) * 100 / (Val(TXTTAX.Text) + 100), 4)
                    grdsales.TextMatrix(grdsales.Row, 6) = Format(Round(Val(TXTsample.Text) * 100 / (Val(grdsales.TextMatrix(grdsales.Row, 9)) + 100), 4), "0.00")
                    grdsales.TextMatrix(grdsales.Row, 22) = Format(Round(Val(TXTsample.Text) * 100 / (Val(grdsales.TextMatrix(grdsales.Row, 9)) + 100), 4), "0.00")
                    If UCase(grdsales.TextMatrix(grdsales.Row, 25)) = "SERVICE CHARGE" Then
                        TXTDISC.Tag = Val(TXTsample.Text) * Val(grdsales.TextMatrix(grdsales.Row, 8)) / 100
                        grdsales.TextMatrix(grdsales.Row, 12) = Format(Round(Val(TXTsample.Text) - Val(TXTDISC.Tag), 4), ".00")
                        grdsales.TextMatrix(grdsales.Row, 34) = Format(Round(Val(grdsales.TextMatrix(grdsales.Row, 6)) - Val(TXTDISC.Tag), 4), ".00")
                    Else
                        TXTDISC.Tag = Val(grdsales.TextMatrix(grdsales.Row, 3)) * Val(TXTsample.Text) * Val(grdsales.TextMatrix(grdsales.Row, 8)) / 100
                        grdsales.TextMatrix(grdsales.Row, 12) = Format(Round((Val(grdsales.TextMatrix(grdsales.Row, 3)) * Val(TXTsample.Text)) - Val(TXTDISC.Tag), 4), ".00")
                        grdsales.TextMatrix(grdsales.Row, 34) = Format(Round((Val(grdsales.TextMatrix(grdsales.Row, 3)) * Val(grdsales.TextMatrix(grdsales.Row, 6))) - Val(TXTDISC.Tag), 4), ".00")
                    End If
                    grdsales.TextMatrix(grdsales.Row, grdsales.Col) = Format(Val(TXTsample.Text), "0.00")
                    LBLTOTAL.Caption = ""
                ...
                Case 9  'TAX
                    TXTDISC.Tag = 0
                    grdsales.TextMatrix(grdsales.Row, 7) = Format(Round(Val(grdsales.TextMatrix(grdsales.Row, 6)) + Val(grdsales.TextMatrix(grdsales.Row, 6)) * Val(TXTsample.Text) / 100, 3), "0.00")
                    grdsales.TextMatrix(grdsales.Row, 21) = Format(Round(Val(grdsales.TextMatrix(grdsales.Row, 6)) + Val(grdsales.TextMatrix(grdsales.Row, 6)) * Val(TXTsample.Text) / 100, 3), "0.00")
                    grdsales.TextMatrix(grdsales.Row, 6) = Format(Round(Val(grdsales.TextMatrix(grdsales.Row, 7)) * 100 / (Val(TXTsample.Text) + 100), 3), "0.00")
                    grdsales.TextMatrix(grdsales.Row, 22) = Format(Round(Val(grdsales.TextMatrix(grdsales.Row, 7)) * 100 / (Val(TXTsample.Text) + 100), 3), "0.00")
                    If UCase(grdsales.TextMatrix(grdsales.Row, 25)) = "SERVICE CHARGE" Then
                        TXTDISC.Tag = Val(grdsales.TextMatrix(grdsales.Row, 7)) * Val(grdsales.TextMatrix(grdsales.Row, 8)) / 100
                        grdsales.TextMatrix(grdsales.Row, 12) = Format(Round(Val(grdsales.TextMatrix(grdsales.Row, 7)) - Val(TXTDISC.Tag), 4), ".00")
                        grdsales.TextMatrix(grdsales.Row, 34) = Format(Round(Val(TXTsample.Text) - Val(TXTDISC.Tag), 4), ".00")
        ...
        Case 113, vbKeyReturn
            ...
            Select Case grdsales.Col
                Case 3
                    ...
                    If UCase(grdsales.TextMatrix(grdsales.Row, 25)) = "OWN" Then Exit Sub
...
SKIP2:
    ...
    Else
        ...
        If lblCBFLAG.Caption = "B" Then
            ...
            'RSTTRXFILE!INV_NO = Val(lblinvno.Caption)
            BANKTRX!TRX_DATE = Format(TXTINVDATE.Text, "DD/MM/YYYY")
            BANKTRX!TRX_AMOUNT = Val(lblnetamount.Caption)
            BANKTRX!act_code = DataList2.BoundText
            BANKTRX!act_name = DataList2.Text
            'RSTTRXFILE!INV_DATE = LBLDATE.Caption
            BANKTRX!REF_NO = "From " & Trim(DataList2.Text)
...
CLOSEFILE:
    ...
    '            If CRXFormulaField.Name = "{@NetGrandTotal}" Then CRXFormulaField.Text = "'" & Format(Round(Val(lblnetamount.Caption), 0), "0.00") & "'"
        ...
        Else
            ...
            Set Report = Nothing
            If thermalprn = False Then Call cmdRefresh_Click
        End If
        Screen.MousePointer = vbNormal
    ...
    Else
        If Trim(lblIGST.Caption) <> "Y" Then
            ...
            If Small_Print = True Then
                'ReportNameVar = MDIMAIN.StatusBar.Panels(7).Text & "EzBiz\rptbillretail"
                ReportNameVar = MDIMAIN.StatusBar.Panels(7).Text & "EzBiz\RPTGSTBILLRA5"
            Else
                ReportNameVar = MDIMAIN.StatusBar.Panels(7).Text & "EzBiz\rptGSTBILLR"
            End If
...
Private Sub TxtCessPer_LostFocus()
    ...
    If UCase(txtcategory.Text) = "SERVICE CHARGE" Then
        ...
        LBLSUBTOTAL.Caption = Format(Round(Val(txtretail.Text) - Val(TxtCessPer.Tag), 2), ".00")
        LblGross.Caption = Format(Round(Val(TXTRETAILNOTAX.Text) - Val(TxtCessPer.Tag), 2), ".00")
    Else
        TxtCessPer.Tag = Val(TXTQTY.Text) * Val(txtretail.Text) * Val(TxtCessPer.Text) / 100
        LBLSUBTOTAL.Caption = Format(Round((Val(TXTQTY.Text) * Val(txtretail.Text)) - Val(TxtCessPer.Tag), 2), ".00")
        LblGross.Caption = Format(Round((Val(TXTQTY.Text) * Val(TXTRETAILNOTAX.Text)) - Val(TxtCessPer.Tag), 2), ".00")
    End If
    
    ''TxtCessPer.Text = Format(TxtCessPer.Text, ".00")
...
Private Sub TxtCessAmt_LostFocus()
    ...
    If UCase(txtcategory.Text) = "SERVICE CHARGE" Then
        ...
        LBLSUBTOTAL.Caption = Format(Round(Val(txtretail.Text) - Val(TxtCessAmt.Tag), 2), ".00")
        LblGross.Caption = Format(Round(Val(TXTRETAILNOTAX.Text) - Val(TxtCessAmt.Tag), 2), ".00")
    Else
        TxtCessAmt.Tag = Val(TXTQTY.Text) * Val(txtretail.Text) * Val(TxtCessAmt.Text) / 100
        LBLSUBTOTAL.Caption = Format(Round((Val(TXTQTY.Text) * Val(txtretail.Text)) - Val(TxtCessAmt.Tag), 2), ".00")
        LblGross.Caption = Format(Round((Val(TXTQTY.Text) * Val(TXTRETAILNOTAX.Text)) - Val(TxtCessAmt.Tag), 2), ".00")
    End If
    
    ''TxtCessAmt.Text = Format(TxtCessAmt.Text, ".00")
