...
Begin VB.Form FRMDAMAGEREG 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "DAMAGE GOODS REPORT"
   ClientHeight    =   9645
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   18660
   ...
   Begin VB.Frame FRMEMAIN 
      ...
      Begin VB.Frame Frmeperiod 
         ...
         Begin VB.Frame Frame1 
            ...
            Top             =   465
            Width           =   2955
            Begin VB.OptionButton OptDmCr 
               BackColor       =   &H00C0FFFF&
               Caption         =   "Damage (Credit Note)"
               BeginProperty Font 
                  Name            =   "Tahoma"
                  Size            =   9.75
                  Charset         =   0
      ...
      Begin vbalProgBarLib6.vbalProgressBar vbalProgressBar1 
         ...
         _ExtentX        =   9895
         _ExtentY        =   582
         Picture         =   "FRMDAMAGEREG.frx":030A
         ForeColor       =   0
         BarPicture      =   "FRMDAMAGEREG.frx":0326
         Max             =   150
         Text            =   "PLEASE WAIT..."
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "Tahoma"
   ...
   End
End
Attribute VB_Name = "FRMDAMAGEREG"
...
Private Sub CmDDisplay_Click()
    ...
    If OPTPERIOD.Value = True Then
        If OptDmCr.Value = True Then
            rstTRANX.Open "SELECT * From DAMAGE_MAST WHERE VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND TRX_TYPE='DG' ORDER BY TRX_TYPE, VCH_DATE,VCH_NO", db, adOpenStatic, adLockReadOnly
        ElseIf OptDm.Value = True Then
            rstTRANX.Open "SELECT * From DAMAGE_MAST WHERE VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND TRX_TYPE='DM' ORDER BY TRX_TYPE, VCH_DATE,VCH_NO", db, adOpenStatic, adLockReadOnly
        Else
            rstTRANX.Open "SELECT * From DAMAGE_MAST WHERE VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND (TRX_TYPE='DG' OR TRX_TYPE='DM') ORDER BY TRX_TYPE, VCH_DATE,VCH_NO", db, adOpenStatic, adLockReadOnly
        End If
    Else
        If OptDmCr.Value = True Then
            rstTRANX.Open "SELECT * From DAMAGE_MAST WHERE ACT_CODE = '" & DataList2.BoundText & "' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND TRX_TYPE='DG' ORDER BY TRX_TYPE, VCH_DATE,VCH_NO", db, adOpenStatic, adLockReadOnly
        ElseIf OptDm.Value = True Then
            rstTRANX.Open "SELECT * From DAMAGE_MAST WHERE ACT_CODE = '" & DataList2.BoundText & "' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND TRX_TYPE='DM' ORDER BY TRX_TYPE, VCH_DATE,VCH_NO", db, adOpenStatic, adLockReadOnly
        Else
            rstTRANX.Open "SELECT * From DAMAGE_MAST WHERE ACT_CODE = '" & DataList2.BoundText & "' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND (TRX_TYPE='DG' OR TRX_TYPE='DM') ORDER BY TRX_TYPE, VCH_DATE,VCH_NO", db, adOpenStatic, adLockReadOnly
        End If
    End If
...
'        End If
        ...
        GRDTranx.TextMatrix(M, 16) = IIf(IsNull(rstTRANX!TRX_YEAR), "", rstTRANX!TRX_YEAR)
        CMDDISPLAY.Tag = ""
        FRMEMAIN.Tag = ""
        FRMEBILL.Tag = ""
...
Private Sub CMDREGISTER_Click()
    ...
    Dim i As Integer
    Screen.MousePointer = vbHourglass
    ReportNameVar = Rptpath & "RPTDAMREG"
...
Private Sub CmdReport_Click()
    ...
    On Error GoTo ERRHAND
    ReportNameVar = Rptpath & "RPTDAMREP"
    Set Report = crxApplication.OpenReport(ReportNameVar, 1)
...
Private Sub cmdview_Click()
    ...
    Screen.MousePointer = vbHourglass
    Set rstTRANX = New ADODB.Recordset
    rstTRANX.Open "SELECT * From DAMAGE_MAST WHERE VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND TRX_TYPE='DG' ", db, adOpenStatic, adLockReadOnly
    If rstTRANX.RecordCount > 0 Then
        vbalProgressBar1.Max = rstTRANX.RecordCount
    Else
        vbalProgressBar1.Max = 100
    ...
    Do Until FROMDATE > TODATE
        ...
        Set RSTTRXFILE = New ADODB.Recordset
        RSTTRXFILE.Open "SELECT * From DAMAGE_MAST WHERE VCH_DATE = '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND TRX_TYPE='DG' ORDER BY VCH_NO", db, adOpenStatic, adLockReadOnly
        Do Until RSTTRXFILE.EOF
            CMDDISPLAY.Tag = ""
    ...
    Sleep (300)
    ReportNameVar = Rptpath & "RPTDAMREGSTR"
    Set Report = crxApplication.OpenReport(ReportNameVar, 1)
...
Private Sub CMDPRINTREGISTER_Click()
    ...
    ReportNameVar = Rptpath & "RPTDAMREPORT"
    Set Report = crxApplication.OpenReport(ReportNameVar, 1)
    ...
    If UCase(dbase1) <> "INVSOFT" And UCase(dbase1) <> "INVDB" And UCase(dbase1) <> "INVSOFT3" Then
        Set oRs = New ADODB.Recordset
        Set oRs = db.Execute("SELECT * FROM TRXFILE ")
        Report.Database.SetDataSource oRs, 3, 1
        Set oRs = Nothing
        
        Set oRs = New ADODB.Recordset
        Set oRs = db.Execute("SELECT * FROM DAMAGE_MAST ")
...
'    Set rstTRXMAST = Nothing
    ...
    Select Case Trim(GRDTranx.TextMatrix(GRDTranx.Row, 1))
        Case "DG"
            If Year(MDIMAIN.DTFROM.Value) <> Val(GRDTranx.TextMatrix(GRDTranx.Row, 16)) Then Exit Sub
            If IsFormLoaded(frmDmgRet) <> True Then
                frmDmgRet.txtBillNo.text = Val(GRDTranx.TextMatrix(GRDTranx.Row, 3))
                frmDmgRet.LBLBILLNO.Caption = Val(GRDTranx.TextMatrix(GRDTranx.Row, 3))
                frmDmgRet.Show
                frmDmgRet.SetFocus
                Call frmDmgRet.txtBillNo_KeyDown(13, 0)
            End If
        Case "DM"
            If Year(MDIMAIN.DTFROM.Value) <> Val(GRDTranx.TextMatrix(GRDTranx.Row, 16)) Then Exit Sub
            If IsFormLoaded(FRMDAMAGE) <> True Then
                FRMDAMAGE.txtBillNo.text = Val(GRDTranx.TextMatrix(GRDTranx.Row, 3))
                FRMDAMAGE.LBLBILLNO.Caption = Val(GRDTranx.TextMatrix(GRDTranx.Row, 3))
                FRMDAMAGE.Show
                FRMDAMAGE.SetFocus
                Call FRMDAMAGE.txtBillNo_KeyDown(13, 0)
            End If
        Case "WO"
    End Select
    Exit Sub
...
CLOSEFILE:
    ...
    'RSTTRXFILE.Open "SELECT * From SALESREG ORDER BY VCH_NO", DB, adOpenStatic,adLockReadOnly
    RSTTRXFILE.Open "SELECT * From DAMAGE_MAST WHERE VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND (TRX_TYPE='DG' OR TRX_TYPE='RI') ORDER BY VCH_NO", db, adOpenStatic, adLockReadOnly
    Do Until RSTTRXFILE.EOF
        SN = SN + 1
        CMDDISPLAY.Tag = ""
        If RSTTRXFILE!SLSM_CODE = "A" Then
