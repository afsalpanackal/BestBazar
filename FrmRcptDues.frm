...
Begin VB.Form FRMRCPTDUES 
   ...
   Begin VB.Frame Frmeperiod 
      ...
      Top             =   -105
      Width           =   7530
      Begin VB.CommandButton CmdMissRcpt 
         Caption         =   "Display &Missing Rcpts"
         BeginProperty Font 
            Name            =   "Arial"
            Size            =   8.25
            Charset         =   204
...
Private Sub CmdMissRcpt_Click()
    ...
    GRDBILL.TextMatrix(0, 4) = "Bill Amount"
    GRDBILL.TextMatrix(0, 5) = "Rcpt Amount"
    GRDBILL.TextMatrix(0, 6) = "Customer"
    GRDBILL.TextMatrix(0, 7) = "Year"
    ...
    If OptAll.Value = True Then
        rstTRANX.Open "SELECT * From TRXMAST WHERE RCPT_AMOUNT <> 0 AND VCH_DATE <= '" & Format(DTTO.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND ACT_CODE <> '130000' AND ACT_CODE <> '130001' AND (TRX_TYPE='SV' OR TRX_TYPE='GI' OR TRX_TYPE='SI' or TRX_TYPE='HI' OR TRX_TYPE='RI' OR TRX_TYPE='WO')  ORDER BY TRX_TYPE, VCH_DATE,VCH_NO", db, adOpenStatic, adLockReadOnly
    Else
        rstTRANX.Open "SELECT * From TRXMAST WHERE RCPT_AMOUNT <> 0 AND VCH_DATE <= '" & Format(DTTO.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND ACT_CODE = '" & DataList2.BoundText & "' AND (TRX_TYPE='SV' OR TRX_TYPE='GI' OR TRX_TYPE='SI' or TRX_TYPE='HI' OR TRX_TYPE='RI' OR TRX_TYPE='WO')  ORDER BY TRX_TYPE, VCH_DATE,VCH_NO", db, adOpenStatic, adLockReadOnly
    End If
    ...
    Do Until rstTRANX.EOF
        ...
        rstBILL.Open "SELECT * From DBTPYMT WHERE TRX_YEAR='" & rstTRANX!TRX_YEAR & "' AND TRX_TYPE='RT' AND INV_NO = " & rstTRANX!VCH_NO & " AND INV_TRX_TYPE = '" & rstTRANX!TRX_TYPE & "'", db, adOpenForwardOnly
        If (rstBILL.EOF And rstBILL.BOF) Then
            ...
            GRDBILL.TextMatrix(i, 5) = IIf(IsNull(rstTRANX!RCPT_AMOUNT), "", rstTRANX!RCPT_AMOUNT)
            GRDBILL.TextMatrix(i, 6) = IIf(IsNull(rstTRANX!ACT_NAME), "", rstTRANX!ACT_NAME)
            GRDBILL.TextMatrix(i, 7) = IIf(IsNull(rstTRANX!TRX_YEAR), "", rstTRANX!TRX_YEAR)
