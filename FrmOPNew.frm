...
Begin VB.Form frmOP 
   ...
   Caption         =   "OP. Stock Entry"
   ClientHeight    =   10995
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   18585
   ControlBox      =   0   'False
   Icon            =   "FrmOPNew.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin VB.Frame Fram 
      ...
      Begin VB.ComboBox Cmbbarcode 
         Height          =   330
         ItemData        =   "FrmOPNew.frx":030A
         Left            =   14640
         List            =   "FrmOPNew.frx":030C
      ...
      Begin VB.Frame FRMECONTROLS 
         ...
         Begin VB.ComboBox cmbfull 
            ...
            ForeColor       =   &H000000FF&
            Height          =   360
            ItemData        =   "FrmOPNew.frx":030E
            Left            =   8040
            List            =   "FrmOPNew.frx":0363
            Style           =   2  'Dropdown List
            TabIndex        =   5
            Top             =   495
            Width           =   825
         ...
         Begin VB.ComboBox CmbWrnty 
            ...
            ForeColor       =   &H000000FF&
            Height          =   360
            ItemData        =   "FrmOPNew.frx":0403
            Left            =   14625
            List            =   "FrmOPNew.frx":040D
            Style           =   2  'Dropdown List
            TabIndex        =   37
            Top             =   1155
            Width           =   840
         ...
         Begin VB.ComboBox CmbPack 
            ...
            ForeColor       =   &H000000FF&
            Height          =   360
            ItemData        =   "FrmOPNew.frx":041E
            Left            =   10155
            List            =   "FrmOPNew.frx":0473
            Style           =   2  'Dropdown List
            TabIndex        =   8
            Top             =   495
            Width           =   780
...
'            End If
    ...
    'grdsales.TextMatrix(Val(TXTSLNO.text), 8) = Format(Round(((Val(LblGross.Caption) / (Val(Los_Pack.text) * (Val(TXTQTY.text) + Val(TXTFREE.text)))) + (((Val(TxtExpense.text) * 100 / (Val(TxttaxMRP.text) + 100)) / ((Val(TXTQTY.text) + Val(TXTFREE.text)) * Val(Los_Pack.text))))), 4), ".0000")
    ...
    If Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 28)) = 0 Then grdsales.TextMatrix(Val(TXTSLNO.Text), 28) = 1
...
'
    ...
    If OLD_BILL = False And Val(txtBillNo.Text) <> 1 Then
        RSTTRXFILE.Open "Select * From TRANSMAST WHERE VCH_NO= (SELECT MAX(VCH_NO) FROM TRANSMAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE = 'OP')", db, adOpenStatic, adLockOptimistic, adCmdText
...
'                End If
                !ITEM_NET_COST = Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 15))
...
'                End If
    ...
    'RSTRTRXFILE!ITEM_NET_COST_PRICE = Round((Val(LBLSUBTOTAL.Caption) / Val(TXTQTY.text)) + (Val(TxtExpense.text) / Val(TXTQTY.text)), 3)
    If (Val(TXTQTY.Text) + Val(TXTFREE.Text)) = 0 Then
...
Private Sub CmdDelete_Click()
    ...
    Do Until RSTRTRXFILE.EOF
        ...
        If Val(grdsales.TextMatrix(i, 28)) = 0 Then grdsales.TextMatrix(i, 28) = 1
...
Private Sub CmdDeleteAll_Click()
    ...
    'db.Execute "delete FROM CASHATRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND INV_NO = " & Val(txtBillNo.Text) & " AND INV_TYPE = 'PY' AND INV_TRX_TYPE = 'OP'"
    For i = 1 To grdsales.rows - 1
...
Private Sub Command5_Click()
    ...
    rstBILL.Open "Select MAX(VCH_NO) From TRANSMAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE = 'OP'", db, adOpenStatic, adLockReadOnly
...
Private Sub Form_Load()
    ...
    grdsales.ColWidth(28) = 800
    grdsales.ColWidth(32) = 900
    ...
    grdsales.TextArray(15) = "Net Rate" '"TAX MODE"
...
Private Sub grdsales_KeyDown(KeyCode As Integer, Shift As Integer)
    Select Case KeyCode
        ...
        Case vbKeyDelete
            ...
            Screen.MousePointer = vbHourglass
            On Error GoTo ERRHAND
            db.Execute "delete  From RTRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='OP' AND VCH_NO = " & Val(txtBillNo.Text) & " AND ITEM_CODE='" & Trim(grdsales.TextMatrix(grdsales.Row, 1)) & "' AND LINE_NO=" & Val(grdsales.TextMatrix(grdsales.Row, 16)) & ""
            Set rststock = New ADODB.Recordset
...
Private Sub grdtmp_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            RSTRXFILE.Open "Select * From RTRXFILE  WHERE ITEM_CODE = '" & Trim(TXTITEMCODE.Text) & "' AND TRX_TYPE = 'OP' ORDER BY VCH_DATE DESC, VCH_NO DESC", db, adOpenStatic, adLockReadOnly, adCmdText
...
'            End If
            ...
            If PHY.RecordCount = 1 Then
                ...
                RSTRXFILE.Open "Select * From RTRXFILE  WHERE ITEM_CODE = '" & Trim(TXTITEMCODE.Text) & "' AND TRX_TYPE = 'OP' ORDER BY VCH_DATE DESC, VCH_NO DESC", db, adOpenStatic, adLockReadOnly
                If Not (RSTRXFILE.EOF And RSTRXFILE.BOF) Then
                    'RSTRXFILE.MoveLast
                    TXTUNIT.Text = 1 'IIf(IsNull(RSTRXFILE!UNIT), "", RSTRXFILE!UNIT)
...
Private Sub TXTPTR_LostFocus()
    ...
    'Call TxtRetailPercent_LostFocus
...
Private Sub TXTRATE_LostFocus()
    If Val(TXTRATE.Tag) <> 0 And Val(TXTRATE.Text) <> 0 And (Val(txtretail.Text) = 0 Or Val(TXTRATE.Text) > Val(TXTRATE.Tag)) Then
        MsgBox "MRP higher than older. Retail price will be affected", , "EzBiz"
        txtretail.Text = Val(TXTRATE.Text)
        Call TxttaxMRP_LostFocus
        Call TXTRETAIL_LostFocus
...
''    End If
    ...
    Else
        ...
        Else
            If Val(TXTQTY.Text) + Val(TXTFREE.Text) = 0 Then
                TXTPTR.Tag = Round(((Val(LblGross.Caption)) + (((Val(TxtExpense.Text) * 100 / (Val(TxttaxMRP.Text) + 100))))), 4)
            Else
                TXTPTR.Tag = Round(((Val(LblGross.Caption) / ((Val(TXTQTY.Text) + Val(TXTFREE.Text)))) + (((Val(TxtExpense.Text) * 100 / (Val(TxttaxMRP.Text) + 100)) / (Val(TXTQTY.Text) + Val(TXTFREE.Text))))), 4)
            End If
        End If
        If MDIMAIN.lblgst.Caption <> "R" Then
...
'
    ...
    Do Until RSTTRXFILE.EOF
        ...
        RSTTRXFILE!RCVD_DATE = Format(Trim(TXTINVDATE.Text), "dd/mm/yyyy")
...
'
'        Set RSTITEMMAST = New ADODB.Recordset
'        RSTITEMMAST.Open "SELECT * FROM CASHATRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND INV_NO = " & Val(txtBillNo.Text) & " AND INV_TYPE = 'PY' AND INV_TRX_TYPE = 'OP'", db, adOpenStatic, adLockOptimistic, adCmdText
'        db.BeginTrans
'        If (RSTITEMMAST.EOF And RSTITEMMAST.BOF) Then
'            RSTITEMMAST.AddNew
'            RSTITEMMAST!rec_no = i + 1
'            RSTITEMMAST!INV_TYPE = "PY"
'            RSTITEMMAST!INV_TRX_TYPE = "OP"
...
Private Function find_small_number()
    ...
    For i = 1 To grdsales.rows - 1
        ...
        lblqty.Caption = Format(Val(lblqty.Caption) + Val(grdsales.TextMatrix(i, 3)), ".00")
        db.Execute "Update RTRXFILE set ITEM_NET_COST_PRICE = " & NETCOST & ", EXPENSE = " & Val(grdsales.TextMatrix(i, 32)) & " WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='OP' AND VCH_NO = " & Val(txtBillNo.Text) & " AND ITEM_CODE='" & Trim(grdsales.TextMatrix(i, 1)) & "'AND LINE_NO=" & Val(grdsales.TextMatrix(i, 16)) & "  "
        'db.Execute "Update RTRXFILE set EXPENSE = " & Val(grdsales.TextMatrix(i, 32)) & " WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='OP' AND VCH_NO = " & Val(txtBillNo.text) & " AND ITEM_CODE='" & Trim(grdsales.TextMatrix(i, 1)) & "'AND LINE_NO=" & Val(grdsales.TextMatrix(i, 16)) & "  "
