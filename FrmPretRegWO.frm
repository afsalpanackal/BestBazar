...
Begin VB.Form FrmPretRegWO 
   ...
   End
End
Attribute VB_Name = "FrmPretRegWO"
...
Private Sub CMDDISPLAY_Click()
    ...
    Select Case OPTALL.Value
        Case True
            If OPTPERIOD.Value = True Then
                ...
                rstTRANX.Open "SELECT DISTINCT VCH_NO From PURCAHSERETURN WHERE [VCH_DATE] <=# " & TODATE & " # AND [VCH_DATE] >=# " & FROMDATE & " # ORDER BY TRX_TYPE,VCH_DATE,VCH_NO", db2, adOpenStatic, adLockReadOnly
            Else
                Set rstTRANX = New ADODB.Recordset
                rstTRANX.Open "SELECT DISTINCT VCH_NO From PURCAHSERETURN WHERE [ACT_CODE] = '" & DataList2.BoundText & "' ORDER BY TRX_TYPE,VCH_DATE,VCH_NO", db2, adOpenStatic, adLockReadOnly
            End If
        Case Else
            If OPTPERIOD.Value = True Then
                Set rstTRANX = New ADODB.Recordset
                rstTRANX.Open "SELECT DISTINCT VCH_NO From PURCAHSERETURN WHERE [CHECK_FLAG] <>'Y' AND [VCH_DATE] <=# " & TODATE & " # AND [VCH_DATE] >=# " & FROMDATE & " # ORDER BY TRX_TYPE,VCH_DATE,VCH_NO", db2, adOpenStatic, adLockReadOnly
            Else
                Set rstTRANX = New ADODB.Recordset
                rstTRANX.Open "SELECT DISTINCT VCH_NO From PURCAHSERETURN WHERE [ACT_CODE] = '" & DataList2.BoundText & "' AND [CHECK_FLAG] <>'Y' ORDER BY TRX_TYPE,VCH_DATE,VCH_NO", db2, adOpenStatic, adLockReadOnly
            End If
    End Select
    Do Until rstTRANX.EOF
        ...
        RSTTRXFILE.Open "SELECT * From PURCAHSERETURN WHERE VCH_NO = " & rstTRANX!VCH_NO & "", db2, adOpenStatic, adLockReadOnly
...
Private Sub GRDTranx_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            i = 0
            Set RSTTRXFILE = New ADODB.Recordset
            RSTTRXFILE.Open "Select * From PURCAHSERETURN WHERE VCH_NO = " & Val(LBLBILLNO.Caption) & " AND TRX_TYPE = '" & Trim(GRDTranx.TextMatrix(GRDTranx.Row, 2)) & "'", db2, adOpenStatic, adLockReadOnly, adCmdText
            Do Until RSTTRXFILE.EOF
                i = i + 1
