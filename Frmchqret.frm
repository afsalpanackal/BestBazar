...
'    End If
    ...
    rstBILL.Open "Select MAX(CR_NO) From CRDTPYMT WHERE TRX_TYPE = 'RC'", db, adOpenForwardOnly
    ...
    TRX_NO = 1
    Set rstBILL = New ADODB.Recordset
    rstBILL.Open "Select MAX(TRX_NO) From BANK_TRX WHERE TRX_TYPE = 'CR' AND BILL_TRX_TYPE = 'RC' AND TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' ", db, adOpenForwardOnly
    If Not (rstBILL.EOF And rstBILL.BOF) Then
...
'    Dim BillNO As Long
'    Set rstBILL = New ADODB.Recordset
'    rstBILL.Open "Select MAX(RCPT_NO) From TRNXRCPT WHERE TRX_TYPE = 'RC'", db, adOpenForwardOnly
'    If Not (rstBILL.EOF And rstBILL.BOF) Then
