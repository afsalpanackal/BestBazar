...
Private Sub CMDADD_Click()
    ...
    grdsales.TextMatrix(Val(TXTSLNO.text), 8) = Format(Round(((Val(LblGross.Caption) / ((Val(TXTQTY.text) + Val(TXTFREE.text)))) + ((Val(TxtExpense.text) / Val(Los_Pack.text)))), 3), ".000")
...
'                !WARRANTY_TYPE = Trim(CmbWrnty.Text)
    ...
    RSTITEMMAST.Open "SELECT * FROM ITEMMAST WHERE ITEM_CODE = '" & grdsales.TextMatrix(Val(TXTSLNO.text), 1) & "' AND ucase(CATEGORY) <> 'SELF' AND ucase(CATEGORY) <> 'SERVICE CHARGE' AND ucase(CATEGORY) <> 'SERVICES' ", db, adOpenStatic, adLockOptimistic, adCmdText
...
'            End If
    ...
    If MDIMAIN.StatusBar.Panels(6).text = "Y" Then
        If MsgBox("Do you want to Print Barcode Labels now?", vbYesNo, "Purchase.....") = vbYes Then
            ...
            'If i > 0 Then Call print_3labels(i, Trim(TxtBarcode.Text), "")
...
Private Sub CmdPrint_Click()
    ...
    For i = 1 To 3
        ...
        For Each CRXFormulaField In CRXFormulaFields
            ...
            If CRXFormulaField.Name = "{@Company}" Then CRXFormulaField.text = "''"
            If CRXFormulaField.Name = "{@CustName}" Then CRXFormulaField.text = "''"
'            If CRXFormulaField.Name = "{@CustAddress}" Then CRXFormulaField.Text = "'" & Trim(lbladdress.Caption) & "'"
    ...
    '        End If
            ...
            If CRXFormulaField.Name = "{@Phone}" Then CRXFormulaField.text = "''"
...
Private Sub cmdRefresh_Click()
    ...
    Else
        If Not IsDate(TXTINVDATE.text) Then
            MsgBox "Enter OP Date", vbOKOnly, "EzBiz"
            Exit Sub
        End If
        'Me.Enabled = False
...
Private Sub TXTPTR_LostFocus()
    ...
    If Val(Los_Pack.text) = 0 Then Los_Pack.text = 1
    Txtgrossamt.text = Round(Val(TXTPTR.text) * (Val(TXTQTY.text) / Val(Los_Pack.text)), 3)
...
Private Sub TXTQTY_LostFocus()
    ...
    LBLSUBTOTAL.Caption = Format((Val(TXTQTY.text) / Val(Los_Pack.text) * Round(Val(TXTPTR.text), 2)), ".000")
    LblGross.Caption = Format((Val(TXTQTY.text) / Val(Los_Pack.text) * Round(Val(TXTPTR.text), 2)), ".000")
    Call TXTPTR_LostFocus
End Sub
...
Public Sub appendpurchase()
    ...
    Do Until RSTTRXFILE.EOF
        ...
        RSTTRXFILE!VCH_DESC = "Received From Opening Stock"
        RSTTRXFILE!PINV = ""
        RSTTRXFILE!M_USER_ID = ""
        RSTTRXFILE.Update
        RSTTRXFILE.MoveNext
...
'        Printer.PaintPicture Picture2.Image, 900, 800 ', wid, hgt  'Item Name      'SREEDEVI
'        Printer.PaintPicture Picture2.Image, 3150, 800 ', wid, hgt  'Item Name
'
...
Private Function print_3labels(i As Long, BAR_LABEL As String, P_RATE As String)
    ...
    Picture2.FontSize = 6
    Picture2.FontBold = False
    Picture2.Print Mid(Trim(grdsales.TextMatrix(Val(TXTSLNO.text), 2)), 1, 10) & " MRP: " & Format(Val(txtretail.text), "0.00") 'ITEM NAME and Price
