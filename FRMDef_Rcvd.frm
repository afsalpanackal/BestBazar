...
Private Sub CMDADD_Click()
    ...
    If OPTSELF.Value = True Then
        Set RSTTRXFILE = New ADODB.Recordset
        RSTTRXFILE.Open "SELECT *  FROM ITEMMAST WHERE ITEM_CODE = '" & grdsales.TextMatrix(Val(TXTSLNO.Text), 6) & "'", db, adOpenStatic, adLockOptimistic, adCmdText
        With RSTTRXFILE
        ...
        Set RSTTRXFILE = New ADODB.Recordset
        RSTTRXFILE.Open "SELECT *  FROM RTRXFILE WHERE RTRXFILE.TRX_TYPE = '" & Trim(grdsales.TextMatrix(Val(TXTSLNO.Text), 7)) & "' AND RTRXFILE.VCH_NO = " & Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 5)) & " AND RTRXFILE.LINE_NO = " & Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 6)) & " AND BAL_QTY > 0", db, adOpenStatic, adLockOptimistic, adCmdText
        With RSTTRXFILE
            If Not (.EOF And .BOF) Then
...
Private Sub cmdRefresh_Click()
    ...
    db.Execute "delete From WAR_TRXFILE WHERE VCH_NO = " & Val(txtBillNo.Text) & ""
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * From WAR_TRXFILE", db, adOpenStatic, adLockOptimistic, adCmdText
    For i = 1 To grdsales.rows - 1
        RSTTRXFILE.AddNew
        RSTTRXFILE!VCH_NO = Val(txtBillNo.Text)
        RSTTRXFILE!VCH_DATE = Format(TXTINVDATE.Text, "DD/MM/YYYY")
        ...
        RSTTRXFILESUB.Open "Select * From RTRXFILE WHERE VCH_NO = " & RSTTRXFILE!R_VCH_NO & " AND LINE_NO = " & RSTTRXFILE!R_LINE_NO & " AND TRX_TYPE = '" & RSTTRXFILE!R_TRX_TYPE & "' ", db, adOpenStatic, adLockReadOnly
    ...
    txtBillNo.Text = 1
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select MAX(VCH_NO) From WAR_TRXFILE", db, adOpenStatic, adLockReadOnly
    If Not (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
...
Private Sub Form_Load()
    ...
    rstBILL.Open "Select MAX(VCH_NO) From WAR_TRXFILE", db, adOpenStatic, adLockReadOnly
...
Select Case KeyCode
    Case vbKeyReturn
        ...
        RSTDN.Open "Select * From WAR_TRXFILE WHERE VCH_NO = " & Val(txtBillNo.Text) & " ORDER BY LINE_NO", db, adOpenStatic, adLockReadOnly
        Do Until RSTDN.EOF
            i = i + 1
...
Private Sub txtBillNo_LostFocus()
    ...
    TRXDN.Open "Select MAX(VCH_NO) From WAR_TRXFILE", db, adOpenStatic, adLockReadOnly
...
Function FILL_BILLGRID()
    ...
    If BILL_FLAG = True Then
        PHY_BILL.Open "Select VCH_NO, VCH_DATE, UNIT, QTY, ITEM_NAME, REF_NO, R_VCH_NO, R_TRX_TYPE, R_LINE_NO From DE_RET_DETAILS ORDER BY VCH_DATE", db, adOpenStatic, adLockReadOnly
        BILL_FLAG = False
    Else
        PHY_BILL.Close
        PHY_BILL.Open "Select VCH_NO, VCH_DATE, UNIT, QTY, ITEM_NAME, REF_NO, R_VCH_NO, R_TRX_TYPE, R_LINE_NO From DE_RET_DETAILS ORDER BY VCH_DATE", db, adOpenStatic, adLockReadOnly
...
Private Sub fillcombo()
    ...
    If ACT_FLAG = True Then
        ACT_REC.Open "select ACT_CODE, ACT_NAME from CUSTMAST ORDER BY ACT_NAME", db, adOpenStatic, adLockReadOnly, adCmdText
        ACT_FLAG = False
    Else
        ACT_REC.Close
        ACT_REC.Open "select ACT_CODE, ACT_NAME from CUSTMAST ORDER BY ACT_NAME", db, adOpenStatic, adLockReadOnly, adCmdText
...
Private Sub DataList2_Click()
    ...
    rstCustomer.Open "select ADDRESS, DL_NO, KGST from CUSTMAST  WHERE ACT_CODE = '" & DataList2.BoundText & "' ", db, adOpenStatic, adLockReadOnly, adCmdText
...
Private Function FILLINVOICE()
    ...
    If INV_FLAG = True Then
        INV_REC.Open "Select DISTINCT VCH_NO From WAR_TRXFILE WHERE ACT_CODE = '" & DataList2.BoundText & "' ORDER BY VCH_NO", db, adOpenStatic, adLockReadOnly
        INV_FLAG = False
    Else
        INV_REC.Close
        INV_REC.Open "Select DISTINCT VCH_NO From WAR_TRXFILE WHERE ACT_CODE = '" & DataList2.BoundText & "' ORDER BY VCH_NO", db, adOpenStatic, adLockReadOnly
...
Private Function FILLBILLDB()
    ...
    B_FLAG = False
    db.Execute "delete From DE_RET_DETAILS"
    Set TRXFILE = New ADODB.Recordset
    TRXFILE.Open "Select * From TRXFILE WHERE (TRX_TYPE='GI' OR TRX_TYPE='RI' OR TRX_TYPE='SI') AND CST <>2 AND ITEM_CODE = '" & TXTITEMCODE.Text & "' AND M_USER_ID = '" & DataList2.BoundText & "'", db, adOpenStatic, adLockReadOnly
    Do Until TRXFILE.EOF
        Set TRXFILESUB = New ADODB.Recordset
        TRXFILESUB.Open "Select * From TRXSUB WHERE VCH_NO = " & TRXFILE!VCH_NO & " AND LINE_NO = " & TRXFILE!LINE_NO & "", db, adOpenStatic, adLockReadOnly

        If Not (TRXFILESUB.EOF And TRXFILESUB.BOF) Then
            Set TRXBILL = New ADODB.Recordset
            TRXBILL.Open "SELECT *  FROM DE_RET_DETAILS", db, adOpenStatic, adLockOptimistic, adCmdText
            B_FLAG = True
            TRXBILL.AddNew
    ...
    Dim i As Long
        ...
        If ITEM_FLAG = True Then
            If NONSTOCKFLAG = True Then
                PHY_ITEM.Open "Select ITEM_CODE, ITEM_NAME, BAL_QTY, REF_NO, LINE_DISC, VCH_NO, LINE_NO, TRX_TYPE, CHECK_FLAG  From RTRXFILE  WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' ORDER BY VCH_DATE", db, adOpenStatic, adLockReadOnly
            Else
                PHY_ITEM.Open "Select ITEM_CODE, ITEM_NAME, BAL_QTY, REF_NO, LINE_DISC, VCH_NO, LINE_NO, TRX_TYPE, CHECK_FLAG  From RTRXFILE  WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY > 0 ORDER BY VCH_DATE", db, adOpenStatic, adLockReadOnly
            End If
            ITEM_FLAG = False
        Else
            PHY_ITEM.Close
...
Function FILL_BATCHGRID()
    ...
    If BATCH_FLAG = True Then
        PHY_BATCH.Open "Select REF_NO, BAL_QTY, ITEM_CODE, ITEM_NAME, VCH_NO, LINE_NO, TRX_TYPE, LINE_DISC, CHECK_FLAG From RTRXFILE  WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY > 0 ORDER BY VCH_DATE", db, adOpenStatic, adLockReadOnly
        BATCH_FLAG = False
    Else
        PHY_BATCH.Close
        PHY_BATCH.Open "Select REF_NO, BAL_QTY, ITEM_CODE, ITEM_NAME, VCH_NO, LINE_NO, TRX_TYPE, LINE_DISC, CHECK_FLAG From RTRXFILE  WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY > 0 ORDER BY VCH_DATE", db, adOpenStatic, adLockReadOnly
