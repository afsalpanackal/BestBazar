...
Dim OLDBILL As Boolean

Private Sub CMDADD_Click()
    Dim i As Long
    If OLDBILL = True And Not (frmLogin.rs!Level = "0" Or frmLogin.rs!Level = "4") Then Exit Sub
...
Private Sub cmdcancel_Click()
    If M_ADD = True Then
        If MsgBox("Changes have been made. Do you want to Cancel?", vbYesNo, "OFFICE EXPENSE") = vbNo Then Exit Sub
    End If
...
'    TRXMAST.Open "Select MAX(VCH_NO) From TRXEXPENSE", db, adOpenStatic, adLockReadOnly
...
Private Sub CmdDelete_Click()
    Dim i As Long
    If OLDBILL = True And Not (frmLogin.rs!Level = "0" Or frmLogin.rs!Level = "4") Then Exit Sub
    If MsgBox("ARE YOU SURE YOU WANT TO DELETE " & """" & grdsales.TextMatrix(Val(TXTSLNO.Text), 2) & """", vbYesNo + vbDefaultButton2, "DELETE.....") = vbNo Then Exit Sub
    ...
    For i = Val(TXTSLNO.Text) To grdsales.rows - 2
        'grdsales.TextMatrix(i, 0) = i + 1 'grdsales.TextMatrix(i + 1, 0)
...
Private Sub CmdDeleteAll_Click()
    If OLDBILL = True And Not (frmLogin.rs!Level = "0" Or frmLogin.rs!Level = "4") Then Exit Sub
    If MsgBox("ARE YOU SURE YOU WANT TO DELETE THE ENTIRE EXPENSE NO. " & txtBillNo.Text, vbYesNo + vbDefaultButton2, "DELETE.....") = vbNo Then Exit Sub
    
    Screen.MousePointer = vbHourglass
    db.Execute "delete From TRXEXPENSE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND VCH_NO = " & Val(txtBillNo.Text) & " AND TRX_TYPE ='EX'"
    db.Execute "delete From TRXEXPMAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND VCH_NO = " & Val(txtBillNo.Text) & " AND TRX_TYPE ='EX'"
    db.Execute "delete FROM CASHATRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND INV_NO = " & Val(txtBillNo.Text) & " AND INV_TYPE = 'EX' AND INV_TRX_TYPE = 'EX'"
    ...
    db.Execute "delete From BANK_TRX WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE ='DR' AND BILL_TRX_TYPE = 'EX' AND B_VCH_NO = " & Val(txtBillNo.Text) & " "
...
Private Sub txtBillNo_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            rstTRXMAST.Open "Select * From TRXEXPENSE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND VCH_NO = " & Val(txtBillNo.Text) & " AND TRX_TYPE ='EX' ORDER BY VCH_NO, VCH_DATE", db, adOpenStatic, adLockReadOnly
            ...
            Set rstTRXMAST = New ADODB.Recordset
            rstTRXMAST.Open "Select * From TRXEXPMAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND VCH_NO = " & Val(txtBillNo.Text) & " AND TRX_TYPE ='EX' ORDER BY VCH_NO, VCH_DATE", db, adOpenStatic, adLockReadOnly
...
'        End If
        ...
        Set RSTITEMMAST = New ADODB.Recordset
        RSTITEMMAST.Open "SELECT * FROM CASHATRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND INV_NO = " & Val(txtBillNo.Text) & " AND INV_TYPE = 'EX' AND INV_TRX_TYPE = 'EX'", db, adOpenStatic, adLockOptimistic, adCmdText
        If (RSTITEMMAST.EOF And RSTITEMMAST.BOF) Then
            RSTITEMMAST.AddNew
            RSTITEMMAST!REC_NO = i + 1
            RSTITEMMAST!INV_TYPE = "EX"
            RSTITEMMAST!INV_TRX_TYPE = "EX"
        ...
        RSTITEMMAST!ACT_NAME = "Office Expense" 'Trim(DataList2.Text)
    ...
    Else
        Set RSTITEMMAST = New ADODB.Recordset
        RSTITEMMAST.Open "Select * From BANK_TRX WHERE TRX_NO= (SELECT MAX(TRX_NO) FROM BANK_TRX WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE = 'DR' AND BILL_TRX_TYPE = 'EX')", db, adOpenStatic, adLockOptimistic, adCmdText
        If RSTITEMMAST.RecordCount = 0 Then
            i = 1
        Else
        ...
        RSTITEMMAST!ACT_NAME = "Office Expense"
    ...
    RSTTRXFILE.Open "SELECT * from TRXEXPMAST ", db, adOpenStatic, adLockOptimistic, adCmdText
    RSTTRXFILE.AddNew
    ...
    RSTTRXFILE!BANK_REMARKS = Trim(lblremarks.Caption)
    ...
    RSTTRXFILE!C_USER_ID = frmLogin.rs!USER_ID ' Left(frmLogin.rs!USER_NAME, 23)
    ...
    For i = 1 To grdsales.rows - 1
        Set RSTTRXFILE = New ADODB.Recordset
        RSTTRXFILE.Open "SELECT * from TRXEXPENSE ", db, adOpenStatic, adLockOptimistic, adCmdText
        RSTTRXFILE.AddNew
        RSTTRXFILE!TRX_TYPE = "EX"
        RSTTRXFILE!VCH_NO = Val(txtBillNo.Text)
...
SKIP:
    ...
    M_ADD = False
    CMDEXIT.Enabled = True
    MsgBox "SAVED SUCCESSFULLY", vbOKOnly, "OFFICE EXPENSE"
    'Exit Sub
    
    Dim rstMaxNo As ADODB.Recordset
    Set rstMaxNo = New ADODB.Recordset
    rstMaxNo.Open "Select MAX(VCH_NO) From TRXEXPMAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "'", db, adOpenStatic, adLockReadOnly
