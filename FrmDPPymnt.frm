...
Private Sub CmdSave_Click()
    ...
    If OptBank.Value = True Then
        ...
        rstBILL.Open "Select MAX(TRX_NO) From BANK_TRX WHERE TRX_TYPE = 'DR' AND BILL_TRX_TYPE = 'FP' AND TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' ", db, adOpenForwardOnly
    ...
    'db.Execute "delete FROM CASHATRXFILE WHERE INV_NO = " & Val(txtBillNo.Text) & " AND INV_TYPE = 'FP' AND INV_TRX_TYPE = 'FP'"
    ...
    If optCash.Value = True Then
        ...
        'db.Execute "delete FROM CASHATRXFILE WHERE INV_NO = " & Val(creditbill.LBLBILLNO.Caption) & " AND INV_TYPE = 'FP'"
        Set RSTITEMMAST = New ADODB.Recordset
        RSTITEMMAST.Open "SELECT * FROM CASHATRXFILE", db, adOpenStatic, adLockOptimistic, adCmdText
        RSTITEMMAST.AddNew
        RSTITEMMAST!REC_NO = i + 1
        RSTITEMMAST!INV_TYPE = "FP"
        RSTITEMMAST!INV_TRX_TYPE = "FP"
...
Private Sub Form_Load()
    ...
    AGNT_FLAG = True
    Set rstBILL = New ADODB.Recordset
    rstBILL.Open "Select MAX(CR_NO) From DBTPYMT WHERE TRX_TYPE = 'FP'", db, adOpenForwardOnly
    If Not (rstBILL.EOF And rstBILL.BOF) Then
