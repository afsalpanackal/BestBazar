...
Begin VB.Form FRMRETURNWO 
   ...
   Begin VB.PictureBox picChecked 
      Height          =   285
      Left            =   390
      Picture         =   "FRMEXPRETURNWO.frx":030A
      ScaleHeight     =   225
      ScaleWidth      =   225
      TabIndex        =   5
      Top             =   1140
   ...
   Begin VB.PictureBox picUnchecked 
      Height          =   285
      Left            =   720
      Picture         =   "FRMEXPRETURNWO.frx":064C
      ScaleHeight     =   225
      ScaleWidth      =   225
      TabIndex        =   4
      Top             =   1170
...
Private Sub Command1_Click()
    ...
    For i = 0 To grdcount.Rows - 1
        ...
        RSTFLAG.Open "SELECT * from [WAR_TRXFILE] where VCH_NO = " & Val(grdcount.TextMatrix(i, 0)) & " AND LINE_NO = " & Val(grdcount.TextMatrix(i, 5)) & " ", db2, adOpenStatic, adLockOptimistic, adCmdText
...
Private Sub Command2_Click()
    ...
    TRXMAST.Open "Select MAX(Val(VCH_NO)) From RTRXFILEWO WHERE TRX_TYPE = 'WR'", db2, adOpenStatic, adLockReadOnly
    ...
    For i = 0 To grdcount.Rows - 1
        ...
        RSTTRXFILE.Open "SELECT *  FROM ITEMMASTWO WHERE ITEM_CODE = '" & grdcount.TextMatrix(i, 6) & "'", db2, adOpenStatic, adLockOptimistic, adCmdText
        With RSTTRXFILE
...
Public Function FILLGRID()
    ...
    With RSTWARTRX
        .Open "SELECT * From [WAR_TRXFILE] WHERE SETTLE_FLAG='Y' AND (ISNULL(RETURN_FLAG ) OR RETURN_FLAG ='N') ORDER BY VCH_NO", db2, adOpenForwardOnly
