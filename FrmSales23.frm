...
Begin VB.Form FRMSALES2 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "SALES BILL - III "
   ClientHeight    =   11100
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   18495
   Icon            =   "FrmSales1.frx":0000
   KeyPreview      =   -1  'True
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin VB.Frame FRMEMAIN 
      ...
      Begin VB.Frame FRMEHEAD 
         ...
         Begin VB.ComboBox cmbtype 
            ...
            ForeColor       =   &H00FF0000&
            Height          =   315
            ItemData        =   "FrmSales1.frx":030A
            Left            =   8355
            List            =   "FrmSales1.frx":031A
    ...
    For i = 1 To grdsales.Rows - 1
        ...
        With RSTUNBILL
            If Not (.EOF And .BOF) Then
                ...
                db.Execute "Update TRXFILE set UN_BILL = '" & Trim(grdsales.TextMatrix(i, 46)) & "' WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE='HI' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(i, 32)) & ""
            End If
        End With
        RSTUNBILL.Close
...
Private Sub Command2_Click()
    ...
    For i = 1 To grdsales.Rows - 1
        ...
        Else
            RSTTRXFILE!VCH_DESC = "Issued to     " & Trim(TxtBillName.Text)
        End If
...
SKIP:
    ...
    If DataList2.BoundText <> "130000" And DataList2.BoundText <> "130001" Then
        ...
        Set RSTTRXFILE = New ADODB.Recordset
        RSTTRXFILE.Open "Select * From DBTPYMT WHERE ACT_CODE = '" & DataList2.BoundText & "' AND NOT(INV_TRX_TYPE = 'HI' AND TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND INV_NO = " & Val(txtBillNo.Text) & ") AND (TRX_TYPE = 'DR' OR TRX_TYPE = 'DB') AND INV_DATE <= '" & Format(TXTINVDATE.Text, "yyyy/mm/dd") & "' ", db, adOpenStatic, adLockReadOnly, adCmdText
        Do Until RSTTRXFILE.EOF
            Select Case RSTTRXFILE!TRX_TYPE
                Case "DB"
...
'    End If
    ...
    If OLD_BILL = False And Val(txtBillNo.Text) <> 1 Then
        RSTTRXFILE.Open "Select * From TRXMAST WHERE VCH_NO= (SELECT MAX(VCH_NO) FROM TRXMAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE = 'HI')", db, adOpenStatic, adLockPessimistic, adCmdText
    ...
    Else
        RSTTRXFILE.Open "Select * FROM TRXMAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE='HI' AND VCH_NO = " & Val(txtBillNo.Text) & "", db, adOpenStatic, adLockPessimistic, adCmdText
        'db.Execute "Lock tables TRXMAST write"
        'db.BeginTrans
        If (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
            RSTTRXFILE.AddNew
...
Private Sub grdsales_DblClick()
    ...
    End If
    On Error GoTo ErrHand
    db.Execute "Update TRXFILE set UN_BILL = '" & Trim(grdsales.TextMatrix(grdsales.Row, 46)) & "' WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE='HI' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(grdsales.Row, 32)) & ""
    Exit Sub
ErrHand:
    MsgBox Err.Description
End Sub
...
'            Set TRXMAST = Nothing
            ...
            TRXFILE.Open "Select * From TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE='HI' AND VCH_NO = " & Val(txtBillNo.Text) & " ORDER BY LINE_NO", db, adOpenStatic, adLockReadOnly
...
'                Set TRXMAST = Nothing
                ...
                TRXSUB.Open "Select * FROM TRXSUB WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE='HI' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(TRXFILE!line_no) & "", db, adOpenStatic, adLockReadOnly
...
'                grdsales.TextMatrix(i, 43) = IIf(IsNull(TRXSUB!R_TRX_YEAR), "", Trim(TRXSUB!R_TRX_YEAR
            ...
            If OLD_BILL = False Then
                Set TRXMAST = New ADODB.Recordset
                TRXMAST.Open "Select MAX(VCH_NO) From TRXMAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE = 'HI'", db, adOpenForwardOnly
                If Not (TRXMAST.EOF And TRXMAST.BOF) Then
...
'                    rstBILL.Open "Select * From TRXMAST WHERE VCH_NO= (SELECT MAX(VCH_NO) FROM TRXMAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE = 'HI')", db, adOpenStatic, adLockPessimistic, adCmdText
...
'                Else
'                    rstBILL.Open "Select * FROM TRXMAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE='HI' AND VCH_NO = " & Val(txtBillNo.Text) & "", db, adOpenStatic, adLockPessimistic, adCmdText
...
Private Function AppendSale()
    ...
    Set RSTITEMMAST = New ADODB.Recordset
    RSTITEMMAST.Open "SELECT * FROM DBTPYMT WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND INV_NO = " & Val(txtBillNo.Text) & " AND TRX_TYPE = 'RT' AND INV_TRX_TYPE = 'HI' ", db, adOpenStatic, adLockOptimistic, adCmdText
    If Not (RSTITEMMAST.EOF And RSTITEMMAST.BOF) Then
...
SKIP2:
    ...
    '    db.Execute "delete From DBTPYMT WHERE TRX_TYPE='DR' AND INV_NO = " & Val(txtBillNo.Text) & " AND INV_TRX_TYPE = 'HI'"
        db.Execute "delete From DBTPYMT WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND INV_NO = " & Val(txtBillNo.Text) & " AND TRX_TYPE = 'DR' AND INV_TRX_TYPE = 'HI' "
        db.Execute "delete From DBTPYMT WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND INV_NO = " & Val(txtBillNo.Text) & " AND TRX_TYPE = 'RT' AND INV_TRX_TYPE = 'HI' "
        If lblCBFLAG.Caption = "B" And BANKCODE <> 0 Then
            db.Execute "delete FROM CASHATRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND INV_NO = " & Val(txtBillNo.Text) & " AND INV_TYPE = 'RT' AND INV_TRX_TYPE = 'HI'"
...
'    db.Execute "Update TRXFILE set VCH_DESC =  '" & Trim(Rcvd_Party) & "' WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE='HI' AND VCH_NO = " & Val(txtBillNo.Text) & " "
...
CLOSEFILE:
        ...
        If MDIMAIN.lblgst.Caption = "C" Then
            ...
            ReportNameVar = Rptpath & "RPTOUTPASSC"
        Else
            ReportNameVar = Rptpath & "RPTOUTPASSN"
        End If
    ...
    If Val(MDIMAIN.StatusBar.Panels(12).Text) = 1 Then
        If Small_Print = True Then
            If MDIMAIN.lblgst.Caption = "C" Then
                ReportNameVar = Rptpath & "RPTGSTBILLCA51"
            Else
                ReportNameVar = Rptpath & "RPTGSTBILLNA51"
            End If
        Else
            If MDIMAIN.lblgst.Caption = "C" Then
                ReportNameVar = Rptpath & "RPTGSTBILLC1"
            Else
                ReportNameVar = Rptpath & "RPTGSTBILLN1"
            End If
        End If
        Set Report = crxApplication.OpenReport(ReportNameVar, 1)
    ...
    Else
        If Small_Print = True Then
            If MDIMAIN.lblgst.Caption = "C" Then
                ReportNameVar = Rptpath & "RPTGSTBILLCA5"
            Else
                ReportNameVar = Rptpath & "RPTGSTBILLNA5"
            End If
        Else
            If MDIMAIN.lblgst.Caption = "C" Then
                ReportNameVar = Rptpath & "rptGSTBILLC"
            Else
                ReportNameVar = Rptpath & "rptGSTBILLN"
            End If
        End If
