...
Private Function Import_Bill()
    ...
    'LBLTOTAL.Caption = ws.Range("AD" & T).Value
    ...
    For i = 2 To 5000
        ...
        grdsales.TextMatrix(n, 2) = Trim(ws.Range("AD" & i).Value) 'ITEM NAME
...
'        End If
        ...
        grdsales.TextMatrix(n, 5) = Val(ws.Range("BG" & i).Value)  'PACK
        If Val(grdsales.TextMatrix(n, 5)) = 0 Then grdsales.TextMatrix(n, 5) = 1
        grdsales.TextMatrix(n, 3) = Val(ws.Range("AL" & i).Value) + Val(ws.Range("AM" & i).Value) 'QTY  + FREE
        grdsales.TextMatrix(n, 4) = 1
        grdsales.TextMatrix(n, 6) = Format(Val(ws.Range("AY" & i).Value), ".000") 'MRP
        grdsales.TextMatrix(n, 7) = Format(Val(grdsales.TextMatrix(n, 6)) / Val(grdsales.TextMatrix(n, 5)), ".000") 'MRP / PACK 'Format(Val(ws.Range("J" & i).value) / Val(grdsales.TextMatrix(N, 5)), ".000")
...
'        End If
        grdsales.TextMatrix(n, 9) = Format(Val(ws.Range("AQ" & i).Value) / Val(grdsales.TextMatrix(n, 5)), ".000") 'RATE / Pack  Format(Val(ws.Range("H" & i).value) / Val(grdsales.TextMatrix(N, 5)), ".000")
        grdsales.TextMatrix(n, 10) = Val(ws.Range("BU" & i).Value) + Val(ws.Range("BV" & i).Value) + Val(ws.Range("BW" & i).Value) 'TAX * 2 'IIf(Val(rstTRXMAST!SALES_TAX) = 0, "", Format(rstTRXMAST!SALES_TAX, ".00"))
        grdsales.TextMatrix(n, 15) = "V" 'IIf(IsNull(rstTRXMAST!CHECK_FLAG), "N", rstTRXMAST!CHECK_FLAG)
            
        grdsales.TextMatrix(n, 11) = Trim(ws.Range("AG" & i).Value) ' BATCH
        'grdsales.TextMatrix(n, 12) = "" ' EXPIRY
        If Len(Trim(ws.Range("AH" & i).Value)) = 4 Then
            MON = Val(Mid(Trim(ws.Range("AH" & i).Value), 1, 2))
            Y = Val(Right(Trim(ws.Range("AH" & i).Value), 2))
            Y = 2000 + Y
            M_DATE = "01" & "/" & MON & "/" & Y
            D = LastDayOfMonth(M_DATE)
            M_DATE = D & "/" & MON & "/" & Y
        ...
        grdsales.TextMatrix(n, 14) = Val(ws.Range("AM" & i).Value)   ' FREE
        grdsales.TextMatrix(n, 17) = Val(ws.Range("AP" & i).Value) 'DISC IIf(IsNull(rstTRXMAST!P_DISC), 0, rstTRXMAST!P_DISC)
...
'        End If
        ...
        grdsales.TextMatrix(n, 23) = "" 'Trim(ws.Range("AD" & i).value)
        grdsales.TextMatrix(n, 24) = "" 'Left(Trim(ws.Range("W" & i).value), 20)
        grdsales.TextMatrix(n, 25) = Left(Trim(ws.Range("A" & i).Value), 8) 'HSN
        grdsales.TextMatrix(n, 26) = "BX" 'R. RATE
        grdsales.TextMatrix(n, 27) = "P" 'DISC FLAG
    Next i
    'or
