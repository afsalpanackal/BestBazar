...
Private Sub CmdSave_Click()
    ...
    If OptBank.Value = True Then
        ...
        rstBILL.Open "Select MAX(TRX_NO) From BANK_TRX WHERE TRX_TYPE = 'CR' AND BILL_TRX_TYPE = 'FR' AND TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' ", db, adOpenForwardOnly
    ...
    'db.Execute "delete FROM CASHATRXFILE WHERE TRX_TYPE = 'CR' AND INV_NO = " & Val(txtBillNo.Text) & " AND INV_TYPE = 'FR'  AND INV_TRX_TYPE = 'FR'"
    ...
    If optCash.Value = True Then
        ...
        'db.Execute "delete FROM CASHATRXFILE WHERE INV_NO = " & Val(creditbill.LBLBILLNO.Caption) & " AND INV_TYPE = 'FR'"
        
        Set RSTITEMMAST = New ADODB.Recordset
        RSTITEMMAST.Open "SELECT * FROM CASHATRXFILE", db, adOpenStatic, adLockOptimistic, adCmdText
        RSTITEMMAST.AddNew
        RSTITEMMAST!REC_NO = i + 1
        RSTITEMMAST!INV_TYPE = "FR"
        RSTITEMMAST!INV_TRX_TYPE = "FR"
    ...
    End If
    'If OptCash.value = False Then db.Execute "delete FROM CASHATRXFILE WHERE TRX_TYPE = 'CR' AND INV_NO = " & Val(txtBillNo.Text) & " AND INV_TYPE = 'FR'  AND INV_TRX_TYPE = 'FR'"
        
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * From DBTPYMT", db, adOpenStatic, adLockOptimistic, adCmdText
    RSTTRXFILE.AddNew
    RSTTRXFILE!TRX_TYPE = "FR"
    RSTTRXFILE!INV_TRX_TYPE = "RP"
...
Private Sub Form_Load()
    ...
    Set rstBILL = New ADODB.Recordset
    rstBILL.Open "Select MAX(CR_NO) From DBTPYMT WHERE TRX_TYPE = 'FR'", db, adOpenForwardOnly
    If Not (rstBILL.EOF And rstBILL.BOF) Then
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    FRMDPReg.Enabled = True
    FRMDPReg.GRDTranx.SetFocus
End Sub
