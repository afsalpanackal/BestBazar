...
Begin VB.Form FrmCustList 
   ...
   Begin VB.Frame Frame2 
      ...
      End
   End
   Begin VB.CommandButton CmdPrevCust 
      Caption         =   "Privilege Customers"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   8.25
         Charset         =   0
...
Private Sub CmdPrevCust_Click()
    ...
    GRDTranx.TextMatrix(0, 3) = "ADDRESS"
    GRDTranx.TextMatrix(0, 4) = "PHONE"
    GRDTranx.TextMatrix(0, 5) = "Total Points"
    GRDTranx.TextMatrix(0, 6) = "Settl. Points"
    GRDTranx.TextMatrix(0, 7) = "Avail. Points"
    
    GRDTranx.ColWidth(0) = 800
    GRDTranx.ColWidth(1) = 1500
    GRDTranx.ColWidth(2) = 4000
    GRDTranx.ColWidth(3) = 2500
    ...
    Dim RSTTRXFILE, rstCust As ADODB.Recordset
    Dim i As Long
    Dim avail_points As Double
    Dim deduct_points As Double
    ...
    Set rstCust = New ADODB.Recordset
    rstCust.Open "SELECT * From cust_details ORDER BY ACT_NAME", db, adOpenStatic, adLockOptimistic
    Do Until rstCust.EOF
        GRDTranx.rows = GRDTranx.rows + 1
        GRDTranx.FixedRows = 1
        GRDTranx.TextMatrix(i, 0) = i
        GRDTranx.TextMatrix(i, 1) = IIf(IsNull(rstCust!ACT_CODE), "", rstCust!ACT_CODE)
        GRDTranx.TextMatrix(i, 2) = IIf(IsNull(rstCust!ACT_NAME), "", rstCust!ACT_NAME)
        GRDTranx.TextMatrix(i, 3) = IIf(IsNull(rstCust!cust_address), "", rstCust!cust_address)
        GRDTranx.TextMatrix(i, 4) = IIf(IsNull(rstCust!cust_phone), "", rstCust!cust_phone)
        
        avail_points = 0
        deduct_points = 0
        'If DataList2.BoundText <> "" Then
            Set RSTTRXFILE = New ADODB.Recordset
            RSTTRXFILE.Open "SELECT SUM(CUST_POINTS) FROM TRXMAST WHERE CUST_CODE = '" & rstCust!ACT_CODE & "' ", db, adOpenForwardOnly
            If Not (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
                avail_points = IIf(IsNull(RSTTRXFILE.Fields(0)), 0, RSTTRXFILE.Fields(0))
            End If
            RSTTRXFILE.Close
            Set RSTTRXFILE = Nothing
            
            Set RSTTRXFILE = New ADODB.Recordset
            RSTTRXFILE.Open "SELECT SUM(CUST_POINTS_DEDUCT) FROM TRXMAST WHERE CUST_CODE = '" & rstCust!ACT_CODE & "' ", db, adOpenForwardOnly
            If Not (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
                deduct_points = IIf(IsNull(RSTTRXFILE.Fields(0)), 0, RSTTRXFILE.Fields(0))
            End If
            RSTTRXFILE.Close
            Set RSTTRXFILE = Nothing
        'End If
    
        GRDTranx.TextMatrix(i, 5) = avail_points
        GRDTranx.TextMatrix(i, 6) = deduct_points
        GRDTranx.TextMatrix(i, 7) = Round(avail_points - deduct_points, 2)
        i = i + 1
        rstCust.MoveNext
    Loop
...
Private Sub GRDTranx_DblClick()
    If GRDTranx.Cols <> 8 Then Exit Sub
    Dim i As Long
    ...
    GRDBILL.TextMatrix(i, 0) = "Sl"
    GRDBILL.TextMatrix(i, 1) = "Bill No."
    GRDBILL.TextMatrix(i, 2) = "Bill Amt"
    GRDBILL.TextMatrix(i, 3) = "Points"
    GRDBILL.TextMatrix(i, 4) = "Deducted"
    
    GRDBILL.ColWidth(0) = 600
    ...
    On Error GoTo ERRHAND
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * From TRXMAST WHERE CUST_CODE = '" & GRDTranx.TextMatrix(GRDTranx.Row, 1) & "' ", db, adOpenStatic, adLockReadOnly
    Do Until RSTTRXFILE.EOF
        i = i + 1
        GRDBILL.rows = GRDBILL.rows + 1
        GRDBILL.FixedRows = 1
        GRDBILL.TextMatrix(i, 0) = i
        GRDBILL.TextMatrix(i, 1) = RSTTRXFILE!VCH_NO
        GRDBILL.TextMatrix(i, 2) = IIf(IsNull(RSTTRXFILE!NET_AMOUNT), 0, RSTTRXFILE!NET_AMOUNT)
        GRDBILL.TextMatrix(i, 3) = IIf(IsNull(RSTTRXFILE!CUST_POINTS), 0, RSTTRXFILE!CUST_POINTS)
        GRDBILL.TextMatrix(i, 4) = IIf(IsNull(RSTTRXFILE!CUST_POINTS_DEDUCT), 0, RSTTRXFILE!CUST_POINTS_DEDUCT)
        RSTTRXFILE.MoveNext
    Loop
    RSTTRXFILE.Close
    Set RSTTRXFILE = Nothing
    GRDTranx.Enabled = False
...
Private Sub GRDTranx_KeyDown(KeyCode As Integer, Shift As Integer)
    Dim sitem As String
    Dim i As Long
    If selection_flag = False Then Exit Sub
    If GRDTranx.Cols = 9 Or GRDTranx.Cols = 8 Then
        If GRDTranx.rows = 1 Then Exit Sub
        Select Case KeyCode
            Case vbKeyReturn
