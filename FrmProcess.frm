   ...
   Begin VB.Frame FRMEMAIN 
      ...
      Begin VB.Frame Frame4 
         ...
         Begin VB.ComboBox combopack 
            ...
            ForeColor       =   &H000000FF&
            Height          =   360
            ItemData        =   "FrmProcess.frx":08CA
            Left            =   5520
            List            =   "FrmProcess.frx":0901
         ...
         Begin VB.ComboBox CmbPack 
            ...
            ForeColor       =   &H000000FF&
            Height          =   360
            ItemData        =   "FrmProcess.frx":0963
            Left            =   1020
            List            =   "FrmProcess.frx":099A
            Style           =   2  'Dropdown List
            TabIndex        =   14
            Top             =   3360
            Width           =   1200
...
Private Sub cmdcancel_Click()
    If MsgBox("ARE YOU SURE YOU WANT TO DELETE THE PROCESS?", vbYesNo + vbDefaultButton2, "DELETE.....") = vbNo Then Exit Sub
    On Error GoTo ErrHand
    db.Execute "delete From TRXFORMULAMAST WHERE TRX_TYPE='PR' AND FOR_NO = " & Val(txtBillNo.text) & ""
    db.Execute "delete FROM TRXFORMULASUB WHERE TRX_TYPE='PR' AND FOR_NO = " & Val(txtBillNo.text) & ""
    Dim rstBILL As ADODB.Recordset
    Set rstBILL = New ADODB.Recordset
    rstBILL.Open "Select MAX(FOR_NO) FROM TRXFORMULAMAST WHERE TRX_TYPE = 'PR'", db, adOpenStatic, adLockReadOnly
...
Private Sub cmdRefresh_Click()
   ...
   ' If grdsales.Rows = 1 Then GoTo SKIP
    ...
    If Trim(TxtFormula.text) = "" Then
        MsgBox "Please enter a name for Process", , "Process Creation"
        TxtFormula.SetFocus
        Exit Sub
    End If
    Dim RSTTRXFILE As ADODB.Recordset
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * From TRXFORMULAMAST WHERE FOR_NAME= '" & Trim(TxtFormula.text) & "' AND FOR_NO <> " & Val(txtBillNo.text) & " ", db, adOpenStatic, adLockReadOnly, adCmdText
    If Not (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
        MsgBox "Already exists", vbOKOnly, "Process Creation"
        RSTTRXFILE.Close
        Set RSTTRXFILE = Nothing
        Exit Sub
    End If
    ...
    If grdsales2.rows <= 1 Then
        MsgBox "Please add the Final Product(s)", , "Process Creation"
        Exit Sub
    End If
'    If DataList1.BoundText = "" Then
'        MsgBox "Please Select a Product", , "Process Creation"
'        TXTPRODUCT2.SetFocus
'        Exit Sub
'    End If
'
'    If Val(TxtqtySer.Text) = 0 Then
'        MsgBox "Please Enter Qty for the Product", , "Process Creation"
'        TxtqtySer.SetFocus
'        Exit Sub
'    End If
'
...
Private Sub CMDADD2_Click()
    On Error GoTo ErrHand
    If DataList1.BoundText = "" Then
        MsgBox "Please select the product", vbOKOnly, "Formula Mixture"
        TXTPRODUCT2.SetFocus
        Exit Sub
    End If
    
    If Val(TxtqtySer.text) = 0 Then
        MsgBox "Please select the qty", vbOKOnly, "Formula Mixture"
        TxtqtySer.SetFocus
        Exit Sub
    End If
    
    If CmbPack.ListIndex = -1 Then
        MsgBox "Please select the pack type", vbOKOnly, "Formula Mixture"
        CmbPack.SetFocus
        Exit Sub
    End If
    ...
    grdsales2.TextMatrix(grdsales2.rows - 1, 0) = grdsales2.rows - 1
    grdsales2.TextMatrix(grdsales2.rows - 1, 1) = DataList1.BoundText
    grdsales2.TextMatrix(grdsales2.rows - 1, 2) = DataList1.text
    grdsales2.TextMatrix(grdsales2.rows - 1, 3) = Val(TxtqtySer.text)
    grdsales2.TextMatrix(grdsales2.rows - 1, 4) = Val(Txtpack.text)
    grdsales2.TextMatrix(grdsales2.rows - 1, 5) = CmbPack.text
    ...
    'grdsales2.TopRow = grdsales2.Rows - 1
Exit Sub
ErrHand:
    MsgBox err.Description
End Sub
...
Private Sub Command1_Click()
    ...
    If MsgBox("ARE YOU SURE YOU WANT TO DELETE " & """" & grdsales2.TextMatrix(grdsales2.Row, 2) & "" & " FROM THE LIST", vbYesNo + vbDefaultButton2, "DELETE.....") = vbNo Then Exit Sub
    Dim selrow As Integer
    Dim i As Integer
    selrow = grdsales2.Row
    For i = selrow To grdsales2.rows - 2
        grdsales2.TextMatrix(selrow, 0) = i
        grdsales2.TextMatrix(selrow, 1) = grdsales2.TextMatrix(i + 1, 1)
        grdsales2.TextMatrix(selrow, 2) = grdsales2.TextMatrix(i + 1, 2)
        grdsales2.TextMatrix(selrow, 3) = grdsales2.TextMatrix(i + 1, 3)
        grdsales2.TextMatrix(selrow, 4) = grdsales2.TextMatrix(i + 1, 4)
        grdsales2.TextMatrix(selrow, 5) = grdsales2.TextMatrix(i + 1, 5)
        selrow = selrow + 1
    Next i
    grdsales2.rows = grdsales2.rows - 1
    grdsales2.SetFocus
...
Private Sub Form_Activate()
    ...
    If TxtFormula.Enabled = True Then TxtFormula.SetFocus
End Sub
...
Private Sub Form_Load()
    ...
    grdsales2.ColWidth(0) = 500
    grdsales2.ColWidth(1) = 0
    grdsales2.ColWidth(2) = 3800
    grdsales2.ColWidth(3) = 1000
    grdsales2.ColWidth(4) = 800
    grdsales2.ColWidth(5) = 700
...
Private Sub TXTBILLNO_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            TRXSUB.Open "Select * From TRXFORMULAMAST WHERE TRX_TYPE='PR' AND FOR_NO = " & Val(txtBillNo.text) & "", db, adOpenStatic, adLockReadOnly
            Do Until TRXSUB.EOF
                ...
                grdsales2.rows = grdsales2.rows + 1
                grdsales2.FixedRows = 1
                grdsales2.TextMatrix(SN, 0) = SN
                grdsales2.TextMatrix(SN, 1) = TRXSUB!ITEM_CODE
                grdsales2.TextMatrix(SN, 2) = TRXSUB!ITEM_NAME
                grdsales2.TextMatrix(SN, 3) = TRXSUB!QTY
                grdsales2.TextMatrix(SN, 4) = IIf(IsNull(TRXSUB!LOOSE_PACK), "1", TRXSUB!LOOSE_PACK)
                grdsales2.TextMatrix(SN, 5) = IIf(IsNull(TRXSUB!PACK_TYPE), "Nos", TRXSUB!PACK_TYPE)
                SN = SN + 1
                TRXSUB.MoveNext
            Loop
            TRXSUB.Close
            ...
            TRXFILE.Open "Select * FROM TRXFORMULASUB WHERE TRX_TYPE='PR' AND FOR_NO = " & Val(txtBillNo.text) & " ORDER BY LINE_NO ", db, adOpenStatic, adLockReadOnly
...
Private Sub txtBillNo_LostFocus()
    ...
    TRXMAST.Open "Select MAX(FOR_NO) FROM TRXFORMULASUB WHERE TRX_TYPE = 'PR'", db, adOpenStatic, adLockReadOnly
    ...
    Set TRXMAST = New ADODB.Recordset
    TRXMAST.Open "Select MIN(FOR_NO) FROM TRXFORMULASUB WHERE TRX_TYPE = 'PR'", db, adOpenStatic, adLockReadOnly
    If Not (TRXMAST.EOF And TRXMAST.BOF) Then
    ...
    Select Case KeyCode
        ...
        Case vbKeyReturn
            ...
            If PHY.RecordCount = 1 Then
                ...
                For i = 1 To grdsales.rows - 1
                    If Trim(grdsales.TextMatrix(i, 1)) = Trim(TXTITEMCODE.text) Then
                        If MsgBox("This Item Already exists in Line No. " & i & " Do yo want to add this item", vbYesNo + vbDefaultButton2, "PROCESS") = vbNo Then Exit Sub
                    End If
                Next i
...
Private Sub TxtqtySer_KeyPress(KeyAscii As Integer)
    Select Case KeyAscii
        Case Asc("'"), Asc("["), Asc("]"), Asc("\")
            KeyAscii = 0
        Case vbKey0 To vbKey9, Asc("."), vbKeyLeft, vbKeyRight, vbKeyBack
            KeyAscii = Asc(UCase(Chr(KeyAscii)))
        Case Else
            KeyAscii = 0
    End Select
...
Private Function AppendSale()
    ...
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * From TRXFORMULAMAST WHERE TRX_TYPE='PR' AND FOR_NO = " & Val(txtBillNo.text) & "", db, adOpenStatic, adLockOptimistic, adCmdText
    For i = 1 To grdsales2.rows - 1
        RSTTRXFILE.AddNew
        RSTTRXFILE!FOR_NO = Val(txtBillNo.text)
        RSTTRXFILE!LINE_NO = i
        RSTTRXFILE!TRX_TYPE = "PR"
        RSTTRXFILE!FOR_NAME = Trim(TxtFormula.text) 'Trim(TXTPRODUCT2.Text)
        RSTTRXFILE!ITEM_CODE = grdsales2.TextMatrix(i, 1) 'DataList1.BoundText
        RSTTRXFILE!ITEM_NAME = grdsales2.TextMatrix(i, 2)
        RSTTRXFILE!QTY = Val(grdsales2.TextMatrix(i, 3))
        RSTTRXFILE!LOOSE_PACK = Val(grdsales2.TextMatrix(i, 4))
        RSTTRXFILE!PACK_TYPE = grdsales2.TextMatrix(i, 5)
...
Private Sub TxtProduct2_Change()
    ...
    If flagchange2.Caption <> "1" Then
        If MIX_FLAG = True Then
            MIX_ITEM.Open "select ITEM_CODE, ITEM_NAME from ITEMMAST where ITEM_NAME Like '%" & TXTPRODUCT2.text & "%' AND ucase(CATEGORY) = 'RAW' ORDER BY ITEM_NAME", db, adOpenStatic, adLockReadOnly, adCmdText
            MIX_FLAG = False
        Else
            MIX_ITEM.Close
            MIX_ITEM.Open "select ITEM_CODE, ITEM_NAME from ITEMMAST  where ITEM_NAME Like '%" & TXTPRODUCT2.text & "%' AND ucase(CATEGORY) = 'RAW' ORDER BY ITEM_NAME", db, adOpenStatic, adLockReadOnly, adCmdText
