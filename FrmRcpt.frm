...
Begin VB.Form FRMRcptLenders 
   ...
   End
End
Attribute VB_Name = "FRMRcptLenders"
...
Private Sub CmdSave_Click()
    ...
    If Val(txtrcptamt.Text) = 0 Then
        MsgBox "Enter receipt Amount", vbOKOnly, "PAYMENT..."
        txtrcptamt.SetFocus
        Exit Sub
    End If
    ...
    rstBILL.Open "Select MAX(CR_NO) From DBTPYMT WHERE TRX_TYPE = 'RL'", db, adOpenForwardOnly
    ...
    If OptBank.value = True Then
        ...
        TRX_NO = 1
        Set rstBILL = New ADODB.Recordset
        rstBILL.Open "Select MAX(TRX_NO) From BANK_TRX WHERE TRX_TYPE = 'CR' AND BILL_TRX_TYPE = 'ML' AND TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' ", db, adOpenForwardOnly
        If Not (rstBILL.EOF And rstBILL.BOF) Then
    ...
    If OptCash.value = True Then
        RSTTRXFILE!C_TRX_TYPE = "DR"
        RSTTRXFILE!C_REC_NO = i
        RSTTRXFILE!C_INV_TRX_TYPE = "RL"
        RSTTRXFILE!C_INV_TYPE = "RL"
