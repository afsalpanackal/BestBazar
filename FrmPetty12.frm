...
Begin VB.Form FRMPETTY1 
   ...
   Caption         =   "PETTY BILL - I "
   ClientHeight    =   11100
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   18495
   Icon            =   "FrmPetty12.frx":0000
   KeyPreview      =   -1  'True
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin VB.Frame FRMEMAIN 
      ...
      Begin VB.Frame FRMEHEAD 
         ...
         Begin VB.ComboBox cmbtype 
            ...
            ForeColor       =   &H00FF0000&
            Height          =   315
            ItemData        =   "FrmPetty12.frx":030A
            Left            =   8355
            List            =   "FrmPetty12.frx":0323
...
'    Set RSTTRXFILE = Nothing
    ...
    db.Execute "INSERT INTO `can_trxfile` SELECT * FROM TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='WO' AND VCH_NO = " & Val(txtBillNo.Text) & " "
    ...
    For n = 1 To grdsales.rows - 1
        Set RSTTRXFILE = New ADODB.Recordset
        RSTTRXFILE.Open "SELECT *  FROM ITEMMAST WHERE ITEM_CODE = '" & grdsales.TextMatrix(n, 13) & "' AND CATEGORY <> 'SERVICES' AND UCASE(CATEGORY) <> 'SERVICE CHARGE'  AND CATEGORY <> 'SELF'", db, adOpenStatic, adLockOptimistic, adCmdText
        With RSTTRXFILE
            If Not (.EOF And .BOF) Then
                .Properties("Update Criteria").Value = adCriteriaKey
                '!ISSUE_QTY = !ISSUE_QTY - (Val(grdsales.TextMatrix(N, 3)) + Val(grdsales.TextMatrix(N, 20)))
        ...
        If grdsales.TextMatrix(n, 19) = "DN" Then
            ...
            RSTTRXFILE.Open "SELECT *  FROM TEMPCN WHERE TRX_TYPE = 'SI' AND TRX_YEAR = '" & Val(grdsales.TextMatrix(n, 47)) & "' AND VCH_NO = " & Val(grdsales.TextMatrix(n, 35)) & " AND LINE_NO = " & Val(grdsales.TextMatrix(n, 37)) & " ", db, adOpenStatic, adLockOptimistic, adCmdText
            With RSTTRXFILE
                If Not (.EOF And .BOF) Then
                    RSTTRXFILE!check_flag = "N"
...
'    End If
    ...
    If DataList2.BoundText = "130000" Or DataList2.BoundText = "130001" Then
        ...
        RSTBANK.Open "select * from BANKCODE WHERE POS_FLAG = 'Y' ", db, adOpenStatic, adLockReadOnly, adCmdText
        If RSTBANK.RecordCount > 0 Then
            If RSTBANK.RecordCount = 1 Then BANKCODE = RSTBANK!BANK_CODE
            RSTBANK.Close
            Set RSTBANK = Nothing
            Me.Enabled = False
            FRMPOS.Show
...
Private Sub cmdratechange_Click()
    ...
    For i = 1 To grdsales.rows - 1
        ...
        db.Execute "Update TRXFILE set SALES_PRICE = " & Val(grdsales.TextMatrix(i, 7)) & ", P_RETAIL = " & Val(grdsales.TextMatrix(i, 7)) & ", PTR = " & Val(grdsales.TextMatrix(i, 6)) & ", P_RETAILWOTAX = " & Val(grdsales.TextMatrix(i, 6)) & ", TRX_TOTAL = " & Val(grdsales.TextMatrix(i, 12)) & ", SCHEME = " & Val(TXTDISC.Tag) & " WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='WO' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(i, 32)) & ""
SKIP:
    Next i
    LBLTOTAL.Caption = ""
    lblnetamount.Caption = ""
...
'                db.Execute "Update TRXFILE set UN_BILL = '" & Trim(grdsales.TextMatrix(i, 46)) & "' WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE='WO' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(i, 32)) & ""
'            End If
'        End With
'        RSTUNBILL.Close
...
SKIP:
    ...
    If DataList2.BoundText <> "130000" And DataList2.BoundText <> "130001" Then
        ...
        Set RSTTRXFILE = New ADODB.Recordset
        RSTTRXFILE.Open "Select * From DBTPYMT WHERE ACT_CODE = '" & DataList2.BoundText & "' AND NOT(INV_TRX_TYPE = 'WO' AND TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND INV_NO = " & Val(txtBillNo.Text) & ") AND (TRX_TYPE = 'DR' OR TRX_TYPE = 'DB') AND INV_DATE <= '" & Format(TXTINVDATE.Text, "yyyy/mm/dd") & "' ", db, adOpenStatic, adLockReadOnly, adCmdText
        Do Until RSTTRXFILE.EOF
            Select Case RSTTRXFILE!TRX_TYPE
                Case "DB"
...
'                End If
    ...
    RSTTRXFILE!DN_YEAR = Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 47))
...
'    Set RSTUNBILL = Nothing
    ...
    If CMBDISTI.BoundText <> "" Or (lblsubdealer.Caption = "D" And Val(lblActAmt.Caption) <> 0) Then
        db.Execute "Update TRXMAST set COMM_AMT = " & Val(lblcomamt.Text) & " WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='WO' AND VCH_NO = " & Val(txtBillNo.Text) & ""
    End If
    ...
    TxtSpec.Text = ""
    txtcategory.Text = ""
    If MDIMAIN.lblitemrepeat.Caption = "N" Then
        CHANGE_NAME = False
        TxtName1.Text = ""
    End If
    CHANGE_NAME = False
...
Private Sub CmdDelete_Click()
    ...
    db.Execute "INSERT INTO `can_trxfile` SELECT * FROM TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='WO' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 32)) & ""
    If grdsales.TextMatrix(Val(TXTSLNO.Text), 19) = "DN" Then
        Select Case MsgBox("Do you want to remove the Item " & """" & grdsales.TextMatrix(Val(TXTSLNO.Text), 2) & """" & " from Delivery List", vbYesNoCancel, "DELETE.....")
            Case vbCancel
                Exit Sub
            Case vbYes
                db.Execute "DELETE  FROM TEMPCN WHERE ACT_CODE = '" & Trim(DataList2.BoundText) & "' AND TRX_YEAR='" & Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 47)) & "' AND TRX_TYPE = 'SI' AND VCH_NO = " & Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 35)) & " AND LINE_NO = " & Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 37)) & " "
                db.Execute "DELETE  FROM TRXFILE WHERE TRX_TYPE = 'DN' TRX_YEAR='" & Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 47)) & "' AND AND VCH_NO = " & Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 35)) & " AND LINE_NO = " & Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 37)) & " "
            Case vbNo
                Set RSTTRXFILE = New ADODB.Recordset
                RSTTRXFILE.Open "SELECT *  FROM TEMPCN WHERE ACT_CODE = '" & Trim(DataList2.BoundText) & "' AND TRX_TYPE = 'SI' AND TRX_YEAR='" & Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 47)) & "' AND VCH_NO = " & Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 35)) & " AND LINE_NO = " & Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 37)) & " ", db, adOpenStatic, adLockOptimistic, adCmdText
                With RSTTRXFILE
                    If Not (.EOF And .BOF) Then
                        RSTTRXFILE!check_flag = "N"
                        RSTTRXFILE!BILL_NO = 0 ' Null
...
'    db.Execute "SET SESSION TRANSACTION ISOLATION LEVEL READ COMMITTED"
    ...
    For n = 0 To grdcount.rows - 1
        ...
        If Not (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
            ...
            grdsales.TextMatrix(grdsales.rows - 1, 47) = IIf(IsNull(RSTTRXFILE!TRX_YEAR), "", RSTTRXFILE!TRX_YEAR)
            'db.CommitTrans
            RSTTRXFILE!check_flag = "Y"
            RSTTRXFILE!BILL_NO = Val(txtBillNo.Text)
        ...
        RSTTRXFILE!DN_YEAR = Val(grdsales.TextMatrix(grdsales.rows - 1, 47))
...
Public Function Generateprint()
    ...
    'Call Print_A4
    'Exit Function
    If (thermalprn = False And MDIMAIN.StatusBar.Panels(8).Text = "Y") Or (thermalprn = True And MDIMAIN.LBLDMPTHERMAL.Caption = "Y") Then
        GoTo SKIP
        If DataList2.BoundText <> "130000" And DataList2.BoundText <> "130001" Then
            Set RSTTRXFILE = New ADODB.Recordset
            RSTTRXFILE.Open "select OPEN_DB from CUSTMAST  WHERE ACT_CODE = '" & DataList2.BoundText & "' ", db, adOpenStatic, adLockReadOnly, adCmdText
...
'    End If
    ...
    RSTCOMPANY.Open "SELECT * FROM TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='WO' AND VCH_NO = " & Val(txtBillNo.Text) & "", db, adOpenForwardOnly
...
Private Sub GRDPOPUPITEM_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            End If
            i = 0
            If ZERO_WARN_FLAG = True And M_STOCK <= 0 Then
...
'    End If
'    On Error GoTo Errhand
'    db.Execute "Update TRXFILE set UN_BILL = '" & Trim(grdsales.TextMatrix(grdsales.Row, 46)) & "' WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE='WO' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(grdsales.Row, 32)) & ""
'    Exit Sub
'Errhand:
'    MsgBox Err.Description
'End Sub
...
'                grdsales.TextMatrix(i, 43) = IIf(IsNull(TRXSUB!R_TRX_YEAR), "", Trim(TRXSUB!R_TRX_YEAR
                ...
                grdsales.TextMatrix(i, 47) = IIf(IsNull(TRXFILE!DN_YEAR), "", TRXFILE!DN_YEAR)
            ...
            If OLD_BILL = False Then
                Set TRXMAST = New ADODB.Recordset
                TRXMAST.Open "Select MAX(VCH_NO) From TRXMAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE = 'WO'", db, adOpenForwardOnly
                If Not (TRXMAST.EOF And TRXMAST.BOF) Then
...
'                    rstBILL.Open "Select * From TRXMAST WHERE VCH_NO= (SELECT MAX(VCH_NO) FROM TRXMAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE = 'WO')", db, adOpenStatic, adLockPessimistic, adCmdText
...
'                Else
'                    rstBILL.Open "Select * FROM TRXMAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE='WO' AND VCH_NO = " & Val(txtBillNo.Text) & "", db, adOpenStatic, adLockPessimistic, adCmdText
...
Private Sub TxtDiscAmt_Change()
    If Val(TxtDiscAmt.Text) = 0 Or Val(txtretail.Text) = 0 Then
        TXTDISC.Text = ""
    Else
        TXTDISC.Text = Round((Val(TxtDiscAmt.Text) * 100) / Val(txtretail.Text), 2)
    End If
...
'            End If
            ...
            If UCase(TXTITEMCODE.Text) <> "OT" Then
                ...
                If PHY.RecordCount = 0 And Left(Trim(TXTITEMCODE.Text), 1) <> "#" Then
                    If MsgBox("Item not exists... Do yo want to create this item", vbYesNo, "BILL..") = vbNo Then
                        TXTPRODUCT.SelStart = 0
                        TXTPRODUCT.SelLength = Len(TXTPRODUCT.Text)
                        Exit Sub
                    Else
                        frmitemmastermini.Show
                        frmitemmastermini.SetFocus
                        Call frmitemmastermini.TxtItemcode_KeyDown(13, 0)
                        frmitemmastermini.TXTITEM = Trim(TXTPRODUCT.Text)
                        Exit Sub
                    End If
                End If
...
'                End If
                'If i <> 0 Then
                    If ZERO_WARN_FLAG = True And M_EDIT = False And SERIAL_FLAG = True And Val(TXTQTY.Text) > (Val(lblactqty.Caption) / Val(LblPack.Text)) Then
                        MsgBox "AVAILABLE STOCK IN THE PARTICULAR STOCK IS  " & (Val(lblactqty.Caption) / Val(LblPack.Text)) & " ", , "SALES"
                        TXTQTY.SelStart = 0
                        TXTQTY.SelLength = Len(TXTQTY.Text)
                        Exit Sub
                    End If
                    If ZERO_WARN_FLAG = True And M_EDIT = False And Val(TXTQTY.Text) > i Then
...
'            End If
            ...
            If Val(TXTSLNO.Text) < grdsales.rows Then
                ...
                txtretail.Text = Round((Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 7)) + Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 6)) * Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 40)) / 100) + Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 41)), 2) 'grdsales.TextMatrix(Val(TXTSLNO.Text), 7)
...
'                TxtCessAmt.Text = IIf(IsNull(grdtmp.Columns(23)), "", grdtmp.Columns(23))
            ...
            If PHY.RecordCount = 0 Then
                ...
                If PHYFLAG = True Then
                    ...
                    PHY.Open "Select ITEM_CODE, ITEM_NAME, BAL_QTY, SALES_TAX, LINE_DISC, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE, BARCODE, REF_NO, VCH_NO, LINE_NO, TRX_TYPE, ITEM_COST, SALES_PRICE, P_WS, P_VAN, CRTN_PACK, P_CRTN, MRP, TAX_MODE, EXP_DATE, TRX_YEAR, P_LWS, PRICE5, PRICE6, PRICE7  From RTRXFILE  WHERE BARCODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY >0", db, adOpenStatic, adLockReadOnly
                    PHYFLAG = False
                Else
                    PHY.Close
                    PHY.Open "Select ITEM_CODE, ITEM_NAME, BAL_QTY, SALES_TAX, LINE_DISC, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE, BARCODE, REF_NO, VCH_NO, LINE_NO, TRX_TYPE, ITEM_COST, SALES_PRICE, P_WS, P_VAN, CRTN_PACK, P_CRTN, MRP, TAX_MODE, EXP_DATE, TRX_YEAR, P_LWS, PRICE5, PRICE6, PRICE7  From RTRXFILE  WHERE BARCODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY >0", db, adOpenStatic, adLockReadOnly
                ...
                If PHY.RecordCount = 0 Then
                    '------
                    If PHYFLAG = True Then
                        PHY.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE, MRP, CUST_DISC, CESS_PER, CESS_AMT, P_LWS, UN_BILL, PRICE5, PRICE6, PRICE7 From ITEMMAST  WHERE BARCODE = '" & Me.TXTITEMCODE.Text & "' ", db, adOpenStatic, adLockReadOnly
                        PHYFLAG = False
                    Else
                        PHY.Close
                        PHY.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE, MRP, CUST_DISC, CESS_PER, CESS_AMT, P_LWS, UN_BILL, PRICE5, PRICE6, PRICE7 From ITEMMAST  WHERE BARCODE = '" & Me.TXTITEMCODE.Text & "' ", db, adOpenStatic, adLockReadOnly
                    ...
                    If PHY.RecordCount = 0 And Left(Trim(TXTITEMCODE.Text), 1) <> "#" Then
                        ...
                        Else
                            ...
                            frmitemmastermini.SetFocus
                            Call frmitemmastermini.TxtItemcode_KeyDown(13, 0)
                            frmitemmastermini.TxtBarcode = Trim(TXTITEMCODE.Text)
                            Exit Sub
                        End If
                    Else
                        Set grdtmp.DataSource = PHY
...
SKIP_BATCH3:
                ...
                Select Case cmbtype.ListIndex
                    ...
                    Case 3
                        ...
                        End If
                    Case 4
                        txtretail.Text = IIf(IsNull(grdtmp.Columns(31)), "", Val(grdtmp.Columns(31)))
                        'TXTRETAILNOTAX.Text = IIf(IsNull(grdtmp.Columns(5)), "", Val(grdtmp.Columns(5)))
                    Case 5
                ...
                lblprice5.Caption = IIf(IsNull(grdtmp.Columns(31)), "", grdtmp.Columns(31))
                lblprice6.Caption = IIf(IsNull(grdtmp.Columns(32)), "", grdtmp.Columns(32))
                lblprice7.Caption = IIf(IsNull(grdtmp.Columns(33)), "", grdtmp.Columns(33))
...
Function FILL_BATCHGRID()
    ...
    GRDPOPUP.Columns(25).Width = 1100
...
'    Dim RSTtax As ADODB.Recordset
'    Set RSTTRXFILE = New ADODB.Recordset
'    RSTTRXFILE.Open "SELECT DISTINCT SALES_TAX From TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE='WO' AND VCH_NO = " & Val(txtBillNo.Text) & " AND SALES_TAX > 0 order by SALES_TAX", db, adOpenStatic, adLockReadOnly
'    Do Until RSTTRXFILE.EOF
'        TaxAmt = 0
'        TaxableAmt = 0
'        Set RSTtax = New ADODB.Recordset
'        RSTtax.Open "Select * From TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND SALES_TAX = " & RSTTRXFILE!SALES_TAX & " AND TRX_TYPE='WO' AND VCH_NO = " & Val(txtBillNo.Text) & "", db, adOpenStatic, adLockReadOnly, adCmdText
'        Do Until RSTtax.EOF
    ...
    Select Case KeyCode
        Case 113, vbKeyReturn
            ...
            Select Case grdsales.Col
                ...
                Case 3
                    If grdsales.TextMatrix(grdsales.Row, 19) = "DN" Then Exit Sub
...
'        End If
        ...
        If lblcredit.Caption = "0" Then
            ...
            If GRDRECEIPT.rows > 1 And Trim(GRDRECEIPT.TextMatrix(2, 0)) = "B" Then
                ...
                BANKTRX!REF_NO = "OTHER SALES - " & txtBillNo.Text 'DataList2.Text
            ...
            If Trim(CMBBRNCH.Text) <> "" Then
                RSTTRXFILE!BR_ADDRESS = Trim(CMBBRNCH.Text)
            Else
                RSTTRXFILE!BR_ADDRESS = Trim(TXTAREA.Text)
            End If
            If Trim(DataList2.Text) <> Trim(TxtBillName.Text) Then
                RSTTRXFILE!BR_ADDRESS = IIf(RSTTRXFILE!BR_ADDRESS = "", Trim(TxtBillName.Text), RSTTRXFILE!BR_ADDRESS & " " & Trim(TxtBillName.Text))
            End If
            RSTTRXFILE.Update
            RSTTRXFILE.Close
            Set RSTTRXFILE = Nothing
    ...
    '    db.Execute "delete From DBTPYMT WHERE TRX_TYPE='DR' AND INV_NO = " & Val(txtBillNo.Text) & " AND INV_TRX_TYPE = 'WO'"
        ...
        If lblCBFLAG.Caption = "B" And BANKCODE <> "" Then
            ...
            Else
                ...
                db.Execute "UPDATE CASHATRXFILE SET AMOUNT = " & Val(lblbankamt.Tag) & " WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND INV_NO = " & Val(txtBillNo.Text) & " AND INV_TYPE = 'RT' AND INV_TRX_TYPE = 'WO'"
            End If
...
'    Set RSTITEMMAST = Nothing
    ...
    RSTBANK.Open "select * from BANKCODE WHERE BANK_CODE = '" & BANKCODE & "' ", db, adOpenStatic, adLockReadOnly, adCmdText
    If Not (RSTBANK.EOF And RSTBANK.BOF) Then
        RSTTRXFILE!BANK_NAME = IIf(IsNull(RSTBANK!BANK_NAME), "", RSTBANK!BANK_NAME)
    End If
    RSTBANK.Close
    Set RSTBANK = Nothing
    RSTTRXFILE.Update
...
'        Set rstBILL = Nothing
'
'        db.Execute "Update TRXMAST set VCH_NO = " & LASTWOBILL & ", TRX_TYPE = 'WO' WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE = 'WO' AND VCH_NO = " & Val(txtBillNo.Text) & ""
'        db.Execute "Update TRXSUB set VCH_NO = " & LASTWOBILL & ", TRX_TYPE = 'WO' WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE = 'WO' AND VCH_NO = " & Val(txtBillNo.Text) & ""
'        db.Execute "Update TRXFILE set VCH_NO = " & LASTWOBILL & ", TRX_TYPE = 'WO' WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE = 'WO' AND VCH_NO = " & Val(txtBillNo.Text) & ""
'
'        db.Execute "Update DBTPYMT set INV_NO = " & LASTWOBILL & ", INV_TRX_TYPE = 'WO' WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE = 'DR' AND INV_NO = " & Val(txtBillNo.Text) & " AND INV_TRX_TYPE = 'WO'"
'        db.Execute "Update BANK_TRX set B_VCH_NO = " & LASTWOBILL & ", B_TRX_TYPE = 'WO' WHERE B_TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND B_TRX_TYPE = 'WO' AND B_VCH_NO = " & Val(txtBillNo.Text) & ""
'        db.Execute "Update DBTPYMT set INV_NO = " & LASTWOBILL & ", INV_TRX_TYPE = 'WO' WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE = 'RT' AND INV_NO = " & Val(txtBillNo.Text) & " AND INV_TRX_TYPE = 'WO'"
'        db.Execute "Update CASHATRXFILE set INV_NO = " & LASTWOBILL & ", INV_TRX_TYPE = 'WO' WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND INV_TYPE = 'RT' AND INV_NO = " & Val(txtBillNo.Text) & " AND INV_TRX_TYPE = 'WO'"
'        db.Execute "Update RTRXFILE set TRX_TYPE= 'WO' WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE= 'WO' AND VCH_NO = " & Val(TxtCN.Text) & ""
...
'    Rcvd_Party = "Issued to     " & Mid(Trim(DataList2.Text), 1, 30)
'    db.Execute "Update TRXFILE set VCH_DESC =  '" & Trim(Rcvd_Party) & "' WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE='WO' AND VCH_NO = " & Val(txtBillNo.Text) & " "
...
'    OLD_BILL = True
    ...
    For i = 1 To grdsales.rows - 1
        ...
        RSTTRXFILE!CESS_PER = 0 ' Val(grdsales.TextMatrix(i, 40))
        RSTTRXFILE!cess_amt = 0 'Val(grdsales.TextMatrix(i, 41))
        RSTTRXFILE!FREE_QTY = Val(grdsales.TextMatrix(i, 20))
        RSTTRXFILE!P_RETAIL = Round((Val(grdsales.TextMatrix(i, 7)) + Val(grdsales.TextMatrix(i, 6)) * Val(grdsales.TextMatrix(i, 40)) / 100) + Val(grdsales.TextMatrix(i, 41)), 2)
        RSTTRXFILE!P_RETAILWOTAX = Round((Val(grdsales.TextMatrix(i, 7)) + Val(grdsales.TextMatrix(i, 6)) * Val(grdsales.TextMatrix(i, 40)) / 100) + Val(grdsales.TextMatrix(i, 41)), 2)
...
'            If i <> 0 And Val(TXTFREE.Text) <> 0 Then
                If ZERO_WARN_FLAG = True And M_EDIT = False And SERIAL_FLAG = True And (Val(TXTFREE.Text) + Val(TXTQTY.Text)) > (Val(lblactqty.Caption) / Val(LblPack.Text)) Then
                    MsgBox "AVAILABLE STOCK IN THE PARTICULAR STOCK IS  " & (Val(lblactqty.Caption) / Val(LblPack.Text)) & " ", , "SALES"
                    TXTQTY.SelStart = 0
                    TXTQTY.SelLength = Len(TXTQTY.Text)
                    Exit Sub
                End If
                If ZERO_WARN_FLAG = True And M_EDIT = False And (Val(TXTFREE.Text) + Val(TXTQTY.Text)) > i Then
                    If SERIAL_FLAG = True Then
                        MsgBox "AVAILABLE STOCK IS  " & i & " ", , "SALES"
                        TXTFREE.SelStart = 0
                        TXTFREE.SelLength = Len(TXTFREE.Text)
...
Function FILL_BATCHGRID2()
    ...
    GRDPOPUP.Columns(29).Width = 0
    GRDPOPUP.Columns(30).Width = 0
    GRDPOPUP.Columns(31).Width = 2800
    GRDPOPUP.Columns(32).Width = 1000
    GRDPOPUP.Columns(33).Width = 1000
    GRDPOPUP.Columns(34).Width = 1000
...
'        End If
        ...
        For i = 1 To grdsales.rows - 1
            Print #1, AlignLeft(Val(i), 4) & _
                Space(0) & AlignLeft(Mid(grdsales.TextMatrix(i, 2), 1, 22), 22) & _
    ...
    If Not (RSTCOMPANY.EOF And RSTCOMPANY.BOF) Then
        ...
        For i = 1 To grdsales.rows - 1
            ...
            Print #1, AlignLeft(Val(i), 3) & _
                ...
                AlignRight(Round(grdsales.TextMatrix(i, 3), 3) & Mid(grdsales.TextMatrix(i, 30), 1, 2), 7) & _
...
Public Function Make_Invoice(BillType As String)
    ...
    db.Execute "INSERT INTO TRXMAST (TRX_TYPE, TRX_YEAR, VCH_NO, VCH_AMOUNT, NET_AMOUNT, VCH_DATE, ACT_CODE, ACT_NAME, DISCOUNT, C_USER_ID, CREATE_DATE, C_TIME, C_USER_NAME, ADD_AMOUNT, ROUNDED_OFF, PAY_AMOUNT, REF_NO, SLSM_CODE, CHECK_FLAG, POST_FLAG, CFORM_NO, REMARKS, DISC_PERS, AST_PERS, AST_AMNT, BANK_CHARGE, VEHICLE, PHONE, TIN, UID_NO, FRIEGHT, MODIFY_DATE, cr_days, AGENT_CODE, AGENT_NAME, COMM_AMT, BILL_TYPE, CN_REF, BILL_NAME, BILL_ADDRESS )" & _
                            ...
                            " '" & disctype & "', 'I', '" & crdtype & "', '" & Format(Time, "HH:MM:SS") & "', '" & DataList2.Text & "', 0, 0, 0, 0, '', '" & Trim(TxtPhone.Text) & "', '', '', " & Val(TxtFrieght.Text) & ", CURDATE(), 0,'','',0,'R',Null, '" & Trim(TxtBillName.Text) & "', '" & Trim(TxtBillAddress.Text) & "')"
...
'    RSTTRXFILE.Open "Select * FROM TRXFILE", db, adOpenStatic, adLockOptimistic, adCmdText
    ...
    For i = 1 To grdsales.rows - 1
        ...
        db.Execute "INSERT INTO TRXFILE (TRX_TYPE, TRX_YEAR, VCH_NO, VCH_DATE, LINE_NO, CATEGORY, ITEM_CODE, ITEM_NAME, QTY, ITEM_COST, MRP, SALES_PRICE, P_RETAIL, PTR, P_RETAILWOTAX, COM_AMT, COM_FLAG, LOOSE_FLAG, LOOSE_PACK, SALES_TAX, UNIT, VCH_DESC, REF_NO, ISSUE_QTY, CHECK_FLAG, MFGR, CST, BAL_QTY, TRX_TOTAL, LINE_DISC, SCHEME, FREE_QTY, MODIFY_DATE, CREATE_DATE, C_USER_ID, M_USER_ID, SALE_1_FLAG, PACK_TYPE, AREA, KFC_TAX )" & _
                            ...
                            " " & Bill_SCHEME & ", " & Val(grdsales.TextMatrix(i, 20)) & ", CURDATE(), CURDATE(), 'SM', '" & DataList2.BoundText & "', '" & Trim(grdsales.TextMatrix(i, 23)) & "', '" & Trim(grdsales.TextMatrix(i, 30)) & "', '" & Trim(TXTAREA.Text) & "', 0)"
'        RSTTRXFILE.AddNew
'        RSTTRXFILE!TRX_TYPE = BillType
'        RSTTRXFILE!TRX_YEAR = Year(MDIMAIN.DTFROM.value)
'        RSTTRXFILE!VCH_NO = BILL_NUM
...
Private Function ReportGeneratION_NONGST()
    Dim RSTCOMPANY As ADODB.Recordset
    Dim RSTTRXFILE As ADODB.Recordset
    Dim Num As Currency
    Dim SN As Integer
...
CLOSEFILE:
    ...
    If Not (RSTCOMPANY.EOF And RSTCOMPANY.BOF) Then
        ...
        Print #1, Chr(27) & Chr(72) & Chr(10) & Space(19) & AlignLeft("CASH BILL", 20)
        'Print #1, Space(12) & "(To be prepared in Triplicate)"
        'Print #1, Space(9) & "(Not Eligible for Input Tax Credit)"
...
'            Set RSTHSNCODE = Nothing
        ...
        Print #1, AlignRight("-------------", 52)
        If Val(LBLDISCAMT.Caption) <> 0 Then
            Print #1, AlignRight("BILL AMOUNT ", 39) & AlignRight((Format(LBLTOTAL.Caption, "####.00")), 12)
            Print #1, AlignRight("DISC AMOUNT ", 39) & AlignRight((Format(LBLDISCAMT.Caption, "####.00")), 12)
        ElseIf Val(LBLDISCAMT.Caption) = 0 Then
            Print #1, AlignRight("BILL AMOUNT ", 39) & AlignRight((Format(LBLTOTAL.Caption, "####.00")), 12)
        End If
        If Val(Txthandle.Text) <> 0 Then
            Print #1, AlignRight("HANDLING CHARGE ", 39) & AlignRight((Format(Txthandle.Text, "####.00")), 12)
        End If
        If Val(TxtFrieght.Text) <> 0 Then
            Print #1, AlignRight("FRIEGHT CHARGE  ", 39) & AlignRight((Format(Txthandle.Text, "####.00")), 12)
        End If
        
        'Print #1, Chr(27) & Chr(71) & Space(10) & AlignRight("Amount ", 53) & AlignRight(Format(LBLTOTAL.Caption, "####.00"), 10)
        Print #1, AlignRight("Round off ", 39) & AlignRight(Format(Round(LBLTOTAL.Caption, 0) - Val(LBLTOTAL.Caption), "0.00"), 12)
        Print #1, Chr(13)
        Print #1, AlignRight("NET AMOUNT ", 39) & AlignRight((Format(Round(lblnetamount.Caption, 0), "####.00")), 12)
        'Print #1, Chr(27) & Chr(71) & Chr(14) & Chr(15) & Space(18) & AlignRight("NET AMOUNT: ", 11) & AlignRight((Format(Val(lbltotalwodiscount.Caption) - Val(LBLRETAMT.Caption), "####.00")), 9)
        Num = CCur(Round(lblnetamount.Caption, 0))
        Print #1, AlignLeft("(Rupees " & Words_1_all(Num) & " only)", 52)
        Print #1, RepeatString("-", 52)
        Print #1, "****Thank You... Visit Again****"
        Print #1, "E.&.O.E" '          SUBJECT TO ALAPPUZHA JURISDICTION"
        'Print #1, Chr(13)
            ...
            If Not (UCase(grdsales.TextMatrix(grdsales.Row, 25)) = "SERVICES" Or UCase(grdsales.TextMatrix(grdsales.Row, 25)) = "SELF") Then
                ...
                With RSTTRXFILE
                    If Not (.EOF And .BOF) Then
                        ...
                        !ISSUE_QTY = !ISSUE_QTY - Round(Val(grdsales.TextMatrix(grdsales.Row, 3)) * Val(grdsales.TextMatrix(grdsales.Row, 27)), 3)
                        !FREE_QTY = !FREE_QTY - Round(Val(grdsales.TextMatrix(grdsales.Row, 20)) * Val(grdsales.TextMatrix(grdsales.Row, 27)), 3)
                        !CLOSE_QTY = !CLOSE_QTY + Round((Val(grdsales.TextMatrix(grdsales.Row, 3)) + Val(grdsales.TextMatrix(grdsales.Row, 20))) * Val(grdsales.TextMatrix(grdsales.Row, 27)), 3)
                ...
                Set RSTTRXFILE = New ADODB.Recordset
                RSTTRXFILE.Open "SELECT *  FROM RTRXFILE WHERE ITEM_CODE = '" & grdsales.TextMatrix(grdsales.Row, 13) & "' AND TRX_TYPE = '" & Trim(grdsales.TextMatrix(grdsales.Row, 16)) & "' AND VCH_NO = " & Val(grdsales.TextMatrix(grdsales.Row, 14)) & " AND LINE_NO = " & Val(grdsales.TextMatrix(grdsales.Row, 15)) & " AND TRX_YEAR = '" & Val(grdsales.TextMatrix(grdsales.Row, 43)) & "' ", db, adOpenStatic, adLockOptimistic, adCmdText
                With RSTTRXFILE
                    If Not (.EOF And .BOF) Then
                        If (IsNull(!ISSUE_QTY)) Then !ISSUE_QTY = 0
                        If (IsNull(!BAL_QTY)) Then !BAL_QTY = 0
...
'''                        If MDIMAIN.StatusBar.Panels(14).text = "Y" Then
'''                            If Not (IsNull(RSTTRXFILE!P_RETAIL) Or RSTTRXFILE!P_RETAIL = 0) Then
'''                                TXTRETAILNOTAX.Tag = (RSTTRXFILE!P_RETAIL - Val(grdsales.TextMatrix(grdsales.Row, 41))) / (1 + ((Val(grdsales.TextMatrix(grdsales.Row, 9))) / 100) + (Val(grdsales.TextMatrix(grdsales.Row, 40)) / 100))
'''                                grdsales.TextMatrix(grdsales.Row, 7) = Round(Val(TXTRETAILNOTAX.Tag) + (Val(TXTRETAILNOTAX.Tag) * Val(grdsales.TextMatrix(grdsales.Row, 9)) / 100), 4)
'''                                grdsales.TextMatrix(grdsales.Row, 7) = Round(Val(txtretail.text) * 100 / ((Val(TXTTAX.text)) + 100), 4)
'''                                grdsales.TextMatrix(grdsales.Row, 6) = Format(Val(TXTRETAILNOTAX.Tag), "0.0000")
'''                                grdsales.TextMatrix(grdsales.Row, 22) = Val(grdsales.TextMatrix(grdsales.Row, 7))
...
'''                        Else
'''                            If Not (IsNull(RSTTRXFILE!P_RETAIL) Or RSTTRXFILE!P_RETAIL = 0) Then
'''                                grdsales.TextMatrix(grdsales.Row, 7) = IIf(IsNull(RSTTRXFILE!P_RETAIL), 0, RSTTRXFILE!P_RETAIL)
'''                                grdsales.TextMatrix(grdsales.Row, 21) = IIf(IsNull(RSTTRXFILE!P_RETAIL), 0, RSTTRXFILE!P_RETAIL)
'''                            End If
'''                            grdsales.TextMatrix(grdsales.Row, 6) = Format(Round(Val(grdsales.TextMatrix(grdsales.Row, 7)) * 100 / ((Val(grdsales.TextMatrix(grdsales.Row, 9))) + 100), 4), "0.0000")
'''                            grdsales.TextMatrix(grdsales.Row, 22) = Format(Round(Val(grdsales.TextMatrix(grdsales.Row, 7)) * 100 / ((Val(grdsales.TextMatrix(grdsales.Row, 9))) + 100), 4), "0.0000")
'''                        End If
...
'''                        If MDIMAIN.StatusBar.Panels(14).text <> "Y" Then
'''                            If Not (IsNull(RSTTRXFILE!P_RETAIL) Or RSTTRXFILE!P_RETAIL = 0) Then
'''                                grdsales.TextMatrix(grdsales.Row, 6) = IIf(IsNull(RSTTRXFILE!P_RETAIL), 0, RSTTRXFILE!P_RETAIL)
'''                                grdsales.TextMatrix(grdsales.Row, 22) = IIf(IsNull(RSTTRXFILE!P_RETAIL), 0, RSTTRXFILE!P_RETAIL)
'''                            End If
'''                            grdsales.TextMatrix(grdsales.Row, 7) = Format(Round(Val(grdsales.TextMatrix(grdsales.Row, 6)) + Val(grdsales.TextMatrix(grdsales.Row, 6)) * (Val(grdsales.TextMatrix(grdsales.Row, 9))) / 100, 4), "0.0000")
'''                            grdsales.TextMatrix(grdsales.Row, 21) = Format(Round(Val(grdsales.TextMatrix(grdsales.Row, 6)) + Val(grdsales.TextMatrix(grdsales.Row, 6)) * (Val(grdsales.TextMatrix(grdsales.Row, 9))) / 100, 4), "0.0000")
'''                        Else
...
'''                            grdsales.TextMatrix(grdsales.Row, 22) = Format(Round(Val(grdsales.TextMatrix(grdsales.Row, 7)) * 100 / ((Val(grdsales.TextMatrix(grdsales.Row, 9))) + 100), 4), "0.0000")
'''                        End If
'''                        grdsales.TextMatrix(grdsales.Row, 39) = IIf(IsNull(RSTTRXFILE!P_RETAIL), 0, RSTTRXFILE!P_RETAIL)
'''                    End If
                    ...
                    TXTRETAILNOTAX.Tag = (TXTRETAILNOTAX.Tag + (TXTRETAILNOTAX.Tag * (Val(grdsales.TextMatrix(grdsales.Row, 9))) / 100)) + (TXTRETAILNOTAX.Tag * Val(grdsales.TextMatrix(grdsales.Row, 40)) / 100) + Val(grdsales.TextMatrix(grdsales.Row, 41))
                    If UCase(grdsales.TextMatrix(grdsales.Row, 25)) = "SERVICE CHARGE" Then
                        'TXTDISC.Tag = Val(grdsales.TextMatrix(grdsales.Row, 7)) * Val(grdsales.TextMatrix(grdsales.Row, 8)) / 100
                    ...
                    For i = 1 To grdsales.rows - 1
                        ...
                        Select Case grdsales.TextMatrix(i, 19)
                            ...
                            Case Else
                                ...
                                LBLTOTAL.Caption = Round(Val(LBLTOTAL.Caption) + Val(grdsales.TextMatrix(i, 12)), 2)
                                If Val(grdsales.TextMatrix(i, 20)) > 0 Then LBLFOT.Caption = Round(Val(LBLFOT.Caption) + (Val(grdsales.TextMatrix(i, 7)) - Val(grdsales.TextMatrix(i, 6))) * Val(grdsales.TextMatrix(i, 20)), 2)
                                'LBLMRPVAL.Caption = Round(Val(LBLMRPVAL.Caption) + Val(grdsales.TextMatrix(i, 5)) * Val(grdsales.TextMatrix(i, 3)), 2)
                                LBLFOT.Caption = ""
                        End Select
                        If Val(grdsales.TextMatrix(i, 3)) = 0 Then
                            lblcomamt.Text = Round(Val(lblcomamt.Text) + Val(grdsales.TextMatrix(i, 24)), 2)
                    ...
                    'db.BeginTrans
                    TXTDISC.Tag = (Val(grdsales.TextMatrix(grdsales.Row, 7)) - Val(grdsales.TextMatrix(grdsales.Row, 6))) * Val(grdsales.TextMatrix(grdsales.Row, 3))
                    db.Execute "Update TRXFILE set KFC_TAX = 0, SALES_PRICE = " & Val(grdsales.TextMatrix(grdsales.Row, 7)) & ", P_RETAIL = " & Val(grdsales.TextMatrix(grdsales.Row, 7)) & ", PTR = " & Val(grdsales.TextMatrix(grdsales.Row, 6)) & ", P_RETAILWOTAX = " & Val(grdsales.TextMatrix(grdsales.Row, 6)) & ", TRX_TOTAL = " & Val(grdsales.TextMatrix(grdsales.Row, 12)) & ", SCHEME = " & Val(TXTDISC.Tag) & " WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='WO' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(grdsales.Row, 32)) & ""
                    If CMBDISTI.BoundText <> "" Or (lblsubdealer.Caption = "D" And Val(lblActAmt.Caption) <> 0) Then
                        db.Execute "Update TRXMAST set COMM_AMT = " & Val(lblcomamt.Text) & ", PAY_AMOUNT = " & Val(LBLTOTALCOST.Caption) & ", ADD_AMOUNT = " & Val(LBLRETAMT.Caption) & ", DISCOUNT = " & Val(TXTTOTALDISC.Text) & ", DISC_PERS  = " & Val(TXTTOTALDISC.Text) & ", VCH_AMOUNT = " & Val(LBLTOTAL.Caption) & ", NET_AMOUNT = " & Val(lblnetamount.Caption) & " WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='WO' AND VCH_NO = " & Val(txtBillNo.Text) & ""
                    Else
                        db.Execute "Update TRXMAST set COMM_AMT = 0, PAY_AMOUNT = " & Val(LBLTOTALCOST.Caption) & ", ADD_AMOUNT = " & Val(LBLRETAMT.Caption) & ", DISCOUNT = " & Val(TXTTOTALDISC.Text) & ", DISC_PERS  = " & Val(TXTTOTALDISC.Text) & ", VCH_AMOUNT = " & Val(LBLTOTAL.Caption) & ", NET_AMOUNT = " & Val(lblnetamount.Caption) & " WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='WO' AND VCH_NO = " & Val(txtBillNo.Text) & ""
...
'
            ...
            If Not (UCase(grdsales.TextMatrix(grdsales.Row, 25)) = "SERVICES" Or UCase(grdsales.TextMatrix(grdsales.Row, 25)) = "SELF") Then
                db.Execute "START TRANSACTION"
                Set RSTTRXFILE = New ADODB.Recordset
                RSTTRXFILE.Open "SELECT *  FROM RTRXFILE WHERE ITEM_CODE = '" & grdsales.TextMatrix(grdsales.Row, 13) & "' AND RTRXFILE.TRX_TYPE = '" & Trim(grdsales.TextMatrix(grdsales.Row, 16)) & "' AND RTRXFILE.VCH_NO = " & Val(grdsales.TextMatrix(grdsales.Row, 14)) & " AND RTRXFILE.LINE_NO = " & Val(grdsales.TextMatrix(grdsales.Row, 15)) & " AND RTRXFILE.TRX_YEAR = '" & Val(grdsales.TextMatrix(grdsales.Row, 43)) & "' AND BAL_QTY > 0", db, adOpenStatic, adLockOptimistic, adCmdText
                With RSTTRXFILE
                    If Not (.EOF And .BOF) Then
                        If (IsNull(!ISSUE_QTY)) Then !ISSUE_QTY = 0
                        If (IsNull(!BAL_QTY)) Then !BAL_QTY = 0
                        ...
                        'grdsales.TextMatrix(grdsales.Row, 9) = IIf(IsNull(RSTTRXFILE!SALES_TAX), 0, RSTTRXFILE!SALES_TAX)
                        grdsales.TextMatrix(grdsales.Row, 45) = IIf(IsNull(RSTTRXFILE!SALES_TAX), 0, RSTTRXFILE!SALES_TAX)
                        If Val(grdsales.TextMatrix(grdsales.Row, 45)) = 0 Then grdsales.TextMatrix(grdsales.Row, 45) = grdsales.TextMatrix(grdsales.Row, 9)
                    ...
                    Else
                        ...
                        Set RSTTRXFILE = Nothing
                        Set RSTTRXFILE = New ADODB.Recordset
                        RSTTRXFILE.Open "SELECT *  FROM RTRXFILE WHERE ITEM_CODE = '" & grdsales.TextMatrix(grdsales.Row, 13) & "' AND BAL_QTY > 0 ORDER BY TRX_TYPE DESC, BAL_QTY DESC", db, adOpenStatic, adLockOptimistic, adCmdText
                        If Not (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
                            If (IsNull(RSTTRXFILE!ISSUE_QTY)) Then RSTTRXFILE!ISSUE_QTY = 0
                            If (IsNull(RSTTRXFILE!BAL_QTY)) Then RSTTRXFILE!BAL_QTY = 0
                            'BALQTY = RSTTRXFILE!BAL_QTY
                            RSTTRXFILE!ISSUE_QTY = RSTTRXFILE!ISSUE_QTY + Round((Val(grdsales.TextMatrix(grdsales.Row, 3)) + Val(grdsales.TextMatrix(grdsales.Row, 20))) * Val(grdsales.TextMatrix(grdsales.Row, 27)), 3)
                            RSTTRXFILE!BAL_QTY = RSTTRXFILE!BAL_QTY - Round((Val(grdsales.TextMatrix(grdsales.Row, 3)) + Val(grdsales.TextMatrix(grdsales.Row, 20))) * Val(grdsales.TextMatrix(grdsales.Row, 27)), 3)
                        ...
                        Else
                            ...
                            Set RSTTRXFILE = Nothing
                            Set RSTTRXFILE = New ADODB.Recordset
                            RSTTRXFILE.Open "SELECT *  FROM RTRXFILE WHERE ITEM_CODE = '" & grdsales.TextMatrix(grdsales.Row, 13) & "' ORDER BY BAL_QTY DESC, VCH_DATE DESC", db, adOpenStatic, adLockReadOnly
                            If Not (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
                                grdsales.TextMatrix(grdsales.Row, 11) = IIf(IsNull(RSTTRXFILE!item_COST), "", RSTTRXFILE!item_COST * Val(grdsales.TextMatrix(grdsales.Row, 27)))
                                grdsales.TextMatrix(grdsales.Row, 14) = RSTTRXFILE!VCH_NO
                                grdsales.TextMatrix(grdsales.Row, 15) = RSTTRXFILE!LINE_NO
            ...
            db.Execute "delete FROM TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='WO' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(grdsales.Row, 32)) & ""
    
            db.Execute "START TRANSACTION"
            Set RSTTRXFILE = New ADODB.Recordset
            RSTTRXFILE.Open "Select * FROM TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='WO' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(grdsales.Row, 32)) & "", db, adOpenStatic, adLockOptimistic, adCmdText
            If (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
                RSTTRXFILE.AddNew
                RSTTRXFILE!TRX_TYPE = "WO"
                RSTTRXFILE!TRX_YEAR = Year(MDIMAIN.DTFROM.Value)
            ...
            RSTTRXFILE!GROSS_AMOUNT = Val(grdsales.TextMatrix(grdsales.Row, 34))
            RSTTRXFILE!DN_NO = Val(grdsales.TextMatrix(grdsales.Row, 35))
            RSTTRXFILE!DN_YEAR = Val(grdsales.TextMatrix(grdsales.Row, 47))
            If IsDate(grdsales.TextMatrix(grdsales.Row, 36)) Then
                RSTTRXFILE!DN_DATE = IIf(IsDate(grdsales.TextMatrix(grdsales.Row, 36)), Format(grdsales.TextMatrix(grdsales.Row, 36), "DD/MM/YYYY"), Null)
            End If
            RSTTRXFILE!DN_LINENO = Val(grdsales.TextMatrix(grdsales.Row, 37))
        ...
        '    Set RSTUNBILL = Nothing
            ...
            db.Execute "START TRANSACTION"
            Set RSTTRXFILE = New ADODB.Recordset
            RSTTRXFILE.Open "Select * FROM TRXSUB WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='WO' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(grdsales.Row, 32)) & "", db, adOpenStatic, adLockOptimistic, adCmdText
            If (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
                RSTTRXFILE.AddNew
                RSTTRXFILE!VCH_NO = Val(txtBillNo.Text)
                RSTTRXFILE!TRX_TYPE = "WO"
                ...
                RSTTRXFILE!LINE_NO = Val(grdsales.TextMatrix(grdsales.Row, 32))
            End If
            RSTTRXFILE!R_VCH_NO = IIf(grdsales.TextMatrix(grdsales.Row, 14) = "", 0, grdsales.TextMatrix(grdsales.Row, 14))
            RSTTRXFILE!R_LINE_NO = IIf(grdsales.TextMatrix(grdsales.Row, 15) = "", 0, grdsales.TextMatrix(grdsales.Row, 15))
            RSTTRXFILE!R_TRX_TYPE = IIf(grdsales.TextMatrix(grdsales.Row, 16) = "", "MI", grdsales.TextMatrix(grdsales.Row, 16))
            RSTTRXFILE!R_TRX_YEAR = IIf(grdsales.TextMatrix(grdsales.Row, 43) = "", "", grdsales.TextMatrix(grdsales.Row, 43))
            RSTTRXFILE!QTY = Val(grdsales.TextMatrix(grdsales.Row, 3))
            RSTTRXFILE.Update
            RSTTRXFILE.Close
            Set RSTTRXFILE = Nothing
