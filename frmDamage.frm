...
Private Sub Form_Load()
    ...
    rstBILL.Open "Select MAX(Val(VCH_NO)) From DAMAGED", db, adOpenStatic, adLockReadOnly
...
Dim chkflag As Integer
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            TRXSUB.Open "Select * From DAMAGED WHERE VCH_NO = " & Val(txtBillNo.Text) & " ORDER BY LINE_NO", db, adOpenStatic, adLockReadOnly
            Do Until TRXSUB.EOF
                    i = i + 1
                    grdsales.Rows = grdsales.Rows + 1
                    grdsales.FixedRows = 1
...
Private Sub txtBillNo_LostFocus()
    ...
    TRXDN.Open "Select MAX(Val(VCH_NO)) From DAMAGED", db, adOpenStatic, adLockReadOnly
...
Public Function AppendSale()
    ...
    i = 0
    On Error GoTo eRRHAND
    db.Execute "delete * From DAMAGED WHERE VCH_NO = " & Val(txtBillNo.Text) & ""
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * From DAMAGED", db, adOpenStatic, adLockOptimistic, adCmdText
    For i = 1 To grdsales.Rows - 1
        RSTTRXFILE.AddNew
    ...
    db.Execute "delete * From TRXFILE WHERE TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & ""
    Set RSTTRXFILE = New ADODB.Recordset
    ...
    txtBillNo.Text = 1
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select MAX(Val(VCH_NO)) From DAMAGED", db, adOpenStatic, adLockReadOnly
    If Not (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
