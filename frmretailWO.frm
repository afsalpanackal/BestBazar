   ...
   Begin VB.Frame FRMEMAIN 
      ...
      Begin VB.Frame FRMEHEAD 
         ...
         Begin VB.ComboBox cmbtype 
            ...
            ForeColor       =   &H00004080&
            Height          =   315
            ItemData        =   "frmretailWO.frx":030A
            Left            =   6585
            List            =   "frmretailWO.frx":0317
...
Private Sub DataList2_Click()
    ...
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "SELECT *  FROM TEMPCN WHERE ACT_CODE = '" & Trim(DataList2.BoundText) & "' AND CHECK_FLAG <> 'Y' AND TRX_TYPE = 'WO'", db, adOpenStatic, adLockReadOnly, adCmdText
...
Private Sub CMDDELIVERY_Click()
    ...
    'If grdsales.Rows <= Val(TXTSLNO.Text) Then grdsales.Rows = grdsales.Rows + 1
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "SELECT *  FROM TEMPCN WHERE ACT_CODE = '" & Trim(DataList2.BoundText) & "' AND CHECK_FLAG <> 'Y' AND TRX_TYPE = 'WO'", db, adOpenStatic, adLockOptimistic, adCmdText
    Do Until RSTTRXFILE.EOF
        grdsales.Rows = grdsales.Rows + 1
        grdsales.FixedRows = 1
...
Private Sub cmdPrint_Click()
    ...
    If DataList2.BoundText = "130000" Then
        FRMRETAILWO.lblcredit.Caption = "0"
        FRMRETAILWO.Generateprint
    Else
        Me.Enabled = False
        FRMDEBITWO.Show
    End If
...
Private Function AppendSale()
    ...
    If grdsales.Rows = 1 Then
        db.Execute "delete * FROM CASHATRXFILE WHERE INV_NO = " & Val(LBLBILLNO.Caption) & " AND INV_TYPE = 'RT' AND BILL_TRX_TYPE = 'WO'"
    Else
        Set RSTITEMMAST = New ADODB.Recordset
        RSTITEMMAST.Open "SELECT * FROM CASHATRXFILE WHERE INV_NO = " & Val(LBLBILLNO.Caption) & " AND INV_TYPE = 'RT' AND BILL_TRX_TYPE = 'WO'", db, adOpenStatic, adLockOptimistic, adCmdText
...
SKIP:
    ...
    If lblcredit.Caption <> "0" Then
        ...
        Set RSTITEMMAST = New ADODB.Recordset
        RSTITEMMAST.Open "SELECT * FROM DBTPYMT WHERE INV_NO = " & Val(LBLBILLNO.Caption) & " AND TRX_TYPE = 'DR' AND BILL_TRX_TYPE = 'WO'", db, adOpenStatic, adLockOptimistic, adCmdText
        If (RSTITEMMAST.EOF And RSTITEMMAST.BOF) Then
            RSTITEMMAST.AddNew
            RSTITEMMAST!TRX_TYPE = "DR"
            RSTITEMMAST!CR_NO = i
        ...
        Set RSTITEMMAST = Nothing
    Else
        db.Execute "delete * From DBTPYMT WHERE TRX_TYPE='DR' AND INV_NO = " & Val(LBLBILLNO.Caption) & " AND BILL_TRX_TYPE = 'WO'"
    End If
