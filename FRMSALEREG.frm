...
'            MRPVALUE = 0
            ...
            RSTITEMMAST.Open "SELECT AREA, KGST FROM CUSTMAST WHERE ACT_CODE = '" & rstTRANX!ACT_CODE & "'", db, adOpenStatic, adLockReadOnly
            If Not (RSTITEMMAST.EOF And RSTITEMMAST.BOF) Then
                RSTSALEREG!TIN_NO = IIf(IsNull(RSTITEMMAST!KGST), "", RSTITEMMAST!KGST)
                RSTSALEREG!Area = IIf(IsNull(RSTITEMMAST!Area), "", RSTITEMMAST!Area)
                GRDTranx.TextMatrix(M, 8) = RSTITEMMAST!Area
            Else
                RSTSALEREG!Area = ""
