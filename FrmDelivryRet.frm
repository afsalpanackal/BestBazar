...
Begin VB.Form FRMDELIVERY1 
   ...
   Begin VB.Frame FRMEMAIN 
      ...
      Begin VB.Frame FRMEHEAD 
         ...
         Begin VB.ComboBox cmbtype 
            ...
            ForeColor       =   &H00004080&
            Height          =   315
            ItemData        =   "FrmDelivryRet.frx":030A
            Left            =   6585
            List            =   "FrmDelivryRet.frx":0317
   ...
   End
End
Attribute VB_Name = "FRMDELIVERY1"
...
Private Sub Form_Load()
    ...
    rstBILL.Open "Select MAX(Val(VCH_NO)) From TEMPCN WHERE TRX_TYPE = 'SI'", db, adOpenStatic, adLockReadOnly
...
Private Sub txtBillNo_LostFocus()
    ...
    Set TRXMAST = New ADODB.Recordset
    TRXMAST.Open "Select MAX(Val(VCH_NO)) From TEMPCN WHERE TRX_TYPE = 'SI'", db, adOpenStatic, adLockReadOnly
    ...
    Set TRXMAST = New ADODB.Recordset
    TRXMAST.Open "Select MIN(Val(VCH_NO)) From TEMPCN WHERE TRX_TYPE = 'SI'", db, adOpenStatic, adLockReadOnly
    If Not (TRXMAST.EOF And TRXMAST.BOF) Then
...
Private Sub TXTDEALER_Change()
    ...
    If flagchange.Caption <> "1" Then
        If ACT_FLAG = True Then
            ACT_REC.Open "select ACT_CODE, ACT_NAME from [ACTMAST]  WHERE ACT_CODE <>'130000' AND (Mid(ACT_CODE, 1, 2)='13')And (len(ACT_CODE)>2) And ACT_NAME Like '" & Me.TXTDEALER.Text & "%'ORDER BY ACT_NAME", db, adOpenStatic, adLockReadOnly, adCmdText
            ACT_FLAG = False
        Else
            ACT_REC.Close
            ACT_REC.Open "select ACT_CODE, ACT_NAME from [ACTMAST]  WHERE ACT_CODE <>'130000' AND (Mid(ACT_CODE, 1, 2)='13')And (len(ACT_CODE)>2) And ACT_NAME Like '" & Me.TXTDEALER.Text & "%'ORDER BY ACT_NAME", db, adOpenStatic, adLockReadOnly, adCmdText
    ...
    Select Case KeyCode
        ...
        Case vbKeyReturn
            ...
            If PHYFLAG = True Then
                PHY.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN, CHECK_FLAG, CATEGORY  From ITEMMAST  WHERE ITEM_NAME Like '" & Me.TXTPRODUCT.Text & "%'ORDER BY [ITEM_NAME] ", db, adOpenStatic, adLockReadOnly
                PHYFLAG = False
            Else
                PHY.Close
                PHY.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN, CHECK_FLAG, CATEGORY  From ITEMMAST  WHERE ITEM_NAME Like '" & Me.TXTPRODUCT.Text & "%'ORDER BY [ITEM_NAME] ", db, adOpenStatic, adLockReadOnly
            ...
            If PHY.RecordCount = 0 Then Exit Sub
