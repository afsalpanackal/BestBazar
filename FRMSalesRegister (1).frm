        ...
        Do Until rstTRANX.EOF
            ...
            Do Until n = GRDTranx.Cols - 3
                ...
                Do Until RSTtax.EOF
                    ...
                    'GRDTranx.TextMatrix(M, N + 1) = Val(GRDTranx.TextMatrix(M, N + 1)) + (((RSTtax!ST_RATE * 100 / (RSTtax!SALES_TAX + 100)) - ((RSTtax!ST_RATE * 100 / (RSTtax!SALES_TAX + 100)) * RSTtax!LINE_DISC) / 100) * RSTtax!SALES_TAX / 100) * RSTtax!QTY
...
Private Sub Command1_Click()
    ...
    db.Execute "INSERT INTO `hsn_trxfile` SELECT * FROM `trxfile` WHERE VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND (TRX_TYPE='HI' or TRX_TYPE='GI' or TRX_TYPE='SV') AND (ISNULL(UN_BILL) OR UN_BILL <> 'Y')"
    ...
    Report.RecordSelectionFormula = "((ISNULL({TRXFILE.UN_BILL}) OR {TRXFILE.UN_BILL} <> 'Y') AND ({TRXFILE.TRX_TYPE}='GI' OR {TRXFILE.TRX_TYPE}='HI' OR {TRXFILE.TRX_TYPE}='SV' OR {TRXFILE.TRX_TYPE}='SR' OR {TRXFILE.TRX_TYPE}='EX' OR {TRXFILE.TRX_TYPE}='E1' OR {TRXFILE.TRX_TYPE}='E2' OR {TRXFILE.TRX_TYPE}='E3') AND {TRXFILE.VCH_DATE} <=# " & Format(DTTo.Value, "MM,DD,YYYY") & " # AND {TRXFILE.VCH_DATE} >=# " & Format(DTFROM.Value, "MM,DD,YYYY") & " # )"
...
Private Function Purchase_Register()
    ...
    If OptNormal.Value = True Then
        If Optinvdate.Value = True Then
            rstTRANX.Open "SELECT DISTINCT SALES_TAX From RTRXFILE WHERE VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND (TRX_TYPE='PI') AND (ISNULL(CATEGORY) OR CATEGORY <> 'SERVICE CHARGE')", db, adOpenStatic, adLockReadOnly
        Else
            rstTRANX.Open "SELECT DISTINCT SALES_TAX From RTRXFILE WHERE RCVD_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND RCVD_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND (TRX_TYPE='PI') AND (ISNULL(CATEGORY) OR CATEGORY <> 'SERVICE CHARGE')", db, adOpenStatic, adLockReadOnly
        End If
    ElseIf OptComm.Value = True Then
        If Optinvdate.Value = True Then
            rstTRANX.Open "SELECT DISTINCT SALES_TAX From RTRXFILE WHERE VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND (TRX_TYPE='PW') AND (ISNULL(CATEGORY) OR CATEGORY <> 'SERVICE CHARGE')", db, adOpenStatic, adLockReadOnly
        Else
            rstTRANX.Open "SELECT DISTINCT SALES_TAX From RTRXFILE WHERE RCVD_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND RCVD_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND (TRX_TYPE='PW') AND (ISNULL(CATEGORY) OR CATEGORY <> 'SERVICE CHARGE')", db, adOpenStatic, adLockReadOnly
        End If
    Else
        If Optinvdate.Value = True Then
            rstTRANX.Open "SELECT DISTINCT SALES_TAX From RTRXFILE WHERE VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND (TRX_TYPE='LP') AND (ISNULL(CATEGORY) OR CATEGORY <> 'SERVICE CHARGE')", db, adOpenStatic, adLockReadOnly
        Else
            rstTRANX.Open "SELECT DISTINCT SALES_TAX From RTRXFILE WHERE RCVD_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND RCVD_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND (TRX_TYPE='LP') AND (ISNULL(CATEGORY) OR CATEGORY <> 'SERVICE CHARGE')", db, adOpenStatic, adLockReadOnly
        End If
    End If
    GRDTranx.rows = 1
...
'    GrdTotal.ColWidth(GRDTranx.Cols - 3) = 1500
    ...
    Do Until rstTRANX.EOF
        ...
        Do Until n = GRDTranx.Cols - 4
            Set RSTtax = New ADODB.Recordset
            RSTtax.Open "Select * From RTRXFILE WHERE TRX_YEAR = '" & rstTRANX!TRX_YEAR & "' AND VCH_NO = " & rstTRANX!VCH_NO & " AND TRX_TYPE = '" & rstTRANX!TRX_TYPE & "' AND SALES_TAX = " & Val(GRDTranx.TextMatrix(0, n)) & " AND (ISNULL(CATEGORY) OR CATEGORY <> 'SERVICE CHARGE')", db, adOpenStatic, adLockReadOnly, adCmdText
            Do Until RSTtax.EOF
                'GRDTranx.TextMatrix(M, N) = Val(GRDTranx.TextMatrix(M, N)) + (RSTtax!TRX_TOTAL * 100) / ((RSTtax!SALES_TAX) + 100)
