...
Begin VB.Form FRMSALEREGWO 
   ...
   End
End
Attribute VB_Name = "FRMSALEREGWO"
...
Private Sub CMDDISPLAY_Click()
    ...
    Select Case OPTALL.Value
        Case True
            If OPTPERIOD.Value = True Then
                ...
                rstTRANX.Open "SELECT DISTINCT VCH_NO From SALERETURN WHERE [VCH_DATE] <=# " & TODATE & " # AND [VCH_DATE] >=# " & FROMDATE & " # ORDER BY TRX_TYPE,VCH_DATE,VCH_NO", db2, adOpenStatic, adLockReadOnly
            Else
                Set rstTRANX = New ADODB.Recordset
                rstTRANX.Open "SELECT DISTINCT VCH_NO From SALERETURN WHERE [ACT_CODE] = '" & DataList2.BoundText & "' ORDER BY TRX_TYPE,VCH_DATE,VCH_NO", db2, adOpenStatic, adLockReadOnly
            End If
        Case Else
            If OPTPERIOD.Value = True Then
                Set rstTRANX = New ADODB.Recordset
                rstTRANX.Open "SELECT DISTINCT VCH_NO From SALERETURN WHERE [CHECK_FLAG] <>'Y' AND [VCH_DATE] <=# " & TODATE & " # AND [VCH_DATE] >=# " & FROMDATE & " # ORDER BY TRX_TYPE,VCH_DATE,VCH_NO", db2, adOpenStatic, adLockReadOnly
            Else
                Set rstTRANX = New ADODB.Recordset
                rstTRANX.Open "SELECT DISTINCT VCH_NO From SALERETURN WHERE [ACT_CODE] = '" & DataList2.BoundText & "' AND [CHECK_FLAG] <>'Y' ORDER BY TRX_TYPE,VCH_DATE,VCH_NO", db2, adOpenStatic, adLockReadOnly
            End If
    End Select
    Do Until rstTRANX.EOF
        ...
        RSTTRXFILE.Open "SELECT * From SALERETURN WHERE VCH_NO = " & rstTRANX!VCH_NO & "", db2, adOpenStatic, adLockReadOnly
        Do Until RSTTRXFILE.EOF
            GRDTranx.TextMatrix(i, 4) = Format(Val(GRDTranx.TextMatrix(i, 4)) + RSTTRXFILE!TRX_TOTAL, "0.00")
            GRDTranx.TextMatrix(i, 5) = IIf(IsNull(RSTTRXFILE!VCH_DESC), "", Mid(RSTTRXFILE!VCH_DESC, 15))
            GRDTranx.TextMatrix(i, 6) = IIf((RSTTRXFILE!CHECK_FLAG = "Y"), "", "Pending")
            GRDTranx.TextMatrix(i, 7) = IIf(IsNull(RSTTRXFILE!BILL_NO), "", RSTTRXFILE!BILL_NO)
            GRDTranx.TextMatrix(i, 8) = IIf(IsNull(RSTTRXFILE!BILL_DATE), "", RSTTRXFILE!BILL_DATE)
...
Private Sub Form_Load()
    ...
    GRDTranx.TextMatrix(0, 5) = "RETURNED FROM"
...
Private Sub GRDTranx_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            i = 0
            Set RSTTRXFILE = New ADODB.Recordset
            RSTTRXFILE.Open "Select * From SALERETURN WHERE VCH_NO = " & Val(LBLBILLNO.Caption) & " AND TRX_TYPE = '" & Trim(GRDTranx.TextMatrix(GRDTranx.Row, 2)) & "'", db2, adOpenStatic, adLockReadOnly, adCmdText
            Do Until RSTTRXFILE.EOF
                i = i + 1
