...
Begin VB.Form FRMGIFT 
   BackColor       =   &H00FFC0FF&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "BUSINESS PROMOTION SAMPLES"
   ClientHeight    =   8580
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   11070
   ControlBox      =   0   'False
   Icon            =   "FRMGIFT.frx":0000
   KeyPreview      =   -1  'True
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
...
Private Sub CMDADD_Click()
    ...
    With RSTTRXFILE
        If Not (.EOF And .BOF) Then
            If (IsNull(!GIFT_QTY)) Then !GIFT_QTY = 0
            !GIFT_QTY = !GIFT_QTY + Val(grdsales.TextMatrix(Val(TXTSLNO.text), 3))
            If (IsNull(!GIFT_VAL)) Then !GIFT_VAL = 0
            !GIFT_VAL = !GIFT_VAL + Val(grdsales.TextMatrix(Val(TXTSLNO.text), 12))
...
Private Sub Form_Load()
    ...
    rstBILL.Open "Select MAX(VCH_NO) From GIFT WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE  'GF'", db, adOpenStatic, adLockReadOnly
...
'''''''                End If
            ...
            If ITEM_FLAG = True Then
                If NONSTOCKFLAG = True Then
                    PHY_ITEM.Open "Select ITEM_CODE, ITEM_NAME, BAL_QTY, SALES_PRICE, SALES_TAX, UNIT, REF_NO, EXP_DATE, VCH_NO, LINE_NO, TRX_TYPE, MRP, CHECK_FLAG From RTRXFILE  WHERE ITEM_CODE = '" & TXTITEMCODE.text & "' ORDER BY VCH_DATE", db, adOpenStatic, adLockReadOnly
                Else
                    PHY_ITEM.Open "Select ITEM_CODE, ITEM_NAME, BAL_QTY, SALES_PRICE, SALES_TAX, UNIT, REF_NO, EXP_DATE, VCH_NO, LINE_NO, TRX_TYPE, MRP, CHECK_FLAG From RTRXFILE  WHERE ITEM_CODE = '" & TXTITEMCODE.text & "' AND BAL_QTY > 0 ORDER BY VCH_DATE", db, adOpenStatic, adLockReadOnly
                End If
                ITEM_FLAG = False
            Else
                PHY_ITEM.Close
                If NONSTOCKFLAG = True Then
                    PHY_ITEM.Open "Select ITEM_CODE, ITEM_NAME, BAL_QTY, ITEM_COST, SALES_TAX, UNIT, REF_NO, EXP_DATE, VCH_NO, LINE_NO, TRX_TYPE, MRP, CHECK_FLAG From RTRXFILE  WHERE ITEM_CODE = '" & TXTITEMCODE.text & "' ORDER BY VCH_DATE", db, adOpenStatic, adLockReadOnly
                Else
                    PHY_ITEM.Open "Select ITEM_CODE, ITEM_NAME, BAL_QTY, ITEM_COST, SALES_TAX, UNIT, REF_NO, EXP_DATE, VCH_NO, LINE_NO, TRX_TYPE, MRP, CHECK_FLAG From RTRXFILE  WHERE ITEM_CODE = '" & TXTITEMCODE.text & "' AND BAL_QTY > 0 ORDER BY VCH_DATE", db, adOpenStatic, adLockReadOnly
                End If
                ITEM_FLAG = False
            End If
...
Dim chkflag As Integer
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            TRXSUB.Open "Select * From GIFT WHERE VCH_NO = " & Val(txtBillNo.text) & " ORDER BY LINE_NO", db, adOpenStatic, adLockReadOnly
            Do Until TRXSUB.EOF
                    i = i + 1
                    grdsales.rows = grdsales.rows + 1
                    grdsales.FixedRows = 1
                    ...
                    TRXMAST.Open "SELECT MANUFACTURER FROM ITEMMAST WHERE ITEMMAST.ITEM_CODE = '" & Trim(TRXSUB!ITEM_CODE) & "'", db, adOpenStatic, adLockReadOnly
                    ...
                    TXTDEALER.text = IIf(IsNull(TRXSUB!VCH_DESC), "", Mid(TRXSUB!VCH_DESC, 15))
...
Private Sub txtBillNo_LostFocus()
    ...
    TRXDN.Open "Select MAX(VCH_NO) From GIFT WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE  'GF'", db, adOpenStatic, adLockReadOnly
        ...
        Case vbKeyReturn
            ...
            If PHY.RecordCount = 1 Then
                ...
                If TMPFLAG = True Then
                    TMPREC.Open "Select ITEM_CODE, ITEM_NAME, BAL_QTY, MRP, ITEM_COST, SALES_TAX, UNIT, REF_NO, EXP_DATE, VCH_NO, LINE_NO, TRX_TYPE, CHECK_FLAG From RTRXFILE  WHERE ITEM_CODE = '" & TXTITEMCODE.text & "' AND BAL_QTY > 0 ORDER BY VCH_DATE", db, adOpenStatic, adLockReadOnly
                    TMPFLAG = False
                Else
                    TMPREC.Close
                    TMPREC.Open "Select ITEM_CODE, ITEM_NAME, BAL_QTY, MRP, ITEM_COST, SALES_TAX, UNIT, REF_NO, EXP_DATE, VCH_NO, LINE_NO, TRX_TYPE, CHECK_FLAG From RTRXFILE  WHERE ITEM_CODE = '" & TXTITEMCODE.text & "' AND BAL_QTY > 0 ORDER BY VCH_DATE", db, adOpenStatic, adLockReadOnly
...
'''''''                            RSTP_RATE.Open "Select * From P_Rate Where CUST_CODE='" & Trim(DataList2.BoundText) & "' And ITEM_CODE='" & RSTZEROSTOCK!ITEM_CODE & "'", DB, adOpenStatic, adLockReadOnly, adCmdText
...
Function FILL_BATCHGRID()
    ...
    If BATCH_FLAG = True Then
        PHY_BATCH.Open "Select REF_NO, BAL_QTY, EXP_DATE, MRP, SALES_PRICE, SALES_TAX,  ITEM_CODE, ITEM_NAME, VCH_NO, LINE_NO, TRX_TYPE, UNIT, CHECK_FLAG From RTRXFILE  WHERE ITEM_CODE = '" & TXTITEMCODE.text & "' AND BAL_QTY > 0 ORDER BY VCH_DATE", db, adOpenStatic, adLockReadOnly
        BATCH_FLAG = False
    Else
        PHY_BATCH.Close
        PHY_BATCH.Open "Select REF_NO, BAL_QTY, EXP_DATE, MRP, SALES_PRICE, SALES_TAX,  ITEM_CODE, ITEM_NAME, VCH_NO, LINE_NO, TRX_TYPE, UNIT, CHECK_FLAG From RTRXFILE  WHERE ITEM_CODE = '" & TXTITEMCODE.text & "' AND BAL_QTY > 0 ORDER BY VCH_DATE", db, adOpenStatic, adLockReadOnly
...
Public Function AppendSale()
    ...
    i = 0
    On Error GoTo ErrHand
    db.Execute "delete From GIFT WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND VCH_NO = " & Val(txtBillNo.text) & " AND TRX_TYPE = 'GF'"
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * From GIFT", db, adOpenStatic, adLockOptimistic, adCmdText
    For i = 1 To grdsales.rows - 1
        RSTTRXFILE.AddNew
        ...
        RSTTRXFILE!VCH_DESC = "Issued to     " & Trim(TXTDEALER.text)
    ...
    txtBillNo.text = 1
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select MAX(VCH_NO) From GIFT WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE  'GF'", db, adOpenStatic, adLockReadOnly
    If Not (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
