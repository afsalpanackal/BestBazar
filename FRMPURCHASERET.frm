...
Begin VB.Form FRMPURRETURN 
   ...
   End
End
Attribute VB_Name = "FRMPURRETURN"
...
Private Sub CMDADD_Click()
    ...
    grdsales.TextMatrix(Val(TXTSLNO.Text), 32) = Format(Val(TxtPTR.Text), ".000")
...
Public Function Generateprint()
    ...
    db.Execute "delete * From PURCH_RETURN WHERE TRX_TYPE='PR' AND VCH_NO = " & Val(txtBillNo.Text) & ""
    
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * From PURCH_RETURN ", db, adOpenStatic, adLockOptimistic, adCmdText
    RSTTRXFILE.AddNew
    ...
    For i = 1 To grdsales.Rows - 1
        ...
        RSTTRXFILE!RETAILER_PRICE = Val(grdsales.TextMatrix(i, 32))
    ...
    m_Rcpt_Amt = 0
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * From TRNXRCPT WHERE ACT_CODE = '" & DataList2.BoundText & "' AND TRX_TYPE = 'RT'", db, adOpenStatic, adLockReadOnly, adCmdText
    Do Until RSTTRXFILE.EOF
        m_Rcpt_Amt = m_Rcpt_Amt + RSTTRXFILE!RCPT_AMOUNT
        RSTTRXFILE.MoveNext
    Loop
    RSTTRXFILE.Close
    Set RSTTRXFILE = Nothing
    ...
    m_Bal_Amt = 0
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * From CRDTPYMT WHERE ACT_CODE = '" & DataList2.BoundText & "' AND INV_NO <> " & Val(LBLBILLNO.Caption) & " AND TRX_TYPE = 'PR'", db, adOpenStatic, adLockReadOnly, adCmdText
    Do Until RSTTRXFILE.EOF
        m_Bal_Amt = m_Bal_Amt + RSTTRXFILE!INV_AMT
        RSTTRXFILE.MoveNext
    Loop
    RSTTRXFILE.Close
    Set RSTTRXFILE = Nothing
    txtOutstanding.Text = m_Bal_Amt - m_Rcpt_Amt
    ...
    If DN_CN_FLag = False Then
        ReportNameVar = App.Path & "\rptbillretail.RPT"
    Else
        ReportNameVar = App.Path & "\RPTBILLRETAILCN.RPT"
    End If
...
Private Sub Form_Load()
    ...
    rstBILL.Open "Select MAX(Val(VCH_NO)) From PURCH_RETURN WHERE TRX_TYPE = 'PR'", db, adOpenStatic, adLockReadOnly
...
Private Sub GRDPOPUP_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            TXTEXPIRY.Text = IIf(GRDPOPUP.Columns(7) = "", "  /  ", Format(GRDPOPUP.Columns(7), "MM/YY"))
            TxtMRP.Text = IIf(IsNull(GRDPOPUP.Columns(8)), "", Format(GRDPOPUP.Columns(8), "0.00"))
...
'                        End If
            ...
            If ITEM_FLAG = True Then
                PHY_ITEM.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, PTR, P_VAN, CHECK_FLAG, LOOSE_PACK, PACK_TYPE, CATEGORY, WARRANTY, WARRANTY_TYPE  From ITEMMAST  WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' ORDER BY ITEM_NAME", db, adOpenStatic, adLockReadOnly
                ITEM_FLAG = False
            Else
                PHY_ITEM.Close
                PHY_ITEM.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, PTR, P_VAN, CHECK_FLAG, LOOSE_PACK, PACK_TYPE, CATEGORY, WARRANTY, WARRANTY_TYPE  From ITEMMAST  WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' ORDER BY ITEM_NAME", db, adOpenStatic, adLockReadOnly
            ...
            RSTBATCH.Open "Select * From RTRXFILE WHERE M_USER_ID = '" & DataList2.BoundText & "' AND ITEM_CODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY >0 ", db, adOpenStatic, adLockReadOnly
            ...
            Else
                RSTBATCH.Close
                Set RSTBATCH = Nothing
                If MsgBox("This Item has not been purchased from " & DataList2.Text & " Yet!! Do You Want to Continue...?", vbYesNo, "SALES RETURN..") = vbNo Then Exit Sub
                Set RSTBATCH = New ADODB.Recordset
            ...
            TxtPTR.Text = IIf(IsNull(GRDPOPUPITEM.Columns(6)), "", GRDPOPUPITEM.Columns(6))
            ...
            lblpack.Caption = IIf(IsNull(GRDPOPUPITEM.Columns(15)) Or GRDPOPUPITEM.Columns(15) = "", "1", GRDPOPUPITEM.Columns(15))
...
Private Sub GRDPRERATE_GotFocus()
    txtretail.Enabled = False
End Sub
...
Private Sub TXTBILLNO_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            TRXSUB.Open "Select * FROM TRXSUB WHERE TRX_TYPE='PR' AND VCH_NO = " & Val(txtBillNo.Text) & " ORDER BY LINE_NO", db, adOpenStatic, adLockReadOnly
            Do Until TRXSUB.EOF
                Set TRXFILE = New ADODB.Recordset
                TRXFILE.Open "Select * From TRXFILE WHERE TRX_TYPE='PR' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(TRXSUB!LINE_NO) & "", db, adOpenStatic, adLockReadOnly
                If Not (TRXFILE.EOF And TRXFILE.BOF) Then
                    ...
                    grdsales.TextMatrix(i, 31) = Format(TRXFILE!EXP_DATE, "MM/YY")
                    grdsales.TextMatrix(i, 32) = IIf(IsNull(TRXFILE!RETAILER_PRICE), 0, Format(TRXFILE!RETAILER_PRICE, "0.00"))
            ...
            Set TRXMAST = New ADODB.Recordset
            TRXMAST.Open "Select * From PURCH_RETURN WHERE TRX_TYPE='PR' AND VCH_NO = " & Val(txtBillNo.Text) & "", db, adOpenStatic, adLockReadOnly
            If Not (TRXMAST.EOF Or TRXMAST.BOF) Then
...
Private Sub txtBillNo_LostFocus()
    ...
    On Error GoTo ErrHand
    Set TRXMAST = New ADODB.Recordset
    TRXMAST.Open "Select MAX(Val(VCH_NO)) From PURCH_RETURN WHERE TRX_TYPE = 'PR'", db, adOpenStatic, adLockReadOnly
...
'    Set TRXMAST = New ADODB.Recordset
'    TRXMAST.Open "Select MIN(Val(VCH_NO)) FROM TRXFILE WHERE TRX_TYPE = 'PR'", db, adOpenStatic, adLockReadOnly
'    If Not (TRXMAST.EOF And TRXMAST.BOF) Then
...
Private Sub TXTPRODUCT_Change()
    ...
    Dim RSTBATCH As ADODB.Recordset
        ...
        If PHYFLAG = True Then
            'PHY.Open "Select [ITEM_CODE], [ITEM_NAME], [CLOSE_QTY], [P_RETAIL], [P_WS], [P_VAN], [P_CRTN], [CATEGORY] From ITEMMAST  WHERE ITEM_NAME Like '" & TXTPRODUCT.Text & "%'ORDER BY [ITEM_NAME]", db, adOpenStatic, adLockReadOnly
            PHY.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, PTR, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE  From ITEMMAST  WHERE ITEM_NAME Like '" & Me.TXTPRODUCT.Text & "%' AND ucase(CATEGORY) <> 'SERVICE CHARGE' ORDER BY [ITEM_NAME] ", db, adOpenStatic, adLockReadOnly
            PHYFLAG = False
        Else
            PHY.Close
            PHY.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, PTR, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE  From ITEMMAST  WHERE ITEM_NAME Like '" & Me.TXTPRODUCT.Text & "%' AND ucase(CATEGORY) <> 'SERVICE CHARGE' ORDER BY [ITEM_NAME] ", db, adOpenStatic, adLockReadOnly
        ...
        grdtmp.Columns(6).Caption = ""
        grdtmp.Columns(6).Width = 0
        ...
        grdtmp.Columns(12).Caption = "RATE"
        grdtmp.Columns(12).Width = 1100
...
'                            End If
                ...
                TxtPTR.Text = IIf(IsNull(grdtmp.Columns(6)), "", grdtmp.Columns(6))
                ...
                lblpack.Caption = IIf(IsNull(grdtmp.Columns(16)) Or grdtmp.Columns(16) = "", "1", grdtmp.Columns(16))
...
Private Sub TxtPTR_LostFocus()
    TxtPTR.Text = Format(Val(TxtPTR.Text), "0.000")
End Sub
...
Private Sub TXTQTY_GotFocus()
    ...
    If M_EDIT = True Then FrmeType.Visible = False
    'Set grdtmp.DataSource = Nothing
    'grdtmp.Visible = False
End Sub
...
'            Set RSTTRXFILE = Nothing
            ...
            Set RSTTRXFILE = New ADODB.Recordset
            RSTTRXFILE.Open "SELECT BAL_QTY  FROM RTRXFILE WHERE ITEM_CODE = '" & Trim(TXTITEMCODE.Text) & "' AND RTRXFILE.TRX_TYPE = '" & Trim(TXTTRXTYPE.Text) & "' AND RTRXFILE.VCH_NO = " & Val(TXTVCHNO.Text) & " AND RTRXFILE.LINE_NO = " & Val(TXTLINENO.Text) & "", db, adOpenStatic, adLockReadOnly
            If Not (RSTTRXFILE.EOF Or RSTTRXFILE.BOF) Then
                If (IsNull(RSTTRXFILE!BAL_QTY)) Then RSTTRXFILE!BAL_QTY = 0
                If OptLoose.value = True Then
                    i = RSTTRXFILE!BAL_QTY * Val(lblpack.Caption)
                Else
...
Private Sub TXTQTY_LostFocus()
    ...
    If Not (RSTITEMCOST.EOF Or RSTITEMCOST.BOF) Then
        If OptLoose.value = True Then
            LBLITEMCOST.Caption = IIf(IsNull(RSTITEMCOST!ITEM_COST), "", RSTITEMCOST!ITEM_COST / Val(lblpack.Caption))
        Else
    ...
    Select Case KeyCode
        Case vbKeyReturn, vbKeyTab
            ...
            If Val(TXTSLNO.Text) < grdsales.Rows Then
                ...
                TxtPTR.Text = grdsales.TextMatrix(Val(TXTSLNO.Text), 32)
...
Function FILL_ITEMGRID()
    ...
    If ITEM_FLAG = True Then
        PHY_ITEM.Open "Select [ITEM_CODE], [ITEM_NAME], [CLOSE_QTY], [P_RETAIL], [PTR], [P_VAN], [P_CRTN], [CATEGORY] From ITEMMAST  WHERE ITEM_NAME Like '" & TXTPRODUCT.Text & "%'ORDER BY [ITEM_NAME]", db, adOpenStatic, adLockReadOnly
        ITEM_FLAG = False
    Else
        PHY_ITEM.Close
        PHY_ITEM.Open "Select [ITEM_CODE], [ITEM_NAME], [CLOSE_QTY], [P_RETAIL], [PTR], [P_VAN], [P_CRTN], [CATEGORY] From ITEMMAST  WHERE ITEM_NAME Like '" & TXTPRODUCT.Text & "%'ORDER BY [ITEM_NAME]", db, adOpenStatic, adLockReadOnly
    ...
    GRDPOPUPITEM.Columns(3).Caption = ""
    GRDPOPUPITEM.Columns(3).Width = 0
    GRDPOPUPITEM.Columns(4).Caption = "RATE"
    GRDPOPUPITEM.Columns(4).Width = 1220
    GRDPOPUPITEM.Columns(5).Caption = ""
...
Private Function COSTCALCULATION()
    ...
    For N = 1 To grdsales.Rows - 1
        ...
        Do Until RSTCOST.EOF
            If grdsales.TextMatrix(N, 26) = "L" And Val(grdsales.TextMatrix(N, 27)) > 1 Then
...
Private Function AppendSale()
    ...
    If OLD_BILL = False Then Call checklastbill
    db.Execute "delete * From CRDTPYMT WHERE TRX_TYPE='PR' AND INV_NO = " & Val(LBLBILLNO.Caption) & ""
    db.Execute "delete * From PURCH_RETURN WHERE TRX_TYPE='PR' AND VCH_NO = " & Val(txtBillNo.Text) & ""
    db.Execute "delete * FROM TRXSUB WHERE TRX_TYPE='PR' AND VCH_NO = " & Val(txtBillNo.Text) & ""
    db.Execute "delete * FROM TRXFILE WHERE TRX_TYPE='PR' AND VCH_NO = " & Val(txtBillNo.Text) & ""
    
    'DB.Execute "delete * From P_Rate WHERE TRX_TYPE='PR' AND VCH_NO = " & Val(txtBillNo.Text) & ""
    ...
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * From SALESLEDGER WHERE TRX_TYPE='PR' AND INV_NO = " & Val(txtBillNo.Text) & "", db, adOpenStatic, adLockOptimistic, adCmdText
    If Not (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
    ...
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * From ATRXFILE  WHERE TRX_TYPE = 'PR' AND VCH_DATE = # " & E_DATE & " # ", db, adOpenStatic, adLockOptimistic, adCmdText
    If Not (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
    ...
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * From PURCH_RETURN WHERE TRX_TYPE='PR' AND VCH_NO = " & Val(txtBillNo.Text) & "", db, adOpenStatic, adLockOptimistic, adCmdText
...
SKIP:
    ...
    If lblcredit.Caption <> "0" Then
        Set rstMaxRec = New ADODB.Recordset
        rstMaxRec.Open "Select MAX(Val(CR_NO)) From CRDTPYMT", db, adOpenForwardOnly
        If Not (rstMaxRec.EOF And rstMaxRec.BOF) Then
        ...
        Set RSTITEMMAST = New ADODB.Recordset
        RSTITEMMAST.Open "SELECT * FROM CRDTPYMT WHERE INV_NO = " & Val(LBLBILLNO.Caption) & " AND TRX_TYPE = 'PR'", db, adOpenStatic, adLockOptimistic, adCmdText
        If (RSTITEMMAST.EOF And RSTITEMMAST.BOF) Then
            RSTITEMMAST.AddNew
        ...
        RSTITEMMAST!RCPT_AMOUNT = Val(LBLTOTAL.Caption)
...
Private Sub TXTRETAIL_LostFocus()
    ...
    If OPTTaxMRP.value = True Then
        TXTRETAILNOTAX.Text = Round(Val(txtretail.Text) - (Val(txtmrpbt.Text) * Val(TXTTAX.Text) / 100), 3)
        'txtretail.Text = Round(Val(TXTRETAILNOTAX.Text) + Val(txtmrpbt.Text) * Val(TXTTAX.Text) / 100, 3)
    End If
    If OPTVAT.value = True Then
...
Private Sub TXTITEMCODE_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        ...
        Case vbKeyReturn
            ...
            Set grdtmp.DataSource = Nothing
            If PHYFLAG = True Then
                PHY.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, PTR, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE  From ITEMMAST  WHERE ITEM_CODE = '" & Me.TXTITEMCODE.Text & "' AND ucase(CATEGORY) <> 'SERVICE CHARGE'", db, adOpenStatic, adLockReadOnly
                PHYFLAG = False
            Else
                PHY.Close
                PHY.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, PTR, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE  From ITEMMAST  WHERE ITEM_CODE = '" & Me.TXTITEMCODE.Text & "' AND ucase(CATEGORY) <> 'SERVICE CHARGE'", db, adOpenStatic, adLockReadOnly
...
Function FILL_BATCHGRID()
    ...
    If BATCH_FLAG = True Then
        PHY_BATCH.Open "Select REF_NO, BAL_QTY, VCH_NO, LINE_NO, TRX_TYPE, VCH_DATE, ITEM_NAME, EXP_DATE, MRP, SALES_TAX, LINE_DISC, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, PTR, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE From RTRXFILE  WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY > 0 ORDER BY VCH_DATE DESC", db, adOpenForwardOnly
        BATCH_FLAG = False
    Else
        PHY_BATCH.Close
        PHY_BATCH.Open "Select REF_NO, BAL_QTY, VCH_NO, LINE_NO, TRX_TYPE, VCH_DATE, ITEM_NAME, EXP_DATE, MRP, SALES_TAX, LINE_DISC, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, PTR, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE From RTRXFILE  WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY > 0 ORDER BY VCH_DATE DESC", db, adOpenForwardOnly
    ...
    GRDPOPUP.Columns(7).Caption = "Expiry"
    GRDPOPUP.Columns(8).Caption = "MRP"
    GRDPOPUP.Columns(11).Caption = ""
    GRDPOPUP.Columns(17).Caption = "RATE"
    
    GRDPOPUP.Columns(0).Width = 2900
    ...
    GRDPOPUP.Columns(7).Width = 1400
    GRDPOPUP.Columns(8).Width = 1100
    GRDPOPUP.Columns(9).Width = 0
    GRDPOPUP.Columns(10).Width = 0
    GRDPOPUP.Columns(11).Width = 0
    ...
    GRDPOPUP.Columns(17).Width = 1100
...
Private Function checklastbill()
    ...
    Set rstBILL = New ADODB.Recordset
    rstBILL.Open "Select MAX(Val(VCH_NO)) From PURCH_RETURN WHERE TRX_TYPE = 'PR'", db, adOpenForwardOnly
    If Not (rstBILL.EOF And rstBILL.BOF) Then
        txtBillNo.Text = IIf(IsNull(rstBILL.Fields(0)), 1, rstBILL.Fields(0) + 1)
    End If
    rstBILL.Close
...
'                            End If
                ...
                If UCase(txtcategory.Text) = "SERVICE CHARGE" Then
                    ...
                    Exit Function
                End If
                TXTRETAILNOTAX.Text = IIf(IsNull(GRDPOPUP.Columns(17)), "", GRDPOPUP.Columns(17))
                TxtPTR.Text = IIf(IsNull(GRDPOPUP.Columns(11)), "", GRDPOPUP.Columns(11))
                lblretail.Caption = IIf(IsNull(GRDPOPUP.Columns(11)), "", GRDPOPUP.Columns(11))
                lblwsale.Caption = IIf(IsNull(GRDPOPUP.Columns(17)), "", GRDPOPUP.Columns(17))
                'lblvan.Caption = IIf(IsNull(GRDPOPUP.Columns(13)), "", GRDPOPUP.Columns(13))
                ...
                lblpack.Caption = IIf(IsNull(GRDPOPUP.Columns(21)) Or GRDPOPUP.Columns(21) = "", "1", GRDPOPUP.Columns(21))
                lblunit.Caption = IIf(IsNull(GRDPOPUP.Columns(22)), "Nos", GRDPOPUP.Columns(22))
                'TxtWarranty.Text = IIf(IsNull(GRDPOPUP.Columns(23)), "", GRDPOPUP.Columns(23))
                'TxtWarranty_type.Text = IIf(IsNull(GRDPOPUP.Columns(24)), "", GRDPOPUP.Columns(24))
                
                If GRDPOPUP.Columns(12) = "A" Then
                    txtretaildummy.Text = IIf(IsNull(GRDPOPUP.Columns(14)), "P", GRDPOPUP.Columns(14))
                    TxtRetailmode.Text = "A"
                Else
                    txtretaildummy.Text = IIf(IsNull(GRDPOPUP.Columns(13)), "P", GRDPOPUP.Columns(13))
                    TxtRetailmode.Text = "P"
                End If
                Select Case PHY!CHECK_FLAG
                    Case "M"
                        OPTTaxMRP.value = True
                        TXTTAX.Text = GRDPOPUP.Columns(9)
                        TXTSALETYPE.Text = "2"
                    Case "V"
                        OPTVAT.value = True
                        TXTSALETYPE.Text = "2"
