   ...
   Begin VB.Frame FRMEMAIN 
      ...
      Begin VB.Frame FRMEHEAD 
         ...
         Begin VB.ComboBox cmbtype 
            ...
            ForeColor       =   &H00004080&
            Height          =   315
            ItemData        =   "FrmDeliverynew.frx":030A
            Left            =   6390
            List            =   "FrmDeliverynew.frx":0314
...
Private Sub cmdnxt_Click()
    ...
    rstBILL.Open "Select MAX(VCH_NO) From TEMPCN WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE = 'SI'", db, adOpenStatic, adLockReadOnly
...
Private Sub CMDMODIFY_Click()
    ...
    'If Val(TXTSLNO.Text) >= grdsales.Rows Then Exit Sub
    If M_EDIT = True Then Exit Sub
'    If ((frmLogin.rs!Level <> "0" And frmLogin.rs!Level <> "4" And frmLogin.rs!Level <> "1")) Or (frmLogin.rs!Level <> "0" And lbloldbills.Caption = "Y") Then
'        MsgBox "Permission Denied", vbOKOnly, "Sales"
'        Exit Sub
'    End If
...
'    LBLFOT.Tag = "(DHS " & Words_1_all(Num) & " Only)"
    ...
    If MDIMAIN.LBLTHPREVIEW.Caption = "Y" Then
        'Preview
        frmreport.Caption = "DELIVERY BILL"
        Call GENERATEREPORT
...
Public Sub txtBillNo_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            TRXFILE.Open "Select * From TEMPCN WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='SI' AND VCH_NO = " & Val(txtBillNo.text) & " ORDER BY LINE_NO", db, adOpenStatic, adLockReadOnly
...
'
'    Set TRXMAST = New ADODB.Recordset
'    TRXMAST.Open "Select MAX(VCH_NO) From TEMPCN WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE = 'SI'", db, adOpenStatic, adLockReadOnly
...
'
'    Set TRXMAST = New ADODB.Recordset
'    TRXMAST.Open "Select MIN(VCH_NO) FROM TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE = 'DN'", db, adOpenStatic, adLockReadOnly
'    If Not (TRXMAST.EOF And TRXMAST.BOF) Then
...
Private Sub TXTPRODUCT_GotFocus()
    ...
    If Trim(TXTPRODUCT.text) <> "" Then Call TXTPRODUCT_Change
End Sub
...
Private Function AppendSale()
    ...
    db.Execute "delete From TEMPCN WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='SI' AND VCH_NO = " & Val(txtBillNo.text) & ""
    db.Execute "delete From TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DN' AND VCH_NO = " & Val(txtBillNo.text) & ""
    ...
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * From TEMPCN WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='SI'", db, adOpenStatic, adLockPessimistic, adCmdText
    For i = 1 To grdsales.rows - 1
        RSTTRXFILE.AddNew
    ...
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * From TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DN'", db, adOpenStatic, adLockOptimistic, adCmdText
    For i = 1 To grdsales.rows - 1
        RSTTRXFILE.AddNew
...
Private Function checklastbill()
    ...
    Set rstBILL = New ADODB.Recordset
    rstBILL.Open "Select MAX(VCH_NO) From TEMPCN WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE = 'SI'", db, adOpenForwardOnly
    If Not (rstBILL.EOF And rstBILL.BOF) Then
        txtBillNo.text = IIf(IsNull(rstBILL.Fields(0)), 1, rstBILL.Fields(0) + 1)
    End If
    rstBILL.Close
