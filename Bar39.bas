...
Sub DrawBarcode(ByVal bc_string As String, bc_MRP As String, obj As Object)

Dim xpos!, Y1!, Y2!, Y3!, dw%, Th!, tw, new_string$
...
'obj.Height = 940
'obj.Height = (xpos + dw) * obj.Width / obj.ScaleWidth
Th = obj.TextHeight(bc_string)                  'text height
tw = obj.TextWidth(bc_string)                  'text width
obj.CurrentX = 0 '(obj.ScaleWidth - tw) / 2
obj.CurrentY = Y2 + 0.25 * Th
obj.Print bc_string
If Val(bc_MRP) <> 0 Then
    tw = obj.TextWidth(bc_MRP)                  'text width
    obj.CurrentX = (obj.ScaleWidth - tw)
    obj.CurrentY = Y2 + 0.25 * Th
    obj.Print "Rs." & Format(Val(bc_MRP), "0.00")
End If

'tw = obj.TextWidth(bc_ItemName)                  'text width
'obj.CurrentX = 0
'obj.CurrentY = Y2 + 0.25 * Th
'obj.Print bc_ItemName
End Sub
