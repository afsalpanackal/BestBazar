...
'                Else
                ...
                If (Val(TXTQTY.text) + Val(TxTfree.text)) = 0 Then
                    !ITEM_NET_COST = Round((Val(LBLSUBTOTAL.Caption) / Val(TXTQTY.text)) + Val(TxtExpense.text), 3)
                Else
                    !ITEM_NET_COST = Round((Val(LBLSUBTOTAL.Caption) / Val(TXTQTY.text)) + (Val(TxtExpense.text) / ((Val(TXTQTY.text) + Val(TxTfree.text)) * Val(Los_Pack.text))), 3)
                End If
    ...
    'RSTRTRXFILE!VCH_DESC = "Received From OP Stock"
...
Private Sub TXTRATE_LostFocus()
    ...
    ElseIf Val(TXTRATE.text) <> 0 And (Val(TXTRETAIL.text) = 0 Or Val(TXTRATE.text) < Val(TXTRATE.Tag)) Then
