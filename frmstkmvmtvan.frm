...
Begin VB.Form FrmStkmovmntvan 
   ...
   End
End
Attribute VB_Name = "FrmStkmovmntvan"
...
''''    Exit Sub
    ...
    RSTITEMMAST.Open "SELECT * From ITEMMASTVAN WHERE ITEM_CODE = '" & DataList2.BoundText & "' AND ucase(CATEGORY) <> 'SELF' AND ucase(CATEGORY) <> 'SERVICE CHARGE' AND ucase(CATEGORY) <> 'SERVICES' ORDER BY ITEM_NAME", db, adOpenStatic, adLockOptimistic, adCmdText
    RSTITEMMAST.Properties("Update Criteria").Value = adCriteriaKey
    If Not (RSTITEMMAST.EOF And RSTITEMMAST.BOF) Then
        
        BALQTY = 0
        Set RSTBALQTY = New ADODB.Recordset
        RSTBALQTY.Open "Select SUM(BAL_QTY) FROM RTRXFILEVAN WHERE  ITEM_CODE = '" & RSTITEMMAST!ITEM_CODE & "'", db, adOpenForwardOnly
        ...
        rststock.Open "SELECT SUM(QTY) FROM RTRXFILEVAN where ITEM_CODE = '" & RSTITEMMAST!ITEM_CODE & "' ", db, adOpenForwardOnly
        ...
        Set rststock = New ADODB.Recordset
        rststock.Open "SELECT SUM(FREE_QTY) FROM RTRXFILEVAN where ITEM_CODE = '" & RSTITEMMAST!ITEM_CODE & "' ", db, adOpenForwardOnly
        If Not (rststock.EOF And rststock.BOF) Then
        ...
        db.Execute "Update RTRXFILEVAN set BAL_QTY = QTY where ITEM_CODE = '" & RSTITEMMAST!ITEM_CODE & "' "
        BALQTY = 0
        
        Set rststock = New ADODB.Recordset
        rststock.Open "SELECT * FROM RTRXFILEVAN WHERE  ITEM_CODE = '" & RSTITEMMAST!ITEM_CODE & "' ", db, adOpenStatic, adLockOptimistic, adCmdText
        Do Until rststock.EOF
            BALQTY = 0
            Set RSTBALQTY = New ADODB.Recordset
        ...
        db.Execute "Update RTRXFILEVAN set BAL_QTY = 0 where ITEM_CODE = '" & RSTITEMMAST!ITEM_CODE & "' AND BAL_QTY <0"
        BALQTY = 0
        
        Set rststock = New ADODB.Recordset
        rststock.Open "SELECT SUM(BAL_QTY) FROM RTRXFILEVAN where ITEM_CODE = '" & RSTITEMMAST!ITEM_CODE & "' ", db, adOpenForwardOnly
        If Not (rststock.EOF And rststock.BOF) Then
        ...
        If Round(INWARD - OUTWARD, 2) < BALQTY Then
            ...
            Set rststock = New ADODB.Recordset
            rststock.Open "SELECT * FROM RTRXFILEVAN where ITEM_CODE = '" & RSTITEMMAST!ITEM_CODE & "' AND BAL_QTY > 0 ORDER BY VCH_DATE ", db, adOpenStatic, adLockOptimistic, adCmdText
            Do Until rststock.EOF
        ...
        ElseIf Round(INWARD - OUTWARD, 2) > BALQTY Then
            ...
            Set rststock = New ADODB.Recordset
            rststock.Open "SELECT * FROM RTRXFILEVAN where ITEM_CODE = '" & RSTITEMMAST!ITEM_CODE & "' ORDER BY VCH_DATE DESC", db, adOpenStatic, adLockOptimistic, adCmdText
            Do Until rststock.EOF
...
Private Sub Command1_Click()
    ...
    'Report.OpenSubreport("Rptinward").RecordSelectionFormula = "( ({TRXFILEVAN.TRX_TYPE} = 'OG' OR {TRXFILEVAN.TRX_TYPE} = 'PI' OR {TRXFILEVAN.TRX_TYPE} = 'OP') AND {TRXFILEVAN.VCH_DATE} <=# " & Format(DTTO.value, "MM,DD,YYYY") & " # AND {TRXFILEVAN.VCH_DATE} >=# " & Format(DTFROM.value, "MM,DD,YYYY") & " # )"
    Report.OpenSubreport("RPTINWRD.rpt").RecordSelectionFormula = "({RTRXFILEVAN.ITEM_CODE} = '" & DataList2.BoundText & "' AND {RTRXFILEVAN.VCH_DATE} <=# " & Format(DTTo.Value, "MM,DD,YYYY") & " # AND {RTRXFILEVAN.VCH_DATE} >=# " & Format(DTFROM.Value, "MM,DD,YYYY") & " # )"
    ...
    Report.OpenSubreport("RPTOUTWARD.rpt").RecordSelectionFormula = "({TRXFILEVAN.ITEM_CODE} = '" & DataList2.BoundText & "' AND {TRXMAST.VCH_DATE} <=# " & Format(DTTo.Value, "MM,DD,YYYY") & " # AND {TRXMAST.VCH_DATE} >=# " & Format(DTFROM.Value, "MM,DD,YYYY") & " # )"
...
Private Sub Command2_Click()
    ...
    Set Report = crxApplication.OpenReport(ReportNameVar, 1)
    'Report.OpenSubreport("Rptinward").RecordSelectionFormula = "( ({TRXFILEVAN.TRX_TYPE} = 'OG' OR {TRXFILEVAN.TRX_TYPE} = 'PI' OR {TRXFILEVAN.TRX_TYPE} = 'OP') AND {TRXFILEVAN.VCH_DATE} <=# " & Format(DTTO.value, "MM,DD,YYYY") & " # AND {TRXFILEVAN.VCH_DATE} >=# " & Format(DTFROM.value, "MM,DD,YYYY") & " # )"
    Report.OpenSubreport("RPTINWRD.rpt").RecordSelectionFormula = "({RTRXFILEVAN.VCH_DATE} <=# " & Format(DTTo.Value, "MM,DD,YYYY") & " # AND {RTRXFILEVAN.VCH_DATE} >=# " & Format(DTFROM.Value, "MM,DD,YYYY") & " # )"
    'Report.OpenSubreport("RPTINWRD.rpt").RecordSelectionFormula = "({TRANSMAST.VCH_DATE} <=# " & Format(DTTO.value, "MM,DD,YYYY") & " # AND {TRANSMAST.VCH_DATE} >=# " & Format(DTFROM.value, "MM,DD,YYYY") & " # )"
    For i = 1 To Report.OpenSubreport("RPTINWRD.rpt").Database.Tables.COUNT
        Report.OpenSubreport("RPTINWRD.rpt").Database.Tables(i).SetLogOnInfo strConnection
        If UCase(dbase1) <> "INVSOFT" And UCase(dbase1) <> "INVDB" And UCase(dbase1) <> "INVSOFT3" Then
...
'    Set rstTRXMAST = Nothing
    ...
    Select Case Trim(GRDOUTWARD.TextMatrix(GRDOUTWARD.Row, 11))
        Case "GI"
            ...
            If IsFormLoaded(FRMGSTBR) <> True Then
                FRMGSTBR.txtBillNo.text = Val(GRDOUTWARD.TextMatrix(GRDOUTWARD.Row, 12))
                FRMGSTBR.LBLBILLNO.Caption = Val(GRDOUTWARD.TextMatrix(GRDOUTWARD.Row, 12))
                FRMGSTBR.Show
                FRMGSTBR.SetFocus
                Call FRMGSTBR.txtBillNo_KeyDown(13, 0)
            End If
...
Private Sub GRDSTOCK_DblClick()
    ...
    Select Case Trim(GRDSTOCK.TextMatrix(GRDSTOCK.Row, 18))
        Case "PI"
            ...
            If IsFormLoaded(FRMTRANSFER) <> True Then
                FRMTRANSFER.txtBillNo.text = Val(GRDSTOCK.TextMatrix(GRDSTOCK.Row, 8))
                FRMTRANSFER.Show
                FRMTRANSFER.SetFocus
                Call FRMTRANSFER.txtBillNo_KeyDown(13, 0)
            End If
    End Select
End Sub
...
Private Sub TXTCODE_Change()
    ...
    If REPFLAG = True Then
        RSTREP.Open "Select * From ITEMMASTVAN  WHERE ITEM_CODE Like '" & TxtCode.text & "%' AND ucase(CATEGORY) <> 'SELF' AND ucase(CATEGORY) <> 'SERVICE CHARGE' AND ucase(CATEGORY) <> 'SERVICES' ORDER BY ITEM_NAME", db, adOpenStatic, adLockReadOnly
        REPFLAG = False
    Else
        RSTREP.Close
        RSTREP.Open "Select * From ITEMMASTVAN  WHERE ITEM_CODE Like '" & TxtCode.text & "%' AND ucase(CATEGORY) <> 'SELF' AND ucase(CATEGORY) <> 'SERVICE CHARGE' AND ucase(CATEGORY) <> 'SERVICES' ORDER BY ITEM_NAME", db, adOpenStatic, adLockReadOnly
...
Private Sub tXTMEDICINE_Change()
    ...
    On Error GoTo ERRHAND
    If REPFLAG = True Then
        RSTREP.Open "Select * From ITEMMASTVAN  WHERE ITEM_NAME Like '" & Me.TxtItemName.text & "%' AND ITEM_NAME Like '%" & Me.tXTMEDICINE.text & "%' AND ITEM_CODE Like '%" & Me.TxtCode.text & "%' AND ucase(CATEGORY) <> 'SELF' AND ucase(CATEGORY) <> 'SERVICE CHARGE' AND ucase(CATEGORY) <> 'SERVICES' ORDER BY ITEM_NAME", db, adOpenStatic, adLockReadOnly
        REPFLAG = False
    Else
        RSTREP.Close
        RSTREP.Open "Select * From ITEMMASTVAN  WHERE ITEM_NAME Like '" & Me.TxtItemName.text & "%' AND ITEM_NAME Like '%" & Me.tXTMEDICINE.text & "%' AND ITEM_CODE Like '%" & Me.TxtCode.text & "%' AND ucase(CATEGORY) <> 'SELF' AND ucase(CATEGORY) <> 'SERVICE CHARGE' AND ucase(CATEGORY) <> 'SERVICES' ORDER BY ITEM_NAME", db, adOpenStatic, adLockReadOnly
...
Private Function Fillgrid()
    ...
    RSTITEM.Open "SELECT *  From ITEMMASTVAN WHERE ITEM_CODE = '" & DataList2.BoundText & "'", db, adOpenStatic, adLockReadOnly, adCmdText
    If Not (RSTITEM.EOF And RSTITEM.BOF) Then
        ...
        RSTTRXFILE.Open "SELECT *  FROM RTRXFILEVAN WHERE ITEM_CODE = '" & RSTITEM!ITEM_CODE & "' AND VCH_DATE <'" & Format(DTFROM.Value, "yyyy/mm/dd") & "'", db, adOpenStatic, adLockReadOnly, adCmdText
        ...
        Set RSTTRXFILE = New ADODB.Recordset
        RSTTRXFILE.Open "SELECT * FROM TRXFILEVAN WHERE  ITEM_CODE = '" & RSTITEM!ITEM_CODE & "' AND ((TRX_TYPE='HI' AND CST =0) OR (TRX_TYPE='GI' AND CST =0) OR (TRX_TYPE='SV' AND CST =0) OR (TRX_TYPE='SI' AND CST =0) OR (TRX_TYPE='RI' AND CST =0) OR TRX_TYPE='DN' OR TRX_TYPE='WP' OR TRX_TYPE='PR' OR TRX_TYPE='DG' OR TRX_TYPE='DM' OR TRX_TYPE='GF' OR TRX_TYPE='RM' OR TRX_TYPE='PC' OR TRX_TYPE='MI') AND VCH_DATE <'" & Format(DTFROM.Value, "yyyy/mm/dd") & "'", db, adOpenStatic, adLockReadOnly, adCmdText
        Do Until RSTTRXFILE.EOF
    ...
    GRDSTOCK.rows = 1
    Set rststock = New ADODB.Recordset
    If optdate.Value = True Then rststock.Open "SELECT * FROM RTRXFILEVAN WHERE  ITEM_CODE = '" & DataList2.BoundText & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' ORDER BY VCH_DATE DESC, VCH_NO", db, adOpenStatic, adLockReadOnly
    If optsupplier.Value = True Then rststock.Open "SELECT * FROM RTRXFILEVAN WHERE  ITEM_CODE = '" & DataList2.BoundText & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' ORDER BY TRX_TYPE,VCH_DESC, VCH_NO", db, adOpenStatic, adLockReadOnly
    If OPTBALQTY.Value = True Then rststock.Open "SELECT * FROM RTRXFILEVAN WHERE  ITEM_CODE = '" & DataList2.BoundText & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' ORDER BY BAL_QTY DESC", db, adOpenStatic, adLockReadOnly
    Do Until rststock.EOF
        i = i + 1
...
'    rststock_bal.Open "SELECT * From ITEMMASTVAN WHERE  ITEM_CODE = '" & DataList2.BoundText & "'", db, adOpenStatic, adLockReadOnly
...
Private Function Fillgrid2()
    ...
    GRDOUTWARD.rows = 1
    Set rststock = New ADODB.Recordset
    If OPTOUTDATE.Value = True Then rststock.Open "SELECT * FROM TRXFILEVAN WHERE  ITEM_CODE = '" & DataList2.BoundText & "' AND ((TRX_TYPE='HI' AND CST =0) OR (TRX_TYPE='GI' AND CST =0) OR (TRX_TYPE='SV' AND CST =0) OR (TRX_TYPE='SI' AND CST =0) OR (TRX_TYPE='RI' AND CST =0) OR (TRX_TYPE='WO' AND CST =0) OR (TRX_TYPE='VI' AND CST =0) OR (TRX_TYPE='TF' AND CST =0) OR TRX_TYPE='DN' OR TRX_TYPE='WP' OR TRX_TYPE='PR' OR TRX_TYPE='RM' OR TRX_TYPE='PC' OR TRX_TYPE='MI' OR TRX_TYPE='DG' OR TRX_TYPE='DM' OR TRX_TYPE='GF' OR TRX_TYPE='RW' OR TRX_TYPE='SR') AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' ORDER BY VCH_DATE DESC, VCH_NO", db, adOpenStatic, adLockReadOnly
    If OPTOUTCUST.Value = True Then rststock.Open "SELECT * FROM TRXFILEVAN WHERE  ITEM_CODE = '" & DataList2.BoundText & "' AND ((TRX_TYPE='HI' AND CST =0) OR (TRX_TYPE='GI' AND CST =0) OR (TRX_TYPE='SV' AND CST =0) OR (TRX_TYPE='SI' AND CST =0) OR (TRX_TYPE='RI' AND CST =0) OR (TRX_TYPE='WO' AND CST =0) OR (TRX_TYPE='VI' AND CST =0) OR (TRX_TYPE='TF' AND CST =0) OR TRX_TYPE='DN' OR TRX_TYPE='WP' OR TRX_TYPE='PR' OR TRX_TYPE='RM' OR TRX_TYPE='PC' OR TRX_TYPE='MI' OR TRX_TYPE='DG' OR TRX_TYPE='DM' OR TRX_TYPE='GF' OR TRX_TYPE='RW' OR TRX_TYPE='SR') AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' ORDER BY VCH_DESC ASC, VCH_DATE DESC, VCH_NO", db, adOpenStatic, adLockReadOnly
    'rststock.Open "SELECT * FROM TRXFILEVAN WHERE  ITEM_CODE = '" & DataList2.BoundText & "' AND CST <>2", db, adOpenStatic, adLockReadOnly
    Do Until rststock.EOF
        i = i + 1
...
'        End Select
        ...
        Select Case rststock!TRX_TYPE
            ...
            Case "VI"
                ...
                GRDOUTWARD.TextMatrix(i, 2) = Mid(rststock!VCH_DESC, 15)
            Case "GI"
                GRDOUTWARD.TextMatrix(i, 1) = "BR. SALE"
                GRDOUTWARD.TextMatrix(i, 2) = Mid(rststock!VCH_DESC, 15)
            Case "SV"
            ...
            Case "SR"
                ...
                GRDOUTWARD.TextMatrix(i, 2) = Mid(rststock!VCH_DESC, 15)
            Case "RW"
                GRDOUTWARD.TextMatrix(i, 1) = "Sales Return(W)"
                GRDOUTWARD.TextMatrix(i, 2) = Mid(rststock!VCH_DESC, 15)
            Case "GF"
        ...
        GRDSTOCK.Tag = ""
        Set RSTTEMP = New ADODB.Recordset
        RSTTEMP.Open "SELECT * From ITEMMASTVAN WHERE  ITEM_CODE = '" & DataList2.BoundText & "' ", db, adOpenStatic, adLockReadOnly
...
'    On Error GoTo Errhand
'    If REPFLAG = True Then
'        RSTREP.Open "Select DISTINCT BARCODE FROM RTRXFILEVAN  WHERE BARCODE Like '" & TxtBarcode.Text & "%' ", db, adOpenStatic, adLockReadOnly
'        REPFLAG = False
'    Else
'        RSTREP.Close
'        RSTREP.Open "Select DISTINCT BARCODE FROM RTRXFILEVAN  WHERE BARCODE Like '" & TxtBarcode.Text & "%' ", db, adOpenStatic, adLockReadOnly
...
Private Function Fillgrid3()
    ...
    Screen.MousePointer = vbHourglass
'    Set RSTITEM = New ADODB.Recordset
'    RSTITEM.Open "SELECT *  FROM RTRXFILEVAN WHERE BARCODE = '" & Trim(TxtBarcode.Text) & "'", db, adOpenStatic, adLockReadOnly, adCmdText
'    If Not (RSTITEM.EOF And RSTITEM.BOF) Then
'        OPQTY = 0
'        OPVAL = 0
'
...
'        'OPVAL = IIf(IsNull(RSTITEM!OPEN_VAL), 0, RSTITEM!OPEN_VAL)
'        Set RSTTRXFILE = New ADODB.Recordset
'        RSTTRXFILE.Open "SELECT *  FROM RTRXFILEVAN WHERE BARCODE = '" & RSTITEM!BARCODE & "' AND VCH_DATE <'" & Format(DTFROM.value, "yyyy/mm/dd") & "'", db, adOpenStatic, adLockReadOnly, adCmdText
'        Do Until RSTTRXFILE.EOF
'            OPQTY = OPQTY + IIf(IsNull(RSTTRXFILE!QTY), 0, RSTTRXFILE!QTY)
'            OPVAL = OPVAL + IIf(IsNull(RSTTRXFILE!TRX_TOTAL), 0, RSTTRXFILE!TRX_TOTAL)
'            RSTTRXFILE.MoveNext
...
'
'        Set RSTTRXFILE = New ADODB.Recordset
'        RSTTRXFILE.Open "SELECT * FROM TRXFILEVAN WHERE  BARCODE = '" & RSTITEM!BARCODE & "' AND ((TRX_TYPE='HI' AND CST =0) OR (TRX_TYPE='GI' AND CST =0) OR (TRX_TYPE='SV' AND CST =0) OR (TRX_TYPE='SI' AND CST =0) OR (TRX_TYPE='RI' AND CST =0) OR TRX_TYPE='DN' OR TRX_TYPE='PR' OR TRX_TYPE='DG' OR TRX_TYPE='DM' OR TRX_TYPE='GF' OR TRX_TYPE='RM' OR TRX_TYPE='PC' OR TRX_TYPE='MI') AND VCH_DATE <'" & Format(DTFROM.value, "yyyy/mm/dd") & "'", db, adOpenStatic, adLockReadOnly, adCmdText
'        Do Until RSTTRXFILE.EOF
'            RCVD_OP = RCVD_OP + RSTTRXFILE!QTY
'            'ISSVAL = ISSVAL + IIf(IsNull(RSTTRXFILE!TRX_TOTAL), 0, RSTTRXFILE!TRX_TOTAL)
...
'    Set RSTITEM = Nothing
    ...
    GRDSTOCK.rows = 1
    Set rststock = New ADODB.Recordset
    If optdate.Value = True Then rststock.Open "SELECT * FROM RTRXFILEVAN WHERE  BARCODE = '" & Trim(TxtBarcode.text) & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' ORDER BY VCH_DATE DESC, VCH_NO", db, adOpenStatic, adLockReadOnly
    If optsupplier.Value = True Then rststock.Open "SELECT * FROM RTRXFILEVAN WHERE  BARCODE = '" & Trim(TxtBarcode.text) & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' ORDER BY TRX_TYPE,VCH_DESC, VCH_NO", db, adOpenStatic, adLockReadOnly
    If OPTBALQTY.Value = True Then rststock.Open "SELECT * FROM RTRXFILEVAN WHERE  BARCODE = '" & Trim(TxtBarcode.text) & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' ORDER BY BAL_QTY DESC", db, adOpenStatic, adLockReadOnly
    Do Until rststock.EOF
        i = i + 1
        GRDSTOCK.rows = GRDSTOCK.rows + 1
        GRDSTOCK.FixedRows = 1
...
'    LBLBALANCE.Caption = ""
'    Set rststock_bal = New ADODB.Recordset
'    rststock_bal.Open "SELECT * From ITEMMASTVAN WHERE  ITEM_CODE = '" & Trim(TxtBarcode.Text) & "'", db, adOpenStatic, adLockReadOnly
'    If Not (rststock_bal.EOF And rststock_bal.BOF) Then
'        Select Case rststock_bal!LOOSE_PACK
'            Case Is > 1
'                LBLBALANCE.Caption = Int(rststock_bal!CLOSE_QTY) & " + " & IIf(Val(Round((rststock_bal!CLOSE_QTY - Int(rststock_bal!CLOSE_QTY)) * rststock_bal!LOOSE_PACK, 0)) = 0, "", Round((rststock_bal!CLOSE_QTY - Int(rststock_bal!CLOSE_QTY)) * rststock_bal!LOOSE_PACK, 0) & " " & rststock_bal!PACK_TYPE)
...
Private Function FILLGRID4()
    ...
    GRDOUTWARD.rows = 1
    Set rststock = New ADODB.Recordset
    If OPTOUTDATE.Value = True Then rststock.Open "SELECT * FROM TRXFILEVAN WHERE  BARCODE = '" & Trim(TxtBarcode.text) & "' AND ((TRX_TYPE='HI' AND CST =0) OR (TRX_TYPE='GI' AND CST =0) OR (TRX_TYPE='SV' AND CST =0) OR (TRX_TYPE='SI' AND CST =0) OR (TRX_TYPE='RI' AND CST =0) OR (TRX_TYPE='WO' AND CST =0) OR (TRX_TYPE='VI' AND CST =0) OR (TRX_TYPE='TF' AND CST =0) OR TRX_TYPE='DN' OR TRX_TYPE='WP' OR TRX_TYPE='PR' OR TRX_TYPE='RM' OR TRX_TYPE='PC' OR TRX_TYPE='MI' OR TRX_TYPE='DG' OR TRX_TYPE='DM' OR TRX_TYPE='GF' OR TRX_TYPE='RW' OR TRX_TYPE='SR') AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' ORDER BY VCH_DATE DESC, VCH_NO", db, adOpenStatic, adLockReadOnly
    If OPTOUTCUST.Value = True Then rststock.Open "SELECT * FROM TRXFILEVAN WHERE  BARCODE = '" & Trim(TxtBarcode.text) & "' AND ((TRX_TYPE='HI' AND CST =0) OR (TRX_TYPE='GI' AND CST =0) OR (TRX_TYPE='SV' AND CST =0) OR (TRX_TYPE='SI' AND CST =0) OR (TRX_TYPE='RI' AND CST =0) OR (TRX_TYPE='WO' AND CST =0) OR (TRX_TYPE='VI' AND CST =0) OR (TRX_TYPE='TF' AND CST =0) OR TRX_TYPE='DN' OR TRX_TYPE='WP' OR TRX_TYPE='PR' OR TRX_TYPE='RM' OR TRX_TYPE='PC' OR TRX_TYPE='MI' OR TRX_TYPE='DG' OR TRX_TYPE='DM' OR TRX_TYPE='GF' OR TRX_TYPE='RW' OR TRX_TYPE='SR') AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' ORDER BY VCH_DESC ASC, VCH_DATE DESC, VCH_NO", db, adOpenStatic, adLockReadOnly
    'rststock.Open "SELECT * FROM TRXFILEVAN WHERE  BARCODE = '" & Trim(TxtBarcode.Text) & "' AND CST <>2", db, adOpenStatic, adLockReadOnly
    Do Until rststock.EOF
        i = i + 1
        GRDOUTWARD.rows = GRDOUTWARD.rows + 1
        GRDOUTWARD.FixedRows = 1
...
'        End Select
        ...
        GRDSTOCK.Tag = ""
        Set RSTTEMP = New ADODB.Recordset
        RSTTEMP.Open "SELECT * From ITEMMASTVAN WHERE  BARCODE = '" & Trim(TxtBarcode.text) & "' ", db, adOpenStatic, adLockReadOnly
        If Not (RSTTEMP.EOF And RSTTEMP.BOF) Then
            GRDOUTWARD.Tag = IIf(IsNull(RSTTEMP!PACK_TYPE), "", RSTTEMP!PACK_TYPE)
            GRDSTOCK.Tag = IIf(IsNull(RSTTEMP!LOOSE_PACK), "", RSTTEMP!LOOSE_PACK)
        End If
