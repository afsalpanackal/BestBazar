...
Begin VB.Form FRMPETTY1 
   ...
   Caption         =   "PETTY BILL - II "
   ClientHeight    =   11100
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   18495
   Icon            =   "FrmPetty5.frx":0000
   KeyPreview      =   -1  'True
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin VB.Frame FRMEMAIN 
      ...
      Begin VB.Frame FRMEHEAD 
         ...
         Begin VB.ComboBox cmbtype 
            ...
            ForeColor       =   &H00FF0000&
            Height          =   315
            ItemData        =   "FrmPetty5.frx":030A
            Left            =   8355
            List            =   "FrmPetty5.frx":031A
...
Private Sub TXTPRODUCT_Change()
        ...
        If PHYFLAG = True Then
            ...
            If Trim(TxtName1.text) = "" Then
                PHY.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE, MRP, CUST_DISC, MANUFACTURER, BIN_LOCATION, CESS_PER, CESS_AMT, P_LWS, UN_BILL  From ITEMMAST  WHERE (ITEM_NAME Like '%" & Trim(Me.TXTPRODUCT.text) & "%' OR MRP Like '" & Trim(Me.TxtName1.text) & "') AND (ISNULL(DEAD_STOCK) OR DEAD_STOCK <> 'Y') ORDER BY ITEM_NAME ", db, adOpenStatic, adLockReadOnly
            Else
                PHY.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE, MRP, CUST_DISC, MANUFACTURER, BIN_LOCATION, CESS_PER, CESS_AMT, P_LWS, UN_BILL  From ITEMMAST  WHERE (ITEM_CODE Like '" & Trim(Me.TxtName1.text) & "%' OR ITEM_NAME Like '%" & Trim(Me.TXTPRODUCT.text) & "%' OR MRP Like '" & Trim(Me.TxtName1.text) & "') AND (ITEM_NAME Like '%" & Trim(Me.TxtName1.text) & "%' OR MRP Like '" & Trim(Me.TxtName1.text) & "') AND (ISNULL(DEAD_STOCK) OR DEAD_STOCK <> 'Y') ORDER BY ITEM_NAME ", db, adOpenStatic, adLockReadOnly
            End If
            PHYFLAG = False
        Else
            PHY.Close
            ...
            If Trim(TxtName1.text) = "" Then
                PHY.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE, MRP, CUST_DISC, MANUFACTURER, BIN_LOCATION, CESS_PER, CESS_AMT, P_LWS, UN_BILL  From ITEMMAST  WHERE (ITEM_NAME Like '%" & Trim(Me.TXTPRODUCT.text) & "%' OR MRP Like '" & Trim(Me.TxtName1.text) & "') AND (ISNULL(DEAD_STOCK) OR DEAD_STOCK <> 'Y') ORDER BY ITEM_NAME ", db, adOpenStatic, adLockReadOnly
            Else
                PHY.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE, MRP, CUST_DISC, MANUFACTURER, BIN_LOCATION, CESS_PER, CESS_AMT, P_LWS, UN_BILL  From ITEMMAST  WHERE (ITEM_CODE Like '" & Trim(Me.TxtName1.text) & "%' OR ITEM_NAME Like '%" & Trim(Me.TxtName1.text) & "%' OR MRP Like '" & Trim(Me.TxtName1.text) & "')  AND (ITEM_NAME Like '%" & Trim(Me.TXTPRODUCT.text) & "%' ) AND (ISNULL(DEAD_STOCK) OR DEAD_STOCK <> 'Y') ORDER BY ITEM_NAME ", db, adOpenStatic, adLockReadOnly
            End If
            PHYFLAG = False
        End If
        Set grdtmp.DataSource = PHY
    ...
    If Not (RSTCOMPANY.EOF And RSTCOMPANY.BOF) Then
       ...
       ' Print #1, Chr(27) & Chr(72) &  "Salesman: CS"
        ...
        For i = 1 To grdsales.rows - 1
            ...
            End If
            Print #1, AlignLeft(grdsales.Tag, 22) & Space(9) & _
...
'            grdsales.Tag = Trim(grdsales.TextMatrix(i, 33))
'        End If
'        Print #1, AlignLeft(grdsales.Tag, 50) & Space(0) & _
...
'    End If
    ...
    For i = 1 To Report.Database.Tables.COUNT
        ...
        End If
    Next i
    Report.RecordSelectionFormula = "({TRXFILE.VCH_NO}= " & Val(txtBillNo.text) & " and ({TRXFILE.TRX_TYPE}= 'WO' or {TRXFILE.TRX_TYPE}= 'XC'))"
