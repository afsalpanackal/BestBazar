...
Begin VB.Form frmDmgRet 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "DAMAGE ENTRY  (Credit Note)"
   ClientHeight    =   11100
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   18495
   Icon            =   "FrmDamageRetNew.frx":0000
   KeyPreview      =   -1  'True
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin VB.Frame FRMEMAIN 
      ...
      Begin VB.Frame Frame4 
         ...
         Begin VB.PictureBox picUnchecked 
            Height          =   285
            Left            =   16080
            Picture         =   "FrmDamageRetNew.frx":030A
            ScaleHeight     =   225
            ScaleWidth      =   225
            TabIndex        =   143
            Top             =   3105
         ...
         Begin VB.PictureBox picChecked 
            Height          =   285
            Left            =   15435
            Picture         =   "FrmDamageRetNew.frx":064C
            ScaleHeight     =   225
            ScaleWidth      =   225
            TabIndex        =   142
            Top             =   3075
...
'    RSTTRXFILE.Open "SELECT *  FROM TEMPCN WHERE ACT_CODE = '" & Trim(DataList2.BoundText) & "' AND BILL_NO = " & Val(txtBillNo.Text) & " AND BILL_TRX_TYPE = 'DG' AND TRX_TYPE = 'DG'", db, adOpenStatic, adLockOptimistic, adCmdText
...
'    Set RSTTRXFILE = Nothing
    ...
    db.Execute "delete FROM TRXSUB WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & " "
    db.Execute "delete From TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & " "
    
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "SELECT *  FROM RTRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(TxtCN.Text) & "", db, adOpenStatic, adLockReadOnly, adCmdText
    With RSTTRXFILE
        Do Until .EOF
    ...
    RSTTRXFILE.Close
    Set RSTTRXFILE = Nothing
    db.Execute "delete From RTRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(TxtCN.Text) & ""
    ...
    For n = 1 To grdsales.rows - 1
        ...
        If grdsales.TextMatrix(n, 19) = "DN" Then
            Set RSTTRXFILE = New ADODB.Recordset
            'RSTTRXFILE.Open "SELECT *  FROM TEMPCN WHERE ACT_CODE = '" & Trim(DataList2.BoundText) & "' AND TRX_TYPE = 'DG' AND VCH_NO = " & Val(grdsales.TextMatrix(n, 35)) & " AND LINE_NO = " & Val(grdsales.TextMatrix(n, 37)) & " ", db, adOpenStatic, adLockOptimistic, adCmdText
...
'    End If
    ...
    If OLD_BILL = False Then
        Dim TRXMAST As ADODB.Recordset
        Set TRXMAST = New ADODB.Recordset
        TRXMAST.Open "Select MAX(VCH_NO) From DAMAGE_MAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE = 'DG'", db, adOpenForwardOnly
        If Not (TRXMAST.EOF And TRXMAST.BOF) Then
    ...
    If OLD_BILL = False And Val(txtBillNo.Text) <> 1 And grdsales.rows <= 1 Then
        RSTTRXFILE.Open "Select * From DAMAGE_MAST WHERE VCH_NO= (SELECT MAX(VCH_NO) From DAMAGE_MAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE = 'DG')", db, adOpenStatic, adLockPessimistic, adCmdText
    ...
    Else
        RSTTRXFILE.Open "Select * From DAMAGE_MAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & "", db, adOpenStatic, adLockPessimistic, adCmdText
    ...
    Else
        ...
        rstMaxNo.Open "Select MAX(LINE_NO) From TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & "", db, adOpenStatic, adLockReadOnly
...
'                End If
    ...
    db.Execute "delete From TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 32)) & ""
    db.Execute "START TRANSACTION"
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * From TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 32)) & "", db, adOpenStatic, adLockOptimistic, adCmdText
    If (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
        RSTTRXFILE.AddNew
        RSTTRXFILE!TRX_TYPE = "DG"
        RSTTRXFILE!TRX_YEAR = Year(MDIMAIN.DTFROM.Value)
...
'    Set RSTUNBILL = Nothing
    ...
    db.Execute "START TRANSACTION"
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * FROM TRXSUB WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 32)) & "", db, adOpenStatic, adLockOptimistic, adCmdText
    If (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
        RSTTRXFILE.AddNew
        RSTTRXFILE!VCH_NO = Val(txtBillNo.Text)
        RSTTRXFILE!TRX_TYPE = "DG"
...
Private Sub CmdDelete_Click()
    ...
    db.Execute "delete FROM TRXSUB WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 32)) & ""
    db.Execute "delete From TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 32)) & ""
...
'    End If
    ...
    RSTCOMPANY.Open "SELECT * From TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & "", db, adOpenForwardOnly
...
Private Sub Form_Load()
    ...
    rstBILL.Open "Select MAX(VCH_NO) From DAMAGE_MAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE = 'DG'", db, adOpenStatic, adLockReadOnly
...
Private Sub grdsales_DblClick()
    ...
    End If
    On Error GoTo ERRHAND
    db.Execute "Update TRXFILE set UN_BILL = '" & Trim(grdsales.TextMatrix(grdsales.Row, 46)) & "' WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(grdsales.Row, 32)) & ""
    Exit Sub
ERRHAND:
    MsgBox err.Description
End Sub
...
Public Sub txtBillNo_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            txtBillNo.Tag = "N"
'            Set TRXMAST = New ADODB.Recordset
'            TRXMAST.Open "Select * From DAMAGE_MAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & " AND (ISNULL(BILL_FLAG) OR BILL_FLAG <>'Y') ", db, adOpenStatic, adLockReadOnly
...
'            Set TRXMAST = Nothing
            ...
            TRXFILE.Open "Select * From TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & " ORDER BY LINE_NO", db, adOpenStatic, adLockReadOnly
...
'                Set TRXMAST = Nothing
                ...
                TRXSUB.Open "Select * FROM TRXSUB WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(TRXFILE!LINE_NO) & "", db, adOpenStatic, adLockReadOnly
...
'                grdsales.TextMatrix(i, 43) = IIf(IsNull(TRXSUB!R_TRX_YEAR), "", Trim(TRXSUB!R_TRX_YEAR))
            ...
            TXTTOTALDISC.Text = ""
            Set TRXMAST = New ADODB.Recordset
            TRXMAST.Open "Select * From DAMAGE_MAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & "", db, adOpenStatic, adLockReadOnly
            If Not (TRXMAST.EOF Or TRXMAST.BOF) Then
...
'                    rstBILL.Open "Select * From DAMAGE_MAST WHERE VCH_NO= (SELECT MAX(VCH_NO) From DAMAGE_MAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE = 'DG')", db, adOpenStatic, adLockPessimistic, adCmdText
...
'                Else
'                    rstBILL.Open "Select * From DAMAGE_MAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & "", db, adOpenStatic, adLockPessimistic, adCmdText
...
'    Set TRXMAST = New ADODB.Recordset
'    TRXMAST.Open "Select MAX(VCH_NO) From DAMAGE_MAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE = 'DG'", db, adOpenStatic, adLockReadOnly
'    If Not (TRXMAST.EOF And TRXMAST.BOF) Then
...
'    Set TRXMAST = New ADODB.Recordset
'    TRXMAST.Open "Select MIN(VCH_NO) From TRXFILE WHERE TRX_TYPE = 'DG'", db, adOpenStatic, adLockReadOnly
'    If Not (TRXMAST.EOF And TRXMAST.BOF) Then
'        i = IIf(IsNull(TRXMAST.Fields(0)), 1, TRXMAST.Fields(0))
'        If Val(txtBillNo.Text) < i Then
...
'                            End If
                ...
                Select Case lblcusttype.Caption
                    Case "R"
                        txtretail.Text = IIf(IsNull(grdtmp.Columns(26)), "", Val(grdtmp.Columns(26)) * Val(LblPack.Text))
                        TXTRETAILNOTAX.Text = IIf(IsNull(grdtmp.Columns(26)), "", Val(grdtmp.Columns(26)) * Val(LblPack.Text))
                    Case "W"
                        txtretail.Text = IIf(IsNull(grdtmp.Columns(12)), "", Val(grdtmp.Columns(12)) * Val(LblPack.Text))
                        TXTRETAILNOTAX.Text = IIf(IsNull(grdtmp.Columns(12)), "", Val(grdtmp.Columns(12)) * Val(LblPack.Text))
                    Case "V"
                        txtretail.Text = IIf(IsNull(grdtmp.Columns(13)), "", Val(grdtmp.Columns(13)) * Val(LblPack.Text))
                        TXTRETAILNOTAX.Text = IIf(IsNull(grdtmp.Columns(13)), "", Val(grdtmp.Columns(13)) * Val(LblPack.Text))
                    Case "M"
                        txtretail.Text = IIf(IsNull(grdtmp.Columns(20)), "", Val(grdtmp.Columns(20)) * Val(LblPack.Text))
                        TXTRETAILNOTAX.Text = IIf(IsNull(grdtmp.Columns(20)), "", Val(grdtmp.Columns(20)) * Val(LblPack.Text))
                    Case "5"
                        txtretail.Text = IIf(IsNull(grdtmp.Columns(27)), "", Val(grdtmp.Columns(27)) * Val(LblPack.Text))
                        TXTRETAILNOTAX.Text = IIf(IsNull(grdtmp.Columns(27)), "", Val(grdtmp.Columns(27)) * Val(LblPack.Text))
                    Case "6"
                        txtretail.Text = IIf(IsNull(grdtmp.Columns(28)), "", Val(grdtmp.Columns(28)) * Val(LblPack.Text))
                        TXTRETAILNOTAX.Text = IIf(IsNull(grdtmp.Columns(28)), "", Val(grdtmp.Columns(28)) * Val(LblPack.Text))
                    Case "7"
                        txtretail.Text = IIf(IsNull(grdtmp.Columns(29)), "", Val(grdtmp.Columns(29)) * Val(LblPack.Text))
                        TXTRETAILNOTAX.Text = IIf(IsNull(grdtmp.Columns(29)), "", Val(grdtmp.Columns(29)) * Val(LblPack.Text))
                    Case Else
                        txtretail.Text = IIf(IsNull(grdtmp.Columns(26)), "", Val(grdtmp.Columns(26)) * Val(LblPack.Text))
                        TXTRETAILNOTAX.Text = IIf(IsNull(grdtmp.Columns(26)), "", Val(grdtmp.Columns(26)) * Val(LblPack.Text))
                End Select
...
Function FILL_PREVIIOUSRATE()
    ...
    If PRERATE_FLAG = True Then
        PHY_PRERATE.Open "Select ITEM_CODE, VCH_DESC, VCH_DATE, QTY, P_RETAIL, M_USER_ID, VCH_NO, ITEM_NAME  From TRXFILE  WHERE TRX_TYPE ='DG' AND ITEM_CODE = '" & TXTITEMCODE.Text & "' AND M_USER_ID = '" & DataList2.BoundText & "' AND VCH_NO <> " & Val(txtBillNo.Text) & " ORDER BY VCH_DATE DESC", db, adOpenStatic, adLockReadOnly
        PRERATE_FLAG = False
    Else
        PHY_PRERATE.Close
        PHY_PRERATE.Open "Select ITEM_CODE, VCH_DESC, VCH_DATE, QTY, P_RETAIL, M_USER_ID, VCH_NO, ITEM_NAME  From TRXFILE  WHERE TRX_TYPE ='DG' AND ITEM_CODE = '" & TXTITEMCODE.Text & "' AND M_USER_ID = '" & DataList2.BoundText & "' AND VCH_NO <> " & Val(txtBillNo.Text) & " ORDER BY VCH_DATE DESC", db, adOpenStatic, adLockReadOnly
...
Private Sub TxtItemcode_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            Set grdtmp.DataSource = Nothing
            If PHYFLAG = True Then
                PHY.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, ITEM_COST, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE, MRP, CUST_DISC, CESS_PER, CESS_AMT, P_LWS, UN_BILL, P_RETAIL, PRICE5, PRICE6, PRICE7 From ITEMMAST  WHERE ITEM_CODE = '" & Me.TXTITEMCODE.Text & "' AND (ISNULL(DEAD_STOCK) OR DEAD_STOCK <> 'Y')", db, adOpenStatic, adLockReadOnly
                PHYFLAG = False
            Else
                PHY.Close
                PHY.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, ITEM_COST, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE, MRP, CUST_DISC, CESS_PER, CESS_AMT, P_LWS, UN_BILL, P_RETAIL, PRICE5, PRICE6, PRICE7 From ITEMMAST  WHERE ITEM_CODE = '" & Me.TXTITEMCODE.Text & "' AND (ISNULL(DEAD_STOCK) OR DEAD_STOCK <> 'Y')", db, adOpenStatic, adLockReadOnly
            ...
            If PHY.RecordCount = 0 Then
                ...
                If PHYFLAG = True Then
                    ...
                    PHY.Open "Select ITEM_CODE, ITEM_NAME, BAL_QTY, SALES_TAX, LINE_DISC, ITEM_COST, COM_FLAG, COM_PER, COM_AMT, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE, BARCODE, REF_NO, VCH_NO, LINE_NO, TRX_TYPE, ITEM_COST, SALES_PRICE, P_WS, P_VAN, CRTN_PACK, P_CRTN, MRP, TAX_MODE, EXP_DATE, TRX_YEAR, P_LWS, CUST_DISC,, P_RETAIL, PRICE5, PRICE6, PRICE7  From RTRXFILE  WHERE BARCODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY >0", db, adOpenStatic, adLockReadOnly
                    PHYFLAG = False
                Else
                    PHY.Close
                    PHY.Open "Select ITEM_CODE, ITEM_NAME, BAL_QTY, SALES_TAX, LINE_DISC, ITEM_COST, COM_FLAG, COM_PER, COM_AMT, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE, BARCODE, REF_NO, VCH_NO, LINE_NO, TRX_TYPE, ITEM_COST, SALES_PRICE, P_WS, P_VAN, CRTN_PACK, P_CRTN, MRP, TAX_MODE, EXP_DATE, TRX_YEAR, P_LWS, CUST_DISC, P_RETAIL, PRICE5, PRICE6, PRICE7  From RTRXFILE  WHERE BARCODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY >0", db, adOpenStatic, adLockReadOnly
                ...
                If PHY.RecordCount = 0 Then
                    If PHYFLAG = True Then
                        PHY.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, ITEM_COST, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE, MRP, CUST_DISC, CESS_PER, CESS_AMT, P_LWS, UN_BILL, P_RETAIL, PRICE5, PRICE6, PRICE7 From ITEMMAST  WHERE BARCODE = '" & Me.TXTITEMCODE.Text & "' ", db, adOpenStatic, adLockReadOnly
                        PHYFLAG = False
                    Else
                        PHY.Close
                        PHY.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, ITEM_COST, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE, MRP, CUST_DISC, CESS_PER, CESS_AMT, P_LWS, UN_BILL, P_RETAIL, PRICE5, PRICE6, PRICE7 From ITEMMAST  WHERE BARCODE = '" & Me.TXTITEMCODE.Text & "' ", db, adOpenStatic, adLockReadOnly
...
'                TXTRETAILNOTAX.text = IIf(IsNull(grdtmp.Columns(5)), "", Val(grdtmp.Columns(5)) * Val(LblPack.text))
                ...
                Select Case lblcusttype.Caption
                    Case "R"
                        txtretail.Text = IIf(IsNull(grdtmp.Columns(32)), "", Val(grdtmp.Columns(32)) * Val(LblPack.Text))
                        TXTRETAILNOTAX.Text = IIf(IsNull(grdtmp.Columns(32)), "", Val(grdtmp.Columns(32)) * Val(LblPack.Text))
                    Case "W"
                        txtretail.Text = IIf(IsNull(grdtmp.Columns(22)), "", Val(grdtmp.Columns(22)) * Val(LblPack.Text))
                        TXTRETAILNOTAX.Text = IIf(IsNull(grdtmp.Columns(22)), "", Val(grdtmp.Columns(22)) * Val(LblPack.Text))
                    Case "V"
                        txtretail.Text = IIf(IsNull(grdtmp.Columns(23)), "", Val(grdtmp.Columns(23)) * Val(LblPack.Text))
                        TXTRETAILNOTAX.Text = IIf(IsNull(grdtmp.Columns(23)), "", Val(grdtmp.Columns(23)) * Val(LblPack.Text))
                    Case "M"
                        txtretail.Text = IIf(IsNull(grdtmp.Columns(26)), "", Val(grdtmp.Columns(26)) * Val(LblPack.Text))
                        TXTRETAILNOTAX.Text = IIf(IsNull(grdtmp.Columns(26)), "", Val(grdtmp.Columns(26)) * Val(LblPack.Text))
                    Case "5"
                        txtretail.Text = IIf(IsNull(grdtmp.Columns(33)), "", Val(grdtmp.Columns(33)) * Val(LblPack.Text))
                        TXTRETAILNOTAX.Text = IIf(IsNull(grdtmp.Columns(33)), "", Val(grdtmp.Columns(33)) * Val(LblPack.Text))
                    Case "6"
                        txtretail.Text = IIf(IsNull(grdtmp.Columns(34)), "", Val(grdtmp.Columns(34)) * Val(LblPack.Text))
                        TXTRETAILNOTAX.Text = IIf(IsNull(grdtmp.Columns(34)), "", Val(grdtmp.Columns(34)) * Val(LblPack.Text))
                    Case "7"
                        txtretail.Text = IIf(IsNull(grdtmp.Columns(35)), "", Val(grdtmp.Columns(35)) * Val(LblPack.Text))
                        TXTRETAILNOTAX.Text = IIf(IsNull(grdtmp.Columns(35)), "", Val(grdtmp.Columns(35)) * Val(LblPack.Text))
                    Case Else
                        txtretail.Text = IIf(IsNull(grdtmp.Columns(32)), "", Val(grdtmp.Columns(32)) * Val(LblPack.Text))
                        TXTRETAILNOTAX.Text = IIf(IsNull(grdtmp.Columns(32)), "", Val(grdtmp.Columns(32)) * Val(LblPack.Text))
                End Select
                ...
                lblretail.Caption = IIf(IsNull(grdtmp.Columns(32)), "", grdtmp.Columns(32))
...
Function FILL_PREVIIOUSRATE2()
    ...
    If PRERATE_FLAG = True Then
        'PHY_PRERATE.Open "Select TOP 10 ITEM_CODE, VCH_DESC, VCH_DATE, QTY, P_RETAIL, M_USER_ID, VCH_NO, ITEM_NAME  From TRXFILE  WHERE TRX_TYPE ='DG' AND ITEM_CODE = '" & TXTITEMCODE.Text & "' AND M_USER_ID = '" & DataList2.BoundText & "' AND VCH_NO <> " & Val(txtBillNo.Text) & " ORDER BY VCH_DATE DESC", db, adOpenStatic, adLockReadOnly
...
Private Function checklastbill()
    ...
    Set rstBILL = New ADODB.Recordset
    rstBILL.Open "Select MAX(VCH_NO) From DAMAGE_MAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE = 'DG'", db, adOpenForwardOnly
    If Not (rstBILL.EOF And rstBILL.BOF) Then
        txtBillNo.Text = IIf(IsNull(rstBILL.Fields(0)), 1, rstBILL.Fields(0) + 1)
    End If
    rstBILL.Close
...
'    End If
    ...
    Dim RSTtax As ADODB.Recordset
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "SELECT DISTINCT SALES_TAX From TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & " AND SALES_TAX > 0 order by SALES_TAX", db, adOpenStatic, adLockReadOnly
    Do Until RSTTRXFILE.EOF
        TaxAmt = 0
        taxableamt = 0
        Set RSTtax = New ADODB.Recordset
        RSTtax.Open "Select * From TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND SALES_TAX = " & RSTTRXFILE!SALES_TAX & " AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & "", db, adOpenStatic, adLockReadOnly, adCmdText
        Do Until RSTtax.EOF
        ...
        Case vbKeyReturn
            Select Case grdsales.Col
                ...
                Case 3 'Qty
                    ...
                    db.Execute "Update TRXFILE set QTY = " & Val(grdsales.TextMatrix(grdsales.Row, 3)) & ", TRX_TOTAL = " & Val(grdsales.TextMatrix(grdsales.Row, 12)) & ", SCHEME = " & Val(TXTDISC.Tag) & " WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(grdsales.Row, 32)) & ""
                    db.Execute "Update TRXMAST set COMM_AMT = 0, PAY_AMOUNT = " & Val(LBLTOTALCOST.Caption) & ", ADD_AMOUNT = " & Val(LBLRETAMT.Caption) & ", DISCOUNT = " & Val(TXTTOTALDISC.Text) & ", DISC_PERS  = " & Val(TXTTOTALDISC.Text) & ", VCH_AMOUNT = " & Val(LBLTOTAL.Caption) & ", NET_AMOUNT = " & Val(lblnetamount.Caption) & " WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & ""
                    db.Execute "COMMIT"
                ...
                Case 5  'MRP
                    ...
                    db.Execute "START TRANSACTION"
                    db.Execute "Update TRXFILE set MRP = " & Val(grdsales.TextMatrix(grdsales.Row, 5)) & " WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(grdsales.Row, 32)) & ""
                    db.Execute "COMMIT"
                    grdsales.Enabled = True
                    TXTsample.Visible = False
                    grdsales.SetFocus
                ...
                Case 8  'Disc
                    ...
                    db.Execute "START TRANSACTION"
                    db.Execute "Update TRXFILE set LINE_DISC = " & Val(grdsales.TextMatrix(grdsales.Row, 8)) & ", TRX_TOTAL = " & Val(grdsales.TextMatrix(grdsales.Row, 12)) & " WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(grdsales.Row, 32)) & ""
                    db.Execute "Update TRXMAST set COMM_AMT = 0, PAY_AMOUNT = " & Val(LBLTOTALCOST.Caption) & ", ADD_AMOUNT = " & Val(LBLRETAMT.Caption) & ", DISCOUNT = " & Val(TXTTOTALDISC.Text) & ", DISC_PERS  = " & Val(TXTTOTALDISC.Text) & ", VCH_AMOUNT = " & Val(LBLTOTAL.Caption) & ", NET_AMOUNT = " & Val(lblnetamount.Caption) & " WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & ""
                    db.Execute "COMMIT"
                    grdsales.Enabled = True
                    TXTsample.Visible = False
                ...
                Case 24  'Commission
                    ...
                    db.Execute "START TRANSACTION"
                    db.Execute "Update TRXFILE set COM_AMT = " & Val(grdsales.TextMatrix(grdsales.Row, 24)) & "  WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(grdsales.Row, 32)) & ""
                    db.Execute "Update TRXMAST set COMM_AMT = " & Val(lblcomamt.Text) & ", PAY_AMOUNT = " & Val(LBLTOTALCOST.Caption) & ", ADD_AMOUNT = " & Val(LBLRETAMT.Caption) & ", DISCOUNT = " & Val(TXTTOTALDISC.Text) & ", DISC_PERS  = " & Val(TXTTOTALDISC.Text) & ", VCH_AMOUNT = " & Val(LBLTOTAL.Caption) & ", NET_AMOUNT = " & Val(lblnetamount.Caption) & " WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & ""
...
'    End If
    ...
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * From TRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & " ", db, adOpenStatic, adLockOptimistic, adCmdText
    Do Until RSTTRXFILE.EOF
        RSTTRXFILE!VCH_DATE = Format(TXTINVDATE.Text, "DD/MM/YYYY")
        RSTTRXFILE!M_USER_ID = DataList2.BoundText
    ...
    If grdsales.rows = 1 Then
        If OLD_BILL = True Then
            ...
            If Val(txtBillNo.Text) >= LASTBILL Then
                db.Execute "delete From DAMAGE_MAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & ""
            End If
        End If
'        Set RSTTRXFILE = New ADODB.Recordset
'        RSTTRXFILE.Open "SELECT *  FROM QTNMAST WHERE BILL_NO = " & Val(txtBillNo.Text) & " AND BILLTYPE = 'DG' ", db, adOpenStatic, adLockOptimistic, adCmdText
'        If Not (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
'            RSTTRXFILE!BILL_NO = Null
...
'        Set RSTTRXFILE = Nothing
    ...
    db.Execute "START TRANSACTION"
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * From DAMAGE_MAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & "", db, adOpenStatic, adLockOptimistic, adCmdText
    If (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
        RSTTRXFILE.AddNew
        RSTTRXFILE!VCH_NO = Val(txtBillNo.Text)
        RSTTRXFILE!TRX_TYPE = "DG"
...
'    Set RSTTRXFILE = New ADODB.Recordset
'    If OLD_BILL = False And Val(txtBillNo.Text) <> 1 Then
'        RSTTRXFILE.Open "Select * From DAMAGE_MAST WHERE VCH_NO= (SELECT MAX(VCH_NO) From DAMAGE_MAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE = 'DG')", db, adOpenStatic, adLockOptimistic, adCmdText
'        db.Execute "Lock tables TRXMAST write"
'        txtBillNo.Text = RSTTRXFILE!VCH_NO + 1
'        LBLBILLNO.Caption = Val(txtBillNo.Text)
'        db.BeginTrans
...
'    db.Execute "unlock tables"
    ...
    'db.Execute "delete from TEMPTRXFILE WHERE VCH_NO = " & Val(txtBillNo.Text) & " AND TRX_TYPE = 'DG' AND TRX_YEAR = '" & Year(MDIMAIN.DTFROM.value) & "' "
...
SKIP_UNBILL:
    ...
    rstTRXMAST.Open "SELECT * from RTRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(TxtCN.Text) & "", db, adOpenStatic, adLockOptimistic, adCmdText
    ...
    Taxsplit = ""
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "SELECT DISTINCT SALES_TAX From TEMPTRXFILE WHERE TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & " order by SALES_TAX", db, adOpenStatic, adLockReadOnly
    Do Until RSTTRXFILE.EOF
        TaxAmt = 0
        taxableamt = 0
        Set RSTtax = New ADODB.Recordset
        RSTtax.Open "Select * From TEMPTRXFILE WHERE SALES_TAX = " & RSTTRXFILE!SALES_TAX & " AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & "", db, adOpenStatic, adLockReadOnly, adCmdText
        Do Until RSTtax.EOF
            If OPTDISCPERCENT.Value = True Then
                grdtmp.Tag = (RSTtax!PTR - (RSTtax!PTR * RSTtax!LINE_DISC) / 100) - ((RSTtax!PTR - (RSTtax!PTR * RSTtax!LINE_DISC) / 100) * Val(TXTTOTALDISC.Text) / 100)
            Else
...
'    End If
    If Val(MDIMAIN.StatusBar.Panels(11).Text) = 1 Then
        If Trim(lblIGST.Caption) <> "Y" Then
            If Small_Print = True Then
                ...
                ReportNameVar = Rptpath & "RPTDMA51"
            Else
                ReportNameVar = Rptpath & "RPTDM1"
            End If
        Else
            ReportNameVar = Rptpath & "RPTDMW1"
        End If
        ...
        If Val(lblnetamount.Caption) >= Val(MDIMAIN.LBLHSNSUM.Caption) Or Trim(lblIGST.Caption) = "Y" Then
            For i = 1 To Report.OpenSubreport("RPTHSN.rpt").Database.Tables.COUNT
                ...
                End If
            Next i
            Report.OpenSubreport("RPTHSN.rpt").RecordSelectionFormula = "({TRXFILE.TRX_TYPE}= 'DG' AND {TRXFILE.VCH_NO}= " & Val(txtBillNo.Text) & " )"
    ...
    Else
        If Trim(lblIGST.Caption) <> "Y" Then
            ...
            If Small_Print = True Then
                'ReportNameVar = Rptpath & "rptbillretail"
                ReportNameVar = Rptpath & "RPTDMA5"
            Else
                ReportNameVar = Rptpath & "rptDM"
            End If
...
'            End If
        Else
            If Small_Print = True Then
                ReportNameVar = Rptpath & "RPTDMWA5"
            Else
                ReportNameVar = Rptpath & "RPTDMW"
            End If
        End If
        Set Report = crxApplication.OpenReport(ReportNameVar, 1)
        ...
        For i = 1 To Report.OpenSubreport("RPTBILL1.rpt").Database.Tables.COUNT
            ...
            End If
        Next i
        Report.OpenSubreport("RPTBILL1.rpt").RecordSelectionFormula = "({TRXFILE.TRX_TYPE}= 'DG' AND {TRXFILE.VCH_NO}= " & Val(txtBillNo.Text) & ")"
        ...
        If Val(MDIMAIN.StatusBar.Panels(11).Text) > 1 Then
            For i = 1 To Report.OpenSubreport("RPTBILL2.rpt").Database.Tables.COUNT
                ...
                End If
            Next i
            Report.OpenSubreport("RPTBILL2.rpt").RecordSelectionFormula = "({TRXFILE.TRX_TYPE}= 'DG' AND {TRXFILE.VCH_NO}= " & Val(txtBillNo.Text) & ")"
        ...
        If Val(MDIMAIN.StatusBar.Panels(11).Text) > 2 Then
            For i = 1 To Report.OpenSubreport("RPTBILL3.rpt").Database.Tables.COUNT
                ...
                End If
            Next i
            Report.OpenSubreport("RPTBILL3.rpt").RecordSelectionFormula = "({TRXFILE.TRX_TYPE}= 'DG' AND {TRXFILE.VCH_NO}= " & Val(txtBillNo.Text) & ")"
        ...
        If Val(MDIMAIN.StatusBar.Panels(11).Text) > 3 Then
            For i = 1 To Report.OpenSubreport("RPTBILL4.rpt").Database.Tables.COUNT
                ...
                End If
            Next i
            Report.OpenSubreport("RPTBILL4.rpt").RecordSelectionFormula = "({TRXFILE.TRX_TYPE}= 'DG' AND {TRXFILE.VCH_NO}= " & Val(txtBillNo.Text) & ")"
    ...
    Select Case KeyCode
        Case vbKeyReturn, vbKeyEscape
            ...
            For i = 1 To grdsales.rows - 1
                ...
                db.Execute "Update TRXFILE set COM_AMT = " & Val(grdsales.TextMatrix(i, 24)) & " WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.Value) & "' AND TRX_TYPE='DG' AND VCH_NO = " & Val(txtBillNo.Text) & " AND LINE_NO = " & Val(grdsales.TextMatrix(i, 32)) & " "
