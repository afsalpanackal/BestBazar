...
'    End If
    ...
    If Dos_Print = False Then
        ...
        For i = 1 To Report.Database.Tables.Count
            Report.Database.Tables(i).SetLogOnInfo "ConnectionName", "G:\dbase\YEAR14-15\INVSOFT.SQL", "admin", "!@#$%^&*())(*&^%$#@!"
        Next i
...
Private Sub GRDPOPUP_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            If Val(lblpack.Caption) <> 1 Then OptLoose.value = True
            Exit Sub
...
Private Sub TXTSLNO_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn, vbKeyTab
            If Trim(TXTTIN.Text) = "" Then
                MsgBox "8B Bill Not allowed", vbOKOnly, "Sales"
                Exit Sub
            End If
...
Private Function AppendSale()
    ...
    i = 0
    If grdsales.Rows = 1 Then
        db.Execute "delete * FROM CASHATRXFILE WHERE INV_NO = " & Val(LBLBILLNO.Caption) & " AND INV_TYPE = 'RT' AND INV_TRX_TYPE = 'SI'"
    Else
        Set RSTITEMMAST = New ADODB.Recordset
        RSTITEMMAST.Open "SELECT * FROM CASHATRXFILE WHERE INV_NO = " & Val(LBLBILLNO.Caption) & " AND INV_TYPE = 'RT' AND INV_TRX_TYPE = 'SI'", db, adOpenStatic, adLockOptimistic, adCmdText
...
SKIP:
    ...
    If lblcredit.Caption <> "0" Then
        ...
        Set RSTITEMMAST = New ADODB.Recordset
        RSTITEMMAST.Open "SELECT * FROM DBTPYMT WHERE INV_NO = " & Val(LBLBILLNO.Caption) & " AND TRX_TYPE = 'DR' AND INV_TRX_TYPE = 'SI'", db, adOpenStatic, adLockOptimistic, adCmdText
        If (RSTITEMMAST.EOF And RSTITEMMAST.BOF) Then
            RSTITEMMAST.AddNew
            RSTITEMMAST!TRX_TYPE = "DR"
            RSTITEMMAST!INV_TRX_TYPE = "SI"
...
'
'    Set RSTITEMMAST = New ADODB.Recordset
'    RSTITEMMAST.Open "SELECT * FROM CRDTPYMT WHERE INV_NO = " & Val(txtBillNo.Text) & " AND TRX_TYPE = 'DR' AND INV_TRX_TYPE = 'RT'", db, adOpenStatic, adLockOptimistic, adCmdText
'    If (RSTITEMMAST.EOF And RSTITEMMAST.BOF) Then
'        RSTITEMMAST.AddNew
'        RSTITEMMAST!TRX_TYPE = "DR"
'        RSTITEMMAST!CR_NO = i
