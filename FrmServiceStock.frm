...
Begin VB.Form FRMServiceStk 
   ...
   End
End
Attribute VB_Name = "FRMServiceStk"
...
Private Sub Form_Load()
    ...
    rstBILL.Open "Select MAX(VCH_NO) From SERVICE_STK", db, adOpenStatic, adLockReadOnly
...
Dim chkflag As Integer
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            TRXSUB.Open "Select * From SERVICE_STK WHERE VCH_NO = " & Val(txtBillNo.text) & " ORDER BY LINE_NO", db, adOpenStatic, adLockReadOnly
            Do Until TRXSUB.EOF
                    i = i + 1
                    grdsales.rows = grdsales.rows + 1
                    grdsales.FixedRows = 1
...
Private Sub txtBillNo_LostFocus()
    ...
    TRXDN.Open "Select MAX(VCH_NO) From SERVICE_STK", db, adOpenStatic, adLockReadOnly
...
Public Function AppendSale()
    ...
    i = 0
    On Error GoTo ErrHand
    db.Execute "delete From SERVICE_STK WHERE VCH_NO = " & Val(txtBillNo.text) & ""
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * From SERVICE_STK", db, adOpenStatic, adLockOptimistic, adCmdText
    For i = 1 To grdsales.rows - 1
        RSTTRXFILE.AddNew
    ...
    db.Execute "delete From TRXFILE WHERE TRX_TYPE='SR' AND VCH_NO = " & Val(txtBillNo.text) & ""
    Set RSTTRXFILE = New ADODB.Recordset
    ...
    txtBillNo.text = 1
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select MAX(VCH_NO) From SERVICE_STK", db, adOpenStatic, adLockReadOnly
    If Not (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
