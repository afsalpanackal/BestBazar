...
'    End If
    ...
    RSTTRXFILE.Open "SELECT *  FROM TEMPCN WHERE ACT_CODE = '" & Trim(DataList2.BoundText) & "' AND BILL_NO = " & Val(txtBillNo.Text) & " AND BILL_TRX_TYPE = 'WO' AND TRX_TYPE = 'SI'", db, adOpenStatic, adLockOptimistic, adCmdText
...
Private Sub CmdPrintA5_Click()
    ...
    If Month(Date) >= 5 And Year(Date) = 2016 Then Exit Sub
    If Month(MDIMAIN.DTFROM.value) >= 5 And Year(MDIMAIN.DTFROM.value) = 2016 Then
...
Private Sub Command1_Click()
    ...
    rstTRXMAST.Open "SELECT * from [RTRXFILE] WHERE TRX_TYPE='WO' AND VCH_NO = " & Val(TxtCN.Text) & "", db, adOpenStatic, adLockOptimistic, adCmdText
    ...
    ElseIf Figre > 0 Then
        LBLFOT.Tag = "(Rupees " & Words_1_all(Num) & " Only)"
    End If
    ReportNameVar = App.Path & "\RPTcommission.RPT"
    ...
    For i = 1 To Report.Database.Tables.Count
        Report.Database.Tables(i).SetLogOnInfo "ConnectionName", "D:\EzBiz\INVSOFT.SQL", "admin", "###DATABASE%%%RET"
    Next i
...
Private Sub Command2_Click()
    ...
    Dim E_DATE As Date
    Dim lastbillnum As Double
    i = 0
    On Error GoTo eRRHAND
    
    lastbillnum = Val(txtBillNo.Text)
    ...
    db.Execute "delete * From DBTPYMT WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE='DR' AND INV_NO = " & Val(LBLBILLNO.Caption) & " AND INV_TRX_TYPE = 'WO'"
    db.Execute "delete  * From BANK_TRX WHERE B_TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND B_VCH_NO = " & Val(LBLBILLNO.Caption) & " AND B_TRX_TYPE = 'WO' "
    db.Execute "delete  * From DBTPYMT WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND INV_NO = " & Val(LBLBILLNO.Caption) & " AND TRX_TYPE = 'RT' AND INV_TRX_TYPE = 'WO' "
...
'    Set RSTITEMMAST = Nothing
    ...
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * From SALESLEDGER WHERE TRX_TYPE='RI' AND INV_NO = " & Val(txtBillNo.Text) & "", db, adOpenStatic, adLockOptimistic, adCmdText
    If Not (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
    ...
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * From ATRXFILE  WHERE TRX_TYPE = 'RI' AND VCH_DATE = # " & E_DATE & " # ", db, adOpenStatic, adLockOptimistic, adCmdText
    If Not (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
...
'    Set RSTITEMMAST = Nothing
    ...
    LBLBILLNO.Caption = lastbillnum + 1
    txtBillNo.Text = lastbillnum + 1
    OLD_BILL = False
...
'    End If
    ...
    If DataList2.BoundText <> "130000" Then
        'If lblcredit.Caption <> "0" Then
        db.Execute "delete  * From DBTPYMT WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND INV_NO = " & Val(LBLBILLNO.Caption) & " AND TRX_TYPE = 'DR' AND INV_TRX_TYPE = 'WO' "
        Set rstMaxRec = New ADODB.Recordset
        rstMaxRec.Open "Select MAX(Val(CR_NO)) From DBTPYMT", db, adOpenForwardOnly
        If Not (rstMaxRec.EOF And rstMaxRec.BOF) Then
            i = IIf(IsNull(rstMaxRec.Fields(0)), 1, rstMaxRec.Fields(0) + 1)
        ...
        Set RSTITEMMAST = New ADODB.Recordset
        RSTITEMMAST.Open "SELECT * FROM DBTPYMT WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND INV_NO = " & Val(LBLBILLNO.Caption) & " AND TRX_TYPE = 'DR' AND INV_TRX_TYPE = 'WO'", db, adOpenStatic, adLockOptimistic, adCmdText
        If (RSTITEMMAST.EOF And RSTITEMMAST.BOF) Then
            RSTITEMMAST.AddNew
            RSTITEMMAST!TRX_TYPE = "DR"
        ...
        If lblcredit.Caption = "0" Then
            Set RSTTRXFILE = New ADODB.Recordset
            RSTTRXFILE.Open "Select * From DBTPYMT WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND INV_NO = " & Val(LBLBILLNO.Caption) & " AND TRX_TYPE = 'RT' AND INV_TRX_TYPE = 'WO' ", db, adOpenStatic, adLockOptimistic, adCmdText
            If (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
                RSTTRXFILE.AddNew
                RSTTRXFILE!TRX_YEAR = Year(MDIMAIN.DTFROM.value)
                RSTTRXFILE!TRX_TYPE = "RT"
    ...
    If DataList2.BoundText <> "130000" Then
        ...
        Set RSTTRXFILE = New ADODB.Recordset
        'db.Execute "delete  * From DBTPYMT WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND INV_NO = " & Val(LBLBILLNO.Caption) & " AND TRX_TYPE = 'RT' AND INV_TRX_TYPE = 'WO' "
        RSTTRXFILE.Open "SELECT * From DBTPYMT WHERE ([ACT_CODE] = '" & DataList2.BoundText & "' AND ([TRX_TYPE] = 'RT' OR [TRX_TYPE] = 'SR')) AND NOT(TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND INV_NO = " & Val(LBLBILLNO.Caption) & " AND TRX_TYPE = 'RT' AND INV_TRX_TYPE = 'WO') ", db, adOpenForwardOnly
        'RSTTRXFILE.Open "Select * From DBTPYMT WHERE ACT_CODE = '" & DataList2.BoundText & "' AND (TRX_TYPE = 'RT' OR TRX_TYPE = 'SR') AND NOT(TRX_TYPE= 'RT' AND INV_TRX_TYPE ='RT' AND TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND INV_NO = " & Val(txtBillNo.Text) & ") ", db, adOpenStatic, adLockReadOnly, adCmdText
        Do Until RSTTRXFILE.EOF
        ...
        m_Bal_Amt = 0
        Set RSTTRXFILE = New ADODB.Recordset
        RSTTRXFILE.Open "Select * From DBTPYMT WHERE ACT_CODE = '" & DataList2.BoundText & "' AND INV_NO <> " & Val(LBLBILLNO.Caption) & " AND TRX_TYPE = 'DR'", db, adOpenStatic, adLockReadOnly, adCmdText
        Do Until RSTTRXFILE.EOF
    ...
    If Trim(TXTTIN.Text) <> "" Then
        If Small_Print = True Then
            ReportNameVar = App.Path & "\RPTESTIMATEA5.RPT"
        Else
            ReportNameVar = App.Path & "\RPTESTIMATE.RPT"
        End If
    Else
        If Small_Print = True Then
            ReportNameVar = App.Path & "\RPTESTIMATEA5.RPT"
...
Private Sub Form_Load()
    ...
    Set rstBILL = New ADODB.Recordset
    rstBILL.Open "Select MAX(Val(VCH_NO)) From TRXMAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE = 'WO'", db, adOpenStatic, adLockReadOnly
    If Not (rstBILL.EOF And rstBILL.BOF) Then
        txtBillNo.Text = IIf(IsNull(rstBILL.Fields(0)), 1, rstBILL.Fields(0) + 1)
...
Private Sub txtBillNo_LostFocus()
    ...
    On Error GoTo eRRHAND
    Set TRXMAST = New ADODB.Recordset
    TRXMAST.Open "Select MAX(Val(VCH_NO)) FROM TRXMAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE = 'WO'", db, adOpenStatic, adLockReadOnly
    If Not (TRXMAST.EOF And TRXMAST.BOF) Then
...
Private Sub TXTPRODUCT_Change()
        ...
        If PHYFLAG = True Then
            ...
            PHY.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE, MRP, BIN_LOCATION  From ITEMMAST  WHERE ITEM_NAME Like '%" & Trim(Me.TXTPRODUCT.Text) & "%' AND ITEM_NAME Like '%" & Trim(Me.TxtName1.Text) & "%' ORDER BY [ITEM_NAME] ", db, adOpenStatic, adLockReadOnly
            PHYFLAG = False
        Else
            PHY.Close
            PHY.Open "Select ITEM_CODE, ITEM_NAME, CLOSE_QTY, SALES_PRICE, SALES_TAX, UNIT, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY, WARRANTY_TYPE, MRP, BIN_LOCATION  From ITEMMAST  WHERE ITEM_NAME Like '%" & Trim(Me.TXTPRODUCT.Text) & "%' AND ITEM_NAME Like '%" & Trim(Me.TxtName1.Text) & "%' ORDER BY [ITEM_NAME] ", db, adOpenStatic, adLockReadOnly
...
Private Function AppendSale()
    ...
    i = 0
    If grdsales.Rows = 1 Then
        db.Execute "delete * FROM CASHATRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND INV_NO = " & Val(LBLBILLNO.Caption) & " AND INV_TYPE = 'RT' AND INV_TRX_TYPE = 'WO'"
    Else
        Set RSTITEMMAST = New ADODB.Recordset
        RSTITEMMAST.Open "SELECT * FROM CASHATRXFILE WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND INV_NO = " & Val(LBLBILLNO.Caption) & " AND INV_TYPE = 'RT' AND INV_TRX_TYPE = 'WO'", db, adOpenStatic, adLockOptimistic, adCmdText
        If (RSTITEMMAST.EOF And RSTITEMMAST.BOF) Then
            RSTITEMMAST.AddNew
            RSTITEMMAST!rec_no = i + 1
            RSTITEMMAST!INV_TYPE = "RT"
    ...
    If grdsales.Rows = 1 Then
        If OLD_BILL = True Then
            ...
            LASTBILL = 1
            Set rstBILL = New ADODB.Recordset
            rstBILL.Open "Select MAX(Val(VCH_NO)) From TRXMAST WHERE TRX_YEAR='" & Year(MDIMAIN.DTFROM.value) & "' AND TRX_TYPE = 'WO'", db, adOpenForwardOnly
            If Not (rstBILL.EOF And rstBILL.BOF) Then
                LASTBILL = IIf(IsNull(rstBILL.Fields(0)), 1, rstBILL.Fields(0))
            End If
            rstBILL.Close
...
Private Sub TXTTYPE_KeyDown(KeyCode As Integer, Shift As Integer)
    Select Case KeyCode
        Case vbKeyReturn, 40
            If Val(TXTTYPE.Text) = 0 Or Val(TXTTYPE.Text) > 2 Then
