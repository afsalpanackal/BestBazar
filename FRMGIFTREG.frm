...
Begin VB.Form FRMGIFTREG 
   BackColor       =   &H00C0FFC0&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "BUSINESS PROMOTION SAMPLE REGISTER"
   ClientHeight    =   7935
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   10755
...
Private Sub CmDDisplay_Click()
    ...
    rstTRANX.Open "SELECT * From GIFTMAST WHERE VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' ORDER BY TRX_TYPE,VCH_DATE,VCH_NO", db, adOpenStatic, adLockReadOnly
    Do Until rstTRANX.EOF
        ...
        GRDTranx.TextMatrix(i, 6) = rstTRANX!TRX_YEAR
        GRDTranx.TextMatrix(i, 2) = rstTRANX!VCH_DATE
        GRDTranx.TextMatrix(i, 3) = IIf(IsNull(rstTRANX!PAY_AMOUNT), "", rstTRANX!PAY_AMOUNT)
        GRDTranx.TextMatrix(i, 4) = IIf(IsNull(rstTRANX!BILL_NAME), "", rstTRANX!BILL_NAME)
'        Set RSTTRXFILE = New ADODB.Recordset
'        RSTTRXFILE.Open "SELECT * From GIFTMAST WHERE VCH_NO = " & rstTRANX!VCH_NO & "", db, adOpenStatic, adLockReadOnly
...
Private Sub CMDPRINTREGISTER_Click()
    ...
    ReportNameVar = Rptpath & "RPTGIFTREP"
    Set Report = crxApplication.OpenReport(ReportNameVar, 1)
    
    Report.RecordSelectionFormula = "({TRXMAST.TRX_TYPE}='GF' AND {TRXMAST.VCH_DATE} <=# " & Format(DTTo.Value, "MM,DD,YYYY") & " # AND {TRXMAST.VCH_DATE} >=# " & Format(DTFROM.Value, "MM,DD,YYYY") & " # )"
...
Private Sub Form_Load()
    ...
    GRDBILL.TextMatrix(0, 3) = "Cost"
    GRDBILL.TextMatrix(0, 4) = "Qty"
    GRDBILL.TextMatrix(0, 5) = "Value"
...
Private Sub GRDTranx_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            i = 0
            Set RSTTRXFILE = New ADODB.Recordset
            RSTTRXFILE.Open "Select * From TRXFILE WHERE VCH_NO = " & Val(LBLBILLNO.Caption) & " AND TRX_TYPE = '" & Trim(GRDTranx.TextMatrix(GRDTranx.Row, 5)) & "' AND TRX_YEAR = '" & Trim(GRDTranx.TextMatrix(GRDTranx.Row, 6)) & "'", db, adOpenStatic, adLockReadOnly, adCmdText
            Do Until RSTTRXFILE.EOF
                i = i + 1
                ...
                GRDBILL.TextMatrix(i, 3) = Format(RSTTRXFILE!ITEM_COST, "0.00")
                GRDBILL.TextMatrix(i, 4) = RSTTRXFILE!QTY
                GRDBILL.TextMatrix(i, 5) = Format(RSTTRXFILE!ITEM_COST * RSTTRXFILE!QTY, "0.00")
