...
'            rstTRANX.Open "SELECT * From TRXEXPMAST WHERE VCH_DATE <= '" & Format(DTTO.value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.value, "yyyy/mm/dd") & "' ORDER BY VCH_DATE,VCH_NO", db, adOpenStatic, adLockReadOnly
'            GoTo MASTER
'        ElseIf OPTCUSTOMER.Value = True Then
'            rstTRANX.Open "SELECT * From TRXEXPMAST WHERE ACT_CODE = '" & Dlstemployee.BoundText & "' ORDER BY VCH_DATE,VCH_NO", db, adOpenStatic, adLockReadOnly
'            GoTo MASTER
'        End If
'    End If
'    If OptEXP.Value = True Then
        ...
        Else
            rstTRANX.Open "SELECT * From TRXEXPENSE WHERE ACT_CODE = '" & Dlstemployee.BoundText & "' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' ORDER BY VCH_DATE,VCH_NO", db, adOpenStatic, adLockReadOnly
        End If
'    End If
...
'    oWB.Font.Bold = True
    ...
    oWS.Range("A" & 2).Value = "OFFICE EXPENSE REGISTER"
...
Private Sub CMDPRINTREGISTER_Click()
    ...
    ReportNameVar = Rptpath & "RPTOfficeExp"
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            If GRDTranx.TextMatrix(GRDTranx.Row, 6) = "M" Then
                RSTTRXFILE.Open "Select * From TRXEXPENSE WHERE VCH_NO = " & Val(LBLBILLNO.Caption) & " ORDER BY LINE_NO", db, adOpenStatic, adLockReadOnly, adCmdText
                ...
                GRDBILL.ColWidth(3) = 1800
            Else
                RSTTRXFILE.Open "Select * From TRXEXPMAST WHERE VCH_NO = " & Val(LBLBILLNO.Caption) & "", db, adOpenStatic, adLockReadOnly, adCmdText
                Do Until RSTTRXFILE.EOF
                    i = i + 1
                    GRDBILL.rows = GRDBILL.rows + 1
                    GRDBILL.FixedRows = 1
...
Private Sub TXTEMPLOYEE_Change()
    ...
    If empflag.Caption <> "1" Then
        If EMP_FLAG = True Then
            EMP_REC.Open "select ACT_CODE, ACT_NAME from ACTMAST  WHERE (Mid(ACT_CODE, 1, 3)='641')And (LENGTH(ACT_CODE)>3) And ACT_NAME Like '" & Me.TXTEMPLOYEE.text & "%'ORDER BY ACT_NAME", db, adOpenStatic, adLockReadOnly, adCmdText
            EMP_FLAG = False
        Else
            EMP_REC.Close
            EMP_REC.Open "select ACT_CODE, ACT_NAME from ACTMAST  WHERE (Mid(ACT_CODE, 1, 3)='641')And (LENGTH(ACT_CODE)>3) And ACT_NAME Like '" & Me.TXTEMPLOYEE.text & "%'ORDER BY ACT_NAME", db, adOpenStatic, adLockReadOnly, adCmdText
