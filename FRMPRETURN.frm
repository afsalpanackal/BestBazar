...
Private Sub CMDADD_Click()
    ...
    With RSTTRXFILE
        If Not (.EOF And .BOF) Then
            !RCPT_QTY = !RCPT_QTY - Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 3)) * Val(Txtpack.Text)
            If (IsNull(!RCPT_VAL)) Then !RCPT_VAL = 0
            !RCPT_VAL = !RCPT_VAL - Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 11))
            !CLOSE_QTY = !CLOSE_QTY - Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 3)) * Val(Txtpack.Text)
    ...
    With RSTTRXFILE
        If Not (.EOF And .BOF) Then
            ...
            !ISSUE_QTY = !ISSUE_QTY + Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 3)) * Val(Txtpack.Text)
            
            If (IsNull(!BAL_QTY)) Then !BAL_QTY = 0
            !BAL_QTY = !BAL_QTY - Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 3)) * Val(Txtpack.Text)
...
Private Sub CmdPrint_Click()
    ...
    For i = 1 To grdsales.Rows - 1
        ...
        RSTTRXFILE!UNIT = 1
        RSTTRXFILE!LOOSE_PACK = Val(grdsales.TextMatrix(i, 4))
        ...
        RSTTRXFILE!LINE_DISC = Val(grdsales.TextMatrix(i, 7))
        RSTTRXFILE!SCHEME = 0
        RSTTRXFILE!EXP_DATE = Null 'grdsales.TextMatrix(i, 10)
...
GOSKIP:
    ...
    ReportNameVar = "D:\EzBiz\rptPR.RPT"
    ...
    For Each CRXFormulaField In CRXFormulaFields
        ...
        If CRXFormulaField.Name = "{@VCH_NO}" Then CRXFormulaField.Text = " '" & Trim(txtBillNo.Text) & "' & ' /PR /' & '" & Year(Date) & "' "
...
Private Sub cmdRefresh_Click()
    ...
    OLD_BILL = True
    If grdsales.Rows = 1 Then
        db.Execute "delete * FROM CRDTPYMT WHERE INV_NO = " & Val(txtBillNo.Text) & " AND TRX_TYPE= 'PR' AND INV_TRX_TYPE = 'PR'"
        GoTo SKIP
    End If

    db.Execute "delete * From PURCAHSERETURN WHERE VCH_NO = " & Val(txtBillNo.Text) & ""
    Set RSTTRXFILE = New ADODB.Recordset
    ...
    For i = 1 To grdsales.Rows - 1
        ...
        'RSTTRXFILE!LOOSE_PACK = grdsales.TextMatrix(i, 4)
        RSTTRXFILE!ITEM_COST = 0
        RSTTRXFILE!MRP = grdsales.TextMatrix(i, 5)
        RSTTRXFILE!PTR = grdsales.TextMatrix(i, 6)
    ...
    txtBillNo.Text = 1
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select MAX(Val(VCH_NO)) From PURCAHSERETURN", db, adOpenStatic, adLockReadOnly
    If Not (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
    ...
    rstMaxNo.Open "Select MAX(Val(CR_NO)) From CRDTPYMT", db, adOpenStatic, adLockReadOnly
    ...
    RSTITEMMAST.Open "SELECT * FROM CRDTPYMT WHERE INV_NO = " & Val(txtBillNo.Text) & " AND TRX_TYPE = 'PR' AND INV_TRX_TYPE = 'PR'", db, adOpenStatic, adLockOptimistic, adCmdText
    If (RSTITEMMAST.EOF And RSTITEMMAST.BOF) Then
        RSTITEMMAST.AddNew
        RSTITEMMAST!TRX_TYPE = "PR"
        RSTITEMMAST!INV_TRX_TYPE = "PR"
        RSTITEMMAST!CR_NO = i
...
Private Sub Form_Load()
    ...
    rstBILL.Open "Select MAX(Val(VCH_NO)) From PURCAHSERETURN", db, adOpenStatic, adLockReadOnly
...
Select Case KeyCode
    Case vbKeyReturn
        ...
        RSTDN.Open "Select * From PURCAHSERETURN WHERE VCH_NO = " & Val(txtBillNo.Text) & " ORDER BY LINE_NO", db, adOpenStatic, adLockReadOnly
        If RSTDN.RecordCount = 0 Then
            OLD_BILL = False
        Else
            OLD_BILL = True
        End If
        Do Until RSTDN.EOF
            ...
            grdsales.TextMatrix(i, 6) = Format(RSTDN!PTR, ".000")
...
Private Sub txtBillNo_LostFocus()
    ...
    TRXDN.Open "Select MAX(Val(VCH_NO)) From PURCAHSERETURN", db, adOpenStatic, adLockReadOnly
...
Private Sub TXTDISC_LostFocus()
    ...
    TXTTAX.Tag = Val(TXTQTY.Text) * (Val(TXTRATE.Text) - Val(TXTRATE.Text) * Val(TXTDISC.Text) / 100) * Val(TXTTAX.Text) / 100
...
Private Sub TXTINVDATE_LostFocus()
    If TXTINVDATE.Text = "  /  /    " Then TXTINVDATE.Text = Format(Date, "DD/MM/YYYY")

    If Not IsDate(TXTINVDATE.Text) Then TXTINVDATE.Text = Format(Date, "DD/MM/YYYY")
    
    If Len(Trim(TXTINVDATE.Text)) < 10 Then TXTINVDATE.Text = Format(Date, "DD/MM/YYYY")
'''''''''''''''''''''''''''''''
End Sub
...
Function FILL_BILLGRID()
    ...
    If BILL_FLAG = True Then
        PHY_BILL.Open "Select VCH_NO, VCH_DATE, UNIT, QTY, MRP, PTR, SALES_TAX, ITEM_NAME, EXP_DATE, REF_NO, R_VCH_NO, R_TRX_TYPE, R_LINE_NO From BILLDETAILS ORDER BY [VCH_DATE]", db, adOpenStatic, adLockReadOnly
        BILL_FLAG = False
    Else
        PHY_BILL.Close
        PHY_BILL.Open "Select VCH_NO, VCH_DATE, UNIT, QTY, MRP, PTR, SALES_TAX, ITEM_NAME, EXP_DATE, REF_NO, R_VCH_NO, R_TRX_TYPE, R_LINE_NO From BILLDETAILS ORDER BY [VCH_DATE]", db, adOpenStatic, adLockReadOnly
...
Private Function FILLINVOICE()
    ...
    If INV_FLAG = True Then
        INV_REC.Open "Select DISTINCT VCH_NO From PURCAHSERETURN WHERE ACT_CODE = '" & DataList2.BoundText & "' ORDER BY VCH_NO", db, adOpenStatic, adLockReadOnly
        INV_FLAG = False
    Else
        INV_REC.Close
        INV_REC.Open "Select DISTINCT VCH_NO From PURCAHSERETURN WHERE ACT_CODE = '" & DataList2.BoundText & "' ORDER BY VCH_NO", db, adOpenStatic, adLockReadOnly
...
Private Function FILLBILLDB()
    ...
    Do Until TRXFILE.EOF
        ...
        TRXBILL.Open "SELECT *  FROM BILLDETAILS", db, adOpenStatic, adLockOptimistic, adCmdText
        B_FLAG = True
        TRXBILL.AddNew
...
Private Sub ReportGeneratION()
   ...
   ' On Error GoTo errHand
    ...
    Open "D:\EzBiz\Report.txt" For Output As #1 '//Report file Creation
CLOSEFILE:
...
'    Set RSTTRXFILE = Nothing
    ...
    'MsgBox "Report file generated at " & "D:\EzBiz\Report.txt" & vbCrLf & "Click Print Report Button to print on paper."
    Exit Sub
...
Private Function checklastbill()
    ...
    Set rstBILL = New ADODB.Recordset
    rstBILL.Open "Select MAX(Val(VCH_NO)) From PURCAHSERETURN ", db, adOpenForwardOnly
    If Not (rstBILL.EOF And rstBILL.BOF) Then
        txtBillNo.Text = IIf(IsNull(rstBILL.Fields(0)), 1, rstBILL.Fields(0) + 1)
    End If
    rstBILL.Close
