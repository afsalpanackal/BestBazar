...
Private Sub Form_Load()
    ...
    If MDIMAIN.lblgst.Caption = "C" Or MDIMAIN.lblgst.Caption = "N" Then
        ...
        LBLB2BSALES(11).Caption = "Sales (CS)"
        'LBLTOTAL(13).Caption = "Sales (C)"
...
Private Sub CmDDisplay_Click()
    ...
    LBLPAIDCASH2.Caption = ""
    LBLRCVDCASH2.Caption = ""
    ...
    Do Until rstTRANX.EOF
        Select Case rstTRANX!TRX_TYPE
            Case "SV"
                LBLSERVICE.Caption = Val(LBLSERVICE.Caption) + rstTRANX!NET_AMOUNT
            Case "HI"
...
'
    ...
    RCVDVAL = 0
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "SELECT SUM(AMOUNT) FROM CASHATRXFILE WHERE INV_TRX_TYPE <> 'WO' AND VCH_DATE < '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND CHECK_FLAG='S'", db, adOpenStatic, adLockReadOnly, adCmdText
    If Not (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
        RCVDVAL = RCVDVAL + IIf(IsNull(RSTTRXFILE.Fields(0)), 0, RSTTRXFILE.Fields(0))
        'RSTTRXFILE.MoveNext
    End If
    ...
    ISSVAL = 0
    lblopcash2.Caption = Round(OPVAL + (RCVDVAL - ISSVAL), 2)

    LBLPAIDCASH2.Caption = 0
    LBLRCVDCASH2.Caption = 0

    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "SELECT SUM(AMOUNT) FROM CASHATRXFILE WHERE CHECK_FLAG='S' AND INV_TRX_TYPE <> 'WO' AND VCH_DATE >= '" & Format(DTFROM.Value, "yyyy/mm/dd") & "' AND VCH_DATE <= '" & Format(DTTo.Value, "yyyy/mm/dd") & "' ", db, adOpenStatic, adLockReadOnly, adCmdText
    If Not (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
        LBLRCVDCASH2.Caption = IIf(IsNull(RSTTRXFILE.Fields(0)), 0, RSTTRXFILE.Fields(0))
    End If
    RSTTRXFILE.Close
    Set RSTTRXFILE = Nothing
    ...
    Else
        LBLWCLOCASH.Caption = Round(Val(lblopcash2.Caption) + (Val(LBLRCVDCASH2.Caption) - Val(LBLPAIDCASH2.Caption)), 2)
...
Private Sub LBLWCLOCASH_DblClick()
    ...
    Dim rstTRANX As ADODB.Recordset
    If LBLPETTYSALE(4).Visible = True Then
        lblopcash2.Visible = True
        LBLRCVDCASH2.Visible = True
        LBLPAIDCASH2.Visible = True
        LBLPETTYSALE(4).Visible = False
        LBLPETTY.Visible = False
        Label1(7).Visible = False
        LblPettyPur.Visible = False
        ...
        Set rstTRANX = Nothing
    Else
        lblopcash2.Visible = False
        LBLRCVDCASH2.Visible = False
        LBLPAIDCASH2.Visible = False
        LBLPETTYSALE(4).Visible = True
        LBLPETTY.Visible = True
        Label1(7).Visible = True
        LblPettyPur.Visible = True
...
Private Sub LBLWSALE_DblClick()
    If Frame2.Visible = True Then
        Frame2.Visible = False
    Else
        Frame2.Visible = True
    End If
    Call LBLWCLOCASH_DblClick
End Sub
