   ...
   Begin VB.Frame FRMEMAIN 
      ...
      Begin VB.Frame FRMEHEAD 
         ...
         Begin VB.ComboBox cmbtype 
            ...
            ForeColor       =   &H00004080&
            Height          =   315
            ItemData        =   "FRMWSALE.frx":030A
            Left            =   6855
            List            =   "FRMWSALE.frx":0317
...
Private Sub cmddeleteall_Click()
    ...
    If grdsales.Rows <> 1 Then If MsgBox("ARE YOU SURE YOU WANT TO CANCEL THE BILL!!!!!", vbYesNo, "DELETE!!!") = vbNo Then Exit Sub
...
Private Sub CMDSALERETURN_Click()
    ...
    lblnetamount.Caption = Round(Val(LBLTOTAL.Caption) - Val(TXTAMOUNT.Text), 0) + Val(LBLFOT.Caption)
...
Private Sub CMDADD_Click()
    ...
    If UCase(txtcategory.Text) = "SERVICE CHARGE" Then
        ...
        TXTAPPENDTOTAL.Text = Format((Round(Val(txtretail.Text), 3)) - Val(TXTDISC.Tag), ".000")
        grdsales.TextMatrix(Val(TXTSLNO.Text), 12) = Format(TXTAPPENDTOTAL.Text, ".000")
...
Private Sub TXTQTY_LostFocus()
    ...
    If Val(TXTRETAILNOTAX.Text) = 0 Then
        ...
        LBLSUBTOTAL.Caption = Format(Round((Val(TXTQTY.Text) * Val(txtretail.Text)) - Val(TXTDISC.Tag) + Val(TXTTAX.Tag), 2), ".000")
    Else
        ...
        LBLSUBTOTAL.Caption = Format(Round((Val(TXTQTY.Text) * Val(TXTRETAILNOTAX.Text)) - Val(TXTDISC.Tag) + Val(TXTTAX.Tag), 2), ".000")
    End If
...
Private Function AppendSale()
    ...
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * From SALESLEDGER WHERE TRX_TYPE='SI' AND INV_NO = " & Val(txtBillNo.Text) & "", db, adOpenStatic, adLockOptimistic, adCmdText
    If Not (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
...
Function FILL_BATCHGRID()
    ...
    If BATCH_FLAG = True Then
        PHY_BATCH.Open "Select REF_NO, BAL_QTY, VCH_NO, LINE_NO, TRX_TYPE, VCH_DATE, ITEM_NAME, WARRANTY, MRP, SALES_TAX, LINE_DISC, P_RETAIL, COM_FLAG, COM_PER, COM_AMT, CRTN_PACK, P_CRTN, P_WS, P_VAN, CHECK_FLAG, CATEGORY, LOOSE_PACK, PACK_TYPE, WARRANTY_TYPE From RTRXFILE  WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY > 0 ORDER BY VCH_DATE DESC", db, adOpenForwardOnly
...
Private Function checklastbill()
    ...
    Set rstBILL = New ADODB.Recordset
    rstBILL.Open "Select MAX(Val(VCH_NO)) From TRXMAST WHERE TRX_TYPE = 'SI'", db, adOpenForwardOnly
    If Not (rstBILL.EOF And rstBILL.BOF) Then
        txtBillNo.Text = IIf(IsNull(rstBILL.Fields(0)), 1, rstBILL.Fields(0) + 1)
    End If
    rstBILL.Close
...
'                            End If
                ...
                If UCase(txtcategory.Text) = "SERVICE CHARGE" Then
                    ...
                    Exit Function
                End If
                'TxtMRP.Text = IIf(IsNull(GRDPOPUP.Columns(8)), "", GRDPOPUP.Columns(8))
