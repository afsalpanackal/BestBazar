...
Private Sub CMDADD_Click()
    ...
    grdsales.TextMatrix(Val(TXTSLNO.Text), 4) = 1 'Val(TxtPack.Text)
    ...
    db.Execute "delete * From RTRXFILE WHERE RTRXFILE.TRX_TYPE = 'CN' AND RTRXFILE.VCH_NO = " & Val(txtBillNo.Text) & " "
    Set RSTTRXFILE = New ADODB.Recordset
    ...
    With RSTTRXFILE
        ...
        RSTTRXFILE!LINE_DISC = Val(TxtPack.Text)
        RSTTRXFILE!SCHEME = 0
...
Private Sub CmdPrint_Click()
    ...
    OLD_BILL = True
    db.Execute "delete * From TRXFILE WHERE TRX_TYPE='CN' AND VCH_NO = " & Val(txtBillNo.Text) & ""
    Set RSTTRXFILE = New ADODB.Recordset
...
Private Sub cmdRefresh_Click()
    ...
    If OLD_BILL = False Then Call checklastbill
    db.Execute "delete * From SALERETURN WHERE VCH_NO = " & Val(txtBillNo.Text) & ""
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select * From SALERETURN", db, adOpenStatic, adLockOptimistic, adCmdText
    For i = 1 To grdsales.Rows - 1
        RSTTRXFILE.AddNew
        RSTTRXFILE!VCH_NO = Val(txtBillNo.Text)
        RSTTRXFILE!VCH_DATE = Format(TXTINVDATE.Text, "DD/MM/YYYY")
        ...
        RSTTRXFILE!P_RETAIL = Round(Val(grdsales.TextMatrix(i, 6)) * 100 / (Val(grdsales.TextMatrix(i, 8)) + 100), 3)
    ...
    txtBillNo.Text = 1
    Set RSTTRXFILE = New ADODB.Recordset
    RSTTRXFILE.Open "Select MAX(Val(VCH_NO)) From SALERETURN", db, adOpenStatic, adLockReadOnly
    If Not (RSTTRXFILE.EOF And RSTTRXFILE.BOF) Then
...
Private Sub Form_Load()
    ...
    rstBILL.Open "Select MAX(Val(VCH_NO)) From SALERETURN", db, adOpenStatic, adLockReadOnly
...
Select Case KeyCode
    Case vbKeyReturn
        ...
        RSTDN.Open "Select * From SALERETURN WHERE VCH_NO = " & Val(txtBillNo.Text) & " ORDER BY LINE_NO", db, adOpenStatic, adLockReadOnly
        ...
        Do Until RSTDN.EOF
            ...
            grdsales.TextMatrix(i, 7) = IIf(IsNull(RSTDN!LINE_DISC), 0, Format(RSTDN!LINE_DISC, ".000")) 'DISC
...
Private Sub txtBillNo_LostFocus()
    ...
    TRXDN.Open "Select MAX(Val(VCH_NO)) From SALERETURN", db, adOpenStatic, adLockReadOnly
    ...
    Select Case KeyCode
        Case vbKeyReturn, vbKeyTab
            ...
            If Val(TXTSLNO.Text) < grdsales.Rows Then
                ...
                TxtPack.Text = 1 'grdsales.TextMatrix(Val(TXTSLNO.Text), 4)
...
Function FILL_BILLGRID()
    ...
    If BILL_FLAG = True Then
        PHY_BILL.Open "Select * From BILLDETAILS ORDER BY [VCH_DATE]", db, adOpenStatic, adLockReadOnly
        BILL_FLAG = False
    Else
        PHY_BILL.Close
        PHY_BILL.Open "Select * From BILLDETAILS ORDER BY [VCH_DATE]", db, adOpenStatic, adLockReadOnly
...
Private Function FILLINVOICE()
    ...
    If INV_FLAG = True Then
        INV_REC.Open "Select DISTINCT VCH_NO From SALERETURN WHERE ACT_CODE = '" & DataList2.BoundText & "' ORDER BY VCH_NO", db, adOpenStatic, adLockReadOnly
        INV_FLAG = False
    Else
        INV_REC.Close
        INV_REC.Open "Select DISTINCT VCH_NO From SALERETURN WHERE ACT_CODE = '" & DataList2.BoundText & "' ORDER BY VCH_NO", db, adOpenStatic, adLockReadOnly
...
Private Function FILLBILLDB()
    ...
    TRXFILE.Open "Select * From TRXFILE WHERE (TRX_TYPE='RI' OR TRX_TYPE='SI') AND CST <>2 AND ITEM_CODE = '" & TXTITEMCODE.Text & "' AND M_USER_ID = '" & DataList2.BoundText & "'", db, adOpenStatic, adLockReadOnly
...
'    On Error GoTo eRRhAND
    Select Case KeyCode
        Case vbKeyReturn
            ...
            Set grdtmp.DataSource = Nothing
            If PHYFLAG = True Then
                PHY.Open "Select DISTINCT [ITEM_CODE], [ITEM_NAME] From ITEMMAST  WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' ", db, adOpenStatic, adLockReadOnly
                PHYFLAG = False
            Else
                PHY.Close
                PHY.Open "Select DISTINCT [ITEM_CODE], [ITEM_NAME] From ITEMMAST  WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' ", db, adOpenStatic, adLockReadOnly
...
Private Function checklastbill()
    ...
    Set rstBILL = New ADODB.Recordset
    rstBILL.Open "Select MAX(Val(VCH_NO)) From SALERETURN ", db, adOpenForwardOnly
    If Not (rstBILL.EOF And rstBILL.BOF) Then
        txtBillNo.Text = IIf(IsNull(rstBILL.Fields(0)), 1, rstBILL.Fields(0) + 1)
    End If
    rstBILL.Close
