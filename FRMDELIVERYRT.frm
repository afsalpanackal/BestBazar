...
Begin VB.Form FRMDELIVERYRT 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "DELIVERY RETAIL SALES......................"
   ClientHeight    =   9705
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   12480
   Icon            =   "FRMDELIVERYRT.frx":0000
   KeyPreview      =   -1  'True
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VB.Frame FRMEMAIN 
      ...
      Begin VB.Frame FRMEHEAD 
         ...
         Begin VB.ComboBox cmbtype 
            ...
            ForeColor       =   &H00004080&
            Height          =   315
            ItemData        =   "FRMDELIVERYRT.frx":030A
            Left            =   6405
            List            =   "FRMDELIVERYRT.frx":0317
   ...
   End
End
Attribute VB_Name = "FRMDELIVERYRT"
...
Public Function Generateprint()
    ...
    'db2.Execute "delete * From TEMPCN WHERE TRX_TYPE='RI' AND VCH_NO = " & Val(txtBillNo.Text) & ""
    ...
    ReportNameVar = App.Path & "\rptDELIVERYRT.RPT"
    Set Report = crxApplication.OpenReport(ReportNameVar, 1)
    Report.RecordSelectionFormula = "( {TEMPCN.TRX_TYPE}='RI' AND {TEMPCN.VCH_NO}= " & Val(txtBillNo.Text) & " )"
...
Private Sub Form_Load()
    ...
    rstBILL.Open "Select MAX(Val(VCH_NO)) From TEMPCN WHERE TRX_TYPE = 'RI'", db2, adOpenStatic, adLockReadOnly
...
''''''                End If
            ...
            Else
                PHY_ITEM.Close
                If NONSTOCKFLAG = True Then
                    PHY_ITEM.Open "Select ITEM_CODE, ITEM_NAME, BAL_QTY, SALES_PRICE, SALES_TAX, UNIT, REF_NO, EXP_DATE, VCH_NO, LINE_NO, TRX_TYPE, MRP, CHECK_FLAG, P_RETAIL From RTRXFILE  WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' ORDER BY [VCH_DATE]", db, adOpenStatic, adLockReadOnly
                Else
                    PHY_ITEM.Open "Select ITEM_CODE, ITEM_NAME, BAL_QTY, SALES_PRICE, SALES_TAX, UNIT, REF_NO, EXP_DATE, VCH_NO, LINE_NO, TRX_TYPE, MRP, CHECK_FLAG, P_RETAIL From RTRXFILE  WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY > 0 ORDER BY [VCH_DATE]", db, adOpenStatic, adLockReadOnly
                End If
                ITEM_FLAG = False
            End If
...
Private Sub TXTBILLNO_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            TRXFILE.Open "Select * From TEMPCN WHERE TRX_TYPE='RI' AND VCH_NO = " & Val(txtBillNo.Text) & " ORDER BY LINE_NO", db2, adOpenStatic, adLockReadOnly
...
Private Sub txtBillNo_LostFocus()
    ...
    Set TRXMAST = New ADODB.Recordset
    TRXMAST.Open "Select MAX(Val(VCH_NO)) From TEMPCN WHERE TRX_TYPE = 'RI'", db2, adOpenStatic, adLockReadOnly
    ...
    Set TRXMAST = New ADODB.Recordset
    TRXMAST.Open "Select MIN(Val(VCH_NO)) From TEMPCN WHERE TRX_TYPE = 'RI'", db2, adOpenStatic, adLockReadOnly
    If Not (TRXMAST.EOF And TRXMAST.BOF) Then
        ...
        Case vbKeyReturn
            ...
            If PHY.RecordCount = 1 Then
                ...
                If TMPFLAG = True Then
                    TMPREC.Open "Select ITEM_CODE, ITEM_NAME, BAL_QTY, MRP, SALES_PRICE, SALES_TAX, UNIT, REF_NO, EXP_DATE, VCH_NO, LINE_NO, TRX_TYPE, CHECK_FLAG, P_RETAIL From RTRXFILE  WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY > 0 ORDER BY [VCH_DATE]", db, adOpenStatic, adLockReadOnly
                    TMPFLAG = False
                Else
                    TMPREC.Close
                    TMPREC.Open "Select ITEM_CODE, ITEM_NAME, BAL_QTY, MRP, SALES_PRICE, SALES_TAX, UNIT, REF_NO, EXP_DATE, VCH_NO, LINE_NO, TRX_TYPE, CHECK_FLAG, P_RETAIL From RTRXFILE  WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY > 0 ORDER BY [VCH_DATE]", db, adOpenStatic, adLockReadOnly
...
Private Sub TXTRETAIL_LostFocus()
    TXTRETAILNOTAX.Text = Format(TXTRETAILNOTAX.Text, ".000")
    'TXTDISC.Tag = 0
    'TXTDISC.Tag = Val(TXTQTY.Text) * Val(TXTRETAILNOTAX.Text) * Val(TXTDISC.Text) / 100
...
Function FILL_BATCHGRID()
    ...
    If BATCH_FLAG = True Then
        PHY_BATCH.Open "Select REF_NO, BAL_QTY, EXP_DATE, MRP, SALES_PRICE, SALES_TAX,  ITEM_CODE, ITEM_NAME, VCH_NO, LINE_NO, TRX_TYPE, UNIT, CHECK_FLAG, P_RETAIL From RTRXFILE  WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY > 0 ORDER BY [VCH_DATE]", db, adOpenStatic, adLockReadOnly
        BATCH_FLAG = False
    Else
        PHY_BATCH.Close
        PHY_BATCH.Open "Select REF_NO, BAL_QTY, EXP_DATE, MRP, SALES_PRICE, SALES_TAX,  ITEM_CODE, ITEM_NAME, VCH_NO, LINE_NO, TRX_TYPE, UNIT, CHECK_FLAG, P_RETAIL From RTRXFILE  WHERE ITEM_CODE = '" & TXTITEMCODE.Text & "' AND BAL_QTY > 0 ORDER BY [VCH_DATE]", db, adOpenStatic, adLockReadOnly
...
Private Function AppendSale()
    ...
    db2.Execute "delete * From TEMPCN WHERE TRX_TYPE='RI' AND VCH_NO = " & Val(txtBillNo.Text) & ""
    'db2.Execute "delete * From P_Rate WHERE TRX_TYPE='RI' AND VCH_NO = " & Val(txtBillNo.Text) & ""
...
CLOSEFILE:
    ...
    SN = 0
'    RSTTRXFILE.Open "SELECT * From SALESREG ORDER BY VCH_NO", Conn, adOpenStatic,adLockReadOnly
