...
Begin VB.Form FRMReceiptreg 
   ...
   End
End
Attribute VB_Name = "FRMReceiptreg"
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            Else
                ...
                RSTTRXFILE.Open "Select * From TRXFILE WHERE VCH_NO = " & Val(LBLBILLNO.Caption) & "", db, adOpenForwardOnly
        ...
        Case vbKeyF6
            'If GRDTranx.Rows <= 1 Then Exit Sub
            Enabled = False
            FRMRECEIPTSHORT.LBLSUPPLIER.Caption = DataList2.Text
            FRMRECEIPTSHORT.lblactcode.Caption = DataList2.BoundText
            FRMRECEIPTSHORT.lblType.Caption = GRDTranx.TextMatrix(GRDTranx.Row, 8)
            ...
            If GRDTranx.TextMatrix(GRDTranx.Row, 0) = "Receipt" Then
                If Val(cmddisplay.Tag) = 68 Or Val(cmddisplay.Tag) = 100 Or Val(cmdexit.Tag) = 69 Or Val(cmdexit.Tag) = 101 Then
                    If MsgBox("Are you sure you want to delete this entry", vbYesNo, "DELETE !!!") = vbYes Then
                        db.Execute "delete * From DBTPYMT WHERE TRX_TYPE='RT' AND CR_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " AND BILL_TRX_TYPE = '" & GRDTranx.TextMatrix(GRDTranx.Row, 8) & "'"
                        db.Execute "delete * FROM CASHATRXFILE WHERE INV_NO = " & GRDTranx.TextMatrix(GRDTranx.Row, 7) & " AND INV_TYPE = 'RT' AND BILL_TRX_TYPE = '" & GRDTranx.TextMatrix(GRDTranx.Row, 8) & "'"
                        Call FILLGRID
                    Else
                        GRDTranx.SetFocus
                    End If
...
Private Sub TXTDEALER_Change()
    ...
    If flagchange.Caption <> "1" Then
        If ACT_FLAG = True Then
            ACT_REC.Open "select ACT_CODE, ACT_NAME from [ACTMAST]  WHERE (Mid(ACT_CODE, 1, 2)='13')And (len(ACT_CODE)>2) And ACT_NAME Like '" & TXTDEALER.Text & "%'ORDER BY ACT_NAME", db, adOpenForwardOnly
            ACT_FLAG = False
        Else
            ACT_REC.Close
            ACT_REC.Open "select ACT_CODE, ACT_NAME from [ACTMAST]  WHERE (Mid(ACT_CODE, 1, 2)='13')And (len(ACT_CODE)>2) And ACT_NAME Like '" & TXTDEALER.Text & "%'ORDER BY ACT_NAME", db, adOpenForwardOnly
...
Private Function FILLGRID()
    ...
    rstTRANX.Open "SELECT * From DBTPYMT WHERE [ACT_CODE] = '" & DataList2.BoundText & "' ORDER BY INV_DATE DESC", db, adOpenForwardOnly
    Do Until rstTRANX.EOF
        ...
        GRDTranx.TextMatrix(i, 8) = IIf(IsNull(rstTRANX!BILL_TRX_TYPE), "SI", rstTRANX!BILL_TRX_TYPE)
        GRDTranx.Row = i
        GRDTranx.Col = 0
