   ...
   Begin VB.Frame Fram 
      ...
      Begin VB.Frame FRMECONTROLS 
         ...
         Begin VB.ComboBox CmbWrnty 
            ...
            ForeColor       =   &H000000FF&
            Height          =   360
            ItemData        =   "FrmLPA.frx":030A
            Left            =   15840
            List            =   "FrmLPA.frx":0314
         ...
         Begin VB.ComboBox CmbPack 
            ...
            ForeColor       =   &H000000FF&
            Height          =   360
            ItemData        =   "FrmLPA.frx":0325
            Left            =   8805
            List            =   "FrmLPA.frx":034D
            Style           =   2  'Dropdown List
            TabIndex        =   4
            Top             =   480
            Width           =   960
    ...
    Select Case KeyCode
        Case vbKeyReturn
            If CMBPO.VisibleCount <> 0 And CMBPO.BoundText = "" Then
                If (MsgBox("Are you sure you want to continue without selecting the Purchase Order No.? !!!!", vbYesNo, "PURCHASE") = vbNo) Then
                    CMBPO.SetFocus
                    Exit Sub
                End If
            End If
            ...
            If CMBPO.Text <> "" And CMBPO.MatchedWithList = False Then
                MsgBox "Please select a valid PO No. from the list", vbOKOnly, "Purchase"
...
Private Sub CMDADD_Click()
    ...
    If Val(TXTQTY.Text) = 0 Then
        MsgBox "Please enter the Qty", vbOKOnly, "Purchase"
        TXTQTY.Enabled = True
        TXTQTY.SetFocus
        Exit Sub
    End If
    If Val(TXTPTR.Text) = 0 Then
        MsgBox "Please enter the Price", vbOKOnly, "Purchase"
        TXTPTR.SetFocus
        Exit Sub
    End If
...
'                Else
                ...
                If Val(Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 20))) <> 0 Then !P_CRTN = Val(grdsales.TextMatrix(Val(TXTSLNO.Text), 20)) ' / Val(Los_Pack.Text), 3)
    ...
    If grdsales.TextMatrix(Val(TXTSLNO.Text), 12) <> "" Then
        RSTRTRXFILE!EXP_DATE = IIf(grdsales.TextMatrix(Val(TXTSLNO.Text), 12) = "", Null, Format(grdsales.TextMatrix(Val(TXTSLNO.Text), 12), "dd/mm/yyyy"))
    End If
...
Private Sub cmdRefresh_Click()
    If CMBPO.VisibleCount = 0 Then CMBPO.Text = ""
    If CMBPO.VisibleCount <> 0 And CMBPO.BoundText = "" Then
        If (MsgBox("Are you sure you want to save the Purchase Bill without selecting the Purchase Order No.? !!!!", vbYesNo, "PURCHASE") = vbNo) Then Exit Sub
    End If
    If CMBPO.Text <> "" And CMBPO.MatchedWithList = False Then
        MsgBox "Please select a valid PO No. from the list", vbOKOnly, "Purchase"
        On Error Resume Next
...
SKIP:
    Screen.MousePointer = vbNormal
    MsgBox "EXPORTED SUCCESSFULLY", vbOKOnly, "PURCHASE ENTRY"
    Exit Sub
eRRhAND:
    Screen.MousePointer = vbNormal
...
Private Sub grdtmp_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            TxtHSN.Text = IIf(IsNull(PHY!REMARKS), "", PHY!REMARKS)
            On Error Resume Next
            ...
            For i = 1 To grdsales.Rows - 1
                If Trim(grdsales.TextMatrix(i, 1)) = Trim(TXTITEMCODE.Text) Then
                    If MsgBox("This Item Already exists in Line No. " & i & " Do yo want to add this item", vbYesNo, "PURCHASE RETURN..") = vbNo Then Exit Sub
                    Exit For
                End If
            Next i
        ...
        Case vbKeyReturn
            If Val(TxttaxMRP.Text) <> 0 Then
                ...
                'If OPTVAT.Value = False Then
                    MsgBox "Tax should be Zero ....", vbOKOnly, "Purchase Entry"
                    TxttaxMRP.Enabled = True
                    TxttaxMRP.SetFocus
                    Exit Sub
                End If
    ...
    Select Case KeyCode
        ...
        Case vbKeyReturn
            ...
            If Trim(TXTITEMCODE.Text) = "" Then
                If MsgBox("Item not exists!!! Do You want to add this item?", vbYesNo, "PURCHASE") = vbNo Then Exit Sub
                TXTPRODUCT.Tag = ""
                Set RSTITEMMAST = New ADODB.Recordset
                ...
                'MsgBox "Item not found!!!!", , "Purchase"
                Exit Sub
            Else
                Call TXTITEMCODE_KeyDown(13, 0)
            End If
    ...
    Select Case KeyCode
        Case vbKeyReturn
            If Val(txtcrtn.Text) <> 0 And Val(txtcrtnpack.Text) = 0 Then
                MsgBox "Please enter the Pack Qty for Loose Qty", vbOKOnly, "Purchase"
                txtcrtnpack.SetFocus
                Exit Sub
            End If
            If Val(Los_Pack.Text) = 1 Then
    ...
    Select Case KeyCode
        Case vbKeyReturn
            ...
            If PHY_CODE.RecordCount = 1 Then
                TXTITEMCODE.Text = grdtmp.Columns(0)
                TXTPRODUCT.Text = grdtmp.Columns(1)
                TxtHSN.Text = IIf(IsNull(PHY_CODE!REMARKS), "", PHY_CODE!REMARKS)
                On Error Resume Next
                Set Image1.DataSource = PHY
                If IsNull(PHY!PHOTO) Then
                    Frame6.Visible = False
                ...
                For i = 1 To grdsales.Rows - 1
                    If Trim(grdsales.TextMatrix(i, 1)) = Trim(TXTITEMCODE.Text) Then
                        If MsgBox("This Item Already exists in Line No. " & i & " Do yo want to add this item", vbYesNo, "PURCHASE") = vbNo Then Exit Sub
                        Exit For
                    End If
                Next i
...
Private Sub txtWsalePercent_LostFocus()
    ...
    Else
        txtWS.Text = Val(TXTRATE.Text) - (Val(TXTRATE.Text) * Val(txtWsalePercent.Text) / 100)
    End If
    ...
    Select Case KeyCode
        Case vbKeyReturn
            If Val(TxtWarranty.Text) <> 0 And CmbWrnty.ListIndex = -1 Then
                MsgBox "Please select the Warranty Period", , "PURCHASE"
                CmbWrnty.SetFocus
                Exit Sub
            End If
