...
'    If OptEXP.Value = True Then
        If OPTPERIOD.Value = True Then
            rstTRANX.Open "SELECT * From TRXEXPENSE WHERE [VCH_DATE] <=# " & TODATE & " # AND [VCH_DATE] >=# " & FROMDATE & " # ORDER BY VCH_DATE,VCH_NO", db2, adOpenStatic, adLockReadOnly
        Else
            rstTRANX.Open "SELECT * From TRXEXPENSE WHERE [ACT_CODE] = '" & Dlstemployee.BoundText & "' ORDER BY VCH_DATE,VCH_NO", db2, adOpenStatic, adLockReadOnly
        End If
'    End If
...
Private Sub TXTEMPLOYEE_Change()
    ...
    If empflag.Caption <> "1" Then
        If EMP_FLAG = True Then
            EMP_REC.Open "select ACT_CODE, ACT_NAME from [ACTMAST]  WHERE (Mid(ACT_CODE, 1, 3)='641')And (len(ACT_CODE)>3) And ACT_NAME Like '" & Me.TXTEMPLOYEE.Text & "%'ORDER BY ACT_NAME", db2, adOpenStatic, adLockReadOnly, adCmdText
            EMP_FLAG = False
        Else
            EMP_REC.Close
            EMP_REC.Open "select ACT_CODE, ACT_NAME from [ACTMAST]  WHERE (Mid(ACT_CODE, 1, 3)='641')And (len(ACT_CODE)>3) And ACT_NAME Like '" & Me.TXTEMPLOYEE.Text & "%'ORDER BY ACT_NAME", db2, adOpenStatic, adLockReadOnly, adCmdText
